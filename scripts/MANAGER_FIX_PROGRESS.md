...
- [ ] MWA-A1b Capability pump completion. Add EventType::{PermissionChanged, BiometricResult, KeyringResult} (+ Hover/Window filter variants + match/filter arms + api.json staging) with EventProvider impls on Permission/Biometric/Keyring managers (pending flags set in set_status/set_last_result); geolocation ERROR channel (push_location_error/drain + last_error + GeolocationError provider event); in-flight counters on biometric/keyring/permission (request dispatched → +1, result folded → −1) feeding desired_interval_ms with ASYNC_RESULT_INTERVAL_MS=100 so prompt outcomes wake an idle loop; clear_pending_event calls for the three new providers in process_window_events. STATUS: TODO
