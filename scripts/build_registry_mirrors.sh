...
# --------------------------------------------------------------------------
# --------------------------------------------------------------------------
# Crystal (Shards) — `shards install` clones any git URL, so a bare repo on
# Pages works exactly like the Homebrew tap and the Scoop bucket:
#   dependencies:
#     azul:
#       git: https://azul.rs/ui/crystal.git
#       version: ~> <V>
#
# The repo has to BE a shard, which means a `shard.yml` naming it and the
# code under `src/`: `shards install` clones into `lib/azul/`, and
# `require "azul"` then resolves `lib/azul/src/azul.cr`. A repo of loose
# files installs with a warning and cannot be required - verified.
#
# The release already carries the whole binding as one self-contained file
# (`lib/azul.cr`, the curl install's payload), so the shard is that file under
# `src/` plus a generated manifest. Nothing else has to be published for it.
# --------------------------------------------------------------------------
build_crystal() {
  command -v git >/dev/null 2>&1 || { echo "  [crystal] git missing — skip"; return; }
  local binding="$RELDIR/lib/azul.cr"
  [ -f "$binding" ] || { echo "  [crystal] $binding missing — skip"; return; }
  local work; work="$(mktemp -d)"

  mkdir -p "$work/src"
  cp "$binding" "$work/src/azul.cr"
  cat > "$work/shard.yml" <<EOF
# Generated by scripts/build_registry_mirrors.sh. Do not edit.
name: azul
version: $V
...
Crystal bindings for [Azul](https://azul.rs), generated from \`api.json\`.
...
The shard is the binding only; the native library is a separate download.
See https://azul.rs/guide/$V/en/hello-world/crystal.
EOF
  ...
  git clone -q --bare "$work" "$SITE/ui/crystal.git" || {
    echo "  [crystal] bare clone failed"; rm -rf "$work"; return; }
  ( cd "$SITE/ui/crystal.git" && git update-server-info )
  rm -rf "$work"

  # The consumer manifest the guide tells the reader to curl. It carries the
  # CONCRETE version: `~> $VERSION` is a placeholder the website substitutes
  # for a reader, and shards fails to resolve it verbatim.
  cat > "$RELDIR/shard.yml" <<EOF
name: hello_world
version: 0.1.0
...
EOF
  ...
  echo "  [crystal] published crystal.git + shard.yml (shard azul $V)"
}
