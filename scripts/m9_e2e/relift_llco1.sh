#!/usr/bin/env bash
# BACKEND-OPT-LEVEL TEST (resumed 2026-08 after month gap; re-added AZ_LLC_O1 gate to
# master's transpiler_remill.rs ~1601). Rebuild dll + cold relift with AZ_LLC_O1=1 →
# per-fn wasm backend (llc) at -O1 (RegStackify ON so locals fit, -O2-only backend opts
# OFF). Splits the solveLayoutReal wild-garbage OOB:
#   solve RETURNS at backend -O1 ⇒ an -O2-only wasm-backend opt miscompiles it → bisect that pass.
#   solve still OOB at -O1       ⇒ bug is -O1-level opt / instruction-selection / a semantic.
