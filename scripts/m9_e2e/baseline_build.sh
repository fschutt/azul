...
echo "=== cold relift (scratch kept) $(date +%H:%M:%S) ===" | tee -a "$LOG"
...
AZ_PORT=8800 "/c/Users/felix/tools/node/node.exe" scripts/m9_e2e/full-cycle.js 2>&1 | head -14 | tee -a "$LOG"
SP=$(powershell -NoProfile -Command "(Get-Process hello-world -EA SilentlyContinue | Select -First 1).Id" 2>/dev/null|tr -d '\r')
echo "server pid=$SP scratch=$TEMP/azul-web-transpiler-$SP" | tee -a "$LOG"
