...
export AZ_LTO_LEVEL=0
echo "=== relift LTO=0 $(date +%H:%M:%S) ===" | tee -a "$LOG"
nohup ./examples/c/hello-world.exe > /c/rb/server_lto0.log 2>&1 &
for i in $(seq 1 300); do
  grep -qE "Listening on" /c/rb/server_lto0.log 2>/dev/null && { echo "READY $(date +%H:%M:%S)" | tee -a "$LOG"; break; }
  a=$(powershell -NoProfile -Command "(Get-Process hello-world -EA SilentlyContinue|Measure-Object).Count" 2>/dev/null | tr -d '\r')
  [ "$a" = "0" ] && { echo "DIED $(date +%H:%M:%S)" | tee -a "$LOG"; tail -10 /c/rb/server_lto0.log | tee -a "$LOG"; exit 1; }
  sleep 10
done
