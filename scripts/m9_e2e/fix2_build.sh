...
nohup ./examples/c/hello-world.exe > /c/rb/server_fix4.log 2>&1 &
for i in $(seq 1 260); do
  grep -qE "Listening on" /c/rb/server_fix4.log 2>/dev/null && { echo "READY ($(wc -l < /c/rb/server_fix4.log)L) $(date +%H:%M:%S)" | tee -a "$LOG"; break; }
  alive=$(powershell -NoProfile -Command "(Get-Process hello-world -EA SilentlyContinue|Measure-Object).Count" 2>/dev/null | tr -d '\r')
  [ "$alive" = "0" ] && { echo "SERVER-DIED" | tee -a "$LOG"; tail -8 /c/rb/server_fix4.log | tee -a "$LOG"; exit 1; }
  sleep 10
done
