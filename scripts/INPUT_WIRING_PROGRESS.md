...
- [x] 10e-i DONE 2026-09-04 (web, not browser-run). The loader binds the mouse trio as POINTER events (same wire kinds, mouse* listeners removed so a press cannot dispatch twice) and forwards `getCoalescedEvents()` / `getPredictedEvents()` in the event tail (`PAYLOAD_LEN`@20, `n_coalesced|n_predicted|(i32,i32)*` at 24 in 24.8 fixed point, capacities 24/4 = exactly the 256-byte buffer, const-asserted). The rAF throttle no longer loses the skipped positions: every event's coalesced list is accumulated and the frame's dispatch carries them minus the event's own sample (the `TouchState::coalesced_points` contract). The wasm side decodes into `EventloopState::pointer_samples`, the web mirror of `touch_state.coalesced_points/predicted_points` (iOS is the only other producer; desktop `record_input_sample` has no history parameter and Windows never calls `GetPointerFrameInfoHistory`), cleared after the up like iOS; a tail whose counts overrun its bytes decodes to nothing. TRAP FOUND: `azDispatch` never wrote offset 20 into a bump-allocated, unzeroed buffer (a stale length) - now always written. 7 codec/accumulator tests + 3 loader-contract tests, deno parses the loader. OPEN: nothing reads the samples until S2's wasm-side `CallbackInfo`. Verification: once at the end of the wave (battery, corpus, 9-target gate), per the user's 2026-09-04 instruction.
...
- [x] 10c-iv DONE 2026-09-04 - `env()` for safe-area and keyboard insets, on the user's design (a dynamic filter like `os` / `os-version`): `env(safe-area-inset-{top,right,bottom,left}[, fallback])` and `env(keyboard-inset-height)` parse into the existing `CssDeclaration::Dynamic` under an `env:` dynamic-id prefix, on longhands AND shorthands (a shorthand expands to its longhands, each reading the variable), and resolve in the cascade against the `DynamicSelectorContext` - `with_safe_area` maps `LayoutWindow::safe_area_insets` (what the shells write, keyboard included) into it, NaN meaning "platform reported none" so the fallback applies (a known name without one gets `0px`). The live value is re-typed through the property's own parser, so `padding-bottom` gets a padding and `top` an inset. A rule block using `env()` counts as context-dependent, so a rotation or keyboard change restyles it through `set_dynamic_selector_context` like an `@media` breakpoint, with no new StyledDom state. Tests in `css/src/parser2.rs`, `css/src/dynamic_selector.rs` and `layout/src/window.rs::env_safe_area_inset_resolves_to_the_live_inset_and_follows_a_change`. Not recognised: `env()` nested inside `calc()` or mixed with other tokens. Verification: once at the end of the wave (battery, corpus, 9-target gate), per the user's 2026-09-04 instruction.
...
- [x] 10b-i-b-i-b DONE 2026-09-04. An explicit `replacementRange` during a composition that is not the marked text is honoured. The offsets index the IME document WITH the preedit spliced in, so the pure `text_edit::rebase_ime_range_onto_committed` maps them onto the committed text: `NSNotFound` untouched; no composition = ordered only; a range before the preedit unchanged; after it, slid back by the preedit's byte length; overlapping it (partial, inside, containing) = the composition's own place, the empty span at its start, which `ime_replacement_action` reads as "act at the caret". Ordering, spelled out in both `macos/mod.rs` callers: composition resolved FIRST, replacement applied to the committed text, then compose/insert at the caret the delete leaves (the reading WebKit's `setCompositionAsync` / `insertTextAsync` imply). `setMarkedText:` un-shapes the preedit but keeps the composition OPEN (an Update, not End+Start); `insertText:` commits first. 6 unit tests on the rebase. Not driven by a real IME (none on hand sends this shape). Verification: the full battery, the e2e corpus and the 9-target cross-compile run ONCE at the end of this wave (user, 2026-09-04: no compiling until everything is implemented).
...
- [x] 9h-i-a-i-d-i-a DONE 2026-09-04 (USER RULING: implement blindly). The AVFAudio path in `media_keys/apple.rs` (dlopen of AVFAudio.framework with the AVFoundation fallback, the double-dereferenced NSString constants, `sharedInstance`, `setCategory:error:` with `AVAudioSessionCategoryPlayback`, `setActive:error:`, the `NotifyOthersOnDeactivation` release, the interruption observer) now compiles for `any(ios, macos)` and `media_keys::set_system_audio_takeover` routes macOS to it. Every step a Mac may lack is guarded (framework, class, `sharedInstance`, each session selector via `respondsToSelector:` since several are `API_UNAVAILABLE(macos)`, the category constant); each guard logs at debug and answers `AVF_ABSENT = Some(cfg!(macos))`, i.e. the desktop's previous "the mixer shares, you own it" no-op on macOS and `Some(false)` on iOS as before. Not run against a live session (no macOS app in hand gates `MPNowPlayingInfoCenter` on one). Verification: the full battery, the e2e corpus and the 9-target cross-compile run ONCE at the end of this wave (user, 2026-09-04: no compiling until everything is implemented).
...
- [x] 9d-i-a DONE 2026-09-04 (web, not browser-run). There was no web `handle_set_pointer_lock` to override (the wasm shell implements no `PlatformWindow`), so the request takes the shell's own change channel: `PATCH_KIND_POINTER_LOCK` (13, `locked:u8`) -> `azSetPointerLock` -> `requestPointerLock()` on `#az-body` / `exitPointerLock()`. The flag is what the BROWSER reports: `pointerlockchange` -> `EVT_POINTERLOCKCHANGE` (17) -> `EventloopState::is_cursor_locked` (a refusal fires `pointerlockerror`, no change). While locked, `pointermove.movementX/Y` accumulate and flush once per frame as `EVT_RAWMOUSEMOTION` (16, fixed-point dx/dy, `pointerId` as device); the wasm side drops it unless its flag is set (X11's `handle_xi_raw_motion` gate), notes it like `note_raw_motion`, broadcasts to nodes registered `rawmousemotion`, and drains like `take_raw_motion`. `window.__azProbe.setPointerLock` lets a headless probe drive it. HONEST GAP: the wasm PRODUCER of patch 13 does not exist (today's callback gets raw event bytes as `info`, so `set_pointer_lock` has no change list) - decoder ahead of producer, like Focus(9)/ScrollTo(10), documented in EVENT_PATCH_SCHEMA.md. Verification: once at the end of the wave (battery, corpus, 9-target gate), per the user's 2026-09-04 instruction.
- [x] 9d-android DONE 2026-09-04 (blind Android): `AndroidWindow::handle_set_pointer_lock` overrides the trait's refuse-by-default with pointer CAPTURE - `set_pointer_capture` (JNI, `NativeTextBridge.setPointerCapture`: `requestFocus` + `requestPointerCapture` / `releasePointerCapture` on the input view, API 26 guard, UI thread). Until this Android answered "no" to every `CallbackInfo::set_pointer_lock`, so `RawMouseMotion` and the pad touch surface (8f-i-a-ii) were unreachable there. Evidence: 9/9 gate EXIT=0, `javac` against android-34 EXIT=0.
  - [x] 9d-android-a DONE 2026-09-04 (blind Android): `AzulInputView.onPointerCaptureChange` -> `nativeOnPointerCaptureChanged` -> `AndroidWindow::pending_pointer_capture` (a `Mutex<Option<bool>>`, the same queue shape as the IME commits: the callback lands on the UI thread, never mid-pass) -> the main loop drains it, and when it differs from `mouse_state.is_cursor_locked` applies it through the OS-sourced `update_window_state` and runs `process_window_events`, so the diff emits `PointerLockChange`. A capture the system takes away on focus loss now drops the flag within a frame, which un-gates `RawMouseMotion`; a refused request corrects the optimistic "requested" answer the same way. Nothing re-captures on focus return (the 2026-09-03 ruling). Evidence: 9/9 gate EXIT=0, host check EXIT=0, `javac` against android-34 EXIT=0. Not run on a device.
  - [x] 9d-android-b DONE 2026-09-04 (blind Android): `AzulInputView.onCapturedPointerEvent` forwards a captured MOUSE (`SOURCE_MOUSE_RELATIVE`, where `getX/getY` ARE the deltas) and a captured touchpad that is not a pad's surface (`AXIS_RELATIVE_X/Y`, after the gamepad path declined) to `nativeOnRawMotion` -> `AndroidWindow::pending_raw_motion` (accumulated `(dx, dy, device)` under a mutex, UI thread) -> the loop drains it while `is_cursor_locked` (X11's `handle_xi_raw_motion` gate) into `device_event_manager.note_raw_motion` and runs one pass, which emits `RawMouseMotion` like every other backend. Evidence: 9/9 gate EXIT=0, host check EXIT=0, `javac` against android-34 EXIT=0. Not run on a device.
...
- [x] 9b-ii-a-i-d-ii-c-iii DONE 2026-09-04: `LayoutWindow::spliced_text_with_preedits` builds the node's committed text with EVERY live composition spliced in (the primary's when `preedit_shaped_node` is this node, each seat's when `seat_preedit_shaped` says so), each at its own caret, in descending (run, byte, seat) order so earlier offsets stay valid; `apply_preedit_to_text_cache`, `apply_seat_preedit_to_text_cache`, `end_preedit_shaping` and `end_seat_preedit_shaping` all shape through it, so ending one composition leaves the others shaped. Before this each shaping call re-read the committed text and spliced ONE preedit, so the last shaped composition erased the others. Test `two_seats_composing_in_one_node_are_both_shaped` (layout/tests/seat_text_session.rs). Verification: the full battery, the e2e corpus and the 9-target cross-compile run ONCE at the end of this wave (user, 2026-09-04: no compiling until everything is implemented).
- [x] 9b-ii-a-i-d-ii-d DONE 2026-09-04, decided as PER-PERSON undo (the collaborative-editor convention): `UndoableOperation.seat_id` attributes every text edit to the seat that made it (`record_operation_for_seat`, threaded through `record_text_edit_undo` / `record_delete_undo` from the seat paths; the primary is seat 0), and the shell's `undo_text_edit_on` / `redo_text_edit_on` take the seat and use `pop_undo_for_seat` / `pop_redo_for_seat`: a seat pops its own latest edit only while it is the TOP of the node's stack. A seat whose edit is buried under another seat's later edit gets nothing - refused rather than undoing someone else's work or restoring a stale snapshot over it. Tests in `undo_redo.rs` (`seat_attribution_tests`) and `a_seats_edit_is_undone_only_by_that_seat_and_only_while_on_top` (seat_text_session.rs). `UndoableOperation` is repr(C) and in api.json, so the `autofix` drift pass runs at the end of the wave with the other type changes.
  - [ ] 9b-ii-a-i-d-ii-d-i Interleaved undo (a seat's edit buried under another seat's): needs operational transformation of the later edits over the undone one; refused today. Logged, not guessed. Verification: the full battery, the e2e corpus and the 9-target cross-compile run ONCE at the end of this wave (user, 2026-09-04: no compiling until everything is implemented).
- [x] 9b-ii-a-i-d-iii DONE 2026-09-04 (overlay half): a NON-primary seat's focused node now paints a 2px ring in that seat's colour around its border box (`LayoutContext::seat_focus_rings`, built by `LayoutWindow::seat_focus_rings` from `FocusManager::seat_focus` with the app-set owner colour or the seat palette the seat carets use; painted in `paint_node_background_and_border_inner` as four `push_rect` strips so every backend paints it identically; several seats on one node ring concentrically). The rings are part of `dl_input_fingerprint`, so a seat's focus move misses the display-list cache like a caret move. This is the ledger's own "seat-coloured overlay" option; `:focus` and its restyle stay the primary's, and a11y keeps one focus (no multi-focus model exists in accesskit) - both by design, not omission. Evidence: corpus scenario e2e/seat-focus-ring.json green (the seat colour appears in the display list only while seat 7 focuses #b, negative steps included), azul-layout check EXIT=0; the full battery and cross-compile are deferred to the end of the wave per the user's 2026-09-04 instruction.
  - [x] 9b-ii-a-i-d-iii-a DONE 2026-09-04 (the pseudo-class half): `:seat-focus` is a real pseudo-class - `CssPathPseudoSelector::SeatFocus` (parser, Display, XML mapping), `PseudoStateType::SeatFocus` / `PseudoStateFlags::seat_focused` in the dynamic selectors, `StyledNodeState::seat_focused` (state index 11) with its own bucket in the property cache (`collect_and_assign!` + the three-tier lookup, mirroring `:focus`), and `StyledDom::restyle_on_seat_focus_change` driven by `apply_seat_focus_restyle` in both the dll shell's `SetSeatFocus` arm and the runner's `set_seat_focus` (the node that lost a seat drops the class only if no other seat still focuses it). The primary's `:focus` never fires for a seat and vice versa. Corpus scenario `e2e/seat-focus-pseudo-class.json` (seat 7's click paints `#b:seat-focus`'s background, not `#b:focus`'s, and it clears on blur); state-index tests extended. `StyledNodeState` is repr(C), so the wave's `autofix` pass carries the new field into api.json. The a11y half stays by design: accesskit has no multi-focus model. Verification: once at the end of the wave (battery, corpus, 9-target gate), per the user's 2026-09-04 instruction.
...
- [x] 9b-ii-a-i-d-v DONE 2026-09-04: the shell's default-action pass (`event.rs`, the `has_key_event` block) now loops over EVERY distinct seat that has a `KeyDown` in `pre_filter.user_events`, in arrival order, each with its own keyboard state, its own focus (`focused_node_for(seat)`), its own editing-query state and its own `SetSeatFocus`/structural-edit records. Before this the first `KeyDown`'s seat won and the second seat's Tab/Enter in the same frame was silently dropped. Evidence: host check EXIT=0, dll 2071 passed, 8/8 gate. Not unit-tested: the block is shell-only (no headless harness drives `process_window_events_recursive`); the e2e runner does not mirror it (see -v-a).
  - [x] 9b-ii-a-i-d-v-a DONE 2026-09-04: the runner's default-action pass (`e2e/runner.rs::run_keyboard_default_action`) now loops over every seat with a `KeyDown` in the pass, each with its own keyboard state, focus (`focused_node_for`), editing-query state, seat focus move (`set_seat_focus`, mirror of the `SetSeatFocusTarget` arm) and structural-edit record. TWO gaps closed: the primary-only read, AND the runner's `ModifyWindowState` change gate omitted `keyboard_seats` (the same hole the `touch_state` comment records), so a seat `key_down` op never ran a pass at all. New corpus scenario `e2e/seat-focus-tab-per-seat.json` (seat 7 Tab walks seat 7's focus a->b while the primary stays on c; seat Escape clears only seat 7's focus), proven live by a negative control (primary-only keyboard read fails at the seat-Tab step). Evidence: host check EXIT=0; core 2810, layout lib 7684, layout --test all 1018 passed; e2e corpus all scenarios green; dll 2071 passed; 8/8 target gate. Seat focus moves dispatch no Blur/Focus in the runner and no `:focus` restyle, matching the dll (9b-ii-a-i-d-iii stays open).
...
- [x] 9b-ii-b-i-b-i-a DONE 2026-09-04 (context-menu half): `get_first_hovered_node_for(seat_id)` reads the seat's own hover point (`InputPointId::for_seat`; the primary getter is now the seat-0 case), and both seat right-click paths open the context menu from it: a seat pen's barrel EDGE in `handle_tablet_frame_for_seat` (was-right captured before the seat's state is overwritten, parity with the primary pen frame) and a seat's right button in `handle_seat_pointer_button` (consumed before the state change, parity with `handle_pointer_button`). Until this a seat's barrel/right button opened nothing: the only opener read the primary's hover. Evidence: host check EXIT=0; azul-dll tests 2071 passed; 8/8 target gate EXIT=0 (x86_64-unknown-linux-gnu compiles the Wayland shell). No headless test: the Wayland window cannot be constructed off-device, and the corpus has no context-menu op; logged as blind Wayland like the rest of the seat work.
  - [x] 9b-ii-b-i-b-i-a-i DONE 2026-09-04 (blind Wayland), the earlier "single device" reading corrected: `zwp_primary_selection_device_manager_v1.get_device` takes a `wl_seat`, so each NON-primary seat now gets its own device (`try_init_seat_primary_selections`, called when a seat appears and when the manager arrives, so global order does not matter; dropped with the seat in `handle_seat_gone`). A seat's left release (`handle_seat_pointer_button`) and pen-up (`handle_tablet_frame_for_seat`, `was_left` captured beside `was_right`) publish THAT seat's selection (`seat_selected_text`) through THAT seat's device with THAT seat's serial (`publish_seat_primary_selection`, one source per seat replaced per publish; the source's `send` serves the text keyed by the source proxy, `cancelled` frees it). The primary's PRIMARY is untouched. Evidence: 9/9 target gate EXIT=0 (x86_64-unknown-linux-gnu compiles the Wayland shell), host check EXIT=0, dll 2073 passed. Not run on a two-seat compositor.
    - [x] 9b-ii-b-i-b-i-a-i-a DONE 2026-09-04 (blind Wayland): the seat device's `data_offer` now attaches the offer listener instead of destroying, and its `selection` event maps the device back to its seat and keeps the offer per seat (`seat_primary_offers`, old offer destroyed, dropped with the seat). A seat's MIDDLE release with a seat caret (`primary_paste_wanted`, per seat) reads THAT seat's selection - our own live source's text directly (the primary path's `native_primary_text` shortcut, since a pipe read of our own offer would wait for a `send` this thread must answer), else `receive_from_offer` on the seat's offer - and records it at the SEAT's caret via `record_text_input_for_seat` BEFORE the pass, the primary's idiom. Evidence: 9/9 target gate EXIT=0 (x86_64-unknown-linux-gnu compiles the Wayland shell), host check EXIT=0, dll 2073 passed. Not run on a two-seat compositor.
...
- [x] 8f-i-a-ii DONE 2026-09-04 (blind Android), research revisited: the earlier "cannot" was true only for the UNCAPTURED case. Under pointer capture (`View.requestPointerCapture`, API 26) Android delivers the pad's touch surface to `View.onCapturedPointerEvent` as a `SOURCE_TOUCHPAD` event with absolute per-pointer positions - the surface itself, not the synthesised mouse. Wired: `AzulInputView.onCapturedPointerEvent` -> `AzulGamepad.onCapturedPointer` (pairs the touchpad InputDevice to the gamepad InputDevice with the same vendor/product when exactly one matches; normalises 0..1 over the device's motion range, y flipped to the bottom-left origin `GamepadState::touchpad_x` documents) -> `nativeOnTouchpad` (`gamepad/android.rs`) -> `touchpad_active/x/y` + `push_gamepad_state`. Uncaptured, `touchpad_active` stays false, which is the truthful "no data" the fields already model, so the API keeps its fields (the ruling's `impl_option!` branch was for a platform with NO data at all; Android has it under capture). Capture itself is 9d-android. Evidence: 9/9 gate EXIT=0 (both Android targets, with and without `jni`), `javac` of all bridge sources against android-34 EXIT=0. Not run on a device or emulator with a pad.
  - [x] 8f-i-a-ii-a DONE 2026-09-04 (second-finger half): `GamepadState` gains `touchpad2_x` / `touchpad2_y` / `touchpad2_active` (api.json via `autofix`, one round, bindings regenerated) and every producer that has a second point fills it - the DualSense report's `points[1]` (bytes 36..40) and the DualShock 4's second point (38..42) in `playstation.rs` (`PadSample::touch2`, new test `a_second_finger_lands_in_touch2`), `touchpadSecondary` on the Game Controller profiles (`apple.rs`), and pointer index 1 of the captured SOURCE_TOUCHPAD event on Android (`nativeOnTouchpad` carries both slots; the lifting pointer of ACTION_POINTER_UP is excluded by index). The manager's change detection and its per-field event test cover the three fields. Evidence: host check EXIT=0; core 2810, layout lib 7684, layout --test all 1018 passed; e2e corpus green; dll 2073 passed (rerun after a borrow fix in the new test); 9/9 target gate EXIT=0; codegen EXIT=0; `javac` against android-34 EXIT=0.
  - [x] 8f-i-a-ii-a-i DONE 2026-09-04 (blind Android): with two identical pads the touchpad node pairs to the gamepad node with the NEAREST InputDevice id (`AzulGamepad.nearestGamepadId`), since Android registers a pad's evdev nodes back to back so their ids are adjacent - the one ordering the platform does guarantee, where the descriptor (a nonce-disambiguated hash over vendor:product:uniqueId shared by both nodes) guarantees nothing. An exact tie (two candidates equally near) still pairs nothing. `javac` against android-34 EXIT=0 at write time. Verification: the full battery, the e2e corpus and the 9-target cross-compile run ONCE at the end of this wave (user, 2026-09-04: no compiling until everything is implemented).
...
- [x] G2-a-ii DONE 2026-09-04. The four drift errors are fixed: `WebConfig::loopback` lacked the `prelift` field the parser had grown; `rust_fontconfig::AZ_IN_WASM_SOLVE` was a diag atomic that existed only in the vendored fork and went away with the crates.io switch (line dropped); and the "two html_render.rs call sites" were ONE site producing two codes - `cache.computed_values.get(node_idx)` became a `(node, prop_type)` lookup when `InheritedValues` was bucketed (E0061) and the `.iter()` on its result was the follow-on E0599 - now `values_for(node_idx)`. Baseline `cargo check -p azul-dll --release --features web` = those 4 errors; after = 0 of them (the remaining red at write time was the stale generated C-ABI for `UndoableOperation.seat_id`, which the wave's `codegen all` regenerates). Still nothing in CI builds `web`. Verification: once at the end of the wave (battery, corpus, 9-target gate), per the user's 2026-09-04 instruction.
