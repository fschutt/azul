...
- [x] 9b-ii-b-i-b-i-a DONE 2026-09-04 (context-menu half): `get_first_hovered_node_for(seat_id)` reads the seat's own hover point (`InputPointId::for_seat`; the primary getter is now the seat-0 case), and both seat right-click paths open the context menu from it: a seat pen's barrel EDGE in `handle_tablet_frame_for_seat` (was-right captured before the seat's state is overwritten, parity with the primary pen frame) and a seat's right button in `handle_seat_pointer_button` (consumed before the state change, parity with `handle_pointer_button`). Until this a seat's barrel/right button opened nothing: the only opener read the primary's hover. Evidence: host check EXIT=0; azul-dll tests 2071 passed; 8/8 target gate EXIT=0 (x86_64-unknown-linux-gnu compiles the Wayland shell). No headless test: the Wayland window cannot be constructed off-device, and the corpus has no context-menu op; logged as blind Wayland like the rest of the seat work.
  - [x] 9b-ii-b-i-b-i-a-i DONE 2026-09-04 (blind Wayland), the earlier "single device" reading corrected: `zwp_primary_selection_device_manager_v1.get_device` takes a `wl_seat`, so each NON-primary seat now gets its own device (`try_init_seat_primary_selections`, called when a seat appears and when the manager arrives, so global order does not matter; dropped with the seat in `handle_seat_gone`). A seat's left release (`handle_seat_pointer_button`) and pen-up (`handle_tablet_frame_for_seat`, `was_left` captured beside `was_right`) publish THAT seat's selection (`seat_selected_text`) through THAT seat's device with THAT seat's serial (`publish_seat_primary_selection`, one source per seat replaced per publish; the source's `send` serves the text keyed by the source proxy, `cancelled` frees it). The primary's PRIMARY is untouched. Evidence: 9/9 target gate EXIT=0 (x86_64-unknown-linux-gnu compiles the Wayland shell), host check EXIT=0, dll 2073 passed. Not run on a two-seat compositor.
    - [ ] 9b-ii-b-i-b-i-a-i-a A seat's MIDDLE-CLICK PASTE still reads the primary's offer: the seat device's `data_offer`/`selection` events are destroyed unread. Wiring it needs a per-seat offer + `receive` through the seat device and a seat-caret insert (`record_text_input_for_seat` exists), which is a second item; logged, not guessed.
