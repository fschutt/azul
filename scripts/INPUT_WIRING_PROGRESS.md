...
- [x] 9d-android DONE 2026-09-04 (blind Android): `AndroidWindow::handle_set_pointer_lock` overrides the trait's refuse-by-default with pointer CAPTURE - `set_pointer_capture` (JNI, `NativeTextBridge.setPointerCapture`: `requestFocus` + `requestPointerCapture` / `releasePointerCapture` on the input view, API 26 guard, UI thread). Until this Android answered "no" to every `CallbackInfo::set_pointer_lock`, so `RawMouseMotion` and the pad touch surface (8f-i-a-ii) were unreachable there. Evidence: 9/9 gate EXIT=0, `javac` against android-34 EXIT=0.
  - [ ] 9d-android-a The capture takes only if the window is focused, and the framework reports the truth ASYNCHRONOUSLY (`View.onPointerCaptureChange`). `handle_set_pointer_lock` answers "requested on API 26+" synchronously, and a capture the system takes away (window loses focus) is not fed back into the window state's pointer-lock flag, so `RawMouseMotion` could stay gated on for a lost capture until the app's next `set_pointer_lock(false)`. Needs a `nativeOnPointerCaptureChanged` -> flag flip + `PointerLockChange` emission on the Android shell; the flag-flip seam was not read here, so it is logged rather than invented.
