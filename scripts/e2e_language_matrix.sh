...
lang_php() {
  ...
  # Reuse an already-loadable extension instead of an incremental rebuild.
  # ext-php-rs's module registration is generated by a proc-macro that does
  # NOT survive Cargo's incremental relink cleanly — an incremental rebuild
  # over a prior build (esp. one killed mid-flight) yields a stale
  # zend_module_entry that PHP rejects with "Invalid library". If a valid
  # extension already exists (php loads it), skip the rebuild; a from-scratch
  # target dir (fresh CI, or `rm -rf target/phpext`) always builds correctly.
  if [ -f "$EXT_LIB" ] && php -d extension="$EXT_LIB" -m 2>/dev/null | grep -qi 'azul-dll'; then
    echo "[php] reusing already-loadable extension at $EXT_LIB" >"$f"
  else
  # Build the extension (debug-server so the AZ_E2E runner is compiled in).
  # Wipe the prior build first: incremental ext-php-rs relinks produce the
  # stale-module bug above, so a clean per-run build is the reliable path.
  ...
  # macOS: a freshly-linked dylib's ad-hoc signature is rejected by PHP's
  # hardened runtime on immediate load ("Invalid library"); re-sign it (same
  # class as the libazul.dylib re-sign the other FFI langs need).
  [ "$(uname -s)" = "Darwin" ] && [ -f "$EXT_LIB" ] && codesign -f -s - "$EXT_LIB" >/dev/null 2>&1
  fi
  if [ ! -f "$EXT_LIB" ]; then
