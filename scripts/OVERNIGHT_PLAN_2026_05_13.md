...
- [⊘] **J.1** Java lifted to a type-driven detector (`smart_callback_setter_info`): "method has 3 args (self, RefAny, callback_wrapper_struct) AND callback_wrapper_name is in HOST_INVOKER_KINDS AND args[2].type_name == callback_wrapper_name". With this, today only Button.with_on_click matches (it's the only widget where the callback arg is the *wrapper struct* directly — every other widget uses the fn-pointer typedef form). Widgets using the typedef form (CheckBox.with_on_toggle, etc.) are not generalized yet — they need an extra `.cb` field extract bridge. Kotlin/C#/Ruby/Node/Lua/OCaml still on the per-binding `if s.name == "Button"` hardcode; not lifted yet. *(this iteration: Java lifted; other 6 bindings open follow-up)*
