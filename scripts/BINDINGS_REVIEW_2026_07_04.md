...
fix = codegen work):
1. ~~C++ enum CONSTANTS not aliased~~ **FIXED 2026-07-04**: every unit enum now emits `namespace Update { inline constexpr AzUpdate RefreshDom = AzUpdate_RefreshDom; … }` (cpp03: `static const`; cpp11/14: `constexpr`) — spelled `Update::RefreshDom`/`ButtonType::Primary`, typed as the raw C enum so it flows through C fn-ptr returns/args with no casts. This REPLACES the old `using Update = AzUpdate;` type alias for unit enums (namespace and type can't share a name; nothing in-tree used the alias as a type); union enums keep their alias. Old `AzUpdate_RefreshDom` spellings keep working. Emitters: lang_cpp/common.rs (generate_enum_constants_namespace), cpp03/cpp11/cpp17/cpp20 generate_enum_wrapper + cpp14 inline loop.
2. ~~C++ callback registration takes raw fn-ptr typedefs~~ **PARTIALLY FIXED 2026-07-04**: all 146 callback fn-ptr typedefs get non-prefixed aliases (`using CallbackType = AzCallbackType;` etc., emitted after the forward decls) and wrapper method signatures now spell the non-prefixed alias (common.rs arg_to_cpp_type_ex). NOT fixable without a trampoline layer: user callback DEFINITIONS must keep raw C parameter/return types (`AzUpdate on_click(AzRefAny, AzCallbackInfo)`) because fn-ptr types must match the C signature exactly and `RefAny`/`CallbackInfo`/`Dom` are wrapper classes with different layout.
3. ~~C++23 toStdExpected / C++17 toStdOptional yield raw Az payloads~~ **FIXED 2026-07-04**: both now yield wrapper classes when one exists (`std::expected<Url, UrlParseError>`, `std::optional<String>`), constructed from the moved-out payload (ownership transfers; Option/Result zeroed). Non-copy Option payloads get a consuming `&&`-qualified toStdOptional (mirrors toStdExpected); Copy payloads keep the const form; raw payloads (primitives, plain enums) unchanged. cpp20.rs emit_cpp23_result_extras + common.rs emit_option_to_std_optional (used by cpp17+cpp20/23).
