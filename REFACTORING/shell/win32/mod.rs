    ...
    return if msg == WM_NCCREATE {
        ...
        let r = match msg {
            ...
            WM_PAINT => {
                ...
                eprintln!("[WM_PAINT] OpenGL context made current");

                let mut rect: RECT = mem::zeroed();
                GetClientRect(hwnd, &mut rect);
                eprintln!("[WM_PAINT] Window rect: {}x{}", rect.width(), rect.height());
                ...
                eprintln!("[WM_PAINT] Flushing scene builder...");
                current_window.render_api.flush_scene_builder();
                eprintln!("[WM_PAINT] Scene builder flushed");
                ...
                eprintln!("[WM_PAINT] OpenGL state configured");
                ...
                // Render
                eprintln!("[WM_PAINT] Calling renderer.update() and render()...");
                if let Some(r) = current_window.renderer.as_mut() {
                    r.update();
                    let render_result = r.render(framebuffer_size, 0);
                    eprintln!("[WM_PAINT] Render result: {:?}", render_result);
                } else {
                    eprintln!("[WM_PAINT] WARNING: No renderer available!");
                }

                eprintln!("[WM_PAINT] Swapping buffers...");
                SwapBuffers(hDC);
                eprintln!("[WM_PAINT] Buffers swapped");
