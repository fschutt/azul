...
pub struct GlContext {
    ...
    pub gl_functions: Option<GlFunctions>,
}
...
impl GlContext {
    pub fn new(
        ...
        let egl_surface = unsafe { (egl.eglCreateWindowSurface)(egl_display, config, egl_window as _, std::ptr::null()) };
        if egl_surface.is_null() {
            return Err(WindowError::PlatformError("eglCreateWindowSurface failed".into()));
        }

        unsafe { (egl.eglMakeCurrent)(egl_display, egl_surface, egl_surface, egl_context) };

        let gl_functions = GlFunctions::initialize(&egl)
            .map_err(|e| WindowError::PlatformError(format!("Failed to load GL functions: {}", e)))?;
        ...
        Ok(Self {
            ...
            gl_functions: Some(gl_functions),
        })
    }

    pub fn swap_buffers(&self) -> Result<(), WindowError> {
        if let (Some(egl), Some(display), Some(surface)) = (self.egl.as_ref(), self.egl_display, self.egl_surface) {
...
impl GlFunctions {
    ...
    pub fn load(&mut self, egl: &Rc<Egl>) {
        fn get_func(egl: &Egl, s: &str, lib: &Option<super::dlopen::Library>) -> *mut c_void {
            ...
            if !addr.is_null() { return addr as *mut _; }
            if let Some(lib) = lib {
                if let Some(addr) = lib.get(s) { return addr as *mut _; }
            }
            std::ptr::null_mut()
        }
