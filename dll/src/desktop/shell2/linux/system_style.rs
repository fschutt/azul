...
fn discover_linux_extras(style: &mut azul_css::system::SystemStyle) {
    // Icon theme
    ...
    if let Some(layout) = gsettings_get("org.gnome.desktop.wm.preferences", "button-layout") {
        ...
        style.metrics.titlebar.button_side = if is_right {
            azul_css::system::TitlebarButtonSide::Right
        } else {
            azul_css::system::TitlebarButtonSide::Left
        };
    }
    ...
    if let Some(v) = gsettings_get("org.gnome.desktop.interface", "toolbar-style") {
        style.visual_hints.toolbar_style = match v.trim() {
            "text" => azul_css::system::ToolbarStyle::TextOnly,
            "both" => azul_css::system::ToolbarStyle::TextBelowIcon,
            "both-horiz" => azul_css::system::ToolbarStyle::TextBesideIcon,
            _ => azul_css::system::ToolbarStyle::IconsOnly,
        };
    }
...
pub(crate) fn discover() -> azul_css::system::SystemStyle {
    ...
    {
        style.platform = azul_css::system::Platform::Linux(azul_css::system::detect_linux_desktop_env());
        style.language = azul_css::system::detect_system_language();
    }
