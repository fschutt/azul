...
impl MenuProtocol {
    ...
    pub fn update_menu(&self, groups: Vec<DbusMenuGroup>) -> Result<(), GnomeMenuError> {
        ...
        menu_groups.clear();
        for group in groups {
            debug_log(&format!("Registering menu group {} with {} items", 
                group.group_id, group.items.len()));
    ...
    pub fn handle_start(&self, subscriptions: Vec<u32>) -> Result<Vec<DbusMenuGroup>, GnomeMenuError> {
        debug_log(&format!("Start method called with subscriptions: {:?}", subscriptions));
        ...
        for group_id in subscriptions {
            if let Some(group) = menu_groups.get(&group_id) {
                ...
                debug_log(&format!("Warning: Subscription for unknown group {}", group_id));
            }
        }
    ...
    pub fn handle_end(&self, subscriptions: Vec<u32>) -> Result<(), GnomeMenuError> {
        debug_log(&format!("End method called with subscriptions: {:?}", subscriptions));
    ...
    pub fn register_with_dbus(&self, connection: &super::DbusConnection) -> Result<(), GnomeMenuError> {
        ...
        #[cfg(all(target_os = "linux", feature = "gnome-menus"))]
        {
            ...
            use dbus::arg::{Variant, Dict, RefArg};
            use std::sync::Arc;
            ...
            let interface = factory
                ...
                .add_m(factory.method("Start", (), move |m| {
                    let subscriptions: Vec<u32> = m.msg.read1()
                    ...
                    for group_id in subscriptions {
                        if let Some(group) = groups.get(&group_id) {
                            ...
                            result.push((group.group_id, group.menu_id, Vec::<(String, Variant<Box<dyn RefArg>>)>::new()));
                        }
                    }
                    
                    Ok(vec![m.msg.method_return().append1(result)])
                }).outarg::<Vec<(u32, u32, Vec<(String, Variant<Box<dyn RefArg>>)>)>, _>("menus"))
            ...
            let tree = factory.tree(()).add(
                factory.object_path(menubar_path, ())
                    .introspectable()
                    .add(interface)
            );
