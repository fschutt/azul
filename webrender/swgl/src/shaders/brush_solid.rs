// swgl_shaders/src/shaders/brush_solid.rs
...
// Corresponds to brush_solid_common
#[derive(Clone, Debug, Default)]
struct BrushSolidCommon {
...
// Corresponds to brush_solid_vert
#[derive(Clone, Debug, Default)]
struct BrushSolidVert {
    common: BrushSolidCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
// Represents the output of the vertex shader passed to the fragment shader.
// Empty for this shader as v_color is flat and doesn't need interpolation.
#[repr(C)]
...
impl BrushSolidVert {
    ...
    fn brush_shader_main_vs(
        ...
        self.brush_vs(context, ph.specific_prim_address, ph.user_data);
    }

    fn brush_vs(&mut self, context: &ShaderContext, prim_address: i32, prim_user_data: ivec4) {
        // A SolidBrush primitive is just a single vec4 color.
        let prim = context.fetch_from_gpu_cache_1(prim_address);
        let opacity = (prim_user_data.x as f32) / 65535.0;
        self.common.v_color = prim * opacity;
    }
}

impl VertexShader for BrushSolidVert {
    fn init_batch(&mut self, _context: &mut ShaderContext) {
        // Corresponds to bind_textures(), but this shader has no textures.
    }
...
// Corresponds to brush_solid_frag
#[derive(Clone, Debug, Default)]
struct BrushSolidFrag {
    vert: BrushSolidVert,
    // No varying inputs, as v_color is flat
}
...
impl FragmentShader for BrushSolidFrag {
    fn read_interp_inputs(&mut self, _init: *const u8, _step: *const u8) {}
    fn read_perspective_inputs(&mut self, _init: *const u8, _step: *const u8, _w: f32) {}
    ...
    fn run_perspective(&mut self, context: &mut ShaderContext, _next_w: &[f32; 4]) {
       self.run(context); // No perspective logic
    }
    
    fn skip_perspective(&mut self, _steps: i32, _next_w: &[f32; 4]) {
        // No-op
    }
...
// Corresponds to brush_solid_program
#[derive(Clone, Debug, Default)]
pub struct BrushSolidProgram {
    frag: BrushSolidFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(BrushSolidProgram::default())
}

impl Program for BrushSolidProgram {
