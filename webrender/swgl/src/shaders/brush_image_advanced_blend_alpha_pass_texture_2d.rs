...
use glam::{bvec2, ivec4, mat4, vec2, vec3, vec4, BVec2, Mat4, Vec2, Vec4};
use std::ffi::CStr;
use std::mem;
use super::strcmp;
...
struct BrushImageAdvancedBlendAlphaPassTexture2DCommon {
...
#[derive(Clone, Debug, Default)]
struct BrushImageAdvancedBlendAlphaPassTexture2DVert {
    common: BrushImageAdvancedBlendAlphaPassTexture2DCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
impl BrushImageAdvancedBlendAlphaPassTexture2DVert {
    #[inline(always)]
    ...
    fn brush_vs(
        ...
        let mut local_image_data_color = image_data.color;
        let opacity = (prim_user_data.z as f32) / 65535.0;

        match blend_mode {
            0 => local_image_data_color.w *= opacity,
            _ => local_image_data_color *= opacity,
        }

        match color_mode {
            0 | 2 => {
                // swgl_blendDropShadow(image_data.color); // This is a backend state call
                self.common.v_mask_swizzle = vec2(1.0, 0.0);
                self.common.v_color = vec4::ONE;
            },
            4 => {
                self.common.v_mask_swizzle = vec2(1.0, 0.0);
                self.common.v_color = local_image_data_color;
            },
            3 => {
                self.common.v_mask_swizzle = vec2(1.0, 0.0);
                self.common.v_color = vec4::splat(local_image_data_color.w);
            },
            1 => {
                self.common.v_mask_swizzle = vec2(local_image_data_color.w, 0.0);
                self.common.v_color = local_image_data_color;
            },
            5 => {
                self.common.v_mask_swizzle = vec2(-local_image_data_color.w, local_image_data_color.w);
                self.common.v_color = local_image_data_color;
            },
            _ => {
...
impl VertexShader for BrushImageAdvancedBlendAlphaPassTexture2DVert {
...
#[derive(Clone, Debug, Default)]
struct BrushImageAdvancedBlendAlphaPassTexture2DFrag {
    vert: BrushImageAdvancedBlendAlphaPassTexture2DVert,
...
impl BrushImageAdvancedBlendAlphaPassTexture2DFrag {
    ...
    fn main(&self, context: &ShaderContext) -> vec4 {
        ...
        let mut frag_color = self.vert.common.v_color * alpha_mask;
...
impl FragmentShader for BrushImageAdvancedBlendAlphaPassTexture2DFrag {
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        if self.vert.common.v_mask_swizzle != vec2(1.0, 0.0) {
            return 0;
        }
        ...
        if self.vert.common.v_color != vec4::ONE {
            context.commit_texture_repeat_color_rgba8(
                SamplerId::SColor0, 
                uv, 
                self.vert.common.v_tile_repeat_bounds,
                self.vert.common.v_uv_bounds, 
                self.vert.common.v_uv_sample_bounds, 
                self.vert.common.v_color
            );
        } else {
            context.commit_texture_repeat_rgba8(
                SamplerId::SColor0, 
                uv, 
                self.vert.common.v_tile_repeat_bounds,
                self.vert.common.v_uv_bounds, 
...
#[derive(Clone, Debug, Default)]
pub struct BrushImageAdvancedBlendAlphaPassTexture2DProgram {
    frag: BrushImageAdvancedBlendAlphaPassTexture2DFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(BrushImageAdvancedBlendAlphaPassTexture2DProgram::default())
}

impl Program for BrushImageAdvancedBlendAlphaPassTexture2DProgram {
