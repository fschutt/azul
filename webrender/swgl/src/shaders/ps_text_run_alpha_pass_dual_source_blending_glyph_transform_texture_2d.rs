...
use glam::{ivec2, ivec4, mat2, mat4, vec2, vec3, vec4, Mat2, Mat4, Vec2, Vec3, Vec4};
use std::ffi::CStr;
use std::mem;
use super::strcmp;
...
#[derive(Clone, Debug, Default)]
struct PsTextRunVert {
    common: PsTextRunCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
    v_uv: vec2,
    gl_position: vec4,
    gl_clip_distance: [f32; 4],
}

#[repr(C)]
#[derive(Default, Copy, Clone)]
struct InterpOutputs {
    swgl_clip_distance: vec4,
    v_uv: vec2,
}
...
impl PsTextRunVert {
    ...
    fn main(&mut self, context: &mut ShaderContext) {
        ...
        let subpx_dir = (instance.flags >> 8) & 0xFF;
        let color_mode = instance.flags & 0xFF;

        let text = self.fetch_text_run(ph.specific_prim_address, context);
        let text_offset = ph.local_rect.p1;
        let mut glyph = self.fetch_glyph(ph.specific_prim_address, glyph_index, context);
        glyph.offset += ph.local_rect.p0;

        let res = self.fetch_glyph_resource(instance.resource_address, context);

        let snap_bias = self.get_snap_bias(subpx_dir);
        let glyph_transform = Mat2::from_cols(transform.m.x_axis.xy(), transform.m.y_axis.xy()) * task.device_pixel_scale;
        let glyph_translation = transform.m.w_axis.xy() * task.device_pixel_scale;
        let glyph_transform_inv = glyph_transform.inverse();

        let raster_glyph_offset = (glyph_transform * glyph.offset + snap_bias).floor();
        let raster_text_offset = ((glyph_transform * text_offset) + glyph_translation + 0.5).floor() - glyph_translation;

        let glyph_origin = res.offset + raster_glyph_offset + raster_text_offset;
        let glyph_rect = RectWithEndpoint {
            p0: glyph_origin,
            p1: glyph_origin + (res.uv_rect.zw() - res.uv_rect.xy()),
        };

        let local_rect = context.transform_rect_mat2(glyph_rect, glyph_transform_inv);
        let mut local_pos = local_rect.p0.lerp(local_rect.p1, self.a_position);

        if context.rect_inside_rect(local_rect, ph.local_clip_rect) {
            local_pos = glyph_transform_inv * (glyph_rect.p0.lerp(glyph_rect.p1, self.a_position));
        }
        ...
        let f = ((glyph_transform * vi.local_pos) - glyph_rect.p0) / context.rect_size(glyph_rect);
        self.gl_clip_distance[0] = f.x;
        self.gl_clip_distance[1] = f.y;
        self.gl_clip_distance[2] = 1.0 - f.x;
        self.gl_clip_distance[3] = 1.0 - f.y;
        ...
        match color_mode {
            ...
            2 => {
                context.swgl_blend_drop_shadow(text.color);
            ...
            1 => {
                context.swgl_blend_subpixel_text(text.color);
                self.common.v_mask_swizzle = vec3(1.0, 0.0, 0.0);
                self.common.v_color = vec4::ONE;
            }
            _ => {
        ...
        let st0 = res.uv_rect.xy() / texture_size;
        let st1 = res.uv_rect.zw() / texture_size;

        self.v_uv = st0.lerp(st1, f);
        self.common.v_uv_bounds = (res.uv_rect + vec4(0.5, 0.5, -0.5, -0.5)) / texture_size.extend(texture_size).xyxy();
    }

    fn fetch_text_run(&self, address: i32, context: &ShaderContext) -> TextRun {
        TextRun { color: context.fetch_from_gpu_cache_1(address) }
    }

    fn fetch_glyph(&self, specific_prim_address: i32, glyph_index: i32, context: &ShaderContext) -> Glyph {
        let glyph_address = specific_prim_address + 1 + (glyph_index / 2);
        let data = context.fetch_from_gpu_cache_1(glyph_address);
        let glyph_offset = if (glyph_index % 2) == 1 { data.zw() } else { data.xy() };
        Glyph { offset: glyph_offset }
    }

    fn fetch_glyph_resource(&self, address: i32, context: &ShaderContext) -> GlyphResource {
    ...
    fn get_snap_bias(&self, subpx_dir: i32) -> vec2 {
        match subpx_dir {
            1 => vec2(0.125, 0.5),
            2 => vec2(0.5, 0.125),
            3 => vec2::splat(0.125),
...
impl VertexShader for PsTextRunVert {
    ...
    fn run_primitive(&mut self, context: &mut ShaderContext, interps: *mut u8, interp_stride: usize) {
        ...
        unsafe {
            ...
            for _ in 0..4 {
                ...
                (*dest_ptr).swgl_clip_distance = self.gl_clip_distance.into();
...
impl PsTextRunFrag {
    fn main(&self, context: &ShaderContext) -> (vec4, vec4) {
        ...
        let frag_color = self.vert.common.v_color * mask;
        let frag_blend = (mask * self.vert.common.v_mask_swizzle.x) + (vec4::splat(mask.a) * self.vert.common.v_mask_swizzle.y);

        let clip_mask = 1.0;
        (frag_color * clip_mask, frag_blend * clip_mask)
    }
}

impl FragmentShader for PsTextRunFrag {
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        ...
        context.commit_texture_linear_rgba8(SamplerId::SColor0, self.v_uv, self.vert.common.v_uv_bounds);
        1
    }
}
...
#[derive(Clone, Debug, Default)]
pub struct PsTextRunAlphaPassDualSourceBlendingGlyphTransformTexture2DProgram {
    frag: PsTextRunFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(PsTextRunAlphaPassDualSourceBlendingGlyphTransformTexture2DProgram::default())
}

impl Program for PsTextRunAlphaPassDualSourceBlendingGlyphTransformTexture2DProgram {
