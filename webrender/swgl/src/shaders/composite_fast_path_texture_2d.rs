...
struct CompositeFastPathTexture2DCommon {
...
impl AttribLocations {
    ...
    fn get_loc(&self, name: &CStr) -> i32 {
        if strcmp(name, "aPosition") { self.a_position as i32 }
        else if strcmp(name, "aDeviceRect") { self.a_device_rect as i32 }
        else if strcmp(name, "aDeviceClipRect") { self.a_device_clip_rect as i32 }
        else if strcmp(name, "aParams") { self.a_params as i32 }
        else if strcmp(name, "aFlip") { self.a_flip as i32 }
        else if strcmp(name, "aUvRect0") { self.a_uv_rect0 as i32 }
        else { -1 }
    }
}
...
#[derive(Clone, Debug, Default)]
struct CompositeFastPathTexture2DVert {
    common: CompositeFastPathTexture2DCommon,
    // Inputs
    a_position: vec2,
    a_device_rect: vec4,
...
impl CompositeFastPathTexture2DVert {
    #[inline(always)]
    fn main(&mut self, context: &ShaderContext) {
        ...
        // aSourceRectType is encoded in aParams.y
...
impl VertexShader for CompositeFastPathTexture2DVert {
    ...
    fn load_attribs(&mut self, attribs: &[&VertexAttrib], start: u32, instance: i32, _count: i32) {
        unsafe {
            ...
            let instance_stride = |attrib: &VertexAttrib| if attrib.divisor == 0 { 0 } else { attrib.stride * instance as usize };

            let device_rect_ptr = (a_device_rect_attrib.data as *const u8).add(instance_stride(a_device_rect_attrib)) as *const Vec4;
            self.a_device_rect = *device_rect_ptr;
            let device_clip_rect_ptr = (a_device_clip_rect_attrib.data as *const u8).add(instance_stride(a_device_clip_rect_attrib)) as *const Vec4;
            self.a_device_clip_rect = *device_clip_rect_ptr;
            let params_ptr = (a_params_attrib.data as *const u8).add(instance_stride(a_params_attrib)) as *const Vec4;
            self.a_params = *params_ptr;
            let flip_ptr = (a_flip_attrib.data as *const u8).add(instance_stride(a_flip_attrib)) as *const Vec2;
            self.a_flip = *flip_ptr;
            let uv_rect0_ptr = (a_uv_rect0_attrib.data as *const u8).add(instance_stride(a_uv_rect0_attrib)) as *const Vec4;
...
#[derive(Clone, Debug, Default)]
struct CompositeFastPathTexture2DFrag {
    vert: CompositeFastPathTexture2DVert,
    // Varying inputs
    v_uv: vec2,
...
impl CompositeFastPathTexture2DFrag {
    #[inline(always)]
    fn main(&self, context: &ShaderContext) -> vec4 {
        context.texture(SamplerId::SColor0, self.v_uv)
    }
}

impl FragmentShader for CompositeFastPathTexture2DFrag {
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        let uv_bounds = vec4(0.0, 0.0, 1.0, 1.0); // Hardcoded in C++ version
        context.commit_texture_rgba8(SamplerId::SColor0, self.v_uv, uv_bounds);
        1
    }
}
...
#[derive(Clone, Debug, Default)]
pub struct CompositeFastPathTexture2DProgram {
    frag: CompositeFastPathTexture2DFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(CompositeFastPathTexture2DProgram::default())
}

impl Program for CompositeFastPathTexture2DProgram {
    ...
    fn get_uniform(&self, name: &CStr) -> i32 {
        ...
        if strcmp(name, "uTransform") { return 2; }
        -1
    }
