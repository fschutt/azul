// swgl_shaders/src/shaders/composite_texture_2d_yuv.rs
...
struct CompositeTexture2DYuvCommon {
    attrib_locations: AttribLocations,
    u_transform: mat4,
    // Flat varyings
    v_ycbcr_bias: vec3,
    v_rgb_from_debiased_ycbcr: mat3,
    v_yuv_format: ivec2, // Renamed from vFormat to be more descriptive
...
#[derive(Clone, Debug, Default)]
struct CompositeTexture2DYuvVert {
    common: CompositeTexture2DYuvCommon,
    // Inputs
    a_position: vec2,
    a_device_rect: vec4,
...
impl CompositeTexture2DYuvVert {
    fn main(&mut self, context: &ShaderContext) {
        ...
        self.write_uv_rect(self.a_uv_rect0.xy(), self.a_uv_rect0.zw(), uv, context.texture_size(SamplerId::SColor0, 0), &mut self.v_uv_y, &mut self.common.v_uv_bounds_y);
        self.write_uv_rect(self.a_uv_rect1.xy(), self.a_uv_rect1.zw(), uv, context.texture_size(SamplerId::SColor1, 0), &mut self.v_uv_u, &mut self.common.v_uv_bounds_u);
        self.write_uv_rect(self.a_uv_rect2.xy(), self.a_uv_rect2.zw(), uv, context.texture_size(SamplerId::SColor2, 0), &mut self.v_uv_v, &mut self.common.v_uv_bounds_v);
...
impl VertexShader for CompositeTexture2DYuvVert {
    ...
    fn load_attribs(&mut self, attribs: &[&VertexAttrib], start: u32, instance: i32, _count: i32) {
        unsafe {
            ...
            let a_uv_rect1_attrib = &*attribs[self.common.attrib_locations.a_uv_rect1];
            let a_uv_rect2_attrib = &*attribs[self.common.attrib_locations.a_uv_rect2];

            let base = instance as usize;
            self.a_device_rect = *(a_device_rect_attrib.data as *const Vec4).add(base);
            self.a_device_clip_rect = *(a_device_clip_rect_attrib.data as *const Vec4).add(base);
            self.a_params = *(a_params_attrib.data as *const Vec4).add(base);
            self.a_flip = *(a_flip_attrib.data as *const Vec2).add(base);
            self.a_uv_rect0 = *(a_uv_rect0_attrib.data as *const Vec4).add(base);
            self.a_uv_rect1 = *(a_uv_rect1_attrib.data as *const Vec4).add(base);
            self.a_uv_rect2 = *(a_uv_rect2_attrib.data as *const Vec4).add(base);
        }
    }
...
#[derive(Clone, Debug, Default)]
struct CompositeTexture2DYuvFrag {
    vert: CompositeTexture2DYuvVert,
...
impl CompositeTexture2DYuvFrag {
...
impl FragmentShader for CompositeTexture2DYuvFrag {
...
#[derive(Clone, Debug, Default)]
pub struct CompositeTexture2DYuvProgram {
    frag: CompositeTexture2DYuvFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(CompositeTexture2DYuvProgram::default())
}

impl Program for CompositeTexture2DYuvProgram {
    ...
    fn get_uniform(&self, name: &CStr) -> i32 {
        ...
        if strcmp(name, "sColor1") { return 2; }
        if strcmp(name, "sColor2") { return 3; }
