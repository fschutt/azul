// swgl_shaders/src/shaders/ps_text_run_alpha_pass_texture_2d.rs

use crate::common::*;
use crate::common_types::*;
use crate::traits::{FragmentShader, Program, VertexShader};
use glam::{ivec2, ivec4, mat4, vec2, vec3, vec4, Mat4, Vec2, Vec4};
use std::ffi::CStr;
use std::mem;
use super::strcmp;
...
struct PsTextRunAlphaPassTexture2DCommon {
...
#[derive(Clone, Debug, Default)]
struct PsTextRunAlphaPassTexture2DVert {
    common: PsTextRunAlphaPassTexture2DCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
impl PsTextRunAlphaPassTexture2DVert {
    fn main(&mut self, context: &mut ShaderContext) {
        ...
        let glyph = self.fetch_glyph(ph.specific_prim_address, glyph_index, context);
        let glyph_offset = glyph.offset + ph.local_rect.p0;
        let res = self.fetch_glyph_resource(instance.resource_address, context);

        let snap_bias = self.get_snap_bias(subpx_dir);
        let raster_scale = (ph.user_data.x as f32) / 65535.0;
        let glyph_raster_scale = raster_scale * task.device_pixel_scale;
        let glyph_scale_inv = res.scale / glyph_raster_scale;

        let raster_glyph_offset =
            ((glyph_offset * glyph_raster_scale) + snap_bias).floor() / res.scale;
        let glyph_origin =
            (glyph_scale_inv * (res.offset + raster_glyph_offset)) + text_offset;

        let glyph_rect = RectWithEndpoint {
            p0: glyph_origin,
            p1: glyph_origin + (glyph_scale_inv * (res.uv_rect.zw() - res.uv_rect.xy())),
        };
    ...
    fn fetch_glyph(&self, specific_prim_address: i32, glyph_index: i32, context: &ShaderContext) -> Glyph {
        let glyph_address = specific_prim_address + 1 + (glyph_index as u32 / 2) as i32;
    ...
    fn get_snap_bias(&self, subpx_dir: i32) -> vec2 {
        match subpx_dir {
            1 => vec2::new(0.125, 0.5),
            2 => vec2::new(0.5, 0.125),
            3 => vec2::splat(0.125),
            _ => vec2::splat(0.5),
        }
    }
...
impl VertexShader for PsTextRunAlphaPassTexture2DVert {
...
#[derive(Clone, Debug, Default)]
struct PsTextRunAlphaPassTexture2DFrag {
    vert: PsTextRunAlphaPassTexture2DVert,
...
impl PsTextRunAlphaPassTexture2DFrag {
    fn main(&self, context: &ShaderContext) -> vec4 {
        let frag_color = self.text_fs(context);
    ...
    fn text_fs(&self, context: &ShaderContext) -> vec4 {
        ...
        mask.truncate() = (mask.rgb() * self.vert.common.v_mask_swizzle.x) + (vec3::splat(mask.a) * self.vert.common.v_mask_swizzle.y);
...
impl FragmentShader for PsTextRunAlphaPassTexture2DFrag {
...
#[derive(Clone, Debug, Default)]
pub struct PsTextRunAlphaPassTexture2DProgram {
    frag: PsTextRunAlphaPassTexture2DFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(PsTextRunAlphaPassTexture2DProgram::default())
}

impl Program for PsTextRunAlphaPassTexture2DProgram {
