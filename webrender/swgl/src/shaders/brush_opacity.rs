// swgl_shaders/src/shaders/brush_opacity.rs
...
// Corresponds to brush_opacity_common
#[derive(Clone, Debug, Default)]
struct BrushOpacityCommon {
...
// Corresponds to brush_opacity_vert
#[derive(Clone, Debug, Default)]
struct BrushOpacityVert {
    common: BrushOpacityCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
impl BrushOpacityVert {
    ...
    fn brush_shader_main_vs(
        ...
        let local_pos = adjusted_segment_rect
            .p0
            .lerp(adjusted_segment_rect.p1, self.a_position);
    ...
    fn brush_vs(
        ...
        self.common.v_uv_sample_bounds = vec4(uv0.x + 0.5, uv0.y + 0.5, uv1.x - 0.5, uv1.y - 0.5)
...
impl VertexShader for BrushOpacityVert {
    fn init_batch(&mut self, _context: &mut ShaderContext) {}

    fn load_attribs(
        &mut self,
        _attribs: &[&VertexAttrib],
        _start: u32,
        _instance: i32,
        _count: i32,
    ) {
    ...
    fn run_primitive(
        ...
        // Store interpolated outputs
        unsafe {
            let mut dest_ptr = interps as *mut InterpOutputs;
            // In a real SIMD implementation, this would iterate through lanes
            for _ in 0..4 {
...
// Corresponds to brush_opacity_frag
#[derive(Clone, Debug, Default)]
struct BrushOpacityFrag {
    vert: BrushOpacityVert,
    // Varying inputs from rasterizer
    v_uv: vec2,
}

impl BrushOpacityFrag {
    fn main(&self, context: &ShaderContext) -> vec4 {
        // Placeholder for gl_FragCoord.w
        let gl_frag_coord_w = 1.0;

        let perspective_divisor = (1.0 - self.vert.common.v_opacity_perspective_vec.y) * gl_frag_coord_w
        ...
        let alpha = self.vert.common.v_opacity_perspective_vec.x;
        
        color * alpha
    }
}

impl FragmentShader for BrushOpacityFrag {
    fn read_interp_inputs(&mut self, _init: *const u8, _step: *const u8) {
        // Placeholder
    }

    fn run(&mut self, context: &ShaderContext) {
        let _color = self.main(context);
    ...
    fn draw_span_rgba8(&mut self, _context: &mut ShaderContext) -> i32 {
        // Placeholder for fast path
        // let perspective_divisor = ...
        // let uv = self.v_uv * perspective_divisor;
        // context.commit_texture_linear_color_rgba8(SamplerId::SColor0, uv, self.vert.common.v_uv_sample_bounds, self.vert.common.v_opacity_perspective_vec.x);
        1
    }
}

// Corresponds to brush_opacity_program
#[derive(Clone, Debug, Default)]
pub struct BrushOpacityProgram {
    frag: BrushOpacityFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(BrushOpacityProgram::default())
}

impl Program for BrushOpacityProgram {
