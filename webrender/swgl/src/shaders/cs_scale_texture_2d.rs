...
// Corresponds to cs_scale_TEXTURE_2D_common
#[derive(Clone, Debug, Default)]
struct CsScaleTexture2DCommon {
...
// Corresponds to cs_scale_TEXTURE_2D_vert
#[derive(Clone, Debug, Default)]
struct CsScaleTexture2DVert {
    common: CsScaleTexture2DCommon,
    // Inputs
    a_position: vec2,
...
impl VertexShader for CsScaleTexture2DVert {
    ...
    fn load_attribs(&mut self, attribs: &[&VertexAttrib], start: u32, instance: i32, _count: i32) {
        unsafe {
            ...
            let a_scale_target_rect_attrib = &*attribs[self.common.attrib_locations.a_scale_target_rect];
            let a_scale_source_rect_attrib = &*attribs[self.common.attrib_locations.a_scale_source_rect];
            let a_source_rect_type_attrib = &*attribs[self.common.attrib_locations.a_source_rect_type];

            let pos_ptr = (a_pos_attrib.data as *const u8).add(a_pos_attrib.stride * start as usize) as *const Vec2;
            self.a_position = *pos_ptr;
            let target_rect_ptr = (a_scale_target_rect_attrib.data as *const u8).add(a_scale_target_rect_attrib.stride * instance as usize) as *const Vec4;
            self.a_scale_target_rect = *target_rect_ptr;
            let source_rect_ptr = (a_scale_source_rect_attrib.data as *const u8).add(a_scale_source_rect_attrib.stride * instance as usize) as *const Vec4;
            self.a_scale_source_rect = *source_rect_ptr;
            let source_rect_type_ptr = (a_source_rect_type_attrib.data as *const u8).add(a_source_rect_type_attrib.stride * instance as usize) as *const f32;
            self.a_source_rect_type = *source_rect_type_ptr;
        }
    }
...
// Corresponds to cs_scale_TEXTURE_2D_frag
#[derive(Clone, Debug, Default)]
struct CsScaleTexture2DFrag {
    vert: CsScaleTexture2DVert,
    // Varying inputs
    v_uv: vec2,
...
impl CsScaleTexture2DFrag {
    ...
    fn main(&self, context: &ShaderContext) -> vec4 {
        ...
        context.texture(SamplerId::SColor0, st)
    }
}

impl FragmentShader for CsScaleTexture2DFrag {
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        context.commit_texture_linear_rgba8(SamplerId::SColor0, self.v_uv, self.vert.common.v_uv_rect);
        1
    }
}
...
// Corresponds to cs_scale_TEXTURE_2D_program
#[derive(Clone, Debug, Default)]
pub struct CsScaleTexture2DProgram {
    frag: CsScaleTexture2DFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(CsScaleTexture2DProgram::default())
}

impl Program for CsScaleTexture2DProgram {
