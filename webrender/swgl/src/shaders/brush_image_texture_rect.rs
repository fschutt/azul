// swgl_shaders/src/shaders/brush_image_texture_rect.rs
...
// Corresponds to brush_image_TEXTURE_RECT_common
#[derive(Clone, Debug, Default)]
struct BrushImageTextureRectCommon {
...
// Corresponds to brush_image_TEXTURE_RECT_vert
#[derive(Clone, Debug, Default)]
struct BrushImageTextureRectVert {
    common: BrushImageTextureRectCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
impl BrushImageTextureRectVert {
    ...
    // This logic appears to be a copy-paste of a repetition shader but is translated faithfully.
    fn brush_vs(
        &mut self,
        context: &ShaderContext,
        vi: VertexInfo,
        ...
        let texture_size = vec2::ONE; // TEXTURE_RECT uses unnormalized coords, this is for the division step
        ...
        self.v_uv *= repeat.xy();
        // The `normalized_offset` logic from other shaders is missing here in the original.
...
impl VertexShader for BrushImageTextureRectVert {
...
// Corresponds to brush_image_TEXTURE_RECT_frag
#[derive(Clone, Debug, Default)]
struct BrushImageTextureRectFrag {
    vert: BrushImageTextureRectVert,
    // Varying inputs from rasterizer
    v_uv: vec2,
    // For storing interpolation steps
    interp_step: InterpOutputs,
    interp_perspective: InterpOutputs,
}

impl BrushImageTextureRectFrag {
    fn compute_repeated_uvs(&self, perspective_divisor: f32) -> vec2 {
        self.v_uv * perspective_divisor + self.vert.common.v_uv_bounds.xy()
    }
...
impl FragmentShader for BrushImageTextureRectFrag {
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        let gl_frag_coord_w = 1.0; // Placeholder, would get from rasterizer state.
        let perspective_divisor = (1.0 - self.vert.common.v_perspective.x) * gl_frag_coord_w + self.vert.common.v_perspective.x;
        let uv = self.compute_repeated_uvs(perspective_divisor);
        context.commit_texture_rgba8_rect(SamplerId::SColor0, uv, self.vert.common.v_uv_sample_bounds);
        1
    }
}

impl BrushImageTextureRectFrag {
    // Helper to advance interpolants for a block of pixels.
    fn step_interp_inputs(&mut self, steps: i32) {
        let chunks = steps as f32 * 0.25;
        self.v_uv += self.interp_step.v_uv * chunks;
    }

    // Helper to advance interpolants with perspective correction.
...
// Corresponds to brush_image_TEXTURE_RECT_program
#[derive(Clone, Debug, Default)]
pub struct BrushImageTextureRectProgram {
    frag: BrushImageTextureRectFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(BrushImageTextureRectProgram::default())
}

impl Program for BrushImageTextureRectProgram {
