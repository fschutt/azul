// swgl_shaders/src/shaders/cs_linear_gradient.rs
...
// Corresponds to cs_linear_gradient_common
#[derive(Clone, Debug, Default)]
struct CsLinearGradientCommon {
...
impl AttribLocations {
    fn bind_loc(&mut self, name: &CStr, index: i32) {
        ...
        else if strcmp(name, "aStartPoint") { self.a_start_point = index as usize; }
        else if strcmp(name, "aEndPoint") { self.a_end_point = index as usize; }
    ...
    fn get_loc(&self, name: &CStr) -> i32 {
        ...
        else if strcmp(name, "aStartPoint") { if self.a_start_point != NULL_ATTRIB { self.a_start_point as i32 } else { -1 } }
        else if strcmp(name, "aEndPoint") { if self.a_end_point != NULL_ATTRIB { self.a_end_point as i32 } else { -1 } }
...
// Corresponds to cs_linear_gradient_vert
#[derive(Clone, Debug, Default)]
struct CsLinearGradientVert {
    common: CsLinearGradientCommon,
    // Inputs
    a_position: vec2,
    a_task_rect: vec4,
    a_start_point: vec2,
...
impl CsLinearGradientVert {
    ...
    fn main(&mut self) {
        ...
        self.v_pos = self.a_position * self.a_scale;

        let dir = self.a_end_point - self.a_start_point;
        self.common.v_scale_dir = dir / dir.dot(dir);
        self.common.v_start_offset.x = self.a_start_point.dot(self.common.v_scale_dir);
        self.common.v_scale_dir *= self.a_task_rect.zw() - self.a_task_rect.xy();
...
impl VertexShader for CsLinearGradientVert {
    ...
    fn load_attribs(&mut self, attribs: &[&VertexAttrib], start: u32, instance: i32, _count: i32) {
        unsafe {
            let pos_attrib = &*attribs[self.common.attrib_locations.a_position];
            let task_rect_attrib = &*attribs[self.common.attrib_locations.a_task_rect];
            let start_point_attrib = &*attribs[self.common.attrib_locations.a_start_point];
            let end_point_attrib = &*attribs[self.common.attrib_locations.a_end_point];
            let scale_attrib = &*attribs[self.common.attrib_locations.a_scale];
            let extend_mode_attrib = &*attribs[self.common.attrib_locations.a_extend_mode];
            let stops_addr_attrib = &*attribs[self.common.attrib_locations.a_gradient_stops_address];

            // Per-vertex
            let pos_ptr = (pos_attrib.data as *const u8).add(pos_attrib.stride * start as usize) as *const Vec2;
            self.a_position = *pos_ptr;

            // Per-instance (flat)
            let base_ptr = |attrib: &VertexAttrib| (attrib.data as *const u8).add(attrib.stride * instance as usize);

            self.a_task_rect = *(base_ptr(task_rect_attrib) as *const Vec4);
            self.a_start_point = *(base_ptr(start_point_attrib) as *const Vec2);
            self.a_end_point = *(base_ptr(end_point_attrib) as *const Vec2);
            self.a_scale = *(base_ptr(scale_attrib) as *const Vec2);
            self.a_extend_mode = *(base_ptr(extend_mode_attrib) as *const i32);
            self.a_gradient_stops_address = *(base_ptr(stops_addr_attrib) as *const i32);
        }
    }
...
// Corresponds to cs_linear_gradient_frag
#[derive(Clone, Debug, Default)]
struct CsLinearGradientFrag {
    vert: CsLinearGradientVert,
    // Varying inputs
    v_pos: vec2,
...
impl CsLinearGradientFrag {
    ...
    fn main(&self, context: &ShaderContext) -> vec4 {
        let offset = self.v_pos.dot(self.vert.common.v_scale_dir) - self.vert.common.v_start_offset.x;
        self.sample_gradient(offset, context)
    }
}

impl FragmentShader for CsLinearGradientFrag {
...
// Corresponds to cs_linear_gradient_program
#[derive(Clone, Debug, Default)]
pub struct CsLinearGradientProgram {
    frag: CsLinearGradientFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(CsLinearGradientProgram::default())
}

impl Program for CsLinearGradientProgram {
