...
use glam::{ivec2, ivec4, mat2, mat4, vec2, vec3, vec4, Mat2, Mat4, Vec2, Vec4};
use std::ffi::CStr;
use std::mem;
use super::strcmp;
...
struct PsTextRunAlphaPassGlyphTransformTexture2DCommon {
...
#[derive(Clone, Debug, Default)]
struct PsTextRunAlphaPassGlyphTransformTexture2DVert {
    common: PsTextRunAlphaPassGlyphTransformTexture2DCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
struct InterpOutputs {
    gl_clip_distance: vec4,
    v_uv: vec2,
}

impl PsTextRunAlphaPassGlyphTransformTexture2DVert {
    fn main(&mut self, context: &mut ShaderContext) {
        ...
        let subpx_dir = (instance.flags >> 8) & 255;
        ...
        let mut glyph = context.fetch_glyph(ph.specific_prim_address, glyph_index);
        glyph.offset += ph.local_rect.p0;

        let res = context.fetch_glyph_resource(instance.resource_address);
        let snap_bias = context.get_snap_bias(subpx_dir);
        ...
        let local_rect = context.transform_rect(glyph_rect, glyph_transform_inv);
...
impl VertexShader for PsTextRunAlphaPassGlyphTransformTexture2DVert {
    ...
    fn run_primitive(&mut self, context: &mut ShaderContext, interps: *mut u8, interp_stride: usize) {
        ...
        unsafe {
            ...
            for _ in 0..4 {
                (*dest_ptr).gl_clip_distance = self.gl_clip_distance.into();
...
#[derive(Clone, Debug, Default)]
struct PsTextRunAlphaPassGlyphTransformTexture2DFrag {
    vert: PsTextRunAlphaPassGlyphTransformTexture2DVert,
...
impl PsTextRunAlphaPassGlyphTransformTexture2DFrag {
    fn text_fs(&self, context: &ShaderContext) -> vec4 {
    ...
    fn main(&self, context: &ShaderContext) -> vec4 {
        let mut frag_color = self.text_fs(context);
        let clip_mask = self.do_clip(context);
        frag_color *= clip_mask;
        frag_color
    }
}

impl FragmentShader for PsTextRunAlphaPassGlyphTransformTexture2DFrag {
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        if context.swgl_is_texture_r8(SamplerId::SColor0) {
            context.swgl_commit_texture_linear_color_r8_to_rgba8(SamplerId::SColor0, self.v_uv, self.vert.common.v_uv_bounds, self.vert.common.v_color);
        } else {
            context.swgl_commit_texture_linear_color_rgba8(SamplerId::SColor0, self.v_uv, self.vert.common.v_uv_bounds, self.vert.common.v_color);
        }
        1
    }
}
...
#[derive(Clone, Debug, Default)]
pub struct PsTextRunAlphaPassGlyphTransformTexture2DProgram {
    frag: PsTextRunAlphaPassGlyphTransformTexture2DFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(PsTextRunAlphaPassGlyphTransformTexture2DProgram::default())
}

impl Program for PsTextRunAlphaPassGlyphTransformTexture2DProgram {
