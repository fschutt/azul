...
use glam::{ivec4, mat4, vec2, vec3, vec4, Mat4, Vec2, Vec4};
use std::ffi::CStr;
use std::mem;
use super::strcmp;

// Corresponds to brush_image_ALPHA_PASS_TEXTURE_RECT_common
#[derive(Clone, Debug, Default)]
struct BrushImageAlphaPassTextureRectCommon {
...
// Corresponds to brush_image_ALPHA_PASS_TEXTURE_RECT_vert
#[derive(Clone, Debug, Default)]
struct BrushImageAlphaPassTextureRectVert {
    common: BrushImageAlphaPassTextureRectCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
impl BrushImageAlphaPassTextureRectVert {
    ...
    fn brush_vs(
        ...
        let (_res_uv_rect, _res_user_data) = context.fetch_image_source(specific_resource_address);
    
        let mut uv0 = _res_uv_rect.p0;
        let mut uv1 = _res_uv_rect.p1;
        ...
        if (brush_flags & 2) != 0 {
            ...
            if (brush_flags & 512) != 0 {
                let uv_size = _res_uv_rect.p1 - _res_uv_rect.p0;
                uv0 = _res_uv_rect.p0 + (segment_data.xy() * uv_size);
                uv1 = _res_uv_rect.p0 + (segment_data.zw() * uv_size);
            }
        }
        ...
        let normalized_offset = vec2::ZERO;
...
impl VertexShader for BrushImageAlphaPassTextureRectVert {
...
// Corresponds to brush_image_ALPHA_PASS_TEXTURE_RECT_frag
#[derive(Clone, Debug, Default)]
struct BrushImageAlphaPassTextureRectFrag {
    vert: BrushImageAlphaPassTextureRectVert,
...
impl BrushImageAlphaPassTextureRectFrag {
    ...
    fn main(&self, context: &ShaderContext) -> vec4 {
        ...
        frag_color * self.do_clip(context)
    }
}

impl FragmentShader for BrushImageAlphaPassTextureRectFrag {
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        ...
        if self.vert.common.v_color != vec4::ONE {
            context.commit_texture_color_rgba8_rect(SamplerId::SColor0, uv, self.vert.common.v_uv_sample_bounds, self.vert.common.v_color);
        } else {
...
// Corresponds to brush_image_ALPHA_PASS_TEXTURE_RECT_program
#[derive(Clone, Debug, Default)]
pub struct BrushImageAlphaPassTextureRectProgram {
    frag: BrushImageAlphaPassTextureRectFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(BrushImageAlphaPassTextureRectProgram::default())
}

impl Program for BrushImageAlphaPassTextureRectProgram {
