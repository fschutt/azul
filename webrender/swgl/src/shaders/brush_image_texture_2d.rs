...
// Corresponds to brush_image_TEXTURE_2D_common
#[derive(Clone, Debug, Default)]
struct BrushImageTexture2DCommon {
...
// Corresponds to brush_image_TEXTURE_2D_vert
#[derive(Clone, Debug, Default)]
struct BrushImageTexture2DVert {
    common: BrushImageTexture2DCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
impl BrushImageTexture2DVert {
    ...
    fn brush_shader_main_vs(
        ...
        // This shader variant does not have antialiasing.
    ...
    fn brush_vs(
        ...
        let _image_data = context.fetch_image_data(prim_address);
        ...
        self.common.v_uv_sample_bounds = vec4(min_uv.x + 0.5, min_uv.y + 0.5, max_uv.x - 0.5, max_uv.y - 0.5) / texture_size.extend(texture_size).xyxy();
        ...
        self.v_uv = uv0.lerp(uv1, f);
        
        if perspective_interpolate == 0.0 {
            self.v_uv *= vi.world_pos.w;
        }

        self.common.v_uv_bounds = vec4(min_uv.x, min_uv.y, max_uv.x, max_uv.y) / texture_size.extend(texture_size).xyxy();
    }
}

impl VertexShader for BrushImageTexture2DVert {
...
// Corresponds to brush_image_TEXTURE_2D_frag
#[derive(Clone, Debug, Default)]
struct BrushImageTexture2DFrag {
    vert: BrushImageTexture2DVert,
...
impl BrushImageTexture2DFrag {
    fn compute_repeated_uvs(&self, perspective_divisor: f32) -> vec2 {
        self.v_uv * perspective_divisor
    }
...
impl FragmentShader for BrushImageTexture2DFrag {
...
// Corresponds to brush_image_TEXTURE_2D_program
#[derive(Clone, Debug, Default)]
pub struct BrushImageTexture2DProgram {
    frag: BrushImageTexture2DFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(BrushImageTexture2DProgram::default())
}

impl Program for BrushImageTexture2DProgram {
