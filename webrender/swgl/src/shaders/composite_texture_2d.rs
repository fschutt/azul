...
// Corresponds to composite_TEXTURE_2D_common
#[derive(Clone, Debug, Default)]
struct CompositeTexture2DCommon {
...
impl AttribLocations {
    ...
    fn get_loc(&self, name: &CStr) -> i32 {
        ...
        else if strcmp(name, "aColor") { self.a_color as i32 }
...
// Corresponds to composite_TEXTURE_2D_vert
#[derive(Clone, Debug, Default)]
struct CompositeTexture2DVert {
    common: CompositeTexture2DCommon,
    // Inputs
    a_position: vec2,
    a_device_rect: vec4,
...
impl CompositeTexture2DVert {
    fn main(&mut self, context: &ShaderContext) {
        ...
        let mut uv_bounds = vec4(
            self.a_uv_rect0.x.min(self.a_uv_rect0.z),
            self.a_uv_rect0.y.min(self.a_uv_rect0.w),
            self.a_uv_rect0.x.max(self.a_uv_rect0.z),
            self.a_uv_rect0.y.max(self.a_uv_rect0.w),
        );
...
impl VertexShader for CompositeTexture2DVert {
    ...
    fn load_attribs(&mut self, attribs: &[&VertexAttrib], start: u32, instance: i32, _count: i32) {
        unsafe {
            ...
            let uv0_ptr = (a_uv_rect0_attrib.data as *const u8).add(a_uv_rect0_attrib.stride * instance as usize) as *const Vec4;
            self.a_uv_rect0 = *uv0_ptr;
        }
    }
...
// Corresponds to composite_TEXTURE_2D_frag
#[derive(Clone, Debug, Default)]
struct CompositeTexture2DFrag {
    vert: CompositeTexture2DVert,
...
impl CompositeTexture2DFrag {
    fn main(&self, context: &ShaderContext) -> vec4 {
        ...
        let texel = context.texture(SamplerId::SColor0, uv);
        self.vert.common.v_color * texel
    }
}

impl FragmentShader for CompositeTexture2DFrag {
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        ...
        let uv_bounds = self.vert.common.v_uv_bounds;

        if color != vec4::ONE {
            context.commit_texture_color_rgba8(SamplerId::SColor0, self.v_uv, uv_bounds, color);
        } else {
...
// Corresponds to composite_TEXTURE_2D_program
#[derive(Clone, Debug, Default)]
pub struct CompositeTexture2DProgram {
    frag: CompositeTexture2DFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(CompositeTexture2DProgram::default())
}

impl Program for CompositeTexture2DProgram {
