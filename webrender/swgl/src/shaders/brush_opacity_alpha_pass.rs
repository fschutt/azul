// swgl_shaders/src/shaders/brush_opacity_alpha_pass.rs
...
// Corresponds to brush_opacity_ALPHA_PASS_common
#[derive(Clone, Debug, Default)]
struct BrushOpacityAlphaPassCommon {
...
// Corresponds to brush_opacity_ALPHA_PASS_vert
#[derive(Clone, Debug, Default)]
struct BrushOpacityAlphaPassVert {
    common: BrushOpacityAlphaPassCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
impl BrushOpacityAlphaPassVert {
    ...
    fn brush_shader_main_vs(
        ...
        if antialiased {
            // In a real implementation, this would modify the rasterizer's state.
...
impl VertexShader for BrushOpacityAlphaPassVert {
    ...
    fn load_attribs(
        ...
        // Simulating the behavior of `load_attrib` and `load_flat_attrib`.
        // This requires unsafe pointer operations to read from the raw buffers
        // provided by the rasterizer backend.
        unsafe {
    ...
    fn run_primitive(
        ...
        // Store the single interpolated value (v_uv) for each of the four
        // vertices of the conceptual quad primitive into the interpolants buffer.
        unsafe {
            let mut dest_ptr = interps as *mut InterpOutputs;
            for _ in 0..4 {
                // In a scalar implementation, we write the same calculated value for all 4 vertices.
                // The rasterizer will then use these four points to set up interpolation across the primitive.
...
// Corresponds to brush_opacity_ALPHA_PASS_frag
#[derive(Clone, Debug, Default)]
struct BrushOpacityAlphaPassFrag {
    vert: BrushOpacityAlphaPassVert,
    // Varying inputs from rasterizer
    v_uv: vec2,
    // For storing interpolation steps
    interp_step: InterpOutputs,
    interp_perspective: InterpOutputs, // For w-correction
}

impl BrushOpacityAlphaPassFrag {
    fn main(&self, context: &ShaderContext) -> vec4 {
        // Placeholder for gl_FragCoord.w, provided by the rasterizer
        ...
        // In the original C++, these are separate functions that might be optimized out.
        // We include them here for a 1:1 translation of logic.
        let antialias_alpha = self.antialias_brush();
        let clip_alpha = self.do_clip();

        let final_color = color * alpha * antialias_alpha * clip_alpha;
        final_color
    }
    ...
    fn do_clip(&self) -> f32 {
        // In a real implementation, this would sample the clip mask texture
        // and return the alpha value.
        // let clip_alpha = context.texture(SamplerId::SClipMask, ...).r;
        1.0
    }
}

impl FragmentShader for BrushOpacityAlphaPassFrag {
    fn read_interp_inputs(&mut self, init: *const u8, step: *const u8) {
        unsafe {
            ...
            // This setup corresponds to `init_interp` and storing the step.
            self.v_uv = init.v_uv;
    ...
    fn run(&mut self, context: &mut ShaderContext) {
        ...
        context.write_output(color); // Assumes context has a method to write to framebuffer
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        // A real implementation would call a highly optimized backend-specific function.
        // We mirror the C++ logic by calling a function on the context.
        ...
        1 // Return 1 to indicate the fast path was taken.
    }
}
...
impl BrushOpacityAlphaPassFrag {
    ...
    fn step_perspective_inputs(&mut self, steps: i32, next_w: &[f32; 4]) {
        let chunks = steps as f32 * 0.25;
        let inv_w = 1.0 / next_w[0]; // Simplified: just use the first w for the next block
...
// Corresponds to brush_opacity_ALPHA_PASS_program
#[derive(Clone, Debug, Default)]
pub struct BrushOpacityAlphaPassProgram {
    frag: BrushOpacityAlphaPassFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(BrushOpacityAlphaPassProgram::default())
}

impl Program for BrushOpacityAlphaPassProgram {
