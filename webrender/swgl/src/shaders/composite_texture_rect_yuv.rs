// swgl_shaders/src/shaders/composite_texture_rect_yuv.rs
...
const RGB_FROM_YUV_REC601: Mat3 = Mat3::from_cols_array(&[
    1.0, 1.0, 1.0, 0.0, -0.17207, 0.886, 0.701, -0.35707, 0.0,
]);
const RGB_FROM_YUV_REC709: Mat3 = Mat3::from_cols_array(&[
    1.0, 1.0, 1.0, 0.0, -0.09366, 0.9278, 0.7874, -0.23406, 0.0,
]);
const RGB_FROM_YUV_REC2020: Mat3 = Mat3::from_cols_array(&[
    1.0, 1.0, 1.0, 0.0, -0.08228, 0.9407, 0.7373, -0.28568, 0.0,
]);
const RGB_FROM_YUV_GBR_IDENTITY: Mat3 = Mat3::from_cols_array(&[
...
struct CompositeTextureRectYuvCommon {
...
impl AttribLocations {
    fn bind_loc(&mut self, name: &CStr, index: i32) {
        ...
        else if strcmp(name, "aUvRect1") { self.a_uv_rect1 = index as usize; }
        else if strcmp(name, "aUvRect2") { self.a_uv_rect2 = index as usize; }
    }
    ...
    fn get_loc(&self, name: &CStr) -> i32 {
        ...
        else if strcmp(name, "aUvRect1") { if self.a_uv_rect1 != NULL_ATTRIB { self.a_uv_rect1 as i32 } else { -1 } }
        else if strcmp(name, "aUvRect2") { if self.a_uv_rect2 != NULL_ATTRIB { self.a_uv_rect2 as i32 } else { -1 } }
        else { -1 }
    }
}
...
#[derive(Clone, Debug, Default)]
struct CompositeTextureRectYuvVert {
    common: CompositeTextureRectYuvCommon,
    // Inputs
    a_position: vec2,
    a_device_rect: vec4,
...
impl CompositeTextureRectYuvVert {
    fn main(&mut self) {
        let device_rect = self.a_device_rect.lerp(self.a_device_rect.zwxy(), self.a_flip.extend(self.a_flip.x).xyxy());
        ...
        let uv = (clipped_world_pos - device_rect.xy()) / (device_rect.zw() - device_rect.xy());

        let prim = self.fetch_yuv_primitive();
        ...
        self.common.v_yuv_format.x = prim.yuv_format;

        self.write_uv_rect(self.a_uv_rect0.xy(), self.a_uv_rect0.zw(), uv, &mut self.v_uv_y, &mut self.common.v_uv_bounds_y);
        self.write_uv_rect(self.a_uv_rect1.xy(), self.a_uv_rect1.zw(), uv, &mut self.v_uv_u, &mut self.common.v_uv_bounds_u);
        self.write_uv_rect(self.a_uv_rect2.xy(), self.a_uv_rect2.zw(), uv, &mut self.v_uv_v, &mut self.common.v_uv_bounds_v);

        self.gl_position = self.common.u_transform * vec4::new(clipped_world_pos.x, clipped_world_pos.y, 0.0, 1.0);
    }

    fn fetch_yuv_primitive(&self) -> YuvPrimitive {
        YuvPrimitive {
            channel_bit_depth: self.a_params.w as i32,
            color_space: self.a_params.y as i32,
            yuv_format: self.a_params.z as i32,
        }
    }

    fn write_uv_rect(&self, uv0: vec2, uv1: vec2, f: vec2, uv: &mut vec2, uv_bounds: &mut vec4) {
        *uv = uv0.lerp(uv1, f);
...
impl VertexShader for CompositeTextureRectYuvVert {
    ...
    fn load_attribs(&mut self, attribs: &[&VertexAttrib], start: u32, instance: i32, _count: i32) {
        unsafe {
            ...
            let a_uv0_attrib = &*attribs[self.common.attrib_locations.a_uv_rect0];
            let a_uv1_attrib = &*attribs[self.common.attrib_locations.a_uv_rect1];
            let a_uv2_attrib = &*attribs[self.common.attrib_locations.a_uv_rect2];

            let pos_ptr = (a_pos_attrib.data as *const u8).add(a_pos_attrib.stride * start as usize) as *const Vec2;
            self.a_position = *pos_ptr;
            
            // All other attributes are flat (per-instance)
            let inst_offset = (start / 4) as usize; // Assuming 4 vertices per instance
            self.a_device_rect = *(a_dev_rect_attrib.data as *const Vec4).add(inst_offset);
            self.a_device_clip_rect = *(a_dev_clip_rect_attrib.data as *const Vec4).add(inst_offset);
            self.a_params = *(a_params_attrib.data as *const Vec4).add(inst_offset);
            self.a_flip = *(a_flip_attrib.data as *const Vec2).add(inst_offset);
            self.a_uv_rect0 = *(a_uv0_attrib.data as *const Vec4).add(inst_offset);
            self.a_uv_rect1 = *(a_uv1_attrib.data as *const Vec4).add(inst_offset);
            self.a_uv_rect2 = *(a_uv2_attrib.data as *const Vec4).add(inst_offset);
        }
    }
...
#[derive(Clone, Debug, Default)]
struct CompositeTextureRectYuvFrag {
    vert: CompositeTextureRectYuvVert,
...
impl CompositeTextureRectYuvFrag {
    fn sample_yuv(&self, context: &ShaderContext, in_uv_y: vec2, in_uv_u: vec2, in_uv_v: vec2) -> vec4 {
        ...
        match self.vert.common.v_yuv_format.x {
            3 => { // YUV444, planar
                ...
                ycbcr_sample.x = context.texture_rect(SamplerId::SColor0, uv_y).x;
                ycbcr_sample.y = context.texture_rect(SamplerId::SColor1, uv_u).x;
                ycbcr_sample.z = context.texture_rect(SamplerId::SColor2, uv_v).x;
            },
            0 | 1 | 2 => { // NV12, etc. (Y plane and UV plane)
                ...
                ycbcr_sample.x = context.texture_rect(SamplerId::SColor0, uv_y).x;
                ycbcr_sample.yz() = context.texture_rect(SamplerId::SColor1, uv_uv).rg();
            },
            4 => { // GBR_IDENTITY special case
                let uv_y = in_uv_y.clamp(self.vert.common.v_uv_bounds_y.xy(), self.vert.common.v_uv_bounds_y.zw());
                ycbcr_sample = context.texture_rect(SamplerId::SColor0, uv_y).gbr();
            },
            _ => { // Should not happen
    ...
    fn main(&self, context: &ShaderContext) -> vec4 {
        self.sample_yuv(context, self.v_uv_y, self.v_uv_u, self.v_uv_v)
    }
}

impl FragmentShader for CompositeTextureRectYuvFrag {
    ...
    fn draw_span_rgba8(&mut self, context: &mut ShaderContext) -> i32 {
        if self.vert.common.v_yuv_format.x == 3 {
            // context.swgl_commit_texture_linear_yuv_rect_3plane(...);
        } else if self.vert.common.v_yuv_format.x == 0 || self.vert.common.v_yuv_format.x == 1 {
            // context.swgl_commit_texture_linear_yuv_rect_2plane(...);
        } else if self.vert.common.v_yuv_format.x == 4 {
            // context.swgl_commit_texture_linear_yuv_rect_1plane(...);
        }
        1
    }
}
...
#[derive(Clone, Debug, Default)]
pub struct CompositeTextureRectYuvProgram {
    frag: CompositeTextureRectYuvFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(CompositeTextureRectYuvProgram::default())
}

impl Program for CompositeTextureRectYuvProgram {
    ...
    fn get_uniform(&self, name: &CStr) -> i32 {
        ...
        if strcmp(name, "sColor1") { return 3; }
        if strcmp(name, "sColor2") { return 4; }
