// swgl_shaders/src/shaders/cs_conic_gradient.rs

use crate::common::*;
use crate::common_types::*;
use crate::traits::{FragmentShader, Program, VertexShader};
use glam::{ivec2, mat4, vec2, vec3, vec4, Mat4, Vec2, Vec4};
use std::ffi::CStr;
use std::mem;
use super::strcmp;

// Corresponds to cs_conic_gradient_common
#[derive(Clone, Debug, Default)]
struct CsConicGradientCommon {
...
impl AttribLocations {
    fn bind_loc(&mut self, name: &CStr, index: i32) {
        ...
        else if strcmp(name, "aStartOffset") { self.a_start_offset = index as usize; }
        else if strcmp(name, "aEndOffset") { self.a_end_offset = index as usize; }
        else if strcmp(name, "aAngle") { self.a_angle = index as usize; }
    ...
    fn get_loc(&self, name: &CStr) -> i32 {
        ...
        else if strcmp(name, "aStartOffset") { if self.a_start_offset != NULL_ATTRIB { self.a_start_offset as i32 } else { -1 } }
        else if strcmp(name, "aEndOffset") { if self.a_end_offset != NULL_ATTRIB { self.a_end_offset as i32 } else { -1 } }
        else if strcmp(name, "aAngle") { if self.a_angle != NULL_ATTRIB { self.a_angle as i32 } else { -1 } }
...
// Corresponds to cs_conic_gradient_vert
#[derive(Clone, Debug, Default)]
struct CsConicGradientVert {
    common: CsConicGradientCommon,
    // Inputs
    a_position: vec2,
    a_task_rect: vec4,
    a_center: vec2,
...
impl CsConicGradientVert {
    #[inline(always)]
    fn main(&mut self) {
        let d = self.a_end_offset - self.a_start_offset;
        self.common.v_start_offset_offset_scale_angle_vec.y = if d != 0.0 { 1.0 / d } else { 0.0 };
        ...
        self.common.v_start_offset_offset_scale_angle_vec.z = std::f32::consts::FRAC_PI_2 - self.a_angle;
        self.common.v_start_offset_offset_scale_angle_vec.x = self.a_start_offset * self.common.v_start_offset_offset_scale_angle_vec.y;
        self.common.v_center = self.a_center * self.common.v_start_offset_offset_scale_angle_vec.y;
        self.v_pos = ((self.a_task_rect.zw() - self.a_task_rect.xy()) * self.a_position) * self.common.v_start_offset_offset_scale_angle_vec.y * self.a_scale;
...
impl VertexShader for CsConicGradientVert {
    ...
    fn load_attribs(&mut self, attribs: &[&VertexAttrib], start: u32, instance: i32, _count: i32) {
        unsafe {
            ...
            macro_rules! load_flat {
                ($field:ident, $loc:ident, $type:ty) => {
                    let attrib = &*attribs[self.common.attrib_locations.$loc];
                    let ptr = (attrib.data as *const u8).add(attrib.stride * instance as usize) as *const $type;
                    self.$field = *ptr;
                };
            }
            load_flat!(a_task_rect, a_task_rect, Vec4);
            load_flat!(a_center, a_center, Vec2);
            load_flat!(a_scale, a_scale, Vec2);
            load_flat!(a_start_offset, a_start_offset, f32);
            load_flat!(a_end_offset, a_end_offset, f32);
            load_flat!(a_angle, a_angle, f32);
            load_flat!(a_extend_mode, a_extend_mode, i32);
            load_flat!(a_gradient_stops_address, a_gradient_stops_address, i32);
        }
    }
...
// Corresponds to cs_conic_gradient_frag
#[derive(Clone, Debug, Default)]
struct CsConicGradientFrag {
    vert: CsConicGradientVert,
    v_pos: vec2,
    interp_step: InterpOutputs,
    interp_perspective: InterpOutputs,
}

impl CsConicGradientFrag {
    fn main(&self, context: &ShaderContext) -> vec4 {
        let current_dir = self.v_pos - self.vert.common.v_center;
        let current_angle = current_dir.y.atan2(current_dir.x) + self.vert.common.v_start_offset_offset_scale_angle_vec.z;
...
impl FragmentShader for CsConicGradientFrag {
    ...
    fn draw_span_rgba8(&mut self, _context: &mut ShaderContext) -> i32 {
        0 // No fast path in the original shader.
    }
}
...
// Corresponds to cs_conic_gradient_program
#[derive(Clone, Debug, Default)]
pub struct CsConicGradientProgram {
    frag: CsConicGradientFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(CsConicGradientProgram::default())
}

impl Program for CsConicGradientProgram {
