...
struct BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectCommon {
...
#[derive(Clone, Debug, Default)]
struct BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectVert {
    common: BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectCommon,
    // Inputs
    a_position: vec2,
    a_data: ivec4,
    // Outputs
...
impl BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectVert {
    #[inline(always)]
    ...
    fn brush_shader_main_vs(
        ...
        self.brush_vs(context, vi, ph.specific_prim_address, ph.local_rect, segment_rect, ph.user_data, instance.resource_address, brush_flags, segment_data);
    }
    ...
    fn brush_vs(
        ...
        let texture_size = vec2::ONE; // For TEXTURE_RECT, UVs are not normalized initially
        let (res_uv_rect, _res_user_data) = context.fetch_image_source(specific_resource_address);
        ...
        let normalized_offset = vec2::ZERO.lerp(vec2::ONE - (repeat * 0.5 + 0.5).fract(), centered.into());

        self.v_uv += normalized_offset * (max_uv - min_uv);
        self.v_uv /= texture_size; // no-op

        if perspective_interpolate == 0.0 {
            self.v_uv *= vi.world_pos.w;
        }

        self.common.v_uv_bounds = vec4(0.0, 0.0, context.texture_size_rect(SamplerId::SColor0).x, context.texture_size_rect(SamplerId::SColor0).y);
        ...
        match color_mode {
            0 | 2 => {
                // context.swgl_blend_drop_shadow(image_data.color);
...
impl VertexShader for BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectVert {
...
#[derive(Clone, Debug, Default)]
struct BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectFrag {
    vert: BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectVert,
    v_uv: vec2,
    interp_step: InterpOutputs,
    interp_perspective: InterpOutputs,
}

impl BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectFrag {
    ...
    fn brush_fs(&self, context: &ShaderContext) -> (vec4, vec4) {
        ...
        let uv = repeated_uv.clamp(self.vert.common.v_uv_sample_bounds.xy(), self.vert.common.v_uv_sample_bounds.zw());

        let mut texel = context.texture_rect(SamplerId::SColor0, uv);
        ...
        let frag_color = self.vert.common.v_color * alpha_mask;
        let frag_blend = (alpha_mask * self.vert.common.v_mask_swizzle.x) + (vec4::splat(alpha_mask.a) * self.vert.common.v_mask_swizzle.y);
        
        (frag_color, frag_blend)
    }

    fn main(&self, context: &ShaderContext) -> (vec4, vec4) {
        let (mut frag_color, mut frag_blend) = self.brush_fs(context);
        let clip_alpha = self.do_clip(context);
        frag_color *= clip_alpha;
        frag_blend *= clip_alpha;
        (frag_color, frag_blend)
    }
}

impl FragmentShader for BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectFrag {
...
#[derive(Clone, Debug, Default)]
pub struct BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectProgram {
    frag: BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectFrag,
}

pub fn loader() -> Box<dyn Program> {
    Box::new(BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectProgram::default())
}

impl Program for BrushImageAlphaPassAntialiasingDualSourceBlendingRepetitionTextureRectProgram {
