...
fn write_unoptimized_shaders(
    ...
    for glsl in glsl_files {
        ...
        let mut compressed = Vec::new();
        {
            ...
            brotli::BrotliCompress(&mut raw_source.as_bytes(), &mut compressed, &params).unwrap();
        }
...
fn main() -> Result<(), std::io::Error> {
    ...
    writeln!(
        shader_file,
        "/// Shader sources are gzip-compressed at build time."
    )?;
    writeln!(
        shader_file,
        "/// Use `decompress_shader()` to get the source string."
    )?;
    ...
    writeln!(
        shader_file,
        "pub fn decompress_shader(compressed: &[u8]) -> String {{"
    )?;
    ...
    writeln!(
        shader_file,
        "    ).expect(\"Failed to decompress shader source\");"
    )?;
