...
use api::{
    units::*, BorderRadius, BorderSide, BorderStyle, ColorF, ColorU,
    NormalBorder as ApiNormalBorder, RepeatMode,
};
use euclid::vec2;
...
use crate::{
    clip::ClipNodeId,
    ellipse::Ellipse,
    gpu_types::{BorderInstance, BorderSegment, BrushFlags},
    internal_types::LayoutPrimitiveInfo,
    prim_store::{
        borders::{NormalBorderData, NormalBorderPrim},
        BorderSegmentInfo, BrushSegment, NinePatchDescriptor,
    },
    scene_building::SceneBuilder,
    segment::EdgeAaSegmentMask,
    spatial_tree::SpatialNodeIndex,
    util::{lerp, RectHelpers},
};
...
/// blurrier.
pub fn get_max_scale_for_border(border_data: &NormalBorderData) -> LayoutToDeviceScale {
    let mut r = 1.0;
...
impl NinePatchDescriptor {
    pub fn create_segments(&self, size: LayoutSize) -> Vec<BrushSegment> {
        let rect = LayoutRect::from_size(size);
        ...
        fn add_segment(
            ...
            let mut brush_flags =
                BrushFlags::SEGMENT_RELATIVE | BrushFlags::SEGMENT_TEXEL_RECT | extra_flags;
            ...
            let segment = BrushSegment::new(
                ...
                [uv_rect.uv0.x, uv_rect.uv0.y, uv_rect.uv1.x, uv_rect.uv1.y],
                brush_flags,
            );
