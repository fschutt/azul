...
//!   - Some special counters such as GPU time queries have specific visualizations ignoring
//!     prefixes.
...
use crate::{
    ...
    renderer::{
        init::wr_has_been_initialized, DebugRenderer, FullFrameStats, MAX_VERTEX_TEXTURE_WIDTH,
    },
};
...
static PROFILER_PRESETS: &'static [(&'static str, &'static str)] = &[
    ...
    (
        &"Default",
        &"FPS,|,Slow indicators,_,Time graphs,|,Frame times, ,Transaction times, ,Frame stats, \
          ,Memory, ,Interners,_,GPU time queries,_,Paint phase graph",
    ),
    ...
    (
        &"Transaction times",
        &"DisplayList,Scene building,Content send,API send",
    ),
    // Timing information for per-frame stages.
    (
        &"Frame times",
        &"Frame CPU total,Frame building,Visibility,Prepare,Batching,Glyph resolve,Texture cache \
          update,Shader build time,Renderer,GPU",
    ),
    // Stats about the content of the frame.
    (
        &"Frame stats",
        &"Primitives,Visible primitives,Draw calls,Vertices,Color passes,Alpha passes,Rendered \
          picture tiles,Rasterized glyphs",
    ),
    // Texture cache allocation stats.
    (
        &"Texture cache stats",
        &"Atlas textures mem, Standalone textures mem, Picture tiles mem, Render targets mem, \
          Depth targets mem, Atlas items mem,
        Texture cache standalone pressure, Texture cache eviction count, Texture cache youngest \
          evicted, ,
        Atlas RGBA8 linear pixels, Atlas RGBA8 glyphs pixels, Atlas A8 glyphs pixels, Atlas A8 \
          pixels, Atlas A16 pixels, Atlas RGBA8 nearest pixels,
        Atlas RGBA8 linear textures, Atlas RGBA8 glyphs textures, Atlas A8 glyphs textures, Atlas \
          A8 textures, Atlas A16 textures, Atlas RGBA8 nearest textures,
        Atlas RGBA8 linear pressure, Atlas RGBA8 glyphs pressure, Atlas A8 glyphs pressure, Atlas \
          A8 pressure, Atlas A16 pressure, Atlas RGBA8 nearest pressure,",
    ),
    // Graphs to investigate driver overhead of texture cache updates.
    (
        &"Texture upload perf",
        &"#Texture cache update,#Texture cache upload, ,#Staging CPU allocation,#Staging GPU \
          allocation,#Staging CPU copy,#Staging GPU copy,#Upload time, ,#Upload copy \
          batches,#Rasterized glyphs, ,#Cache texture creation,#Cache texture deletion",
    ),
    // Graphs:
    ...
    (
        &"Time graphs",
        &"#DisplayList,#Scene building,#Blob rasterization, ,#Frame CPU total,#Frame \
          building,#Renderer,#Texture cache update, ,#GPU,",
    ),
    // Useful when investigating render backend bottlenecks.
    (
        &"Backend graphs",
        &"#Frame building, #Visibility, #Prepare, #Batching, #Glyph resolve",
    ),
    // Useful when investigating renderer bottlenecks.
    (
        &"Renderer graphs",
        &"#Rendered picture tiles,#Draw calls,#Rasterized glyphs,#Texture uploads,#Texture \
          uploads mem, ,#Texture cache update,#Renderer,",
    ),
    // Misc:
    (
        &"GPU Memory",
        &"External image mem, Atlas textures mem, Standalone textures mem, Picture tiles mem, \
          Render targets mem, Depth targets mem, Atlas items mem, GPU cache mem, GPU buffer mem, \
          GPU total mem",
    ),
    (
        &"CPU Memory",
        &"Image templates, Image templates mem, Font templates,Font templates mem, DisplayList mem",
    ),
    (&"Memory", &"$CPU,CPU Memory, ,$GPU,GPU Memory"),
    (
        &"Interners",
        "Interned primitives,Interned clips,Interned pictures,Interned text runs,Interned normal \
         borders,Interned image borders,Interned images,Interned YUV images,Interned line \
         decorations,Interned linear gradients,Interned radial gradients,Interned conic \
         gradients,Interned filter data,Interned backdrop renders, Interned backdrop captures",
    ),
    // Gpu sampler queries (need the pref gfx.webrender.debug.gpu-sampler-queries).
    (
        &"GPU samplers",
        &"Alpha targets samplers,Transparent pass samplers,Opaque pass samplers,Total samplers",
    ),
    (
        &"Render reasons",
        &"Reason scene, Reason animated property, Reason resource update, Reason async image, \
          Reason clear resources, Reason APZ, Reason resize, Reason widget, Reason cache flush, \
          Reason snapshot, Reason resource hook, Reason config change, Reason content sync, \
          Reason flush, On vsync, Reason testing, Reason other",
    ),
    (
        &"Slow frame breakdown",
        &"Total slow frames CPU, Total slow frames GPU, Slow: frame build, Slow: upload, Slow: \
          render, Slow: draw calls, Slow: targets, Slow: blobs, Slow: after scene, Slow scroll \
          frames",
    ),
    (
        &"Compositor",
        &"Compositor surface underlays,Compositor surface overlays,Compositor surface blits",
    ),
...
impl Profiler {
    ...
    // Call at the end of every frame, after setting the counter values and before drawing the
    // counters.
