...
use api::{
    units::Au, ColorSpace, ColorU, CompositeOperator, FilterOpGraphPictureBufferId,
    FilterPrimitiveInput, FilterPrimitiveKind, MixBlendMode, PropertyBinding, PropertyBindingId,
    RasterSpace,
};

use crate::{
    filterdata::SFilterData,
    intern::{Handle as InternHandle, InternDebug, Internable, ItemUid},
    internal_types::{
        Filter, FilterGraphNode, FilterGraphOp, FilterGraphPictureReference, LayoutPrimitiveInfo,
        SVGFE_CONVOLVE_VALUES_LIMIT,
    },
    picture::PictureCompositeMode,
    prim_store::{InternablePrimitive, PrimitiveInstanceKind, PrimitiveStore, VectorKey},
        ...
        FilterGraphPictureReferenceKey {
            buffer_id: match pic.buffer_id {
                ...
                FilterOpGraphPictureBufferId::BufferId(id) => {
                    FilterGraphPictureBufferIdKey::BufferId(id)
                }
            },
            subregion: [
            ...
            offset: [Au::from_f32_px(pic.offset.x), Au::from_f32_px(pic.offset.y)],
        }
    }
}
...
pub enum FilterGraphOpKey {
    ...
    SVGFEConvolveMatrixEdgeModeDuplicate {
        order_x: i32,
        order_y: i32,
        kernel: [Au; SVGFE_CONVOLVE_VALUES_LIMIT],
        divisor: Au,
        bias: Au,
        target_x: i32,
        target_y: i32,
    ...
    fn from(op: FilterGraphOp) -> Self {
        match op {
            ...
            FilterGraphOp::SVGFEColorMatrix {
                ...
                FilterGraphOpKey::SVGFEColorMatrix {
            ...
            FilterGraphOp::SVGFEComponentTransferInterned {
                handle,
                creates_pixels,
            } => FilterGraphOpKey::SVGFEComponentTransferInterned {
                handle: handle.uid(),
                creates_pixels,
            },
            ...
            FilterGraphOp::SVGFEConvolveMatrixEdgeModeDuplicate {
                ...
                let mut values: [Au; SVGFE_CONVOLVE_VALUES_LIMIT] =
                    [Au(0); SVGFE_CONVOLVE_VALUES_LIMIT];
            ...
            FilterGraphOp::SVGFEFlood { color } => FilterGraphOpKey::SVGFEFlood {
                color: color.into(),
            },
            ...
            FilterGraphOp::SVGFEOpacity {
                ...
                PropertyBinding::Value(value) => FilterGraphOpKey::SVGFEOpacity {
                    value: Au::from_f32_px(value),
                },
                PropertyBinding::Binding(key, default) => FilterGraphOpKey::SVGFEOpacityBinding {
                    valuebindingid: key.id,
                    value: Au::from_f32_px(default),
                },
            },
                    ...
                    filter_primitives
                        ...
                        .map(|primitive| match primitive.kind {
                            ...
                            FilterPrimitiveKind::Blur(blur) => FilterPrimitiveKey::Blur(
                                primitive.color_space,
                                Au::from_f32_px(blur.width),
                                Au::from_f32_px(blur.height),
                                blur.input,
                            ),
