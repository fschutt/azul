...
use crate::{
    batch::BatchTextures,
    device::{
        Device, DrawTarget, Texture, TextureFilter, TextureFlags, TextureUploader, UploadMethod,
        UploadStagingBuffer,
    },
    gpu_types::CopyInstance,
    internal_types::{
        CacheTextureId, FastHashMap, RenderTargetInfo, Swizzle, TextureCacheUpdate,
        TextureUpdateSource,
    },
    profiler,
    render_api::MemoryReport,
    renderer::{
        Renderer, RendererStats, TextureSampler, VertexArrayKind, TEXTURE_CACHE_DBG_CLEAR_COLOR,
    },
    texture_pack::{FreeRectSlice, GuillotineAllocator},
};
...
pub fn upload_to_texture_cache(
                ...
                TextureUpdateSource::External { id, channel_index } => {
                    ...
                    match handler.lock(id, channel_index).source {
                        ...
                        ExternalImageSource::NativeTexture(eid) => {
                            panic!(
                                "Unexpected external texture {:?} for the texture cache update of \
                                 {:?}",
                                eid, id
                            );
                        }
    ...
    if !batch_upload_copies.is_empty() {
        ...
        if renderer.device.use_draw_calls_for_texture_copy() {
            ...
            // commands (low end intel drivers on Windows for example can take take 100+ ms
            // submitting a few hundred blits). In this case we do the copy with batched
            // draw calls.
