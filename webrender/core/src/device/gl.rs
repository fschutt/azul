...
use api::{
    units::*, BoolParameter, CrashAnnotation, CrashAnnotator, CrashAnnotatorGuard, ImageBufferKind,
    ...
    fn new(device: &Device, name: &'static str, features: &[&'static str]) -> Self {
        ...
        let source_type = match optimized_source {
            Some(source_and_digest) => {
                ...
                // We use the hashes that were computed at build time, and verify it in debug
                // builds.
...
impl Device {
    pub fn new(
        ...
        //   - with glTexStorage, the internal format needs to match the external format, or the
        //     driver would have to do the conversion, which is slow
        //   - on desktop GL, there is no BGRA internal format. However, initializing the textures
        //     with glTexImage as RGBA appears to use BGRA internally, preferring BGRA external data
        //     [4].
        //   - when glTexStorage + BGRA internal format is not supported, and the external data is
        //     BGRA, we have the following options:
        //       1. use glTexImage with RGBA internal format, this costs us VRAM for mipmaps
        //       2. use glTexStorage with RGBA internal format, this costs us the conversion by the
        //          driver
        //       3. pretend we are uploading RGBA and set up the swizzling of the texture unit -
        //          this costs us batch breaks
        //
        ...
        // Prefer 24-bit depth format. While 16-bit depth also works, it may exhaust depth ids
        // easily.
        ...
        let supports_qcom_tiled_rendering = if is_adreno && version_string.contains("V@0490") {
            ...
            // driver version V@0490, so block this extension on that driver version. See bug
            // 1828248.
            false
    ...
    pub fn copy_texture_sub_region(
        ...
        if self.capabilities.supports_copy_image_sub_data {
            assert_ne!(
                ...
                "glCopyImageSubData's behaviour is undefined if src and dst images are identical \
                 and the rectangles overlap."
            );
            unsafe {
    ...
    /// Once uploads have been performed the uploader must be flushed with
    ...
    fn get_pbo(&mut self, device: &mut Device, min_size: usize) -> Result<UploadPBO, String> {
            ...
            if let Some(mut buffer) = self.available_buffers.pop() {
                ...
                match buffer.mapping {
                    ...
                    PBOMapping::Transient(_) => {
                        unreachable!(
                            "Transiently mapped UploadPBO must be unmapped before returning to \
                             pool."
                        );
                    }
        ...
        let mapping = if device.capabilities.supports_buffer_storage && can_recycle {
            ...
            let ptr = device.gl.map_buffer_range(
                ...
                0,
                pbo.reserved_size as _,
                // GL_MAP_COHERENT_BIT doesn't seem to work on Adreno, so use
                // glFlushMappedBufferRange. kvark notes that coherent memory can
                // be faster on some platforms, such as nvidia, so in the future we
                // could choose which to use at run time.
            ...
            let ptr = device.gl.map_buffer_range(
                ...
                0,
                pbo.reserved_size as _,
                // Unlike the above code path, where we are re-mapping a buffer that has previously
                // been unmapped, this buffer has just been created there is no
                // need for GL_MAP_UNSYNCHRONIZED_BIT.
                gl::MAP_WRITE_BIT,
            ) as *mut _;
