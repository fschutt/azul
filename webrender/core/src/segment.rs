...
//! building.
//!
//! - The first step is to determine the segmentation and write the associated GPU data. See
//!   `PrimitiveInstance::build_segments_if_needed` and `write_brush_segment_description` in
//!   `prim_store/mod.rs` which uses the segment builder of this module.
//! - The second step is to generate the mask render tasks. See `BrushSegment::update_clip_task` and
//!   `RenderTask::new_mask`. For each segment that needs a mask, the contribution of all clips that
//!   affect the segment is added to the mask's render task.
//! - Segments are assigned to batches (See `batch.rs`). Segments of a given primitive can be
//!   assigned to different batches.
//!
...
use api::{units::*, BorderRadius, ClipMode};
...
fn emit_segment_if_needed(
    ...
    let segment_rect = LayoutRect {
        min: LayoutPoint::new(x0.to_f32_px(), y0.to_f32_px()),
        max: LayoutPoint::new(x1.to_f32_px(), y1.to_f32_px()),
    };
...
mod test {
    ...
    use api::{
        ...
        BorderRadius, ClipMode,
    };

    use super::{EdgeAaSegmentMask, Segment, SegmentBuilder};
    ...
    fn segment_sorter(s0: &Segment, s1: &Segment) -> cmp::Ordering {
        ...
        ((r0.min.x, r0.min.y, r0.max.x, r0.max.y))
            .partial_cmp(&(r1.min.x, r1.min.y, r1.max.x, r1.max.y))
            .unwrap()
    }
