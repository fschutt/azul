...
use api::{
    ...
    DocumentId, ExternalEvent, GlyphDimensionRequest, GlyphIndexRequest, IdNamespace,
...
use crate::{
    ...
    clip::{ClipIntern, PolygonIntern},
    filterdata::FilterDataIntern,
    frame_builder::FrameBuilderConfig,
    intern::{Internable, Interner, UpdateList},
    internal_types::{FastHashMap, FastHashSet},
    prim_store::{
    ...
    render_api::{ApiMsg, FrameMsg, MemoryReport, ResourceUpdate, SceneMsg, TransactionMsg},
    render_backend::SceneView,
    renderer::{FullFrameStats, PipelineInfo},
    scene::{BuiltScene, Scene, SceneStats},
    scene_building::{SceneBuilder, SceneRecycler},
    spatial_tree::{SceneSpatialTree, SpatialTreeUpdates},
    telemetry::Telemetry,
    util::drain_filter,
    SceneBuilderHooks,
};
...
impl SceneBuilderThread {
        ...
        loop {
            ...
            match self.rx.recv() {
                ...
                Ok(SceneBuilderRequest::Transactions(txns)) => {
                    eprintln!(
                        "[SCENE_BUILDER_THREAD] Received Transactions, count: {}",
                        txns.len()
                    );
    ...
    fn process_transaction(&mut self, mut txn: TransactionMsg) -> Box<BuiltTransaction> {
        ...
        eprintln!(
            "[SCENE_BUILDER_THREAD::process_transaction] Getting document {:?}",
            txn.document_id
        );
        ...
        eprintln!(
            "[SCENE_BUILDER_THREAD::process_transaction] Processing {} scene_ops",
            txn.scene_ops.len()
        );
        ...
        for message in txn.scene_ops.drain(..) {
            match message {
                ...
                SceneMsg::SetDisplayList {
                    ...
                    eprintln!(
                        "[SCENE_BUILDER_THREAD::process_transaction] Processing SetDisplayList \
                         for pipeline {:?}, epoch {:?}",
                        pipeline_id, epoch
                    );
                    ...
                    if self.removed_pipelines.contains(&pipeline_id) {
                        eprintln!(
                            "[SCENE_BUILDER_THREAD::process_transaction] Pipeline {:?} was \
                             removed, skipping",
                            pipeline_id
                        );
                        continue;
                    ...
                    eprintln!(
                        "[SCENE_BUILDER_THREAD::process_transaction] Calling \
                         scene.set_display_list()..."
                    );
                    scene.set_display_list(pipeline_id, epoch, display_list);
                    eprintln!(
                        "[SCENE_BUILDER_THREAD::process_transaction] scene.set_display_list() \
                         RETURNED"
                    );
                }
                SceneMsg::SetRootPipeline(pipeline_id) => {
                    eprintln!(
                        "[SCENE_BUILDER_THREAD::process_transaction] Processing SetRootPipeline \
                         {:?}",
                        pipeline_id
                    );
                ...
                SceneMsg::RemovePipeline(pipeline_id) => {
                    eprintln!(
                        "[SCENE_BUILDER_THREAD::process_transaction] Processing RemovePipeline \
                         {:?}",
                        pipeline_id
                    );
        ...
        eprintln!(
            "[SCENE_BUILDER_THREAD::process_transaction] All scene_ops processed, \
             removed_pipelines cleared"
        );
        ...
        eprintln!(
            "[SCENE_BUILDER_THREAD::process_transaction] rebuild_scene={}, has_root_pipeline={}",
            rebuild_scene,
            scene.has_root_pipeline()
        );

        if scene.has_root_pipeline() && rebuild_scene {
            eprintln!(
                "[SCENE_BUILDER_THREAD::process_transaction] >>>>> Calling SceneBuilder::build() \
                 <<<<<"
            );
            ...
            eprintln!(
                "[SCENE_BUILDER_THREAD::process_transaction] >>>>> SceneBuilder::build() RETURNED \
                 <<<<<"
            );
        ...
        let scene_build_time_ms = profiler::ns_to_ms(precise_time_ns() - scene_build_start);
    ...
    fn forward_built_transactions(&mut self, txns: Vec<Box<BuiltTransaction>>) {
        ...
        if let Some(pipeline_info) = pipeline_info {
            ...
            self.hooks
                .as_ref()
                .unwrap()
                .post_scene_swap(&document_ids, pipeline_info);
