...
- **(B) Wire WindowType::Menu -> xdg_popup** (run.rs:~1228 TODO): the pending-window-create path still makes a fresh xdg_toplevel for `WindowType::Menu` instead of an xdg_popup parented via `xdg_surface.get_popup` + an `xdg_positioner` anchored to `MenuLayoutData::trigger_rect` (+ parent serial for the grab). `try_show_context_menu` (right-click) already runs in handle_pointer_button; `create_menu_popup_options` + `menu_layout_callback` + `menu_renderer::create_menu_dom_with_css` already build the menu as a renderable StyledDom.
