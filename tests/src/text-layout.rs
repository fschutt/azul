#[cfg(test)]
use azul_core::{
    app_resources::{LineCaretIntersection, Word, WordType, Words},
    window::{LogicalPosition, LogicalRect, LogicalSize},
};
#[cfg(test)]
use azul_css::U32Vec;
#[cfg(test)]
use azul_layout::text2::layout::split_text_into_words;
...
fn test_split_words() {
    ...
    fn string_to_vec(s: String) -> U32Vec {
        s.chars().map(|q| q as u32).collect::<Vec<_>>().into()
    }
    ...
    let words_ascii_expected = Words {
        internal_str: ascii_str.clone().into(),
        is_rtl: false,
    ...
    let words_unicode_expected = Words {
        internal_str: unicode_str.clone().into(),
        is_rtl: false,
    ...
    let words_single_str_expected = Words {
        internal_str: single_str.clone().into(),
        is_rtl: false,
...
fn test_caret_intersects_with_holes_1() {
    ...
    let result = LineCaretIntersection::new(
        line_caret_x,
        line_number,
        font_size_px,
        line_height_px,
