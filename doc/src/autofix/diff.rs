...
/// 2. Having a type name ending with "Vec" (e.g., "U8Vec", "SvgPathVec")
fn check_vec_functions(
    ...
    // 2. Inferred from type name ending with "Vec"
    let element_type = if let Some(ref elem_type) = api_info.vec_element_type {
        elem_type.clone()
    } else if type_name.ends_with("Vec") && !type_name.ends_with("VecRef") && !type_name.ends_with("VecRefMut") {
        // Extract element type from name: "SvgPathVec" -> "SvgPath"
