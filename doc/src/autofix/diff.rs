...
fn generate_diff_v2(
    ...
    // Collect types that are required by Vec types for their macro-generated functions
    // These types should NOT be removed even if not directly reachable from function signatures
    let vec_required_types = collect_vec_required_types(current_api_types, &element_to_option_map);
