...
use quote::ToTokens;
use syn::{
    Type, TypeArray, TypeBareFn, TypeParen, TypePath, TypePtr, TypeReference, TypeSlice, TypeTuple,
};
        ...
        match &typedef.kind {
            ...
            TypeDefKind::MacroGenerated {
                ...
                if matches!(kind, MacroGeneratedKind::Vec) {
                    ...
                    // The destructor type is: {VecTypeName}DestructorType (e.g.,
        ...
        match ty {
            Type::Path(type_path) => {
                if let Some(last) = type_path.path.segments.last() {
                            ...
                            if let syn::PathArguments::AngleBracketed(args) = &last.arguments {
                                for arg in &args.args {
                                    if let syn::GenericArgument::Type(inner_ty) = arg {
                                        ...
                                        return self
                                            .check_type_c_compatibility_with_index(inner_ty);
                                    }
                                }
                            }
                            return None;
                    ...
                    // If we get here, the type was NOT found in the index (neither with nor without
                    // Az prefix)
                    match name.as_str() {
                        "Vec" => {
                            return Some(
                                "Vec<T> is not C-compatible. Use a custom Vec type like FooVec \
                                 instead."
                                    .to_string(),
                            );
                        }
                        ...
                        "Arc" | "Rc" => {
                            return Some(
                                "Arc<T>/Rc<T> are not C-compatible. Use a custom \
                    ...
                    if let syn::PathArguments::AngleBracketed(args) = &last.arguments {
                        for arg in &args.args {
                            if let syn::GenericArgument::Type(inner_ty) = arg {
                                if let Some(warning) =
                                    self.check_type_c_compatibility_with_index(inner_ty)
                                {
                                    return Some(warning);
                                }
                            }
            ...
            Type::Reference(_) => Some(
                "References (&T) are not C-compatible. Use pointers (*const T, *mut T) instead."
                    .to_string(),
            ),
            Type::Ptr(type_ptr) => {
            ...
            Type::Array(type_array) => self.check_type_c_compatibility_with_index(&type_array.elem),
            Type::Slice(_) => Some(
                "Slices ([T]) are not C-compatible. Use a pointer with length or a custom slice \
                 type."
                    .to_string(),
            ),
            ...
            Type::BareFn(_) => None,
            Type::Paren(type_paren) => self.check_type_c_compatibility_with_index(&type_paren.elem),
            _ => None,
        }
    }
}
                            ...
                            for arg in &f.sig.inputs {
                                match arg {
                                    ...
                                    syn::FnArg::Typed(pat_type) => {
                                        ...
                                        let ty =
                                            clean_type(&pat_type.ty.to_token_stream().to_string());
                                        params.push(ParameterInfo { name, ty });
                                    }
                                }
                            }
...
fn check_type_c_compatibility(ty: &Type) -> Option<String> {
    match ty {
        Type::Path(type_path) => {
            ...
            if let Some(last) = type_path.path.segments.last() {
                ...
                match name.as_str() {
                    ...
                    "Arc" | "Rc" => {
                        return Some(
                            "Arc<T>/Rc<T> are not C-compatible. Use a custom reference-counted \
                             type."
                                .to_string(),
                        )
                    }
        ...
        Type::Ptr(type_ptr) => {
            ...
            // Use is_known_c_compatible_type to be conservative: unknown types are treated as
            // opaque
            if is_known_c_compatible_type(&type_ptr.elem) {
        ...
        Type::Slice(_) => Some(
            "Slices ([T]) are not C-compatible. Use a pointer with length or a custom slice type."
                .to_string(),
        ),
        Type::Tuple(type_tuple) if !type_tuple.elems.is_empty() => {
            Some("Non-empty tuples are not C-compatible. Use a struct instead.".to_string())
        ...
        Type::Paren(type_paren) => check_type_c_compatibility(&type_paren.elem),
        _ => None,
    }
}

/// Check if a type is C-compatible (returns true if compatible, false if not)
/// This is the inverse of check_type_c_compatibility - returns true if no warning would be
/// generated
        ...
        Type::Path(type_path) => {
            if let Some(last) = type_path.path.segments.last() {
                ...
                match name.as_str() {
                    ...
                    | "u16" | "u32" | "u64" | "u128" | "usize" | "f32" | "f64" | "c_void"
                    | "c_char" | "c_int" | "c_uint" | "c_long" | "c_ulong" => true,
...
fn extract_all_types_from_expr_with_options(
    type_str: &str,
    trace_through_pointers: bool,
) -> Vec<String> {
    // Try to parse as syn::Type
    let ty: Type = match syn::parse_str(type_str) {
        Ok(t) => t,
...
fn extract_types_from_syn_type(ty: &Type, types: &mut Vec<String>, trace_through_pointers: bool) {
    match ty {
        ...
        Type::Ptr(type_ptr) => {
            ...
            // If not tracing through pointers (struct fields), stop here - pointer becomes *const
            // c_void
        }
        ...
        Type::BareFn(type_fn) => {
            // Function pointer: extract argument and return types (always trace through pointers
            // for fn args)
...
fn extract_types_from_type_path(
    type_path: &TypePath,
...
mod tests {
    ...
    fn test_extract_all_types_from_expr() {
        ...
        assert_eq!(
            extract_all_types_from_expr("*const FontCache"),
            Vec::<String>::new()
        );
        assert_eq!(
            extract_all_types_from_expr("*mut FontCache"),
            Vec::<String>::new()
        );

        // Pointers to known non-C-compatible types are also "trace blockers"
        assert_eq!(
            extract_all_types_from_expr("*const String"),
            Vec::<String>::new()
        );
        assert_eq!(
            extract_all_types_from_expr("*mut Vec<i32>"),
            Vec::<String>::new()
        );
        assert_eq!(
            extract_all_types_from_expr("*const FcFontCache"),
            Vec::<String>::new()
        );

        // Pointers to primitives ARE extracted (primitives are known C-compatible)
        assert_eq!(
            extract_all_types_from_expr("*const u8"),
            Vec::<String>::new()
        ); // u8 is primitive, not added
        assert_eq!(
            extract_all_types_from_expr("*mut i32"),
            Vec::<String>::new()
        ); // i32 is primitive
        ...
        assert_eq!(
            extract_all_types_from_expr("Option<FontCache>"),
            vec!["FontCache"]
        );
        assert_eq!(
            extract_all_types_from_expr("Vec<FontCache>"),
            vec!["FontCache"]
        );

        // Box<T> is a "stop type" - it maps to *const c_void, so we don't extract inner type
        assert_eq!(
            extract_all_types_from_expr("Box<FontCache>"),
            Vec::<String>::new()
        );

        // Option<Box<T>> is also a "stop type"
        assert_eq!(
            extract_all_types_from_expr("Option<Box<FontCache>>"),
            Vec::<String>::new()
        );
        ...
        assert_eq!(
            extract_all_types_from_expr("[FontCache; 4]"),
            vec!["FontCache"]
        );
    }
    ...
    fn test_extract_all_types_with_pointer_tracing() {
        ...
        assert_eq!(
            extract_all_types_from_expr("*mut TessellatedGPUSvgNode"),
            Vec::<String>::new()
        );

        // With pointer tracing (for function arguments): extract types behind pointers
        assert_eq!(
            extract_all_types_from_expr_with_options("*const FontCache", true),
            vec!["FontCache"]
        );
        assert_eq!(
            extract_all_types_from_expr_with_options("*mut TessellatedGPUSvgNode", true),
            vec!["TessellatedGPUSvgNode"]
        );
        assert_eq!(
            extract_all_types_from_expr_with_options("*const *const Foo", true),
            vec!["Foo"]
        );

        // With pointer tracing but primitives are still not added
        assert_eq!(
            extract_all_types_from_expr_with_options("*const u8", true),
            Vec::<String>::new()
        );
        assert_eq!(
            extract_all_types_from_expr_with_options("*mut i32", true),
            Vec::<String>::new()
        );
    }
