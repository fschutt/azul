...
impl<'a> TypeResolver<'a> {
    ...
    pub fn resolve_type_with_context(&mut self, type_name: &str, ctx: &ResolutionContext, parent_context: Option<&str>) {
        ...
        let extracted = extract_all_types_from_expr(type_name);
    ...
    fn resolve_referenced_types(&mut self, typedef: &TypeDefinition, ctx: &ResolutionContext) {
        ...
        match &typedef.kind {
            ...
            TypeDefKind::MacroGenerated { base_type, kind, .. } => {
...
fn extract_all_types_from_expr(type_str: &str) -> Vec<String> {
    ...
    extract_types_from_syn_type(&ty, &mut types);
    types
}

/// Extract type names from a parsed syn::Type
fn extract_types_from_syn_type(ty: &Type, types: &mut Vec<String>) {
    match ty {
        Type::Path(type_path) => {
            extract_types_from_type_path(type_path, types);
        }
        Type::Ptr(type_ptr) => {
            // Pointer: check if inner type is KNOWN to be C-compatible
            // If inner type is NOT known to be C-compatible, this pointer becomes *const c_void
            // and we should NOT extract the inner type (it's a "trace blocker")
            // This prevents tracing into opaque types like FcFontCache, ImageCache etc.
            if is_known_c_compatible_type(&type_ptr.elem) {
                // Inner type is known C-compatible (primitive, pointer, etc.), extract it
                extract_types_from_syn_type(&type_ptr.elem, types);
            }
            // If not known C-compatible, don't extract - pointer becomes *const c_void
        }
        Type::Reference(type_ref) => {
            // Reference: extract the inner type
            extract_types_from_syn_type(&type_ref.elem, types);
        }
        Type::Array(type_array) => {
            // Array [T; N]: extract the element type
            extract_types_from_syn_type(&type_array.elem, types);
        }
        Type::Slice(type_slice) => {
            // Slice [T]: extract the element type
            extract_types_from_syn_type(&type_slice.elem, types);
        }
        Type::Tuple(type_tuple) => {
            // Tuple (T1, T2, ...): extract all element types
            for elem in &type_tuple.elems {
                extract_types_from_syn_type(elem, types);
            }
        }
        Type::BareFn(type_fn) => {
            // Function pointer: extract argument and return types
            for arg in &type_fn.inputs {
                extract_types_from_syn_type(&arg.ty, types);
            }
            if let syn::ReturnType::Type(_, ret_ty) = &type_fn.output {
                extract_types_from_syn_type(ret_ty, types);
            }
        }
        Type::Paren(type_paren) => {
            extract_types_from_syn_type(&type_paren.elem, types);
        }
        Type::Group(type_group) => {
            extract_types_from_syn_type(&type_group.elem, types);
        }
...
fn extract_types_from_type_path(type_path: &TypePath, types: &mut Vec<String>) {
            ...
            if let syn::PathArguments::AngleBracketed(args) = &last.arguments {
                for arg in &args.args {
                    if let syn::GenericArgument::Type(inner_ty) = arg {
                        ...
                        extract_types_from_syn_type(inner_ty, types);
                        return;
                    }
                }
            }
