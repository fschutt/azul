...
pub fn generate_api_html(api_data: &ApiData, version: &str) -> String {
    ...
    html.push_str(&format!("<h1>API Documentation - Version {}</h1>\n", version));
    
    // Add version documentation if available
    if let Some(doc) = &version_data.doc {
        html.push_str(&format!("<p class=\"version doc\">{}</p>\n", format_doc(doc)));
    }
    ...
    for (module_name, module) in &version_data.modules {
        ...
        html.push_str(&format!("<h3>mod <a href=\"#m.{}\">{}</a>:</h3>", module_name, module_name));
        html.push_str("<ul>");
        ...
        for (class_name, class_data) in &module.classes {
            ...
            let destructor_warning = if class_has_custom_destructor || treat_external_as_ptr || class_has_recursive_destructor {
            ...
            if let Some(enum_fields) = &class_data.enum_fields {
                ...
                let enum_type = if enum_is_union(enum_fields) { "union enum" } else { "enum" };
                
                html.push_str(&format!("<h4>{} <a href=\"#st.{}\">{}</a>{}</h4>", 
                    ...
                    for (variant_name, variant_data) in variant_map {
                        ...
                        if let Some(variant_type) = &variant_data.r#type {
                            ...
                            if is_primitive_arg(&type_name) {
                                html.push_str(&format!("<p class=\"f\">{}({})</p>", variant_name, variant_type));
                            } else if let Some((_, class_name)) = search_for_class_by_class_name(api_data, &type_name) {
                                html.push_str(&format!("<p class=\"f\">{}({}<a href=\"#st.{}\">{}</a>{})</p>", 
            ...
            // Handle structs
                ...
                html.push_str(&format!("<h4>struct <a href=\"#st.{}\">{}</a>{}</h4>", 
                ...
                for field_map in struct_fields {
                    for (field_name, field_data) in field_map {
                        ...
                        if is_primitive_arg(&type_name) {
                            html.push_str(&format!("<p class=\"f\">{}: {}</p>", field_name, field_type));
                        } else if let Some((_, class_name)) = search_for_class_by_class_name(api_data, &type_name) {
                            html.push_str(&format!("<p class=\"f\">{}: {}<a href=\"#st.{}\">{}</a>{}</p>", 
            ...
            // Handle typedefs
                ...
                html.push_str(&format!("<h4>fnptr <a href=\"#fnty.{}\">{}</a></h4>", class_name, class_name));
                ...
                if !callback_typedef.fn_args.is_empty() {
                    ...
                    for arg in &callback_typedef.fn_args {
                        ...
                        if is_primitive_arg(&type_name) {
                            html.push_str(&format!("<li><p class=\"f\">arg {}</p></li>", type_name));
                        } else if let Some((_, class_name)) = search_for_class_by_class_name(api_data, &type_name) {
                            html.push_str(&format!("<li><p class=\"fnty arg\">arg {} <a href=\"#st.{}\">{}</a></p></li>", 
                ...
                if let Some(returns) = &callback_typedef.returns {
                    ...
                    if is_primitive_arg(&type_name) {
                        ...
                        html.push_str(&format!("<p class=\"fnty ret\">->&nbsp;<a href=\"#st.{}\">{}</a></p>", 
                                             type_name, class_name));
                    } else {
                        html.push_str(&format!("<p class=\"fnty ret\">->&nbsp;{}</p>", return_type));
                    }
                }
            }
            ...
            if let Some(constructors) = &class_data.constructors {
                ...
                for (constructor_name, constructor) in constructors {
                    ...
                    html.push_str(&format!("<li class=\"cn\" id=\"{}.{}\">", class_name, constructor_name));
                    html.push_str(&format!("<p>constructor <a href=\"#{}.{}\">{}</a>:</p>", 
                    ...
                    for arg_map in &constructor.fn_args {
                        for (arg_name, arg_type) in arg_map {
                            ...
                            if is_primitive_arg(&type_name) {
                                html.push_str(&format!("<li><p class=\"arg\">arg {}: {}</p></li>", arg_name, arg_type));
                            } else if let Some((_, class_name)) = search_for_class_by_class_name(api_data, &type_name) {
                                html.push_str(&format!("<li><p class=\"arg\">arg {}: {}<a href=\"#st.{}\">{}</a>{}</p></li>", 
                    ...
                    if let Some(returns) = &constructor.returns {
                        ...
                        if is_primitive_arg(&type_name) {
                            html.push_str(&format!("<p class=\"cn ret\">->&nbsp;{}</p>", type_name));
                        } else if let Some((_, class_name)) = search_for_class_by_class_name(api_data, &type_name) {
                            html.push_str(&format!("<p class=\"cn ret\">->&nbsp;{}<a href=\"#st.{}\">{}</a>{}</p>", 
                                                 prefix, type_name, class_name, suffix));
                        } else {
                            html.push_str(&format!("<p class=\"cn ret\">->&nbsp;{}</p>", return_type));
                        }
                        
                        html.push_str("</li>");
                    } else {
                        html.push_str(&format!("<li><p class=\"ret\">->&nbsp;<a href=\"#st.{}\">{}</a></p></li>", 
            ...
            if let Some(functions) = &class_data.functions {
                ...
                for (function_name, function) in functions {
                    ...
                    html.push_str(&format!("<li class=\"fn\" id=\"{}.{}\">", class_name, function_name));
                    html.push_str(&format!("<p>fn <a href=\"#{}.{}\">{}</a>:</p>", 
                    ...
                    if let Some(returns) = &function.returns {
                        ...
                        if is_primitive_arg(&type_name) {
                            html.push_str(&format!("<p class=\"fn ret\">->&nbsp;{}</p>", type_name));
                        } else if let Some((_, class_name)) = search_for_class_by_class_name(api_data, &type_name) {
                            html.push_str(&format!("<p class=\"fn ret\">->&nbsp;{}<a href=\"#st.{}\">{}</a>{}</p>", 
                                                 prefix, type_name, class_name, suffix));
                        } else {
                            html.push_str(&format!("<p class=\"fn ret\">->&nbsp;{}</p>", return_type));
                        }
...
pub fn generate_api_index(api_data: &ApiData) -> String {
    ...
    for version in api_data.get_sorted_versions() {
        html.push_str(&format!("<li><a href=\"{}/api/{}\">{}</a></li>\n", HTML_ROOT, version, version));
    }
