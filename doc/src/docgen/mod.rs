...
use crate::utils::string::{format_doc, render_example_description, render_example_code};
...
pub fn generate_docs(api_data: &ApiData) -> HashMap<String, String> {
    ...
    docs.insert("api.html".to_string(), apidocs::generate_api_index(api_data));
    
    // Generate guide pages
    for guide in guide::get_guide_list() {
        let guide_html = guide::generate_guide_html(&guide, latest_version);
        docs.insert(format!("guide/{}/{}.html", latest_version, guide.file_name), guide_html);
    }
    
    // Generate combined guide page
    docs.insert("guide.html".to_string(), guide::generate_guide_index(latest_version));
    ...
    docs.insert("releases.html".to_string(), release::generate_releases_index(api_data));
