...
pub fn analyze_type(arg_type: &str) -> (String, String, String) {
    ...
    if arg_type_clean.starts_with('[') && arg_type_clean.ends_with(']') {
        let arg_type_parts: Vec<String> = arg_type_clean[1..arg_type_clean.len()-1].split(';').map(|s| s.to_string()).collect();
...
pub fn get_all_imports(api_data: &ApiData, module: &ModuleData, module_name: &str) -> String {
    ...
    for arg in arg_types_to_search.iter() {
        ...
        if let Some((found_module, found_class)) = search_for_class_by_class_name(api_data, &arg) {
            if found_module != module_name {
                imports.entry(found_module.to_string())
    ...
    for (module_name, classes) in imports {
        if classes.len() == 1 {
            imports_str.push_str(&format!("    use crate::{}::{};\r\n", module_name, classes[0]));
        } else {
...
pub fn get_class<'a>(
    ...
    version_data.modules.get(module_name)?.classes.get(class_name)
}
        ...
        for field_map in struct_fields {
            for (field_name, field_data) in field_map {
                ...
                if !is_primitive_arg(&field_type_base) {
                    if let Some((module_name, class_name)) = search_for_class_by_class_name(api_data, &field_type_base) {
            ...
            for (_, variant_data) in variant_map {
                if let Some(variant_type) = &variant_data.r#type {
                    ...
                    if !is_primitive_arg(&variant_type_base) {
                        if let Some((module_name, class_name)) = search_for_class_by_class_name(api_data, &variant_type_base) {
                            if let Some(variant_class) = get_class(api_data, module_name, class_name) {
...
pub fn enum_is_union(enum_fields: &Vec<indexmap::IndexMap<String, crate::api::EnumVariantData>>) -> bool {
