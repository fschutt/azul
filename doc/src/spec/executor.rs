...
fn write_review_header(f: &mut fs::File, count: usize) {
    ...
    writeln!(
        f,
        "1. Annotating the source code with `// +spec:feature-HASH` markers where behavior is \
         already implemented"
    )
    .unwrap();
    writeln!(
    ...
    writeln!(
        f,
        "- **Code duplication**: same logic repeated in multiple places (should be extracted to a \
         helper)"
    )
    .unwrap();
    writeln!(
    ...
    writeln!(
        f,
        "- **Conflicting patches**: multiple patches that modify the same code region \
         independently\n"
    )
    .unwrap();
...
pub fn cmd_review_arch(
    ...
    writeln!(
        f,
        "You are reviewing {} patches generated by parallel AI agents against a CSS layout engine \
         (Rust).",
        patches.len()
    )
    .unwrap();
...
pub fn cmd_refactor_md(
    ...
    writeln!(
        f,
        "You are planning refactoring work needed before applying patches to a CSS layout engine \
         (Rust).\n"
    )
    .unwrap();
    writeln!(
...
pub fn cmd_groups_json(
    ...
    writeln!(
        f,
        "- **`agent_context`**: A DETAILED instruction block for the applying agent. THIS IS \
         CRITICAL.\n"
    )
    .unwrap();
    ...
    writeln!(
        f,
        "- **Known bugs in the patch** from the review (e.g., \"uses width > 0.0 as auto proxy — \
         fix this\")"
    )
    .unwrap();
    writeln!(
        f,
        "- **Refactoring needed**: if the patch duplicates existing helpers, name the helper to \
         reuse"
    )
    .unwrap();
    writeln!(
        f,
        "- **Functions/types to reuse**: specific existing functions the agent should call \
    ...
    writeln!(
        f,
        "- **For MERGE groups**: which parts of each patch to take, where they conflict, how to \
         combine them"
    )
    .unwrap();
    writeln!(
    ...
    writeln!(
        f,
        "- **Compilation notes**: if missing imports or signature changes are needed, mention \
         them\n"
    )
    .unwrap();
    ...
    writeln!(
        f,
        "Respond with a JSON array of merge groups. The `agent_context` field is the most \
         important —"
    )
    .unwrap();
    writeln!(
    ...
    writeln!(
        f,
        "    \"agent_context\": \"## Intent\\nFix float overlap: use float MARGIN BOX instead of \
         content box for overlap checks (CSS 2.2 §9.5).\\n\\n## Spec\\nCSS 2.2 §9.5: 'The border \
         box of a table, a block-level replaced element, or an element in the normal flow that \
         establishes a new BFC MUST NOT overlap the margin box of any floats in the same \
         BFC.'\\n\\n## Known Issues\\n- `is_block_level_replaced()` only checks Image, not \
         video/canvas/input. This is OK for now, just apply as-is.\\n- The margin-box math in \
         `available_line_box_space` is correct: verified origin - margin_start + size + \
         margin_start + margin_end = correct margin box end.\\n\\n## Apply Instructions\\nApply \
         directly. No conflicts expected. Verify `cargo check -p azul-layout` passes.\""
    )
    .unwrap();
    writeln!(f, "  }},").unwrap();
    writeln!(f, "  {{").unwrap();
    ...
    writeln!(
        f,
        "    \"patches\": [\"line-breaking_008.md.done.001.patch\", \
         \"line-breaking_015.md.done.001.patch\", \"line-breaking_040.md.done.001.patch\"],"
    )
    .unwrap();
    writeln!(
        f,
    ...
    writeln!(
        f,
        "    \"agent_context\": \"## Intent\\nAdd `word-break` CSS property (CSS Text 3 §5.2): \
         break-all, keep-all, normal.\\n\\n## Why _008 is preferred\\n- Correctly suppresses \
         hyphenation for break-all (spec: 'Hyphenation is not applied')\\n- Has proper Hash/Eq \
         derives needed for caching\\n- Clean 3-way match in peek_next_unit_with_word_break\\n- \
         _015 misses hyphenation suppression; _040 also misses it and has less explicit CJK \
         detection\\n\\n## Known Issues to Fix\\n- No CSS property wiring: `word_break` field in \
         UnifiedConstraints defaults to Normal but is never read from CSS. You must add wiring in \
         fc.rs where UnifiedConstraints is built — read word-break from the style and map to the \
         WordBreak enum.\\n- The `break_one_line` signature changes — update ALL callers.\\n\\n## \
         Existing Code to Reuse\\n- `is_word_separator()` already exists in cache.rs — the patch \
         correctly uses it.\\n- `UnifiedConstraints` is built in \
         `translate_to_text3_constraints()` in fc.rs — add word_break there.\""
    )
    .unwrap();
    writeln!(f, "  }},").unwrap();
    writeln!(f, "  {{").unwrap();
    ...
    writeln!(
        f,
        "    \"agent_context\": \"SKIP — regression: replaces comprehensive get_display_type() \
         blockification (handles TableRowGroup, RunIn, etc.) with a simpler version that loses \
         these mappings.\""
    )
    .unwrap();
    writeln!(f, "  }}").unwrap();
    ...
    writeln!(
        f,
        "- The `agent_context` must be SELF-CONTAINED. The applying agent sees ONLY this field + \
         the patch diffs."
    )
    .unwrap();
    writeln!(
        f,
        "  It does NOT see the review, the conflict map, or any other groups. Include everything \
         it needs."
    )
    .unwrap();
    writeln!(
    ...
    writeln!(
        f,
        "- For ANNOT-only groups, `agent_context` can be brief: just say what spec paragraph to \
         annotate."
    )
    .unwrap();
...
fn build_apply_prompt(
    ...
    prompt.push_str(
        "For MERGE groups: combine the best parts of all patches into one coherent \
         implementation.\n",
    );
    prompt.push_str(
        "For PICK_ONE groups: apply the preferred patch, verify nothing unique is lost from \
         others.\n",
    );
    ...
    match group.action.as_str() {
        "APPLY" if is_annot_only => {
            ...
            prompt.push_str(
                "2. If it fails, run `git am --abort`, skip that patch, and continue with the \
                 next\n",
            );
            prompt.push_str(
        ...
        "PICK_ONE" => {
            ...
            prompt.push_str(
                "Multiple patches implement the same feature. The preferred patch has been \
                 selected.\n",
            );
            prompt
    ...
    if !is_annot_only {
        ...
        prompt.push_str(
            "The compilation check (`cargo check -p azul-dll --features build-dll`) covers all of \
             these.\n\n",
        );
...
fn build_full_prompt(prompt_path: &Path, working_dir: &Path) -> Result<String, String> {
    ...
    full_prompt.push_str(&format!(
        "## Working Directory\n\nYour current working directory is: `{}`\nYou are in a git \
         worktree. ALL file paths are relative to this directory.\nDo NOT `cd` anywhere else — \
         your commits will be lost if you do.\n\n",
        working_dir.display()
    ));
...
fn preflight_checks(config: &SpecConfig, workspace_root: &Path) -> Result<(), String> {
    ...
    if std::env::var("ANTHROPIC_API_KEY").is_ok() {
        return Err(
            "ANTHROPIC_API_KEY is set in environment.\nThis would route claude CLI through the \
             paid API instead of your Pro/Max subscription.\nUnset it first:  unset \
