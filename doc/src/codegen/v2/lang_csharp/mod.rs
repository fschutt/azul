...
pub fn generate(ir: &CodegenIR, config: &CodegenConfig) -> Result<String> {
    ...
    // 6. Public `HostInvoker` class — per-kind invokers, typed `<Kind>WithData<T>` delegates +
    //    `Register<Kind><T>`, and RefanyCreate / RefanyGet for user data.
...
fn generate_header(builder: &mut CodeBuilder) {
    ...
    builder.line("// ============================================================================");
    builder.blank();
    // Azul.cs is compiled inside the USER's project (the guide's
    // `dotnet run` flow), whatever its <Nullable> setting: annotations
    // are enabled so the `Dom?` / `string?` spellings below are legal
    // under a project with nullable disabled (CS8632), and the nullable
    // WARNINGS are disabled so the un-annotated `object` signatures
    // (kept for PowerShell's Add-Type, which has no nullable context)
    // do not raise CS8765/CS8603 under a project with nullable enabled.
    builder.line("#nullable enable annotations");
    builder.line("#nullable disable warnings");
    builder.blank();
}
