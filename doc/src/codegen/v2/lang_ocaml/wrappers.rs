...
fn emit_method_impl(
    ...
    for a in &user_args {
        ...
        } else if let Some(e) = ir.find_enum(a.type_name.trim()) {
            if !e.is_union {
                let module = super::ocaml_module_name(&e.name);
                call_args.push(format!("({}.to_int {})", module, id));
            } else {
                call_args.push(id);
            }
        } else {
    ...
    let mut call_str = if call_args.is_empty() {
        format!("{} ()", raw_binding)
    } else {
        format!("{} {}", raw_binding, call_args.join(" "))
    };

    if let Some(r) = func.return_type.as_deref() {
        if let Some(e) = ir.find_enum(r.trim()) {
            if !e.is_union {
                let module = super::ocaml_module_name(&e.name);
                call_str = format!("(match {}.of_int ({}) with | Some x -> x | None -> failwith \"invalid {} enum\")", module, call_str, module);
            }
        }
    }
...
fn emit_unit_enum_module(builder: &mut CodeBuilder, e: &EnumDef, ir: &CodegenIR, interface: bool) {
    ...
    builder.indent();
    for v in &e.variants {
        let variant = sanitize_identifier(&super::to_pascal_case(&v.name));
