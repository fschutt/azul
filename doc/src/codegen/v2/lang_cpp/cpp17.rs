...
impl CppDialect for Cpp17Generator {
    ...
    fn generate_class_declaration(
        ...
        // again. `Az*_delete` is NOT safe on a zeroed struct (it drops an
        // invalid value -> e.g. a zeroed BTreeMap segfaults), so the destructor
        // checks `owned_` rather than relying on delete-of-zero being a no-op.
        if needs_dtor {
        ...
        // `release()`/r-value conversion hand the raw value to a consuming
        // C-ABI function. For owning types this relinquishes ownership (dtor
        // must NOT delete afterwards); non-owning types just return + zero.
        if needs_dtor {
            code.push_str(&format!(
    ...
    fn generate_method_implementations(
            ...
            .filter(|f| !is_constructor_or_default(f))
        {
            ...
            // A value-self (consuming) method hands inner_ to a C-ABI that takes
            // ownership of it; such a method must NOT be const (it relinquishes
            // ownership via release() below) -- otherwise `this`'s destructor
            // would double-free the already-consumed inner_ (e.g. the nested
            // DynamicSelectorVec inside a Button), crashing in its Drop.
            ...
            // release() (relinquish ownership: owned_=false / zero the source) so
            // the consumed inner_ is not double-freed; a by-ref method borrows.
            ...
            if func_takes_string_arg(func) {
                ...
                let sv_call_args = generate_call_args_sv_overload(&func.args, ir, true, class_name);
