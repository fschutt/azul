...
impl CppDialect for Cpp23Generator {
    ...
    fn generate(&self, ir: &CodegenIR, config: &CodegenConfig) -> Result<String> {
        ...
        code.push_str("// Forward declarations\r\n");
        for struct_def in &sorted_structs {
            if !config.should_include_type(&struct_def.name) { continue; }
            if should_skip_class(struct_def) || renders_as_type_alias(struct_def) { continue; }
            code.push_str(&format!("class {};\r\n", struct_def.name));
        }
        code.push_str("\r\n");
        ...
        for enum_def in &ir.enums {
            if !config.should_include_type(&enum_def.name) { continue; }
            self.generate_enum_wrapper(&mut code, enum_def, config);
        }
    ...
    fn generate_copy_move_semantics(&self, code: &mut String, class_name: &str, c_type_name: &str, is_copy: bool, needs_destructor: bool) {
        Cpp20Generator.generate_copy_move_semantics(code, class_name, c_type_name, is_copy, needs_destructor);
    }
