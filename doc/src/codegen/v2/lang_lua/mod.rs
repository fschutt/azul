...
pub fn generate(ir: &CodegenIR, _config: &CodegenConfig) -> Result<String> {
    ...
    builder.line("        -- Byref routing: LuaJIT's C call frame holds 256 bytes of");
    builder.line("        -- stack-passed arguments IN TOTAL (CCALL_MAXSTACK), and on x86-64");
    builder.line("        -- SysV every aggregate over 16 bytes goes on the stack — so the");
    builder.line("        -- SUM of the aggregate arguments decides, not the largest one");
    builder.line("        -- (two 150-byte structs NYI just like one 2 KiB config). Route");
    builder.line("        -- through the <name>Byref twin whenever the sum is anywhere near.");
    builder.line("        local meta = __az_fn_meta[name]");
    builder.line("        if meta then");
    builder.line("            local total = 0");
