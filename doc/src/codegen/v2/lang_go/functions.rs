...
fn emit_go_function(b: &mut CodeBuilder, p: &CallPlan) {
    ...
    // Determine purego lib_ func signature
    let mut lib_args = Vec::new();
    let via_out = p.target != Target::Direct && p.ret != Ret::Void;
    if via_out {
        lib_args.push("uintptr".to_string());
    }
    for a in &p.args {
        lib_args.push(match &a.pass {
            Pass::Prim { .. } => a.go_type.clone(),
            Pass::Pointer | Pass::ByAddress => "uintptr".to_string(),
        });
    }
    
    let lib_ret = if via_out {
        String::new()
    } else {
        match &p.ret {
            Ret::Void => String::new(),
            Ret::Prim { go, .. } => go.clone(),
            Ret::Pointer { .. } => "uintptr".to_string(),
            Ret::Aggregate { .. } => unreachable!("aggregate returns never go Direct"),
        }
    };
    
    b.line(&format!("var lib_{} func({}) {}", p.c_call_name(), lib_args.join(", "), lib_ret));
    b.line(&format!("func init_{}(lib uintptr) {{", p.c_call_name()));
    b.indent();
    b.line(&format!("purego.RegisterLibFunc(&lib_{}, lib, \"{}\")", p.c_call_name(), p.c_call_name()));
    b.dedent();
    b.line("}");
    b.blank();
    ...
    if via_out {
        ...
        call_args.push("uintptr(unsafe.Pointer(&azRet))".into());
    }
    for a in &p.args {
        call_args.push(match &a.pass {
            Pass::Prim { .. } => a.go_name.clone(),
            Pass::Pointer => format!("uintptr(unsafe.Pointer({}))", a.go_name),
            Pass::ByAddress => format!("uintptr(unsafe.Pointer(&{}))", a.go_name),
        });
    }
    let call = format!("lib_{}({})", p.c_call_name(), call_args.join(", "));
    if via_out {
        b.line(&call);
        b.line("return azRet");
    } else {
        match &p.ret {
            ...
            Ret::Prim { .. } => b.line(&format!("return {}", call)),
            Ret::Pointer { go, .. } => {
                if go == "unsafe.Pointer" {
                    b.line(&format!("return unsafe.Pointer({})", call));
                } else {
                    b.line(&format!("return ({})(unsafe.Pointer({}))", go, call));
                }
            }
            Ret::Aggregate { .. } => unreachable!("aggregate returns never go Direct"),
        }
...
fn emit_file(b: &mut CodeBuilder, plans: &[&CallPlan], shim: bool, index: usize, total: usize, init_name: &str) {
    b.line("// ============================================================================");
    b.line(&format!(
        "// functions_{}.go - raw C-call bindings ({}/{}).",
        index, index, total
    ));
    ...
    b.line(&format!("func {}(lib uintptr) {{", init_name));
    b.indent();
    for p in plans {
        b.line(&format!("init_{}(lib)", p.c_call_name()));
    }
    b.dedent();
    b.line("}");
    b.blank();
...
pub fn generate_files(ir: &CodegenIR, config: &CodegenConfig) -> Result<Vec<(String, String)>> {
    ...
    for (shim, set) in [(false, direct), (true, shims)] {
        ...
        for (i, chunk) in chunks.into_iter().enumerate() {
            ...
            let init_name = format!("initFunctions_{}_{}", if shim { "shim" } else { "direct" }, i);
            init_calls.push(init_name.clone());
            
            emit_file(&mut b, chunk, shim, i, total, &init_name);
    ...
    b.line("package azul");
    b.blank();
    b.line("func initFunctions(lib uintptr) {");
    b.indent();
    for init_name in init_calls {
        b.line(&format!("{}(lib)", init_name));
    }
    b.dedent();
    b.line("}");
    files.push(("functions_init.go".to_string(), b.finish()));
