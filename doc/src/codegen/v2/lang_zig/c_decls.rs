//! Pre-translated C ABI for Zig: emits `azul_c.zig`.
//!
...
//! so `azul.zig` can `@import("azul_c.zig")` instead. Zig's lazy
//! analysis then only touches the declarations a program uses.
//!
//! # Fidelity contract
//!
...
//! macros). They are C conveniences; Zig users have `switch` and the
//! wrapper layer, and they are what made `@cImport` slow in the first
//! place.
...
/// File name `azul.zig` imports; also the release artefact name.
pub const C_DECLS_FILE: &str = "azul_c.zig";
...
fn emit_header(out: &mut String) {
    out.push_str(
        "// ============================================================================\n\
         // azul_c.zig — the C ABI of libazul, pre-translated to Zig.\n\
         // Auto-generated by azul-doc codegen v2 (lang_zig). DO NOT EDIT MANUALLY.\n\
         // ============================================================================\n\
         //\n\
         // This file replaces `@cImport(@cInclude(\"azul.h\"))`: it declares every\n\
         // struct, union, enum, callback typedef, constant and exported function\n\
         // of libazul with the same names and layouts the C header has, so\n\
         // `azul.C.AzApp_create(...)` works exactly as it did through @cImport —\n\
         // without the compiler translating and analysing a 5.6 MB header on\n\
         // every cold build. `azul.h` is no longer needed on the include path;\n\
         // the shared library still is on the link line (`-lazul`).\n\
         //\n\
         // Layout notes (mirroring azul.h):\n\
         //   * C enums are `c_uint` aliases with `c_int` constants per variant.\n\
         //   * Tagged unions are `extern union`s over one `Variant_<V>` struct each;\n\
         //     the tag field is `u8` for #[repr(C, u8)] enums.\n\
         //   * Pointers are C pointers (`[*c]T`); `void*` is `?*anyopaque`.\n\
         // ============================================================================\n\n\
...
fn emit_function(out: &mut String, f: &FunctionDef, config: &CodegenConfig) {
    let self_snake = to_snake_case(&f.class_name);
    ...
    let is_self = |a: &FunctionArg| a.name == "self" || a.name == self_snake;
...
/// `StyleTextView` → `style_text_view` (the IR builder's self-arg name).
