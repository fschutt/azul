//! Odin type emission: structs, enums, tagged unions (`struct
//! #raw_union`), callback proc-types, and type aliases.
//!
//! Odin package-scope declarations are order-independent, so we simply
//! walk the IR and emit each type once. Skipped categories (recursive /
//! generic template) get an opaque `AzName :: rawptr` alias so any
...
//! collected into a `struct #raw_union`. Because every variant struct
//! shares the same first `tag: u8` at offset 0 and Odin uses C struct /
//! union layout (natural alignment, no field reordering), the size and
...
pub fn generate_types(
    ...
    // enums; simple aliases become `AzName :: <target>`.
    for ta in &ir.type_aliases {
        ...
        match &ta.monomorphized_def {
            ...
            None => {
                let target = map_type_to_odin(&ta.target, ir);
                b.line(&format!("{} :: {}", name, target));
                b.blank();
            }
        }
    }

    // Callback typedefs -> Odin proc-type aliases (C function pointers).
...
fn emit_opaque(b: &mut CodeBuilder, ffi_name: &str, orig: &str) {
    // Pointer-sized alias so by-pointer references still resolve (the
    // type is only ever reached through a `Box`/pointer in these
    // categories). Matches the Pascal backend's `T = Pointer`.
    b.line(&format!("{} :: rawptr // opaque: {}", ffi_name, orig));
    b.blank();
}
...
fn emit_simple_enum(b: &mut CodeBuilder, e: &EnumDef) {
    ...
    if e.variants.is_empty() {
        // Odin has no zero-variant enum; degrade to the backing integer.
        b.line(&format!("{} :: {}", name, backing));
        b.blank();
        return;
    }
...
// ============================================================================
// Tagged union (struct #raw_union of per-variant structs)
// ============================================================================
...
fn emit_tagged_union(b: &mut CodeBuilder, e: &EnumDef, ir: &CodegenIR) {
    ...
    b.line(&format!("{} :: struct #raw_union {{", name));
    for v in &e.variants {
...
// ============================================================================
// Callback typedef -> Odin proc type
// ============================================================================
...
fn emit_callback_typedef(
    ...
    // Unnamed parameter types keep proc-type declarations free of
...
/// Backing integer for a simple enum. Rust `#[repr(C, u8)]` -> `u8`,
/// otherwise a `#[repr(C)]` enum is C-`int` sized -> `i32`. (Odin's
/// default enum backing is pointer-sized `int`, which would be wrong.)
...
/// tagged unions use a 1-byte tag; a plain `#[repr(C)]` data enum uses
/// a C-`int` tag.
