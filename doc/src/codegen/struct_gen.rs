...
impl StructMetadata {
    ...
    pub fn from_class_data(name: String, class_data: &ClassData) -> Self {
        ...
        // Callback wrapper structs (like ButtonOnClick with fields {cb, data: RefAny})
        // can always have Clone derived since both callback pointers and RefAny implement Clone
        if !derive.contains(&"Clone".to_string()) {
            if let Some(ref fields) = class_data.struct_fields {
                let is_callback_wrapper = fields.len() == 1 && fields[0].len() == 2;
                if is_callback_wrapper {
                    let field_map = &fields[0];
                    let has_cb_field = field_map.contains_key("cb");
                    let has_data_refany = field_map.get("data")
                        .and_then(|f| Some(f.r#type.as_str() == "RefAny"))
                        .unwrap_or(false);
                    if has_cb_field && has_data_refany {
                        derive.push("Clone".to_string());
                    }
                }
            }
...
fn generate_struct_definition(
    ...
    if !config.is_memtest {
        ...
        // Check if this is a callback wrapper struct - if so, skip trait generation here
        // because generate_callback_trait_impls will generate them later
        ...
        let should_gen_via_external = |trait_name: &str| -> bool {
            ...
            // Skip callback wrapper structs - their trait impls are generated by generate_callback_trait_impls
            if is_callback_wrapper {
                return false;
            }
            // Vec types: Generate Clone/Drop/Default/Debug always,
            // also generate PartialEq/PartialOrd/Eq/Ord/Hash if explicitly in custom_impls
            if is_vec_type {
                let always_allowed = matches!(trait_name, "Clone" | "Drop" | "Default" | "Debug");
                let in_custom_impls = struct_meta.custom_impls.contains(&trait_name.to_string());
                if !always_allowed && !in_custom_impls {
                    return false;
                }
            }
            // If already in derive, don't generate manual impl
            // EXCEPTION: In drop_via_external mode, PartialEq/PartialOrd/Eq/Ord/Hash are filtered
            // from #[derive()] so they need manual impl even if listed in derive
            let is_filtered_in_drop_via_external = config.drop_via_external 
                && matches!(trait_name, "PartialEq" | "PartialOrd" | "Eq" | "Ord" | "Hash" | "Copy");
            if struct_meta.derive.contains(&trait_name.to_string()) && !is_filtered_in_drop_via_external {
                return false;
            }
            ...
            // - Also generate for types with external + custom_destructor, BUT only for
            //   Clone/Drop/Default/Debug - NOT for PartialEq/PartialOrd/Ord/Eq/Hash
            //   (comparison traits may not be implementable via external delegation)
            ...
            // - For traits that are in derive but filtered out (PartialEq/PartialOrd/Eq/Ord/Hash),
            //   generate via external if the type has an external path
            let is_derive_but_filtered = is_filtered_in_drop_via_external 
                && struct_meta.derive.contains(&trait_name.to_string())
                && has_external;
            
            // Check if Default is explicitly declared (in derive or custom_impls)
            let has_default_declared = struct_meta.derive.contains(&"Default".to_string()) 
                || struct_meta.custom_impls.contains(&"Default".to_string());
            
            struct_meta.custom_impls.contains(&trait_name.to_string()) 
                || (has_external && struct_meta.has_custom_destructor && matches!(trait_name, "Clone" | "Drop" | "Debug"))
                || (has_external && struct_meta.has_custom_destructor && trait_name == "Default" && has_default_declared)
                || (has_external && is_container_type && matches!(trait_name, "Clone" | "Drop" | "Debug"))
                || (has_external && is_container_type && trait_name == "Default" && has_default_declared)
                || (has_external && trait_name == "Debug")
                || is_derive_but_filtered
        };
        ...
        // Generate impl Default - use should_gen_via_external for consistency
        // This handles:
        // - Types with Default in custom_impls (like WindowsWindowOptions)
        // - Types with external + custom_destructor
        // - Container types (Vec/Option/Result)
        if should_gen_via_external("Default") {
            code.push_str(&format!(
                "{}impl Default for {} {{\n",
                indent_str, struct_name
            ));
...
fn generate_enum_definition(
    ...
    // For drop_via_external mode: generate Clone impl via external delegation for enums
    // This is needed because enums may have derive: [] (empty) but still need Clone
    // for struct fields that use #[derive(Clone)]
    ...
    if config.drop_via_external 
        ...
        && !is_vec_destructor_type
        && !struct_meta.derive.contains(&"Clone".to_string())  // Not already derived
    {
        let external_path = struct_meta.external.as_deref().unwrap_or(struct_name);
        let external_crate = external_path.replace("azul_dll", "crate");
