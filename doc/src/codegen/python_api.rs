...
fn is_primitive_or_void_alias(class_data: &ClassData) -> bool {
    if let Some(ref type_alias) = class_data.type_alias {
        ...
        matches!(target, "c_void" | "usize" | "u8" | "u16" | "u32" | "u64" 
    ...
    if let Some(ref struct_fields) = class_data.struct_fields {
        for field_map in struct_fields {
            for (_, field_data) in field_map {
                ...
                if field_data.r#type.contains("*const") || field_data.r#type.contains("*mut") 
    ...
    if let Some(ref struct_fields) = class_data.struct_fields {
        for field_map in struct_fields {
            for (_, field_data) in field_map {
                ...
                if type_str.contains("*const") || type_str.contains("*mut") || type_str.contains('*') {
                    return false;
                }
                
                // Skip if type is a Ref/RefMut type (borrowing types not constructable from Python)
                if type_str.ends_with("Ref") || type_str.ends_with("RefMut") 
                   || type_str.contains("VecRef") || type_str.contains("Refstr") {
                    return false;
                }
...
/// Returns Some((callback_field_name, callback_type, callback_info_type, return_type)) if it's a pair
...
fn get_type_external_path(type_name: &str, version_data: &VersionData) -> String {
    ...
    if matches!(type_name, "()" | "bool" | "u8" | "u16" | "u32" | "u64" | "usize" 
...
fn get_callback_signature(callback_wrapper_type: &str, version_data: &VersionData) -> Option<CallbackSignature> {
    ...
    // Get return type
    let return_type = callback_typedef.returns.as_ref()
    ...
    if let Some((module, _)) = search_for_class_by_class_name(version_data, &base_type) {
        if let Some(class_data) = get_class(version_data, module, &base_type) {
            ...
            if let Some(ref type_alias) = class_data.type_alias {
                ...
                if target.ends_with("Callback") || target.ends_with("Type") || target.ends_with("Destructor") {
                    return true;
                }
            }
        }
        ...
        if base_type.contains("Callback") || (base_type.contains("Data") && base_type != "StyleFontFamiliesValue") {
            return true;
        }
    }
        ...
        for field_map in struct_fields {
            for (field_name, field_data) in field_map {
                ...
                if is_vec || is_boxed {
                    ...
                    if is_boxed && (field_data.ref_kind == RefKind::ConstPtr || field_data.ref_kind == RefKind::MutPtr) {
                        continue;
                    }
                }
...
fn resolve_type_alias_chain<'a>(type_name: &'a str, version_data: &'a VersionData) -> &'a str {
    ...
    for _ in 0..10 { // Limit recursion depth
...
fn instantiate_type_alias(
    ...
    let resolved_args: Vec<&str> = type_alias.generic_args.iter()
...
pub fn generate_python_api(api_data: &ApiData, version: &str) -> String {
    ...
    for (_module_name, module) in &version_data.api {
        for (class_name, class_data) in &module.classes {
            ...
            if class_data.type_alias.is_some() {
                if let Some(instantiated) = instantiate_type_alias(class_name, class_data, version_data) {
    ...
    for (class_name, class_data) in &structs {
        if let Some((cb_field, cb_type, cb_sig)) = is_callback_data_pair_struct(class_data, version_data) {
            callback_pair_info.push((class_name.clone(), cb_field.clone(), cb_type.clone()));
            code.push_str(&generate_callback_data_pair_wrapper(class_name, &cb_field, &cb_type, &cb_sig, &prefix));
        }
    }
    code.push_str("\r\n");
    ...
    for (class_name, class_data) in &structs {
        ...
        if let Some((_, cb_field, cb_type)) = callback_pair_info.iter().find(|(n, _, _)| n == class_name) {
            code.push_str(&generate_callback_data_pair_struct(class_name, cb_field, cb_type, &prefix));
        } else {
            code.push_str(&generate_struct_definition(class_name, class_data, &prefix, version_data));
        }
    }
    ...
    for (class_name, class_data) in &enums {
        code.push_str(&generate_enum_definition(class_name, class_data, &prefix, version_data));
    }
    ...
    for (class_name, class_data) in &enums {
        let is_union = class_data.enum_fields.as_ref().map(|f| enum_is_union(f)).unwrap_or(false);
        if is_union {
            code.push_str(&generate_clone_impl(class_name, class_data, &prefix));
        }
    }
    ...
    for (class_name, class_data) in &structs {
        ...
        let is_callback_pair = callback_pair_info.iter().any(|(n, _, _)| n == class_name);
        if !is_callback_pair {
            code.push_str(&generate_struct_pymethods(class_name, class_data, &prefix, version_data));
        }
    }
    for (class_name, class_data) in &enums {
        code.push_str(&generate_enum_pymethods(class_name, class_data, &prefix, version_data));
    }

    // Generate Python module
    code.push_str(&generate_python_module(&structs, &enums, &prefix, version_data));
...
fn generate_struct_definition(
    ...
    let unsendable = if needs_unsendable(class_data) { ", unsendable" } else { "" };
    ...
    if let Some(struct_fields) = &class_data.struct_fields {
        ...
        let mut fields_with_setters: std::collections::HashSet<&str> = std::collections::HashSet::new();
        ...
        for field_map in struct_fields {
            for (field_name, field_data) in field_map {
                let is_raw_ptr = field_data.ref_kind == RefKind::ConstPtr || field_data.ref_kind == RefKind::MutPtr;
                let is_destructor = field_name == "destructor" || field_data.r#type.ends_with("DestructorCallbackType");
                ...
                let field_type = if is_boxed && (is_raw_ptr || is_destructor) {
                    ...
                    rust_type_to_python_type_with_ref(&field_data.r#type, field_data.ref_kind.clone(), prefix, version_data)
                };
            ...
            for variant_map in enum_fields {
                for (_, variant_data) in variant_map {
                    if let Some(ref variant_type) = variant_data.r#type {
                        ...
                        let should_skip = if let Some((module, _)) = search_for_class_by_class_name(version_data, variant_type) {
                            if let Some(variant_class_data) = get_class(version_data, module, variant_type) {
                                // Skip callback typedefs, VecRef, recursive types, but NOT manual types
...
fn generate_clone_impl(class_name: &str, class_data: &ClassData, prefix: &str) -> String {
    ...
    if has_custom_clone {
        ...
        code.push_str(&format!(
            "        unsafe {{ mem::transmute(crate::ffi::dll::{}{}_deepCopy(mem::transmute(self))) }}\r\n",
            prefix, class_name
        ));
        code.push_str("    }\r\n");
        code.push_str("}\r\n\r\n");
...
fn generate_debug_impl(class_name: &str, class_data: &ClassData, prefix: &str) -> String {
    ...
    if has_derive_debug || has_custom_debug {
        ...
        code.push_str(&format!("        let capi: &crate::ffi::dll::{} = unsafe {{ mem::transmute(self) }};\r\n", type_name));
...
fn generate_struct_pymethods(
    ...
    if class_data.struct_fields.is_some() {
        code.push_str(&generate_default_constructor(class_name, class_data, prefix, version_data));
    }
    ...
    if let Some(constructors) = &class_data.constructors {
        for (ctor_name, ctor_data) in constructors {
            ...
            code.push_str(&generate_function(
                class_name, ctor_name, ctor_data, prefix, version_data, true,
            ));
        }
    }
    ...
    if let Some(functions) = &class_data.functions {
        for (fn_name, fn_data) in functions {
            ...
            code.push_str(&generate_function(
                class_name, fn_name, fn_data, prefix, version_data, false,
            ));
        }
    }
                    ...
                    if let Some(ref variant_type) = variant_data.r#type {
                        ...
                        if let Some((module, _)) = search_for_class_by_class_name(version_data, &base_type) {
                            if let Some(variant_class) = get_class(version_data, module, &base_type) {
                                if is_callback_typedef(variant_class) || is_vec_ref_type(variant_class) {
                                    continue;
                                }
...
fn generate_default_constructor(
    ...
    for field_map in struct_fields {
        for (field_name, field_data) in field_map {
            let field_type = rust_type_to_python_type_with_ref(&field_data.r#type, field_data.ref_kind.clone(), prefix, version_data);
            code.push_str(&format!("        {}: {},\r\n", field_name, field_type));
        }
    }
...
fn generate_function(
    ...
    let c_fn_name = format!("{}{}_{}", prefix, class_name, snake_case_to_lower_camel(fn_name));
...
fn generate_callback_data_pair_wrapper(
    ...
    code.push_str(&format!("/// Python object wrapper for {} callback+data\r\n", class_name));
    code.push_str("#[repr(C)]\r\n");
    code.push_str(&format!("pub struct {} {{\r\n", wrapper_name));
    ...
    code.push_str(&format!("/// Trampoline for {} - called by C-API, invokes Python\r\n", class_name));
    // Callbacks now take by-value arguments using C-API types
    code.push_str(&format!(
        "extern \"C\" fn {}(\r\n    data: {},\r\n    info: {}{}\r\n) -> {} {{\r\n",
        trampoline_name, capi_refany_type, capi_info_type, extra_args_sig, 
        capi_return_type
    ));
    ...
    let default_expr = match cb_sig.return_type.as_str() {
        ...
        "OnTextInputReturn" => format!("{} {{ update: crate::ffi::dll::AzUpdate::DoNothing, valid: crate::ffi::dll::AzTextInputValid::Yes }}", capi_return_type),
        "()" => "()".to_string(),
    ...
    code.push_str(&format!("        let info_py: {} = unsafe {{ mem::transmute(info) }};\r\n", info_type_az));
    ...
    for (name, type_name, ref_kind, _) in &cb_sig.extra_args {
        ...
        code.push_str(&format!("        let {}_py: {} = unsafe {{ mem::transmute({}) }};\r\n", name, transmute_type, name));
...
fn generate_callback_data_pair_struct(
    ...
    code.push_str(&format!("/// {} - Python wrapper for callback+data pair\r\n", class_name));
    code.push_str(&format!(
        "#[pyclass(name = \"{}\", module = \"azul\", unsendable)]\r\n",
        class_name
    ));
    code.push_str(&format!("pub struct {} {{\r\n", struct_name));
    code.push_str(&format!("    pub inner: crate::ffi::dll::{},\r\n", struct_name));
    code.push_str("}\r\n\r\n");
    ...
    code.push_str("    ///     callback: A callable that receives (data, info) and returns the appropriate type\r\n");
    code.push_str("    #[new]\r\n");
    ...
    code.push_str("                return Err(PyException::new_err(\"callback must be callable\"));\r\n");
    code.push_str("            }\r\n");
    code.push_str("            Ok(())\r\n");
    code.push_str("        })?;\r\n\r\n");
    ...
    code.push_str(&format!("            inner: crate::ffi::dll::{} {{\r\n", struct_name));
    code.push_str(&format!("                {}: crate::ffi::dll::{} {{\r\n", cb_field_name, cb_struct_name));
...
fn generate_python_module(
    ...
    code.push_str("    #[cfg(all(feature = \"use_pyo3_logger\", not(feature = \"use_fern_logger\")))] {\r\n");
...
fn rust_type_to_python_type_with_ref(rust_type: &str, ref_kind: RefKind, prefix: &str, version_data: &VersionData) -> String {
...
fn rust_type_to_python_type(rust_type: &str, prefix: &str, version_data: &VersionData) -> String {
    ...
    if CAPI_TYPES.contains(&base_type.as_str()) {
        return format!("{}crate::ffi::dll::Az{}{}", ptr_prefix, base_type, array_suffix);
    }
        ...
        if let Some(class_data) = get_class(version_data, module, &base_type) {
            if let Some(ref type_alias) = class_data.type_alias {
                ...
                if type_alias.generic_args.is_empty() {
                    ...
                    let resolved = rust_type_to_python_type(&type_alias.target, prefix, version_data);
                    return format!("{}{}{}{}", alias_ptr_prefix, resolved, alias_ptr_suffix, array_suffix);
                }
            }
        }
    }
