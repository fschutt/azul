use crate::api::{ApiData, ClassData};
...
pub fn generate_python_api(api_data: &ApiData) -> String {
            ...
            for field_map in struct_fields {
                for (field_name, field_data) in field_map {
                    if field_name == "ptr" && field_data.r#type.contains("*") {
                        ...
                        code.push_str(&format!("    pub(crate) {}: {}, // raw pointer not exposed to Python\r\n", field_name, field_data.r#type));
                    } else if field_name != "cb" { // Don't expose callback fields
    ...
    code.push_str("\r\n");
    code.push_str("// Necessary because the Python interpreter may send structs across different threads\r\n");
    
    // Implement Send for all structs with raw pointers
    let raw_pointer_structs: Vec<&String> = struct_map.keys()
        .filter(|name| {
    ...
    for (struct_name, class_data) in &struct_map {
        ...
        if class_data.external.is_some() {
            code.push_str(&format!("impl Clone for {} {{ fn clone(&self) -> Self {{ let r: &{} = unsafe {{ mem::transmute(self) }}; unsafe {{ mem::transmute(r.clone()) }} }} }}\r\n", 
                                 struct_name, class_data.external.as_ref().unwrap()));
        }
    }
    ...
    for (enum_name, class_data) in &enum_map {
        ...
        if class_data.external.is_some() {
            code.push_str(&format!("impl Clone for {}EnumWrapper {{ fn clone(&self) -> Self {{ let r: &{} = unsafe {{ mem::transmute(&self.inner) }}; unsafe {{ mem::transmute(r.clone()) }} }} }}\r\n", 
                                 enum_name, class_data.external.as_ref().unwrap()));
        }
    }
    ...
    for (struct_name, class_data) in &struct_map {
        ...
        if should_impl_drop {
            code.push_str(&format!("impl Drop for {} {{ fn drop(&mut self) {{ crate::{}_delete(unsafe {{ mem::transmute(self) }}); }} }}\r\n", 
    ...
    for (enum_name, class_data) in &enum_map {
        ...
        if should_impl_drop {
            code.push_str(&format!("impl Drop for {}EnumWrapper {{ fn drop(&mut self) {{ crate::{}_delete(unsafe {{ mem::transmute(self) }}); }} }}\r\n", 
                                 enum_name, enum_name));
        }
    }
    ...
    for (module_name, module) in &version_data.modules {
        for (class_name, class_data) in &module.classes {
            ...
            if class_data.struct_fields.is_some() {
                ...
                if let Some(constructors) = &class_data.constructors {
                    for (constructor_name, constructor) in constructors {
                        ...
                        if constructor.fn_args.iter().any(|arg| {
                            arg.iter().any(|(_, arg_type)| arg_type == "RefAny" || arg_type.contains('*'))
                        }) {
                            continue;
                        }
                        ...
                        code.push_str(&format!("    fn {}(/* args */) -> {} {{\r\n", constructor_name, class_name_with_prefix));
                        code.push_str("        // Implementation would convert Python args to Rust types\r\n");
                ...
                if let Some(functions) = &class_data.functions {
                    for (function_name, function) in functions {
                        ...
                        if function.fn_args.iter().any(|arg| {
                            arg.iter().any(|(name, arg_type)| name != "self" && (arg_type == "RefAny" || arg_type.contains('*')))
                        }) {
                            continue;
                        }
                        
                        // Generate simplified Python method
                        code.push_str(&format!("    fn {}(&self/* , args */) -> PyResult<()> {{\r\n", function_name));
                        code.push_str("        // Implementation would convert Python args to Rust types,\r\n");
                        code.push_str("        // call the appropriate C function, and convert results back\r\n");
                        code.push_str("        Ok(())\r\n");
                        code.push_str("    }\r\n\r\n");
                    }
                }
                ...
                code.push_str(&format!("impl PyObjectProtocol for {} {{\r\n", class_name_with_prefix));
                code.push_str("    fn __str__(&self) -> Result<String, PyErr> { \r\n");
                if let Some(external) = &class_data.external {
                    code.push_str(&format!("        let m: &{} = unsafe {{ mem::transmute(self) }}; Ok(format!(\"{{:#?}}\", m))\r\n", external));
                } else {
            ...
            // Generate methods for enums
                    ...
                    for (variant_name, variant_data) in variant_map {
                        if is_union {
                            ...
                            if let Some(variant_type) = &variant_data.r#type {
                                ...
                                code.push_str(&format!("        {} {{ inner: {}::{}", wrapper_name, class_name_with_prefix, variant_name));
                                code.push_str("(v) }}\r\n");
                                code.push_str("    }\r\n");
                            } else {
                                // Unit variant
                            ...
                            code.push_str("    #[classattr]\r\n");
                            code.push_str(&format!("    const {}: {} = {}::{};\r\n", 
                                                 variant_name, class_name_with_prefix, class_name_with_prefix, variant_name));
                        }
                    }
                }
                ...
                if is_union {
                    ...
                    code.push_str(&format!("        use crate::python::{};\r\n", class_name_with_prefix));
                    ...
                    for variant_map in enum_fields {
                        for (variant_name, variant_data) in variant_map {
                            let opt_variant_type_match = if variant_data.r#type.is_some() { "(v)" } else { "" };
                            ...
                            let opt_variant_value = if let Some(variant_type) = &variant_data.r#type {
                                if variant_type == "*const c_void" || variant_type == "*mut c_void" {
                                    ...
                                    if is_primitive_arg(&base_type) {
                                        ...
                                        if let Some(found_class) = get_class(api_data, &type_class_name, &base_type) {
                                            if found_class.callback_typedef.is_some() {
                                                "()" // Function pointer
                                            } else if found_class.enum_fields.is_some() {
                                                "{ let m: &AzEnumWrapper = unsafe { mem::transmute(v) }; m.clone() }"
                                            } else {
                                                "v.clone()"
                                            }
                                        } else {
                            ...
                            code.push_str(&format!("            {}::{}{} => Ok(vec![\"{}\".into_py(py), {}.into_py(py)]),\r\n", 
                                                 class_name_with_prefix, variant_name, opt_variant_type_match, variant_name, opt_variant_value));
                        }
                    }
                ...
                code.push_str("    fn __str__(&self) -> Result<String, PyErr> { \r\n");
                if let Some(external) = &class_data.external {
                    code.push_str(&format!("        let m: &{} = unsafe {{ mem::transmute(&self.inner) }}; Ok(format!(\"{{:#?}}\", m))\r\n", external));
                } else {
                ...
                if !is_union {
                    ...
                    code.push_str("            pyo3::class::basic::CompareOp::Lt => { Ok((self.clone().inner as usize) <  (other.clone().inner as usize)) }\r\n");
                    code.push_str("            pyo3::class::basic::CompareOp::Le => { Ok((self.clone().inner as usize) <= (other.clone().inner as usize)) }\r\n");
                    code.push_str("            pyo3::class::basic::CompareOp::Eq => { Ok((self.clone().inner as usize) == (other.clone().inner as usize)) }\r\n");
                    code.push_str("            pyo3::class::basic::CompareOp::Ne => { Ok((self.clone().inner as usize) != (other.clone().inner as usize)) }\r\n");
                    code.push_str("            pyo3::class::basic::CompareOp::Gt => { Ok((self.clone().inner as usize) >  (other.clone().inner as usize)) }\r\n");
                    code.push_str("            pyo3::class::basic::CompareOp::Ge => { Ok((self.clone().inner as usize) >= (other.clone().inner as usize)) }\r\n");
                    code.push_str("        }\r\n");
                    code.push_str("    }\r\n");
                }
    ...
    code.push_str("\r\n");
    code.push_str("        if std::env::var(\"AZUL_PY_LOGLEVEL_ERROR\").is_ok() { filter = log::LevelFilter::Error; }\r\n");
    code.push_str("        if std::env::var(\"AZUL_PY_LOGLEVEL_WARN\").is_ok() { filter = log::LevelFilter::Warn; }\r\n");
    code.push_str("        if std::env::var(\"AZUL_PY_LOGLEVEL_INFO\").is_ok() { filter = log::LevelFilter::Info; }\r\n");
    code.push_str("        if std::env::var(\"AZUL_PY_LOGLEVEL_DEBUG\").is_ok() { filter = log::LevelFilter::Debug; }\r\n");
    code.push_str("        if std::env::var(\"AZUL_PY_LOGLEVEL_TRACE\").is_ok() { filter = log::LevelFilter::Trace; }\r\n");
    code.push_str("        if std::env::var(\"AZUL_PY_LOGLEVEL_OFF\").is_ok() { filter = log::LevelFilter::Off; }\r\n");
    code.push_str("    }\r\n");
    code.push_str("\r\n");
    ...
    for (module_name, module) in &version_data.modules {
        for (class_name, class_data) in &module.classes {
            ...
            if class_data.struct_fields.is_some() {
                ...
                code.push_str(&format!("    m.add_class::<{}EnumWrapper>()?;\r\n", class_name_with_prefix));
            }
        }
        code.push_str("\r\n");
    }
...
fn collect_structs_and_enums<'a>(
    api_data: &'a ApiData
) -> (IndexMap<String, &'a ClassData>, IndexMap<String, &'a ClassData>) {
