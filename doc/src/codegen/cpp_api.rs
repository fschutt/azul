...
impl CppVersion {
    ...
    pub fn has_optional(&self) -> bool {
        matches!(
            self,
            CppVersion::Cpp17 | CppVersion::Cpp20 | CppVersion::Cpp23
        )
    }
...
fn class_is_option_type(class_name: &str, class_data: &ClassData) -> bool {
    ...
    if let Some(enum_fields) = &class_data.enum_fields {
        let variant_names: Vec<&str> = enum_fields
            .iter()
            .flat_map(|field_map| field_map.keys().map(|s| s.as_str()))
            .collect();
...
pub fn generate_cpp_api_for_version(
    api_data: &ApiData,
    version: &str,
...
pub fn generate_cpp_api_versioned(
    api_data: &ApiData,
    version: &str,
    cpp_version: CppVersion,
) -> String {
    ...
    if !cpp_version.has_move_semantics() {
        ...
        code.push_str(
            "// RAII objects by value. This header uses the \"Colvin-Gibbons trick\" (also\r\n",
        );
        code.push_str(
            "// known as the \"Move Constructor Idiom\") to work around this limitation.\r\n",
        );
        code.push_str("//\r\n");
        code.push_str(
            "// This is essentially a manual, type-safe implementation of std::auto_ptr's\r\n",
        );
        ...
        code.push_str(
            "//   String a = String::fromConstStr(\"hello\");  // OK: uses Proxy path\r\n",
        );
        code.push_str(
            "//   String b = a;                               // OK but TRANSFERS ownership from \
             a!\r\n",
        );
        ...
        code.push_str("//\r\n");
        code.push_str(
            "// C++03 containers require \"Copy Constructible\" and \"Assignable\" semantics\r\n",
        );
        code.push_str("// where copying A to B leaves both A and B as equivalent copies.\r\n");
        code.push_str(
            "// These wrappers use destructive copy (like std::auto_ptr), which violates\r\n",
        );
        ...
        code.push_str(
            "// 3. Upgrade to C++11 where move semantics work correctly with containers\r\n",
        );
        code.push_str("//\r\n");
        code.push_str(
            "// Reference: https://en.wikibooks.org/wiki/More_C%2B%2B_Idioms/Move_Constructor\r\n",
        );
        code.push_str("//\r\n");
    }
...
fn generate_cpp_class_declaration(
    ...
    // For C++03 non-Copy types: use mutable to allow "stealing" in const copy constructor (like
    // std::auto_ptr) For C++11+ and Copy types: no mutable needed
    if !cpp_version.has_move_semantics() && !is_copy {
    ...
    let noexcept = if cpp_version.has_noexcept() {
        " noexcept"
    } else {
        ""
    };
    ...
    if !cpp_version.has_move_semantics() && !is_copy {
        ...
        code.push_str(&format!(
            "    {}(const {}& other) : inner_(other.inner_) {{ std::memset(&other.inner_, 0, \
             sizeof(other.inner_)); }}\r\n",
            class_name, class_name
        ));
        code.push_str(&format!(
    ...
    if let Some(constructors) = &class_data.constructors {
        ...
        for (fn_name, constructor) in constructors {
            ...
            let cpp_args =
                generate_cpp_args_signature(constructor, version_data, cpp_version, false);
            let nodiscard = if cpp_version.has_nodiscard() {
                "[[nodiscard]] "
            } else {
                ""
            };
    ...
    if cpp_version.has_move_semantics() {
        ...
        code.push_str(&format!(
            "    {} release() {{ {} result = inner_; std::memset(&inner_, 0, sizeof(inner_)); \
             return result; }}\r\n",
            c_type_name, c_type_name
        ));
    }
    ...
    if is_vec {
        if let Some(elem_type) = get_vec_element_type(class_data) {
            ...
            if cpp_version.has_move_semantics() {
                code.push_str(&format!(
                    "    std::vector<{}> toStdVector() const {{ return std::vector<{}>(begin(), \
                     end()); }}\r\n",
                    c_elem_type, c_elem_type
                ));
            }
            ...
            if cpp_version.has_span() {
                code.push_str(&format!(
                    "    std::span<const {}> toSpan() const {{ return std::span<const \
                     {}>(begin(), size()); }}\r\n",
                    c_elem_type, c_elem_type
                ));
                code.push_str(&format!(
        ...
        if cpp_version.has_move_semantics() {
            ...
            code.push_str(
                "    explicit String(const char* s) : \
                 inner_(AzString_copyFromBytes(reinterpret_cast<const uint8_t*>(s), 0, \
                 std::strlen(s))) {}\r\n",
            );
        }
    ...
    if class_is_option_type(class_name, class_data) {
        if let Some(inner_type) = get_option_inner_type(class_data) {
            ...
            code.push_str(&format!(
                "    {} unwrapOr(const {}& defaultValue) const {{ return isSome() ? \
                 inner_.Some.payload : defaultValue; }}\r\n",
    ...
    if class_is_result_type(class_name, class_data) {
        if let Some((ok_type, err_type)) = get_result_types(class_data) {
            ...
            code.push_str(&format!(
                "    const {}& unwrap() const {{ return inner_.Ok.payload; }}\r\n",
                c_ok_type
            ));
            code.push_str(&format!(
                "    {}& unwrap() {{ return inner_.Ok.payload; }}\r\n",
                c_ok_type
            ));

            // unwrapErr() - returns the Err value, undefined behavior if Ok
            code.push_str(&format!(
                "    const {}& unwrapErr() const {{ return inner_.Err.payload; }}\r\n",
                c_err_type
            ));
            code.push_str(&format!(
                "    {}& unwrapErr() {{ return inner_.Err.payload; }}\r\n",
                c_err_type
            ));
            ...
            if cpp_version.has_expected() {
                code.push_str(&format!(
                    "    std::expected<{}, {}> toStdExpected() const {{\r\n",
                    c_ok_type, c_err_type
                ));
                code.push_str(&format!(
                    "        return isOk() ? std::expected<{}, {}>(inner_.Ok.payload) : \
                     std::unexpected(inner_.Err.payload);\r\n",
                    c_ok_type, c_err_type
                ));
                code.push_str("    }\r\n");
                code.push_str(&format!(
                    "    operator std::expected<{}, {}>() const {{ return toStdExpected(); }}\r\n",
                    c_ok_type, c_err_type
                ));
            }
        }
...
fn generate_cpp_method_implementations(
    ...
    if let Some(functions) = &class_data.functions {
        for (fn_name, function) in functions {
            ...
            let (is_const, self_is_value) = function
                .fn_args
                .first()
                .and_then(|arg| arg.iter().next())
                .map(|(name, typ)| {
            ...
            if cpp_return_type == "void" {
                ...
                if let Some(ret) = &function.returns {
                    ...
                    if !is_primitive_arg(&base_type)
                        && type_has_cpp_wrapper(&base_type, version_data)
                    {
                        // For C++03, check if return type needs Proxy path
                        if !cpp_version.has_move_semantics()
                            && type_needs_proxy_for_cpp03(&base_type, version_data)
                        {
                            // Use explicit variable to avoid syntax ambiguity with functional cast
                            code.push_str(&format!(
                                "    {}::Proxy _p({}({}));\r\n",
...
fn generate_cpp_call_args(
    ...
    for (i, arg_map) in func.fn_args.iter().enumerate() {
        for (arg_name, arg_type) in arg_map {
            ...
            // Check if this is a pointer type
            let is_pointer = prefix.contains("*const")
                || prefix.contains("* const")
                || prefix.contains("*mut")
                || prefix.contains("* mut");
