...
pub fn generate_release_html(version: &str, api_data: &ApiData, assets: &ReleaseAssets) -> String {
    ...
    for (key, asset) in &assets.bindings {
        // Only the shipped tier (docgen::SHIPPED_LANGUAGES, what CI e2e-gates
        // on) is shown here — the rest are generated but not something we
        // actually ship.
        if !crate::docgen::is_shipped_language(key) {
            continue;
        }
    ...
    const ANDROID_READY: &[&str] = &["AzMaps", "AzPaint", "AzWidgets"];
    ...
    <section class='docs-body'>
      <div class='container'>
        ...
        <div class='docs-content'>
              ...
              <p>For detailed installation instructions please see the main <a href='{HTML_ROOT}/'>/ui page</a>.</p>
              ...
              <pre><code class='language-bash'>docker pull ghcr.io/fschutt/azul:{version}</code></pre>
