...
/// Extract the language code from a path like `doc/guide/<lang>/foo.md`.
/// Returns None if the path doesn't fit the layout.
pub fn path_language(path: &str) -> Option<&str> {
    let rest = path.strip_prefix("doc/guide/")?;
...
/// Strip `doc/guide/<lang>/` from a path, returning the part relative to
/// that language's root. Used when emitting SUMMARY.md links.
fn path_within_language(path: &str) -> Option<&str> {
    let rest = path.strip_prefix("doc/guide/")?;
    rest.split_once('/').map(|x| x.1)
}

// ── SUMMARY.md generation (per-language) ──────────────────────────────

pub fn regenerate_summary(project_root: &Path, manifest: &Manifest) -> Result<(), String> {
    // Bucket outputs by language extracted from the output path.
    let mut by_lang: BTreeMap<String, Vec<(&Output, &Group)>> = BTreeMap::new();
    for g in &manifest.groups {
        for o in &g.outputs {
            let lang = match path_language(&o.path) {
                Some(l) => l.to_string(),
                None => {
                    eprintln!(
                        "warning: output `{}` is not under doc/guide/<lang>/, skipping",
                        o.path
                    );
                    continue;
                }
            };
            by_lang.entry(lang).or_default().push((o, g));
        }
    }

    for (lang, outs) in by_lang {
        write_summary_for_language(project_root, &lang, &outs)?;
    }
    Ok(())
}

fn write_summary_for_language(
    project_root: &Path,
    lang: &str,
    outs: &[(&Output, &Group)],
) -> Result<(), String> {
    let path = project_root.join(format!("doc/guide/{}/SUMMARY.md", lang));
    let mut s = String::new();
    s.push_str(&format!("# Summary ({})\n\n", lang));
    s.push_str("> Auto-generated by `azul-doc autoreview autodoc-summary`.  \
                Hand-edits in this file will be overwritten on regeneration.\n\n");

    let mut external_linear: Vec<&Output> = Vec::new();
    let mut external_topics: Vec<&Output> = Vec::new();
    let mut contributor: Vec<&Output> = Vec::new();
    ...
    for (o, g) in outs {
        ...
        match aud {
            Some("external") => {
                if o.topic_only {
                    external_topics.push(o);
                } else {
                    external_linear.push(o);
                }
            }
            Some("contributor") => contributor.push(o),
            _ => {}
        }
    }

    let badge = |m: &str| match m {
        "wip" => " (WIP)",
    ...
    s.push_str("# Guide\n\n");
    external_linear.sort_by_key(|o| o.guide_order.unwrap_or(i32::MAX));
    for o in &external_linear {
        let rel = path_within_language(&o.path).unwrap_or(&o.path);
        s.push_str(&format!("- [{}{}]({})\n", o.title, badge(&o.maturity), rel));
    }

    if !external_topics.is_empty() {
        s.push_str("\n# Topics\n\n");
        external_topics.sort_by(|a, b| a.slug.cmp(&b.slug));
        for o in &external_topics {
            let rel = path_within_language(&o.path).unwrap_or(&o.path);
            s.push_str(&format!("- [{}{}]({})\n", o.title, badge(&o.maturity), rel));
        }
    }
