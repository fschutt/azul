...
fn run_summarize_highlevel(config: AutoreviewConfig) -> Result<(), String> {
    ...
    if all_reports.is_empty() {
        return Err(format!(
            "No review reports found at {}*.md on '{}'. Has `autoreview` been run \
             on that branch?",
    ...
    if !superplan.exists() {
        ...
        let header = format!(
            "# High-level / architectural cleanup super-plan\n\
             ...
             reports on `{}` ({}). Each bullet is one architectural item an agent\n\
                ...
                match parse_highlevel_verdict(&text) {
                    ...
                    None => {
                        ...
                        append_superplan_line(
                            &superplan,
                            &format!("- [UNCERTAIN] agent gave no clear verdict — review {} by hand  _(from {})_", name, name),
                        )?;
