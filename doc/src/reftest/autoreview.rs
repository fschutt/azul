...
pub struct AutoreviewConfig {
    ...
    /// `summarize-highlevel`: stop after this many reports this run (`None` =
    /// process all remaining).
    pub limit: Option<usize>,
    /// `summarize-highlevel`: the branch/tag whose committed review reports
    /// (`doc/target/autoreview/reports/*.md`) are summarized. Defaults to
    /// `midlevel-fixes-reference`.
...
pub enum AutoreviewSubcommand {
    ...
    AutodocCheck,
    AutodocScreenshots,
    /// For each committed review report, ask a low-effort agent whether its
    /// high-level / architectural finding still applies to the current tree,
    /// and append a one-line bullet to `scripts/HIGHLEVEL_SUPERPLAN.md`.
    SummarizeHighlevel,
}
...
fn preflight_checks(project_root: &Path, dry_run: bool) -> Result<(), String> {
    ...
    if !dry_run {
        ...
        if std::env::var("ANTHROPIC_API_KEY").is_ok() {
            return Err(
                "ANTHROPIC_API_KEY is set.\nUnset it first: unset ANTHROPIC_API_KEY".into(),
            );
        }
...
// ── Subcommand: summarize-highlevel ──────────────────────────────────────

/// The committed review reports live on the reference branch under this path.
/// (On master they're gitignored + wiped by `cargo clean`, so we read them
/// straight out of the reference branch's tree.)
const HIGHLEVEL_REPORTS_PREFIX: &str = "doc/target/autoreview/reports/";

fn highlevel_superplan_path(project_root: &Path) -> PathBuf {
    project_root.join("scripts/HIGHLEVEL_SUPERPLAN.md")
}

/// Done-list lives in the cargo-clean-safe `.apply-midlevel/` dir (NOT under
/// `target/`, which the apply agents wipe), one report path per line.
fn highlevel_progress_path(project_root: &Path) -> PathBuf {
    project_root.join(".apply-midlevel/highlevel-progress.txt")
}

/// `autoreview summarize-highlevel`: for each committed review report on the
/// reference branch, spawn one low-effort agent to decide whether the report's
/// HIGH-LEVEL / architectural finding still applies to the current (drifted)
/// tree, and append a single bullet to `scripts/HIGHLEVEL_SUPERPLAN.md`.
///
/// Sequential (one agent at a time), resumable (a done-list is kept so re-runs
/// pick up where they left off), and NON-committing — the resulting super-plan
/// is left uncommitted so a human can curate it into the final cleanup plan
...
fn run_summarize_highlevel(config: AutoreviewConfig) -> Result<(), String> {
    ...
    if all_reports.is_empty() {
        return Err(format!(
            "No review reports found at {}*.md on '{}'. Has `autoreview` been run on that branch?",
            HIGHLEVEL_REPORTS_PREFIX, reference
        ));
    }

    // Load the resume set.
    let progress_path = highlevel_progress_path(&project_root);
    fs::create_dir_all(progress_path.parent().unwrap()).ok();
    let mut done: std::collections::HashSet<String> = fs::read_to_string(&progress_path)
        .unwrap_or_default()
        .lines()
    ...
    // Seed the super-plan with a header on first run.
    let superplan = highlevel_superplan_path(&project_root);
    if !superplan.exists() {
        fs::create_dir_all(superplan.parent().unwrap()).ok();
        let header = format!(
            "# High-level / architectural cleanup super-plan\n\nGenerated by `azul-doc autoreview \
             summarize-highlevel` from the review\nreports on `{}` ({}). Each bullet is one \
             architectural item an agent\njudged STILL RELEVANT to the current tree. \
             `[UNCERTAIN]` items need a\nsecond look. This file is intentionally left UNCOMMITTED \
    ...
    let work_dir = project_root.join(".apply-midlevel/highlevel");
    ...
    for (i, report) in todo.iter().enumerate() {
        ...
        if let Some(n) = config.limit {
            if processed_this_run >= n {
                println!("\n--limit={} reached. Re-run to continue.", n);
                break;
            }
        }

        let name = report.rsplit('/').next().unwrap_or(report);
        println!("[{}/{}] {}", i + 1, todo.len(), name);

        // Pull the report's content out of the reference branch's tree.
        let content = match git_show_file(&project_root, &reference, report) {
            Ok(c) => c,
            Err(e) => {
                println!("  [warn] could not read report ({}); skipping", e);
                errored += 1;
                continue;
            }
        };
        ...
        match result {
            Ok(text) => {
                match parse_highlevel_verdict(&text) {
                    ...
                    Some(HighlevelVerdict::Obsolete(reason)) => {
                        println!("  → OBSOLETE: {}", reason);
                        obsolete += 1;
                    }
                    None => {
                        // No parseable verdict — record an UNCERTAIN pointer so
                        // the item isn't silently lost, and move on.
                        append_superplan_line(
                            &superplan,
                            &format!(
                                "- [UNCERTAIN] agent gave no clear verdict — review {} by hand  \
                                 _(from {})_",
                                name, name
                            ),
                        )?;
                        println!("  → no VERDICT line parsed; recorded as UNCERTAIN");
                        uncertain += 1;
                    }
                }
            }
            Err(e) => {
                println!("  [error] agent failed: {} — leaving for a later run", e);
                errored += 1;
                // Do NOT mark done, so a re-run retries this report.
                continue;
            }
        }

        // Mark done (append to the resume list) and advance.
        done.insert(report.clone());
        append_superplan_line(&progress_path, report)?;
...
enum HighlevelVerdict {
    Relevant(String),
    Obsolete(String),
    Uncertain(String),
}

/// Parse the agent's final `VERDICT: <KIND> | <text>` line (last one wins).
fn parse_highlevel_verdict(text: &str) -> Option<HighlevelVerdict> {
    for line in text.lines().rev() {
        let t = line.trim();
        let rest = match t.strip_prefix("VERDICT:") {
            Some(r) => r.trim(),
            None => continue,
        };
        ...
        return Some(match kind.as_str() {
            ...
            _ => HighlevelVerdict::Uncertain(payload),
        });
    }
    None
}

fn build_highlevel_prompt(report_name: &str, report_content: &str) -> String {
    let mut p = String::new();
    p.push_str("You are evaluating ONE automated code-review report to decide whether its\n");
    p.push_str("HIGH-LEVEL / ARCHITECTURAL finding(s) still apply to the CURRENT state of\n");
    p.push_str("this repository. The tree has drifted ~1000 commits since the report was\n");
    p.push_str("written, but the overall architecture is unchanged.\n\n");

    p.push_str("STRICT RULES:\n");
    p.push_str("  - DO NOT modify any files. DO NOT commit. This is read-only analysis.\n");
    p.push_str("  - Investigate the CURRENT tree (Read / Grep / `git log`) to verify each\n");
    p.push_str("    architectural concern is still real — cite file:line evidence.\n");
    p.push_str("  - We care ONLY about HIGH-LEVEL / architectural items: systemic design\n");
    p.push_str("    issues, cross-cutting concerns, missing/half-wired subsystems, and\n");
    p.push_str("    performance/logic-architecture problems. IGNORE purely local nits\n");
    p.push_str("    (a single unused import, one typo, a micro-refactor in one function) —\n");
    p.push_str("    a separate mid-level pass handles those.\n\n");

    p.push_str("STEPS:\n");
    p.push_str("  1. Read the report below.\n");
    p.push_str("  2. Extract any HIGH-LEVEL / architectural finding. If the report only\n");
    p.push_str("     contains trivial local nits, there is nothing high-level here.\n");
    p.push_str("  3. Check the current tree: is the architectural concern still present /\n");
    p.push_str("     unresolved / relevant?\n");
    p.push_str("  4. Produce ONE overall verdict for this report.\n\n");

    p.push_str("OUTPUT: end your response with EXACTLY ONE line in this format. It is parsed\n");
    p.push_str("by a script, so no other line may start with `VERDICT:`:\n\n");
    p.push_str("  VERDICT: RELEVANT | <one imperative bullet for the cleanup super-plan;\n");
    p.push_str("                       name the system + the concrete action; <=200 chars>\n");
    p.push_str("  VERDICT: OBSOLETE | <<=120-char reason it no longer applies>\n");
    p.push_str("  VERDICT: UNCERTAIN | <one bullet + what a human should double-check>\n\n");
    p.push_str("Use OBSOLETE when the architectural issue is already fixed or there was no\n");
    p.push_str("high-level item to begin with. Be terse.\n\n");

    p.push_str(&format!("Report file: {}\n", report_name));
    p.push_str("=== REPORT ===\n");
    p.push_str(report_content);
    p.push_str("\n=== END REPORT ===\n");
    p
}

/// Spawn one read-only Claude agent for a single report and return its text.
/// Modeled on `run_midlevel_agent` but captures the result and forbids edits.
fn run_highlevel_agent(
    project_root: &Path,
    work_dir: &Path,
    report_name: &str,
    prompt_text: &str,
...
fn append_superplan_line(path: &Path, line: &str) -> Result<(), String> {
...
fn git_rev_parse(project_root: &Path, refname: &str) -> Result<String, String> {
...
fn git_show_file(project_root: &Path, refname: &str, path: &str) -> Result<String, String> {
    let out = Command::new("git")
