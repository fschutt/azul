...
pub fn run_debug_analysis(config: DebugConfig) -> anyhow::Result<()> {
    println!("[DEBUG] Starting debug analysis for test: {}", config.test_name);
    ...
    if is_test_passed(&config)? {
        println!("[DEBUG] Test '{}' has already passed, skipping", config.test_name);
        return Ok(());
    }
    ...
    if diff_count <= PASS_THRESHOLD {
        ...
        println!("\n✅ Test '{}' passes ({} pixels different = {:.3}%, threshold 0.5%). No debug needed.", 
            config.test_name, diff_count, percentage);
        return Ok(());
    }
    
    let percentage = (diff_count as f64 / (1920.0 * 1080.0)) * 100.0;
    println!("[DEBUG] Pixel difference: {} ({:.3}% of screen)", diff_count, percentage);
    ...
    println!("[DEBUG] Final prompt: {} tokens ({} chars)", total_tokens, prompt.len());
    
    // Save prompt to file
    fs::create_dir_all(&config.output_dir)?;
    
    let prompt_path = config.output_dir.join(format!("{}_prompt.md", config.test_name));
    fs::write(&prompt_path, &prompt)?;
    println!("[DEBUG] Prompt saved to: {:?}", prompt_path);
    
    // Save debug data as JSON
    let json_path = config.output_dir.join(format!("{}_debug.json", config.test_name));
    ...
    "source_files": {}
        ...
        debug_data.chrome_layout_data.as_ref().map(|s| s.len()).unwrap_or(0),
        debug_data.azul_display_list.as_ref().map(|s| s.len()).unwrap_or(0),
    ...
    println!("\n🤖 Sending to Gemini API... This will take a while ({} tokens sent)", total_tokens);
    
    // Call Gemini API
    let response = call_gemini_api(&api_key, &debug_data, &prompt)?;
    
    // Save response
    let response_path = config.output_dir.join(format!("{}_response.md", config.test_name));
...
fn find_test_file(config: &DebugConfig) -> anyhow::Result<PathBuf> {
    ...
    Err(anyhow::anyhow!("Test file not found for: {}", config.test_name))
}

/// Run the test and collect all debug data
fn run_test_and_collect_data(config: &DebugConfig, test_file: &Path) -> anyhow::Result<TestDebugData> {
    // Read XHTML source
...
fn generate_azul_with_debug(
    ...
    let css_warnings = if debug_data.css_warnings.is_empty() {
        ...
        debug_data.css_warnings.lines().map(|s| s.to_string()).collect()
    };
...
fn collect_source_code_with_budget(config: &DebugConfig) -> anyhow::Result<Vec<(String, String)>> {
    ...
    println!("[DEBUG] Reserved {} tokens for images (2 x ULTRA_HIGH)", image_tokens);
    ...
    for (priority, path) in source_files {
        ...
        if total_tokens + tokens > token_budget {
            println!("[DEBUG] Token budget reached, skipping {:?} ({} tokens)", path, tokens);
            break;
        }
        
        let relative_path = path.strip_prefix(&config.azul_root)
            .unwrap_or(&path)
            .to_string_lossy()
            .to_string();
        
        println!("[DEBUG] Including {:?}: {} tokens (priority: {:?})", 
    ...
    println!("[DEBUG] Total source code tokens: {} / {}", total_tokens, token_budget);
...
fn build_prompt(
    ...
    prompt.push_str("The test below is failing - the Azul layout engine produces different output than Chrome.\n\n");
    ...
    prompt.push_str("Below are the screenshots from Chrome (reference) and Azul (implementation).\n");
    prompt.push_str("Compare them visually to identify layout differences.\n\n");
    
    if let Some(ref chrome_base64) = debug_data.chrome_image_base64 {
        prompt.push_str("### Chrome Reference Screenshot\n\n");
        prompt.push_str(&format!("![Chrome Reference](data:image/png;base64,{})\n\n", chrome_base64));
    }
    
    if let Some(ref azul_base64) = debug_data.azul_image_base64 {
        prompt.push_str("### Azul Implementation Screenshot\n\n");
        prompt.push_str(&format!("![Azul Implementation](data:image/webp;base64,{})\n\n", azul_base64));
    }
    ...
    prompt.push_str("**IMPORTANT: If the Chrome and Azul screenshots look identical (test passes), respond with just: SUCCESS**\n\n");
    prompt.push_str("Otherwise, if there are differences:\n");
    prompt.push_str("1. Compare the Chrome and Azul screenshots visually to identify differences.\n");
    prompt.push_str("2. Analyze the Chrome reference layout JSON and compare it to Azul's display list.\n");
    prompt.push_str("3. Identify specific differences in positioning, sizing, or styling.\n");
    prompt.push_str("4. Trace through the source code to find the likely cause of the discrepancy.\n");
