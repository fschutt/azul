...
The CSS parser turns a `&str` stylesheet into a `Css` value (`css/src/css.rs`), a `Vec<Stylesheet>` of typed [`CssRuleBlock`](https://github.com/maps4print/azul/blob/master/css/src/css.rs) entries, each pairing a [`CssPath`](https://github.com/maps4print/azul/blob/master/css/src/css.rs) selector with a `Vec<CssDeclaration>`. The entry point is `new_from_str` (`css/src/parser2.rs::new_from_str`); it never panics.

## File map

- **`new_from_str`.** Top-level CSS parser; returns `(Css, Vec<CssParseWarnMsg>)`.
  - `css/src/parser2.rs::new_from_str`
- **`parse_css_path`.** Selector parser used by `<style>` elements with explicit selectors and by codegen.
  - `css/src/parser2.rs::parse_css_path`
- **`CssProperty` enum.** 180 typed property variants.
  - `css/src/props/property.rs::CssProperty`
- **`CssPropertyType` enum.** 180 discriminants for property keys.
  - `css/src/props/property.rs::CssPropertyType`
- **`parse_css_property(key, value) -> Result<CssProperty, _>`.** The dispatch table.
  - `css/src/props/property.rs::parse_css_property`
- **`impl_pixel_value!`, `impl_percentage_value!`, `css_property_from_type!` macros.** Boilerplate generators for property types.
  - `css/src/props/macros.rs`
- **Primitive types.** Length, color, angle, font, time, geometry, image, and direction.
  - `css/src/props/basic/`
- **Layout properties.** Display, flex, grid, position, dimensions, and more.
  - `css/src/props/layout/`
- **Paint properties.** Background, border, shadow, transform, filter, and more.
  - `css/src/props/style/`
...
- The `*` token maps to `Global`.
- A bare `tag` maps to `Type(NodeTypeTag)` if the tag is recognized; it's silently dropped otherwise.
- The `#id` token maps to `Id(AzString)`.
- The `.class` token maps to `Class(AzString)`.
- A space maps to `Children` (descendant).
- The `>` token maps to `DirectChildren`.
- The `+` token maps to `AdjacentSibling`.
- The `~` token maps to `GeneralSibling`.
- The `:foo` and `:foo(arg)` tokens map to `PseudoSelector` (see `pseudo_selector_from_str`).
...
Defined at `css/src/props/property.rs::parse_css_property`. Three short-circuits run before the per-property dispatch:
...
`has_typed_auto` / `has_typed_none` (`css/src/props/property.rs::has_typed_auto`) list the properties for which `auto`/`none` is a *typed* value rather than the generic CSS keyword (e.g. `display: none` is `LayoutDisplay::None`, not `CssPropertyValue::None`). The dispatch then matches the 180 variants of `CssPropertyType` to their parser:
...
`impl_pixel_value!(LayoutWidth)` (`macros.rs::impl_pixel_value`) generates 16 methods on a struct with an `inner: PixelValue` field: `zero()`, `const_px(isize)`, `const_em`, `const_pt`, `const_percent`, `const_in`, `const_cm`, `const_mm`, `const_from_metric`, `px(f32)`, `em`, `pt`, `percent`, `from_metric`, `interpolate`. Every numeric layout property uses this macro.

`impl_percentage_value!(StyleOpacity)` (`macros.rs::impl_percentage_value`) does the equivalent for percentage wrappers, plus `Display`/`Debug` impls that print as `"X%"`.
...
- The `parse_parentheses(input, &["url"]) -> Result<(stopword, inside)>` helper matches `<stopword>(...)` and returns the contents.
- The `strip_quotes(input) -> Result<QuoteStripped>` helper strips matching `"..."` or `'...'`. It errors if quotes don't match.
- The `parse_image(input) -> Result<AzString>` helper calls `strip_quotes`, falling back to the trimmed input on no-quotes.
...
`ParseFloatError` and `ParseIntError` (`basic/error.rs`) are `#[repr(C)]` mirrors of the `core::num` types. Rust's privacy on `ParseFloatError::kind` means we can't pass it through FFI directly, so the kind is reconstructed by comparing against known instances (`error.rs::reconstruct_float_kind`).
...
3. Add a variant to `CssProperty` (`property.rs::CssProperty`) and `CssPropertyType` (`property.rs::CssPropertyType`).
4. Add an arm to `parse_css_property` (`property.rs::parse_css_property`) routing to your parser.
5. Add the new variant to `css_property_from_type!` (`macros.rs::css_property_from_type`) so `auto`/`none`/`initial`/`inherit` work generically.
