---
slug: internals/rendering/webrender-bridge
title: WebRender Bridge
language: en
canonical_slug: internals/rendering/webrender-bridge
...
- `compositor2.rs` translates `DisplayListItem` → WebRender display-list
  `push_*` calls (covered in [Rendering](../rendering.md)).
- `wr_translate2.rs` translates everything else: pipeline IDs, image keys,
  font keys, border radii, colors, clip-chain IDs, and the per-frame
...
display list references `FontInstanceKey`s and `ImageKey`s that must
...
The root DOM gets `PipelineId(0, document_id)`. IFrames and `VirtualView`s
...
documented in detail at [Rendering](../rendering.md).
...
The reverse path (cursor to tag) is owned by the platform shell. It
queries WebRender's hit tester each frame and routes results through the
event dispatch system. See [Hit Testing](../events/hit-testing.md) for the
full namespace layout and result-type mapping.
...
- **Property-only update** (`display_list_was_rebuilt = false`).
  ...
  `set_scroll_offsets`, `append_dynamic_properties`, and
  `set_document_view` apply.
...
- **Resource not registered before display list.** `txn.update_resources(...)`
  must run before `txn.set_display_list(...)` *in the same transaction*.
  If the call sites are reordered, the scene builder fails to resolve the
  key and the entire DOM renders blank.
- **Epoch not incremented after a rebuild.** WebRender uses `Epoch` to
  ...
  must run on every rebuild path.
- **Forgetting `set_root_pipeline`.** Required even if the display list
  was reused — WebRender needs to know which pipeline is the root for
  hit-testing and viewport calculations.
- **Dynamic property without a corresponding binding in the display list.**
  ...
  `compositor2` actually pushed a stacking context with the binding
  (search for the `OpacityKey::id` in the compositor logs).
...
- [Hit Testing](../events/hit-testing.md) — tag namespaces and reverse routing
