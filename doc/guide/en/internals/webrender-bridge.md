...
`generate_frame` in `wr_translate2.rs:1548` is the single entry point that
the platform shells call once per frame.
...
inside the parent display list (`compositor2.rs:1455`).
...
`webrender::ResourceUpdate`:

| Azul type | WebRender type |
|---|---|
| `ResourceUpdate::AddFont(AddFont { font, key })` | `ResourceUpdate::AddFont(WrFontKey, WrFontTemplate, ...)` |
| `ResourceUpdate::AddFontInstance(AddFontInstance { font_key, glyph_size, key })` | `ResourceUpdate::AddFontInstance(WrFontInstanceKey, WrFontKey, ...)` |
| `ResourceUpdate::AddImage(AddImage { key, descriptor, data })` | `ResourceUpdate::AddImage(WrImageKey, WrImageDescriptor, WrImageData)` |
| Removals | Symmetric `Delete*` variants |
...
- `font_hash_map: HashMap<u64, FontKey>` — `compositor2`'s `push_text` looks
  up `FontKey` by `font_hash` (`compositor2.rs:2350`).
- `currently_registered_fonts: HashMap<FontKey, (FontRef, BTreeMap<(Au,
  DpiScaleFactor), FontInstanceKey>)>` — same path resolves the
  `FontInstanceKey` by `(size, dpi)`.
- `image_key_map: HashMap<ImageKey, ImageRefHash>` — reverse lookup for hit-
  testing.
...
`synchronize_gpu_values` (`wr_translate2.rs:1875`) is the bridge between
`core/src/gpu.rs` and WebRender's `Transaction::append_dynamic_properties`. It
walks `layout_window.gpu_value_cache` (a `GpuValueCache` — see
`core/src/gpu.rs:46`) and emits one `PropertyValue<T>` per active key:
...
already in physical pixels. `compositor2.rs:1336` does this on display-list
emission; `synchronize_gpu_values` does the equivalent on every dynamic
update.
...
| Space | Where | Units |
|---|---|---|
| **Window logical (CSS)** | `DisplayList` items | Logical pixels (CSS px), absolute window coordinates |
| **Window physical** | WebRender display list | Physical pixels (logical × DPI), absolute window coordinates |
| **Stacking-context-relative physical** | Inside a stacking context that pushed an origin | Physical pixels minus the stacking context origin |

`compositor2`'s `resolve_rect` (`compositor2.rs:81`) and `resolve_point`
(`compositor2.rs:98`) fuse the DPI multiply and the offset subtract. The
*scroll* frame does **not** add to the offset stack, because WebRender scroll
frames share their parent's coordinate space — only stacking contexts and
reference frames push offsets. This is documented in detail at
[Rendering Pipeline § Coordinate offset](rendering-pipeline.md#coordinate-offset-stacking-context-scroll-frame).

Scroll positions: `scroll_all_nodes` (`wr_translate2.rs:1821`) reads the
`ScrollManager`'s state (logical CSS pixels), multiplies by `hidpi_factor`,
and submits via `txn.set_scroll_offsets`. The scroll offset and the display
list both end up in physical pixels, so the addition in WebRender's spatial
tree resolves correctly.
...
- `0x0500` — `TAG_TYPE_SCROLL_CONTAINER` (the scroll container itself, used
  for wheel/trackpad scroll target lookup, `compositor2.rs:1006`)

The reverse path (cursor → tag) is owned by the platform shell — it queries
...
- **Display list rebuilt** (`display_list_was_rebuilt = true`): full scene
  build. Resources, display lists, root pipeline all submitted.
...
callbacks: `Update::RefreshDom` → full rebuild; `Update::DoNothing` with only
GPU/scroll changes → property-only.
...
- **Epoch not incremented after a rebuild.** WebRender uses `Epoch` to
  ...
  must run on every rebuild path; check `wr_translate2.rs:1763`.
