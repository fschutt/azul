...
- **Accessibility tree push** — `A11yManager::update_tree` produces an `accesskit::TreeUpdate` that each platform adapter forwards to the OS bridge (`accesskit_unix`, `accesskit_macos`, `accesskit_windows`). Action requests come back through the same channel and are decoded by `decode_a11y_node_id` + `map_accesskit_action` (in `azul_layout::managers::a11y`).
