...
Notice the required `--features build-dll`. The DLL is built in `target/release/libazul.{so,dylib}` (or `azul.dll`). The header/bindings are previously generated by `azul-doc codegen all` and end up at `target/codegen/`. Copy both somewhere `dotnet` can find them.
...
While the bindings use things like `IntPtr` and a `HostInvoker` under the hood, these internals are abstracted away in the bindings, so the above example reads as "native" C#. Note that `WithCss("...")` accepts any CSS string, including inline `:hover { }`, `@media ... { }` and `@os(...)` queries. Callbacks are strongly-typed delegates:
...
The generated `Azul.cs` bindings automatically handle wrapping your data model into a type-erased `RefAny` handle and, crucially, perform the safe runtime downcast (as `T`) before invoking your callback. If the type mismatches, the wrapper catches it and returns `Update.DoNothing` (or an empty `Dom`), meaning there is no `IntPtr` ceremony or manual upcasting / downcasting code needed. Using `var app` disposes deterministically: `Dispose()` calls the C-side `_delete()`, so the native memory is released when the `App` goes out of scope.

At run time the generated `DllImportResolver` probes the project directory and the directory of the published executable for `libazul.dylib` / `libazul.so` / `azul.dll`, with `LD_LIBRARY_PATH` / `DYLD_LIBRARY_PATH` as a fallback for non-standard layouts.
