...
the [release page](https://azul.rs/ui/release/$VERSION) instead:
...
Azul is not published on crates.io yet, and the crate's Rust API surface is
GENERATED by the in-repo `azul-doc` tool (the generated sources are not
committed), so a bare `--git` dependency cannot build. Clone the repo at the
release tag, run the generator once, then depend on the crate by path (the
package is named `azul-dll`; renaming it to `azul` gives you the usual
`use azul::prelude::*` paths):

```sh
# one-time: clone the repo at the release tag + generate the bindings
...
# create new project and depend on the dll crate by path
...
(If you keep the generated files somewhere else, point the build at them with
`export AZ_CODEGEN_DIR=/path/to/generated-sources`.)
