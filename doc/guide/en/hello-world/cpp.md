...
all six `azul<NN>.hpp` and `hello-world.cpp` - in one download:
...
and end up at `target/codegen/`: `azul.h`, the six `azul<NN>.hpp` wrappers and `azul.cppm` for the C++20+ 
module partition. Copy the wrapper for your standard plus the DLL somewhere your compiler can find.
...
Since C++03 has no template member functions, `azul03.hpp` generates the same three functions from a 
macro instead: `AZ_REFLECT(MyDataModel)` emits `MyDataModel_upcast` / `_downcast_ref` / `_downcast_mut`. 
...
AzJson MyDataModel_toJson(AzRefAny refany) {
    MyDataModel const* d = MyDataModel_downcast_ref(*(azul::RefAny*)&refany);
    if (!d) return AzJson_null();
    AzJsonKeyValue kv = AzJsonKeyValue_create(
        az_string_from_literal("counter"), AzJson_int((int64_t)d->counter));
    return AzJson_object(AzJsonKeyValueVec_fromItem(kv));   // { "counter": 5 }
}

AzResultRefAnyString MyDataModel_fromJson(AzJson json) {
    AzOptionJson field = AzJson_getKey(&json, az_string_from_literal("counter"));
    if (field.None.tag == AzOptionJson_Tag_None) {
        return AzResultRefAnyString_err(az_string_from_literal("Expected object with 'counter'"));
    }
    AzOptionI64 counter_opt = AzJson_asInt(&field.Some.payload);
    if (counter_opt.None.tag == AzOptionI64_Tag_None) {
        return AzResultRefAnyString_err(az_string_from_literal("'counter' is not an integer"));
    }
    MyDataModel model;
    model.counter = (uint32_t)counter_opt.Some.payload;
    return AzResultRefAnyString_ok(MyDataModel_upcast(model).release());
}
...
Note that `RefAny::create<T>` passes no serialization hooks, so on C++11 and up the JSON round-trip 
means building the `AzRefAny` through the raw `AzRefAny_newC` function and handing it your `toJson` / 
`fromJson` pointers yourself.
...
and `auto`-return functions. The address-of-static trick backing the type ID is not a constant 
expression, so `type_id_v` can be checked at runtime but not `static_assert`-ed.
...
- Structured bindings on every `ResultXxx` wrapper: `auto [ok, err] = std::move(result);` works 
  without per-class hooks, through the codegen's `tuple_size` / `tuple_element` specializations.
...
- a sibling `azul.cppm` module partition (generated by `azul-doc codegen all`, not 
  part of the release downloads): with a modules-aware toolchain you can `import azul;` 
  instead of `#include "azul20.hpp"`.
...
- deducing-`this` builder methods: every `with_*` is emitted as 
  ...
  on l-values and r-values without separate `const&` / `&&` overloads.
...
# C++03 - same DLL, different wrapper header
g++ -std=c++03 hello-world.cpp -I. -L. -lazul -o hello-world
...
clang++ -std=c++20 -fmodules hello-world.cpp -L. -lazul -o hello-world
```
