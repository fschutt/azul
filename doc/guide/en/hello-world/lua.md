...
In order to use Azul from Lua, you need the native prebuilt `libazul` library and the pre-rendered Lua bindings, which handle the `ffi` abstraction for you.

Azul's Lua bindings rely on the `ffi` module. The easiest way to get this is by using **LuaJIT 2.1+**, which includes `ffi` built-in. If you are using **vanilla PUC Lua** (5.1-5.4), you will need to install a third-party FFI module such as `luaffi` or `cffi-lua` via LuaRocks first.

## Installation

You can install the Lua bindings via LuaRocks using the provided `.rockspec` (available on the release page or generated from source), or install manually:
...
If you prefer to install manually, simply download `azul.lua` and put it next to your script (or point `LUA_PATH` at it):
