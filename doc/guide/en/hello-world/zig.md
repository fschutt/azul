...
fn layout(data: C.AzRefAny, _: C.AzLayoutCallbackInfo) callconv(.c) C.AzDom {
    ...
    var label_wrapper = C.AzDom_createPWithText(counter_str);
    
    const css_bytes = "font-size: 32px; margin: 0;";
    const css_str = C.AzString_fromUtf8(css_bytes.ptr, css_bytes.len);
    C.AzDom_setCss(&label_wrapper, css_str);
