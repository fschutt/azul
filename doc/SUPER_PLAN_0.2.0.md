...
- (2026-06-18) **azul-self-test: removed the Udp probe** (step 2b): excised `probe_udp`/`recv_retry`/
  ...
  azul-self-test` green (CHECK_EXIT=0). DEFERRED (cosmetic, dead internal code): remove the dll
  `udp` modules + `core::udp_framing` + the `video_codec` doc comment (all since done in
  1f5cbd437); and the real `web-transport-quinn` engine (v1 is the loopback stub).
