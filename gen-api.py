...
rust_c_api_path = "./azul/src/c-api.rs"

bindings_c_path = "./azul/bindings/c/azul.h"
bindings_cpp_path = "./azul/bindings/cpp/azul.h"
bindings_rust_path = "./azul/bindings/rust/lib.rs"
bindings_python_path = "./azul/bindings/python/lib.py"
...
def generate_c_api_code(apiData):
    ...
    for td in apiData["rust_typedefs"]:
        name = td["name"]
        item_type = td["rust_type"]
        if "doc" in td.keys():
            code += "/// " + td["doc"] + "\r\n"
    ...
    for c in apiData["classes"]:
        ...
        if "doc" in c.keys():
            ...
            code += "/// Pointer to rust-allocated `" + class_name + "` struct\r\n"
        ...
        code += "#[no_mangle] pub extern \"C\" fn " + fn_prefix + to_snake_case(class_name) + "_delete(ptr: " + class_ptr_name + ") { "
        ...
        if do_downcast:
            ...
            code += "fn " + fn_prefix + to_snake_case(class_name) + "_downcast(ptr: " + class_ptr_name + ") -> Box<" + rust_class_name + "> { "
...
def generate_c_bindings(apiData):
    header = "// WARNING: autogenerated code for azul api version " + str(apiData.keys()[0]) + "\r\n\r\n"
    apiData = apiData.values()[0]
    ...
    header += apiData["c_typedefs"] # temporary solution, C isn't easy to generate
    ...
    for c in apiData["classes"]:
        ...
        if "doc" in c.keys():
            ...
            header += "// Pointer to rust-allocated `" + class_name + "` struct\r\n"
...
def main():
    ...
    latestApi = apiData["apis"][-1]
    write_file(generate_c_api_code(latestApi), rust_c_api_path)
    write_file(generate_c_bindings(latestApi), bindings_c_path)
    write_file(generate_cpp_bindings(latestApi), bindings_cpp_path)
    write_file(generate_rust_bindings(latestApi), bindings_rust_path)
    write_file(generate_python_bindings(latestApi), bindings_python_path)
