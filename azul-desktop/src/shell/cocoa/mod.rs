...
/* 
// Creating a custom NSOpenGLView that overrides drawRect:
// We define a new Objective-C class "MyOpenGLView" that inherits from NSOpenGLView.
// We'll store a pointer to our AppData in it, so we can call 'rebuild_display_list()' etc.
pub fn create_opengl_view(frame: NSRect, data: Arc<Mutex<AppData>>) -> id {
    ...
    unsafe {
        let mut superclass = class!(NSOpenGLView);
        let c = CString::new("MyOpenGLView").unwrap();
        let mut decl = ClassDecl::new(&c.as_c_str(), superclass).unwrap();

        // We add an ivar to store an Arc<Mutex<AppData>> pointer. We can store any user data here.
        let i = CString::new("myAppDataPointer").unwrap();
        decl.add_ivar::<*mut core::ffi::c_void>(&i.as_c_str());
/* 
        // Override 'initWithFrame:pixelFormat:' to set up the ivar:
        decl.add_method(
            sel!(initWithFrame:pixelFormat:),
            init_with_frame as extern "C" fn(&Object, Sel, NSRect, id) -> id,
        );
...
*/
        ...
        /*
        // Override 'drawRect:' to do our custom rendering:
        ...
        // Allocate our custom view:
        let view: id =  msg_send![cls, alloc];
        // let view: id = msg_send![view, initWithFrame:frame pixelFormat:pixel_format];

        // Store the Arc<Mutex<AppData>> pointer in the ivar:
        let ptr_to_appdata = Arc::into_raw(data) as *mut core::ffi::c_void;
        *(*view).get_mut_ivar("myAppDataPointer") = ptr_to_appdata;

        // We want the best resolution (Retina) handling:
        let _: () = msg_send![view, setWantsBestResolutionOpenGLSurface: YES];
...
// Called when our custom NSOpenGLView is initialized.
extern "C" fn init_with_frame(
    ...
    unsafe {
        ...
        let this: id = msg_send![super(this, superclass), initWithFrame: frame pixelFormat: format];
        this
    }
}

// Called whenever macOS wants us to redraw. This is where we call `rebuild_display_list()`.
extern "C" fn draw_rect(this: &Object, _sel: Sel, _dirty_rect: NSRect) {
    unsafe {

        // Get the pointer to our AppData:
        let ptr: *mut core::ffi::c_void = *this.get_ivar("myAppDataPointer");
        // Convert back to Arc<Mutex<AppData>>:
        let my_data: Arc<Mutex<AppData>> = Arc::from_raw(ptr as *const Mutex<AppData>);
        // Immediately convert back to a raw pointer so we don't drop it:
        let _ = Arc::into_raw(my_data.clone());

        // Retrieve the OpenGL context from the NSOpenGLView
        let gl_context: id = msg_send![this, openGLContext];

        // Make the context current
        let _: () = msg_send![gl_context, makeCurrentContext];

        // Perform OpenGL rendering calls
