...
mod table_width_distribution_tests {
    ...
    fn test_table_width_100_percent_distributes_columns() {
        ...
        assert!((total_computed - table_width).abs() < 0.01, 
            "Columns should sum to table width. Expected: {}, Got: {}", table_width, total_computed);
        
        // Each column should be approximately equal (same max-content width)
        assert!(columns[0].computed_width.unwrap() > 200.0,
            "Column should be expanded beyond max-content width");
        assert!((columns[0].computed_width.unwrap() - columns[1].computed_width.unwrap()).abs() < 0.01,
...
mod vertical_alignment_tests {
    ...
    fn test_vertical_alignment_uses_content_box() {
        ...
        assert!((content_box_height - 16.0).abs() < 0.01, 
            "Content-box height should be ~16px, got {}", content_box_height);
        assert!(y_offset.abs() < 2.0, 
...
mod cell_padding_tests {
    ...
    fn test_cell_padding_subtracted_for_ifc() {
        ...
        let content_box_width: f32 = cell_border_box_width 
            - padding_left - padding_right
            - border_left - border_right;
        
        // Verify
        assert!((content_box_width - 259.64).abs() < 0.01,
            "Content-box width should be ~259.64px, got {}", content_box_width);
        
        // OLD WRONG: Would pass full border-box width
        assert!(cell_border_box_width > content_box_width + 15.0,
...
mod body_margin_tests {
    ...
    fn test_body_margin_preserved() {
        ...
        assert_ne!(expected_margin, default_ua_margin,
...
mod type_safety_tests {
    ...
    fn test_border_box_to_content_box_conversion() {
        ...
        let border_box = LogicalRect {
            ...
            size: LogicalSize { width: 300.0, height: 200.0 },
        };
