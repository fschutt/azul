...
fn test_taffy_align_items_stretch() {
    ...
    println!(
        ...
        container_layout.size, container_layout.location
    );
    assert_eq!(
        container_layout.size.width, 600.0,
        "Container width should be 600px"
    );
    assert_eq!(
        container_layout.size.height, 100.0,
        "Container height should be 100px"
    );

    // Check item 1 layout (flex-grow: 1, should get 1/6 = 100px width)
    let item1_layout = taffy.layout(item1).unwrap();
    println!(
        "Item 1: size={:?}, location={:?}",
        item1_layout.size, item1_layout.location
    );
    assert_eq!(
        item1_layout.size.width, 100.0,
        "Item 1 width should be 100px (1/6 of 600px)"
    );
    assert_eq!(
        item1_layout.size.height, 100.0,
        "Item 1 height should be STRETCHED to 100px"
    );
    assert_eq!(item1_layout.location.x, 0.0, "Item 1 x should be 0");
    assert_eq!(
        item1_layout.location.y, 0.0,
        "Item 1 y should be 0 (top-aligned when stretched)"
    );

    // Check item 2 layout (flex-grow: 2, should get 2/6 = 200px width)
    let item2_layout = taffy.layout(item2).unwrap();
    println!(
        "Item 2: size={:?}, location={:?}",
        item2_layout.size, item2_layout.location
    );
    assert_eq!(
        item2_layout.size.width, 200.0,
        "Item 2 width should be 200px (2/6 of 600px)"
    );
    assert_eq!(
        item2_layout.size.height, 100.0,
        "Item 2 height should be STRETCHED to 100px"
    );
    assert_eq!(item2_layout.location.x, 100.0, "Item 2 x should be 100");
    assert_eq!(
        item2_layout.location.y, 0.0,
        "Item 2 y should be 0 (top-aligned when stretched)"
    );

    // Check item 3 layout (flex-grow: 3, should get 3/6 = 300px width)
    let item3_layout = taffy.layout(item3).unwrap();
    println!(
        "Item 3: size={:?}, location={:?}",
        item3_layout.size, item3_layout.location
    );
    assert_eq!(
        item3_layout.size.width, 300.0,
        "Item 3 width should be 300px (3/6 of 600px)"
    );
    assert_eq!(
        item3_layout.size.height, 100.0,
        "Item 3 height should be STRETCHED to 100px"
    );
    assert_eq!(item3_layout.location.x, 300.0, "Item 3 x should be 300");
    assert_eq!(
        item3_layout.location.y, 0.0,
        "Item 3 y should be 0 (top-aligned when stretched)"
    );
}
...
fn test_taffy_align_items_stretch_with_intrinsic_size() {
    ...
    println!(
        "Item with intrinsic: size={:?}, location={:?}",
        item_layout.size, item_layout.location
    );

    // Key question: Does Taffy call measure with known_dimensions.height = Some(100.0)?
    // And does the final layout have height = 100.0?
    assert_eq!(
        item_layout.size.width, 600.0,
        "Item width should fill container"
    );
    assert_eq!(
    ...
    assert_eq!(
        item_layout.location.y, 0.0,
        "Item should be top-aligned when stretched"
    );
}
