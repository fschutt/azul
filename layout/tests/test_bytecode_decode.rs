    ...
    while ip < bytecode.len() {
        ...
        match op {
            ...
            0x40 => {
                ...
                let vals: Vec<i32> = bytecode[ip + 2..ip + 2 + n]
                    .iter()
                    .map(|&b| b as i32)
                    .collect();
                result.push(DecodedInstr {
                    ip: start_ip,
                    opcode: op,
                    mnemonic: format!("NPUSHB[{n}]"),
                    push_data: vals,
                });
                ip += 2 + n;
            }
            ...
            0x41 => {
                ...
                let vals: Vec<i32> = vals
                    .into_iter()
                    .map(|v| if v >= 0x8000 { v - 0x10000 } else { v })
                    .collect();
                result.push(DecodedInstr {
                    ip: start_ip,
                    opcode: op,
                    mnemonic: format!("NPUSHW[{n}]"),
                    push_data: vals,
                });
                ip += 2 + n * 2;
            }
            ...
            0xB0..=0xB7 => {
                ...
                let vals: Vec<i32> = bytecode[ip + 1..ip + 1 + n]
                    .iter()
                    .map(|&b| b as i32)
                    .collect();
                result.push(DecodedInstr {
...
fn test_bytecode_decode_u() {
    ...
    for &(ft_prefix, ft_ip) in FONTTOOLS_U_INSTRS {
        ...
        match ours {
            Some(d) => {
                ...
                if !matches {
                    eprintln!(
                        "  MISMATCH ip={}: fonttools={} ours={}",
                        ft_ip, ft_prefix, d.mnemonic
                    );
                    mismatches += 1;
                }
            }
    ...
    if mismatches == 0 {
        eprintln!(
            "  All {} reference instructions match ✓",
            FONTTOOLS_U_INSTRS.len()
        );
    }
    assert_eq!(
        mismatches, 0,
        "'u' bytecode decode has {mismatches} mismatches"
    );
}
...
fn test_bytecode_decode_l() {
    ...
    if mismatches == 0 {
        eprintln!(
            "  All {} reference instructions match ✓",
            FONTTOOLS_L_INSTRS.len()
        );
    }
    assert_eq!(
        mismatches, 0,
        "'L' bytecode decode has {mismatches} mismatches"
    );
}
...
fn test_decode_all_lorem_glyphs() {
    ...
    for ch in chars.chars() {
        ...
        if let Some(last) = last {
            let end = last.ip
                + 1
                + if last.mnemonic.starts_with("PUSH") || last.mnemonic.starts_with("NPUSH") {
    ...
    eprintln!(
        "Decoded {} glyphs: {} bytes → {} instructions, {} unknown opcodes",
        chars.len(),
        total_bytes,
        total_instrs,
        unknown
