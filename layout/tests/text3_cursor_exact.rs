...
fn cannot_move_right_past_end() {
    ...
    assert_px(
        l.get_cursor_rect(&again).unwrap().origin.x,
        110.0,
        "right at end stays",
    );
}
...
fn next_word_lands_on_word_starts() {
    ...
    assert_eq!(
        byte_of(&w1),
        4,
        "first next-word => start of 'bar' (byte 4)"
    );
    assert_eq!(
        byte_of(&w2),
        8,
        "second next-word => start of 'baz' (byte 8)"
    );
}
...
fn prev_word_lands_on_word_starts() {
    ...
    assert_eq!(
        byte_of(&p1),
        8,
        "prev-word from end => start of 'baz' (byte 8)"
    );
...
fn home_end_on_single_line() {
    ...
    assert_px(
        l.get_cursor_rect(&home).unwrap().origin.x,
        0.0,
        "home => x=0",
    );
    assert_px(
        l.get_cursor_rect(&end).unwrap().origin.x,
        110.0,
        "end => x=110",
    );
}
...
fn down_preserves_column_between_wrapped_lines() {
    ...
    assert!(
        rect.origin.y > 15.0,
        "down moved to the next line (y>15), got y={}",
        rect.origin.y
    );
...
fn cursor_skips_whole_grapheme_cluster() {
    ...
    assert_ne!(
        byte_of(&r1),
        1,
        "cursor must never land on the combining-mark byte 1"
    );
