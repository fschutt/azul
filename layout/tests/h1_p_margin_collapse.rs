...
fn test_comparison_old_vs_new_behavior() {
    ...
    assert!(
        (old_margin - 10.72).abs() < 0.01,
        "Old behavior should give 10.72px"
    );

    // Verify new behavior is correct
    assert!(
        (new_margin - 21.44).abs() < 0.01,
        "New behavior should give 21.44px"
    );
    ...
    assert!(
        (new_collapsed - 21.44).abs() < 0.01,
        "New: max(20, 21.44) should be 21.44"
    );
}
