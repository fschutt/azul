...
fn test_arabic_unicode_bidi_detection() {
    ...
    assert!(
        matches!(bidi_dir, UBidiDirection::Rtl),
        "Arabic text should be detected as RTL, got {:?}",
        bidi_dir
    );
}
...
fn test_mixed_arabic_latin() {
    ...
    println!(
        ...
        unicode_bidi::get_base_direction(mixed)
    );
}
...
fn test_bidi_with_css_direction_ltr() {
    ...
    use crate::text3::cache::{reorder_logical_items, ContentIndex, Direction, LogicalItem};
    ...
    let logical_items = vec![LogicalItem::Text {
        source: ContentIndex {
            run_index: 0,
            item_index: 0,
        },
    ...
    let arabic_item = visual_items_ltr
        .iter()
    ...
    println!("\nAssertion checks:");
    println!(
        "  Arabic portion BiDi level: {}",
        arabic_item.bidi_level.level()
    );
    ...
    let latin_item = visual_items_ltr
        .iter()
        .find(|item| item.text.contains('H'))
        .expect("Should find Latin text");

    println!(
        "  Latin portion BiDi level: {}",
        latin_item.bidi_level.level()
    );
