...
fn test_real_menubar_widget_not_clipped() {
    ...
    if let Some(msgs) = debug_messages.as_ref() {
        for m in msgs.iter() {
            ...
            if s.contains("TAFFY OUTPUT")
                || s.contains("TAFFY CHILD RESULT")
                || s.contains("set_unrounded")
            {
                println!("{s}");
            }
        }
    ...
    for (idx, w) in &item_widths {
        assert!(
            *w > 30.0,
            "real menubar item node {idx} clipped to {w} (bug #11)"
        );
    }
}
...
fn test_app_path_menubar_not_clipped() {
    ...
    for i in 0..12 {
        if let Some(node) = tree.get(LayoutNodeId::new(i)) {
            ...
            let intr = tree
                ...
                .unwrap_or_default();
            println!(
                "Node {}: fc={:?} size=({:.1},{:.1}) intr_min_w={:.1} max_w={:.1}",
                i,
                node.formatting_context,
                size.width,
                size.height,
                intr.min_content_width,
                intr.max_content_width
            );
        }
    }
    let mut item_widths = Vec::new();
    ...
    for (idx, w) in &item_widths {
        assert!(
            *w > 30.0,
            "APP PATH: menubar item node {idx} clipped to {w} — bug is in the app, not just paged"
        );
    }
    assert_eq!(
        item_widths.len(),
        3,
        "expected 3 items, got {item_widths:?}"
    );
}
...
fn test_probe_words_glyph_counts() {
    ...
    let words = [
        "File", "Edit", "View", "View", "Wiew", "Xiew", "Viww", "Vie", "Vi", "Help", "AAAA", "Open",
    ];
    let items: Vec<MenuItem> = words
        .iter()
    ...
    let viewport = LogicalRect {
        origin: LogicalPosition::zero(),
        size: LogicalSize::new(1200.0, 600.0),
    };
    let renderer_resources = RendererResources::default();
    let mut debug_messages = Some(Vec::new());
    let loader = PathLoader::new();
    ...
    for (i, (node, gc)) in text_runs.iter().enumerate() {
        ...
        let flag = if *gc != word.chars().count() {
            "  <-- TRUNCATED"
        } else {
            ""
        };
        println!(
            "word {:>5?} (node {}): {} glyphs (expected {}){}",
            word,
            node,
            gc,
            word.chars().count(),
...
fn test_menubar_item_text_not_clipped() {
    ...
    assert_eq!(
        item_widths.len(),
        3,
        "expected 3 menubar items, got {item_widths:?}"
    );
}
