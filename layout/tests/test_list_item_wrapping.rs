...
use azul_layout::text3::cache::{
    UnifiedConstraints, ShapedItem, ShapedCluster, ParsedFontTrait, Point, GlyphKind,
};
use azul_layout::text3::knuth_plass::kp_layout;
...
fn test_no_premature_line_break_with_sufficient_space() {
    ...
    let max_y_wide = layout_wide.items.iter()
        .filter_map(|item| {
    ...
    let max_y_narrow = layout_narrow.items.iter()
        .filter_map(|item| {
            if let azul_layout::text3::cache::PositionedItem::Cluster(cluster) = item {
                Some(cluster.position.y)
            } else {
