...
use azul_layout::xml::{parse_xml_string, XmlParseError, XmlNodeChild};
...
fn test_no_text_duplication_in_paragraph() {
    ...
    if let XmlNodeChild::Element(p_node) = &nodes[0] {
        ...
        assert_eq!(p_node.children.as_ref().len(), 3, "Expected 3 children in <p>");
        
        // First child: "Text before "
        if let XmlNodeChild::Text(text1) = &p_node.children.as_ref()[0] {
            assert_eq!(text1.as_str(), "Text before ", "First child should be 'Text before '");
        } else {
            panic!("First child should be Text, got: {:?}", p_node.children.as_ref()[0]);
        }
        
        // Second child: <span>inline</span>
        if let XmlNodeChild::Element(span_node) = &p_node.children.as_ref()[1] {
            assert_eq!(span_node.node_type.as_str(), "span", "Second child should be <span>");
            assert_eq!(span_node.children.as_ref().len(), 1, "Span should have 1 text child");
            ...
            panic!("Second child should be Element(span), got: {:?}", p_node.children.as_ref()[1]);
        }
        
        // Third child: " text after."
        if let XmlNodeChild::Text(text2) = &p_node.children.as_ref()[2] {
            assert_eq!(text2.as_str(), " text after.", "Third child should be ' text after.'");
        } else {
            panic!("Third child should be Text, got: {:?}", p_node.children.as_ref()[2]);
        }
    } else {
...
fn test_no_text_duplication_complex() {
    ...
    if let XmlNodeChild::Element(p_node) = &nodes[0] {
        ...
        assert_eq!(combined, "This is important text.", "Text should not be duplicated");
        
        // Verify each text appears only once
        assert_eq!(all_text.iter().filter(|&&t| t == "This is ").count(), 1, "'This is ' should appear exactly once");
        assert_eq!(all_text.iter().filter(|&&t| t == "important").count(), 1, "'important' should appear exactly once");
        assert_eq!(all_text.iter().filter(|&&t| t == " text.").count(), 1, "' text.' should appear exactly once");
    }
}
