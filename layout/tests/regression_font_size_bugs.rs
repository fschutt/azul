...
//!   - Problem: When merging DOM trees via append_child, dependency chains' source_node IDs weren't adjusted
...
//!   - Root cause: NodeId encoding uses 0=None, 1=NodeId(0), 2=NodeId(1), but code didn't decode properly
...
fn test_bug1_dependency_chain_concept() {
    ...
    assert_eq!(adjusted_source_node.index(), 3, 
...
fn test_bug2_parent_node_id_decoding() {
    ...
    assert_eq!(NodeId::into_usize(&Some(NodeId::new(0))), 1, "NodeId(0) should encode to 1");
    
    // Encoding test: Some(NodeId(1)) -> 2
    assert_eq!(NodeId::into_usize(&Some(NodeId::new(1))), 2, "NodeId(1) should encode to 2");
    
    // Encoding test: Some(NodeId(2)) -> 3
    assert_eq!(NodeId::into_usize(&Some(NodeId::new(2))), 3, "NodeId(2) should encode to 3");
    
    // Decoding test: 0 -> None
    assert_eq!(NodeId::from_usize(0), None, "0 should decode to None");
    
    // Decoding test: 1 -> Some(NodeId(0))
    assert_eq!(NodeId::from_usize(1), Some(NodeId::new(0)), "1 should decode to NodeId(0)");
    
    // Decoding test: 2 -> Some(NodeId(1))
    assert_eq!(NodeId::from_usize(2), Some(NodeId::new(1)), "2 should decode to NodeId(1)");
    
    // Decoding test: 3 -> Some(NodeId(2))
    assert_eq!(NodeId::from_usize(3), Some(NodeId::new(2)), "3 should decode to NodeId(2)");
    ...
    assert_eq!(wrong_parent.index(), 1, "Wrong approach: treats 1 as NodeId(1)");
    
    // Correct approach (Bug fix):
    let correct_parent = NodeId::from_usize(parent_usize).expect("Should decode to Some");
    assert_eq!(correct_parent.index(), 0, "Correct approach: decodes 1 to NodeId(0)");
    
    // This is the off-by-one error that caused the self-reference bug!
    assert_ne!(wrong_parent, correct_parent, 
...
fn test_bug2_h1_parent_is_body_not_self() {
    ...
    assert_eq!(wrong_parent.index(), 2, "Bug: H1 thinks parent is node 2 (itself!)");
    
    // Correct decoding:
    let correct_parent = NodeId::from_usize(h1_parent_encoded)
        .expect("Should decode to Some(NodeId)");
    assert_eq!(correct_parent.index(), 1, "Fix: H1 correctly identifies parent as node 1 (Body)");
...
fn test_bug3_font_size_dependency_chain_inheritance() {
    ...
    let wrong_result = wrong_resolution.resolve_with_context(&child_context, PropertyContext::FontSize);
    assert_eq!(wrong_result, 64.0, "If child inherited 2em chain: 2em * 32px = 64px (WRONG)");
    ...
    assert_ne!(correct_value, wrong_result, 
...
fn test_bug3_font_size_computed_value_inheritance() {
    ...
    assert_eq!(child_correct, 32.0, "Correct: inheriting computed 32px stays 32px");
...
fn test_all_bugs_combined_h1_32px_not_64px() {
    ...
    assert_ne!(resolved, buggy_resolved, 
...
fn test_h1_margin_uses_own_font_size() {
    ...
    assert!((resolved_margin - expected).abs() < 0.01,
    ...
    let resolved_font_size = font_size_em.resolve_with_context(&h1_context, PropertyContext::FontSize);
    
    // Em in font-size uses parent_font_size
    assert_eq!(resolved_font_size, 32.0, "Font-size: 2em * 16px (parent) = 32px");
}
...
fn test_regression_summary() {
    ...
    println!("  Example: Tree B node 0 -> node {} after append to Tree A ({} nodes)", 
        adjusted_node.index(), tree_a_nodes);
    ...
    println!("  Wrong: NodeId::new({}) = NodeId({})", parent_encoded, wrong.index());
    println!("  Correct: NodeId::from_usize({}) = NodeId({})", parent_encoded, correct.index());
    ...
    println!("  Wrong: Child inherits 2em chain, resolves to 2*32 = {}px", wrong_inherit);
    println!("  Correct: Child inherits computed 32px value = {}px", correct_inherit);
