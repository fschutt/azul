...
azul_core::impl_managed_callback! {
    ...
    from_handle_byref_fn: AzResumeCallback_createFromHostHandleByref,
...
azul_core::impl_managed_callback! {
    ...
    from_handle_byref_fn: AzCallback_createFromHostHandleByref,
}
...
// `Callback::invoke(data, info)` is generated by `impl_managed_callback!`
// below (it installs the wrapper's host ctx into `info` first).

impl azul_core::host_invoker::HostCtxCarrier for CallbackInfo {
    fn install_host_ctx(&mut self, ctx: &OptionRefAny) {
        // SAFETY: `ref_data` points at the dispatcher's per-callback
        // stack record, alive for the whole invocation; the slot is a
        // `RefCell`, so this is an ordinary interior-mutable write.
        unsafe { *(*self.ref_data).ctx.borrow_mut() = ctx.clone() };
    }
}
...
pub struct CallbackInfoRefData<'a> {
    ...
    /// Interior-mutable so `<Wrapper>::invoke` can install a nested
    /// callback's ctx through the `*const` the `Copy`able `CallbackInfo`
    /// carries (see `azul_core::host_invoker::HostCtxCarrier`).
    pub ctx: core::cell::RefCell<OptionRefAny>,
}
...
impl CallbackInfo {
    ...
    pub fn get_ctx(&self) -> OptionRefAny {
        unsafe { (*self.ref_data).ctx.borrow().clone() }
    }
