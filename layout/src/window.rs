...
impl LayoutWindow {
    ...
    /// rasteriser composites it at the host `VirtualView` ITEM's
    /// `bounds.origin + content_offset` under the host's scroll/reference
    /// frames. This reads THAT record (`headless::nested_dom_window_origin`,
    /// the same placement resolution the hit-tester uses) instead of
    /// re-deriving from `calculated_positions` — the old walk knew nothing of
    /// ancestor scroll frames or transforms in the host, so an IME candidate
    /// window inside a scrolled/transformed host drifted off the caret while
    /// clicks (placement-translated) still landed. One derivation, three
...
/// What produces the frames of an [`AnimTrack`].
#[derive(Debug, Clone)]
pub enum TrackSource {
    /// The compiled per-channel stop lists (from `@keyframes` or a builtin).
    Compiled,
    /// A registered native animation function (`-azul-animation-out:
    /// fooFunc 1s`): called once per frame with eased progress and a
    /// [`azul_core::resources::ZombieAnimInfo`] it may use to analyze the
    /// tree it is animating (e.g. measure its own component's text).
    Native {
        callback: azul_core::resources::ZombieAnimCallback,
        data: RefAny,
    },
}
