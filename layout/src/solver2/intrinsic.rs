use azul_core::{
    app_resources::{DecodedImage, RendererResources}, dom::NodeType, id_tree::{NodeDataContainer, NodeDataContainerRef}, styled_dom::StyledDom, window::LogicalSize
};
use azul_css::*;
...
pub fn calculate_intrinsic_sizes(
    ...
    for node_id in 0..node_data.len() {
        ...
        match node_data_ref.get_node_type() {
            ...
            NodeType::Image(image) => {
                ...
                if let Some(image_sizes) = calculate_image_intrinsic_sizes(
                    image,
                    styled_dom,
                    renderer_resources,
                ) {
    ...
    for depth_item in styled_dom.non_leaf_nodes.iter().rev() {
        if let Some(parent_id) = depth_item.node_id.into_crate_internal() {
            ...
            let min_width = css_property_cache.get_min_width(node_data_ref, &parent_id, styled_node_state);
            let max_width = css_property_cache.get_max_width(node_data_ref, &parent_id, styled_node_state);
            let height = css_property_cache.get_height(node_data_ref, &parent_id, styled_node_state);
            let min_height = css_property_cache.get_min_height(node_data_ref, &parent_id, styled_node_state);
            let max_height = css_property_cache.get_max_height(node_data_ref, &parent_id, styled_node_state);
...
fn calculate_text_intrinsic_sizes(
    ...
    use crate::text::layout::{split_text_into_words, shape_words};
    use crate::text::shaping::ParsedFont;
    ...
    let css_font_families = css_property_cache.get_font_id_or_default(
        node_data,
        &node_id,
        styled_node_state,
    );
    ...
    let max_content_width = shaped_words.items.iter()
        .map(|word| word.word_width as f32)
        .sum::<f32>() * font_size_px / parsed_font.font_metrics.units_per_em as f32;
...
fn calculate_parent_intrinsic_sizes(
    ...
    match formatting_context {
        Block { establishes_new_context: _ } => {
            calculate_block_intrinsic_sizes(&children, intrinsic_sizes)
        },
        Inline => {
            calculate_inline_intrinsic_sizes(&children, intrinsic_sizes)
        },
        InlineBlock => {
            calculate_inline_block_intrinsic_sizes(&children, intrinsic_sizes)
        },
        Flex => {
            calculate_flex_intrinsic_sizes(parent_id, &children, styled_dom, intrinsic_sizes)
        },
        Float(_) => {
            calculate_block_intrinsic_sizes(&children, intrinsic_sizes)
        },
...
fn calculate_block_intrinsic_sizes(
    ...
    // - Width: maximum of children's widths (children stack vertically, so width is determined by widest child)
