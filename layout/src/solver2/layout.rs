use azul_core::{
    ...
    ui_solver::{InlineTextLayout, InlineTextLayoutRustInternal, LayoutDebugMessage, LayoutResult, PositionInfo, PositionInfoInner, PositionedRectangle, ResolvedOffsets, ResolvedTextLayoutOptions, DEFAULT_LETTER_SPACING, DEFAULT_WORD_SPACING},
...
use super::intrinsic::IntrinsicSizes;
use crate::text::layout::{position_words, shape_words, split_text_into_words};
...
pub fn calculate_layout(
    ...
    if let Some(messages) = debug_messages {
        messages.push(LayoutDebugMessage {
            message: format!("Starting layout calculation with root bounds: {:?}", root_bounds).into(),
    ...
    let scrollable_nodes = finalize_scrollable_areas(
        &positioned_rects,
        styled_dom,
        debug_messages,
    );
...
fn layout_node_recursive(
    ...
    match formatting_context {
        ...
        FormattingContext::Float(float_direction) => {
            layout_float(
                node_id,
                positioned_rects,
                styled_dom,
        ...
        FormattingContext::OutOfFlow(position) => {
            ...
            let size = calculate_intrinsic_size(
                node_id,
                intrinsic_sizes,
                available_space,
                styled_dom,
...
fn layout_block_context(
    ...
    for child_id in node_id.az_children(&styled_dom.node_hierarchy.as_container()) {
        ...
        if !establishes_new_context {
            adjusted_content_rect = adjust_rect_for_floats(
                adjusted_content_rect,
                &local_exclusion_areas,
                node_id,
            );
        }
...
fn layout_inline_context(
    ...
    for child_id in node_id.az_children(&styled_dom.node_hierarchy.as_container()) {
        ...
        let child_intrinsic_size = calculate_intrinsic_size(
            child_id,
            intrinsic_sizes,
            content_box,
            styled_dom,
        ...
        if matches!(formatting_context, FormattingContext::InlineBlock) {
            let child_rect = LogicalRect::new(
                ...
                LogicalSize::new(
                    child_intrinsic_size.width,
                    child_intrinsic_size.height,
                ),
            );
    ...
    for (line, y_position, height) in line_boxes {
        ...
        for (child_id, width, height, is_inline) in line {
            ...
            update_inline_element_position(
                child_id,
                positioned_rects,
                styled_dom,
                child_space,
...
fn layout_flex_context(
    ...
    for (child_id, mut size, flex_grow) in flex_items {
        ...
        let child_rect = if is_row {
            LogicalRect::new(
                LogicalPosition::new(current_main_pos, cross_start),
                size,
            )
        } else {
            LogicalRect::new(
                LogicalPosition::new(cross_start, current_main_pos),
                size,
            )
        };
    ...
    match float_direction {
        ...
        LayoutFloat::Right => {
            float_position.x = available_space.origin.x + available_space.size.width 
                             - constrained_size.width - margin.right;
...
fn position_absolute_elements(
    ...
    for (node_id, position_type) in absolute_elements {
        ...
        let containing_block = if position_type == LayoutPosition::Fixed {
            ...
            find_positioned_ancestor(node_id, &positioned_rects.as_borrowing_ref(), styled_dom, root_bounds)
        };
...
fn find_positioned_ancestor(
    ...
    while let Some(parent_id) = current_id {
        if matches!(positioned_rects[parent_id].position, 
                  PositionInfo::Relative(_) | 
                  PositionInfo::Absolute(_) | 
                  PositionInfo::Fixed(_)) {
...
fn position_absolute_element(
    ...
    let left = css_property_cache
        ...
        .and_then(|l| Some(l.get_property()?.inner.to_pixels(containing_block.size.width)));
    
    let right = css_property_cache
        .get_right(node_data, &node_id, styled_node_state)
        .and_then(|r| Some(r.get_property()?.inner.to_pixels(containing_block.size.width)));
    
    let top = css_property_cache
        .get_top(node_data, &node_id, styled_node_state)
        .and_then(|t| Some(t.get_property()?.inner.to_pixels(containing_block.size.height)));
    
    let bottom = css_property_cache
        .get_bottom(node_data, &node_id, styled_node_state)
        .and_then(|b| Some(b.get_property()?.inner.to_pixels(containing_block.size.height)));
    ...
    if let Some(left_value) = left {
        ...
        position.x = containing_block.origin.x + containing_block.size.width 
                   - element_size.width - right_value;
    }
    ...
    if let Some(top_value) = top {
        ...
        position.y = containing_block.origin.y + containing_block.size.height 
                   - element_size.height - bottom_value;
    }
...
fn calculate_constrained_size(
    ...
    let final_width = match width {
        ...
        None => match intrinsic_size.preferred_width {
            ...
            None => intrinsic_size.max_content_width.max(min_width).min(max_width).min(parent_width),
        },
    };
    ...
    let final_height = match height {
        ...
        None => match intrinsic_size.preferred_height {
            ...
            None => intrinsic_size.max_content_height.max(min_height).min(max_height),
        },
    };
...
fn calculate_intrinsic_size(
    ...
    let width = intrinsic_size.preferred_width
        .unwrap_or_else(|| intrinsic_size.max_content_width);
    
    let height = intrinsic_size.preferred_height
...
fn extract_text_layout_options(node_id: NodeId, styled_dom: &StyledDom) -> ResolvedTextLayoutOptions {
    ...
    let font_size = css_property_cache
        ...
        .map_or(DEFAULT_FONT_SIZE_PX as f32, |fs| fs.inner.to_pixels(100.0 /* percent - TODO */));
