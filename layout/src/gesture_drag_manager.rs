...
impl DragData {
    ...
    pub fn get_data(&self, mime_type: &str) -> Option<&[u8]> {
        self.data.get(&azul_css::AzString::from(mime_type)).map(|v| v.as_slice())
    }
...
impl GestureAndDragManager {
    ...
    pub fn start_input_session(&mut self, position: LogicalPosition, timestamp: CoreInstant, button_state: u8) -> u64 {
    ...
    pub fn record_input_sample(&mut self, position: LogicalPosition, timestamp: CoreInstant, button_state: u8) -> bool {
    ...
    pub fn clear_old_sessions(&mut self, current_time: CoreInstant) {
        ...
        let valid_session_ids: Vec<u64> = self.input_sessions.iter().map(|s| s.session_id).collect();
        self.long_press_callbacks_invoked.retain(|id| valid_session_ids.contains(id));
    }
    ...
    pub fn detect_long_press(&self) -> Option<DetectedLongPress> {
        ...
        if distance <= self.config.long_press_distance_threshold {
            let first = session.first_sample()?;
            let callback_invoked = self.long_press_callbacks_invoked.contains(&session.session_id);
    ...
    pub fn detect_double_click(&self) -> bool {
        ...
        let duration = last_first
            .timestamp
            .duration_since(&prev_first.timestamp);
    ...
    fn get_current_session(&self) -> Option<&InputSession> {
    ...
    pub fn get_window_position_from_drag(&self) -> Option<azul_core::window::WindowPosition> {
        ...
        match drag.initial_window_position {
            azul_core::window::WindowPosition::Initialized(initial_pos) => {
                Some(azul_core::window::WindowPosition::Initialized(
                    PhysicalPositionI32::new(
...
mod tests {
    ...
    fn test_activate_window_drag_with_hit_test() {
        ...
        manager.activate_window_drag(
            WindowPosition::Initialized(azul_core::window::PhysicalPositionI32::new(100, 100)),
            Some(hit_test.clone()),
        );
        ...
        assert_eq!(saved_hit_test.regular_hit_test_nodes.len(), 1);
        assert!(saved_hit_test.regular_hit_test_nodes.contains_key(&NodeId::new(99)));
    }
