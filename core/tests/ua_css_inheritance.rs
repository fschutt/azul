...
use azul_core::{
    dom::{Dom, NodeDataInlineCssProperty, NodeType, NodeId},
...
fn test_ua_css_h1_font_weight_bold() {
    ...
    let mut dom = Dom::body()
        .with_child(
            Dom::new(NodeType::H1)
                .with_child(Dom::text("Test Heading"))
        );
    ...
    println!("Note: UA CSS properties are applied during layout, not necessarily in computed_values");
    ...
    if let Some(h1_computed) = cache.computed_values.get(&h1_id) {
        if let Some(CssProperty::FontSize(size_value)) = h1_computed.get(&CssPropertyType::FontSize) {
            if let Some(size) = size_value.get_property() {
                ...
                // 2em = 2 * 16px = 32px
                assert_eq!(pixels, 32.0, "H1 should have font-size: 2em (32px) from UA CSS");
...
fn test_ua_css_p_margins_1em() {
    ...
    let mut dom = Dom::body()
        .with_child(
            Dom::new(NodeType::P)
                .with_child(Dom::text("Test paragraph"))
        );
    ...
    for i in 1..=6 {
        ...
        if let Some(computed) = cache.computed_values.get(&heading_id) {
            ...
            if let Some(margin_bottom) = computed.get(&CssPropertyType::MarginBottom) {
                println!("  ✓ {} has margin-bottom: {:?}", heading_name, margin_bottom);
            }
        } else {
...
fn test_font_weight_inheritance_body_to_children() {
    ...
    let mut dom = Dom::body()
        .with_inline_css_props(vec![
            NodeDataInlineCssProperty::Normal(CssProperty::font_weight(StyleFontWeight::Normal))
        ].into())
        .with_child(
            Dom::new(NodeType::H1)
    ...
    if let Some(body_computed) = cache.computed_values.get(&body_id) {
        if let Some(CssProperty::FontWeight(weight_value)) = body_computed.get(&CssPropertyType::FontWeight) {
            if let Some(weight) = weight_value.get_property() {
                assert_eq!(*weight, StyleFontWeight::Normal, "Body should have font-weight: normal");
    ...
    if let Some(h1_computed) = cache.computed_values.get(&h1_id) {
        if let Some(CssProperty::FontWeight(weight_value)) = h1_computed.get(&CssPropertyType::FontWeight) {
            if let Some(weight) = weight_value.get_property() {
                ...
                assert_eq!(*weight, StyleFontWeight::Normal, "H1 inherits font-weight from body (overrides UA CSS)");
    ...
    if let Some(p_computed) = cache.computed_values.get(&p_id) {
        if let Some(CssProperty::FontWeight(weight_value)) = p_computed.get(&CssPropertyType::FontWeight) {
            if let Some(weight) = weight_value.get_property() {
                assert_eq!(*weight, StyleFontWeight::Normal, "P should inherit font-weight: normal from body");
