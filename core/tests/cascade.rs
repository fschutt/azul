...
use azul_css::props::{
    ...
    basic::{StyleFontSize, pixel::PixelValue, ColorU},
...
fn test_inherit_keyword_exists() {
    ...
    let font_size_inherit = CssProperty::FontSize(
...
fn test_initial_keyword_exists() {
    ...
    let display_initial = CssProperty::Display(
...
fn test_revert_keyword_exists() {
    ...
    let margin_revert = CssProperty::MarginTop(
...
fn test_unset_keyword_exists() {
    ...
    let text_color_unset = CssProperty::TextColor(
...
fn test_all_keywords_on_same_property() {
    ...
    // Verify each one
    if let CssProperty::Width(ref v) = auto { assert!(v.is_auto()); }
    if let CssProperty::Width(ref v) = none { assert!(v.is_none()); }
    if let CssProperty::Width(ref v) = initial { assert!(v.is_initial()); }
    if let CssProperty::Width(ref v) = inherit { assert!(v.is_inherit()); }
    if let CssProperty::Width(ref v) = revert { assert!(v.is_revert()); }
    if let CssProperty::Width(ref v) = unset { assert!(v.is_unset()); }
...
fn test_property_type_is_inheritable() {
    ...
    assert!(CssPropertyType::FontSize.is_inheritable(), "FontSize should be inheritable");
    assert!(CssPropertyType::TextColor.is_inheritable(), "TextColor should be inheritable");
    assert!(CssPropertyType::FontFamily.is_inheritable(), "FontFamily should be inheritable");
    
    // These should NOT be inheritable
    assert!(!CssPropertyType::Width.is_inheritable(), "Width should not be inheritable");
    assert!(!CssPropertyType::Height.is_inheritable(), "Height should not be inheritable");
    assert!(!CssPropertyType::MarginTop.is_inheritable(), "MarginTop should not be inheritable");
    assert!(!CssPropertyType::PaddingLeft.is_inheritable(), "PaddingLeft should not be inheritable");
    assert!(!CssPropertyType::BorderTopWidth.is_inheritable(), "BorderTopWidth should not be inheritable");
...
fn test_cascade_keyword_semantics_inherit() {
    ...
    let parent_value = CssProperty::FontSize(
        CssPropertyValue::Exact(StyleFontSize { inner: PixelValue::px(20.0) })
    );
    
    // Child has font-size: inherit
    let child_value = CssProperty::FontSize(
        CssPropertyValue::Inherit
    );
...
fn test_cascade_keyword_semantics_unset_inheritable() {
    ...
    let color_with_unset = CssProperty::TextColor(
        CssPropertyValue::Unset
    );
...
fn test_cascade_keyword_semantics_unset_non_inheritable() {
    ...
    let width_with_unset = CssProperty::Width(
        CssPropertyValue::Unset
    );
...
fn test_em_values_in_properties() {
    ...
    let font_size_em = CssProperty::FontSize(
        CssPropertyValue::Exact(StyleFontSize { inner: PixelValue::em(1.5) })
    );
...
fn test_percentage_values_in_properties() {
    ...
    let font_size_percent = CssProperty::FontSize(
        CssPropertyValue::Exact(StyleFontSize { inner: PixelValue::percent(150.0) })
    );
...
fn test_cascade_chain_example() {
    ...
    let root = CssProperty::FontSize(
        CssPropertyValue::Exact(StyleFontSize { inner: PixelValue::px(16.0) })
    );
    
    let parent = CssProperty::FontSize(
        CssPropertyValue::Exact(StyleFontSize { inner: PixelValue::percent(150.0) })
    );
    
    let child = CssProperty::FontSize(
        CssPropertyValue::Exact(StyleFontSize { inner: PixelValue::em(2.0) })
    );
