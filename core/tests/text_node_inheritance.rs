...
fn test_text_node_inherits_font_weight_from_h1() {
    ...
    let mut dom = Dom::body().with_child(Dom::new(NodeType::H1).with_child(Dom::text("Bold Text")));
            ...
            if let Some(weight) = weight_value.get_property() {
                ...
                assert_eq!(
                    *weight,
                    ...
                    "H1 should have font-weight: bold from UA CSS"
                );
            }
        }
    } else {
    ...
    if let Some(text_computed) = cache.computed_values.get(&text_id) {
        if let Some(CssProperty::FontWeight(weight_value)) =
            text_computed.get(&CssPropertyType::FontWeight)
        {
            if let Some(weight) = weight_value.get_property() {
                ...
                assert_eq!(
                    *weight,
                    StyleFontWeight::Bold,
                    "Text node should inherit font-weight: bold from H1"
                );
...
fn test_text_node_inherits_from_parent_with_explicit_style() {
    ...
    let mut dom = Dom::body().with_child(
        Dom::div()
    ...
    if let Some(div_computed) = cache.computed_values.get(&div_id) {
        if let Some(CssProperty::FontWeight(weight_value)) =
            div_computed.get(&CssPropertyType::FontWeight)
        {
            if let Some(weight) = weight_value.get_property() {
            ...
            if let Some(weight) = weight_value.get_property() {
                ...
                assert_eq!(
                    *weight,
                    ...
                    "Text should inherit 900 weight"
                );
            } else {
...
fn test_nested_text_node_inheritance() {
    ...
    let mut dom = Dom::body()
        ...
        .with_child(
            Dom::div().with_child(
                Dom::new(NodeType::Span).with_child(
...
fn test_multiple_text_nodes_same_parent() {
    ...
    let mut dom = Dom::body()
        .with_child(Dom::new(NodeType::H1).with_child(Dom::text("First Bold")))
        .with_child(Dom::new(NodeType::H1).with_child(Dom::text("Second Bold")));
    ...
    let text1_has_bold = cache
        .computed_values
        .get(&text1_id)
        .and_then(|cv| cv.get(&CssPropertyType::FontWeight))
        .is_some();

    let text2_has_bold = cache
        .computed_values
        .get(&text2_id)
        .and_then(|cv| cv.get(&CssPropertyType::FontWeight))
        .is_some();
    ...
    assert!(
        text2_has_bold,
        "Second text node should inherit font-weight"
    );
