...
fn test_dom_with_child() {
    let dom = Dom::new_div().with_child(Dom::text("Child"));
...
fn test_dom_nested_structure() {
    let dom = Dom::new_div().with_child(Dom::new_div().with_child(Dom::text("Nested")));
    assert_eq!(dom.children.len(), 1);
}

#[test]
fn test_dom_deeply_nested() {
    let dom = Dom::new_div().with_child(
...
fn test_dom_node_types() {
    ...
    let p = Dom::new_node(NodeType::P);
    assert!(matches!(p.root.node_type, NodeType::P));

    let span = Dom::new_node(NodeType::Span);
    assert!(matches!(span.root.node_type, NodeType::Span));

    let h1 = Dom::new_node(NodeType::H1);
...
fn test_dom_with_empty_children_vec() {
    let dom = Dom::new_div().with_children(vec![].into());
    assert!(dom.children.is_empty());
}
...
fn test_dom_mixed_node_types() {
    let dom = Dom::new_div()
        ...
        .with_child(Dom::new_node(NodeType::Span).with_child(Dom::text("Span")))
...
fn test_dom_table_structure() {
    let dom = Dom::new_node(NodeType::Table)
        .with_child(
            Dom::new_node(NodeType::Tr)
                .with_child(Dom::new_node(NodeType::Td).with_child(Dom::text("Cell 1")))
                .with_child(Dom::new_node(NodeType::Td).with_child(Dom::text("Cell 2"))),
        )
        .with_child(
            Dom::new_node(NodeType::Tr)
                .with_child(Dom::new_node(NodeType::Td).with_child(Dom::text("Cell 3")))
                .with_child(Dom::new_node(NodeType::Td).with_child(Dom::text("Cell 4"))),
        );
...
fn test_dom_list_structure() {
    let dom = Dom::new_node(NodeType::Ul)
        .with_child(Dom::new_node(NodeType::Li).with_child(Dom::text("Item 1")))
        .with_child(Dom::new_node(NodeType::Li).with_child(Dom::text("Item 2")))
        .with_child(Dom::new_node(NodeType::Li).with_child(Dom::text("Item 3")));
    assert_eq!(dom.children.len(), 3);
}

#[test]
fn test_dom_form_structure() {
    let dom = Dom::new_node(NodeType::Form)
        .with_child(Dom::new_node(NodeType::Input))
        .with_child(Dom::new_node(NodeType::Button).with_child(Dom::text("Submit")));
    assert_eq!(dom.children.len(), 2);
}

#[test]
fn test_dom_semantic_elements() {
    let dom = Dom::new_node(NodeType::Article)
        .with_child(Dom::new_node(NodeType::Header).with_child(Dom::text("Title")))
        .with_child(Dom::new_node(NodeType::Section).with_child(Dom::text("Content")))
        .with_child(Dom::new_node(NodeType::Footer).with_child(Dom::text("Footer")));
    assert_eq!(dom.children.len(), 3);
}
...
fn test_dom_all_heading_levels() {
    let dom = Dom::new_div()
        .with_child(Dom::new_node(NodeType::H1).with_child(Dom::text("H1")))
        .with_child(Dom::new_node(NodeType::H2).with_child(Dom::text("H2")))
        .with_child(Dom::new_node(NodeType::H3).with_child(Dom::text("H3")))
        .with_child(Dom::new_node(NodeType::H4).with_child(Dom::text("H4")))
        .with_child(Dom::new_node(NodeType::H5).with_child(Dom::text("H5")))
        .with_child(Dom::new_node(NodeType::H6).with_child(Dom::text("H6")));
...
fn test_dom_inline_elements() {
    let dom = Dom::new_node(NodeType::P)
        ...
        .with_child(Dom::new_node(NodeType::Strong).with_child(Dom::text("bold")))
        .with_child(Dom::text(" and "))
        .with_child(Dom::new_node(NodeType::Em).with_child(Dom::text("italic")))
...
fn test_dom_many_children() {
    let children: Vec<Dom> = (0..100)
        ...
        .collect();
    let dom = Dom::new_div().with_children(children.into());
...
fn test_dom_deep_tree() {
    ...
    fn create_deep(depth: usize) -> Dom {
        if depth == 0 {
            ...
            Dom::new_div().with_child(create_deep(depth - 1))
        }
    }
