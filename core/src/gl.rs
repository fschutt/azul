...
impl RefstrVecRef {
    #[must_use]
    pub const fn as_slice(&self) -> &[Refstr] {
...
impl GLint64VecRefMut {
    #[must_use]
    pub const fn as_slice(&self) -> &[GLint64] {
        // AUDIT: `from_raw_parts` is UB on a null ptr; guard FFI null/empty.
        if self.ptr.is_null() || self.len == 0 {
            return &[];
        }
...
impl GLfloatVecRefMut {
    #[must_use]
    pub const fn as_slice(&self) -> &[GLfloat] {
        // AUDIT: `from_raw_parts` is UB on a null ptr; guard FFI null/empty.
        if self.ptr.is_null() || self.len == 0 {
            return &[];
        }
...
impl GLintVecRefMut {
    #[must_use]
    pub const fn as_slice(&self) -> &[GLint] {
        // AUDIT: `from_raw_parts` is UB on a null ptr; guard FFI null/empty.
        if self.ptr.is_null() || self.len == 0 {
            return &[];
        }
...
impl GLuintVecRef {
    #[must_use]
    pub const fn as_slice(&self) -> &[GLuint] {
        // AUDIT: `from_raw_parts` is UB on a null ptr; guard FFI null/empty.
        if self.ptr.is_null() || self.len == 0 {
            return &[];
        }
...
impl GLenumVecRef {
    #[must_use]
    pub const fn as_slice(&self) -> &[GLenum] {
        // AUDIT: `from_raw_parts` is UB on a null ptr; guard FFI null/empty.
        if self.ptr.is_null() || self.len == 0 {
            return &[];
        }
...
impl GLbooleanVecRefMut {
    #[must_use]
    pub const fn as_slice(&self) -> &[GLboolean] {
        // AUDIT: `from_raw_parts` is UB on a null ptr; guard FFI null/empty.
        if self.ptr.is_null() || self.len == 0 {
            return &[];
        }
...
impl GLsyncPtr {
    #[must_use]
    pub const fn new(p: GLsync) -> Self {
        Self {
            ptr: p,
            run_destructor: true,
        }
...
fn try_compile_program(
    ...
    gl_context.shader_source(
        vs,
        &[shader_with_glsl_version(vert_src, version_line).as_slice()],
    );
    gl_context.compile_shader(vs);
    let fs = gl_context.create_shader(gl::FRAGMENT_SHADER);
    gl_context.shader_source(
        fs,
        &[shader_with_glsl_version(frag_src, version_line).as_slice()],
    );
...
impl GlContextPtr {
    ...
    pub fn bind_attrib_location(&self, program: GLuint, index: GLuint, name: &str) {
        self.get().bind_attrib_location(program, index, name);
    }
    ...
    pub fn push_debug_group_khr(&self, source: GLenum, id: GLuint, message: &str) {
        self.get().push_debug_group_khr(source, id, message);
    }
    ...
    pub fn egl_image_target_renderbuffer_storage_oes(&self, target: u32, image: GlVoidPtrConst) {
        self.get()
            .egl_image_target_renderbuffer_storage_oes(target, image.ptr as *const c_void);
    }
...
impl GlStateSave {
    fn save(gl_context: &GlContextPtr) -> Self {
        ...
        gl_context.get_integer_v(
            ...
            (&mut s.current_vertex_buffer[..]).into(),
        );
        gl_context.get_integer_v(
            gl::ELEMENT_ARRAY_BUFFER_BINDING,
            (&mut s.current_index_buffer[..]).into(),
        );
        gl_context.get_integer_v(gl::CURRENT_PROGRAM, (&mut s.current_program[..]).into());
        gl_context.get_integer_v(
            gl::VERTEX_ARRAY_BINDING,
            (&mut s.current_vertex_array_object[..]).into(),
        );
    ...
    fn restore(&self, gl_context: &GlContextPtr) {
        ...
        gl_context.bind_buffer(
            ...
            self.current_index_buffer[0] as u32,
        );
...
impl Texture {
    ...
    )] // OpenGL/graphics binding: GL-bounded numeric casts to GL* types
...
impl VertexAttribute {
    #[must_use]
    pub const fn get_stride(&self) -> usize {
...
impl VertexAttributeType {
    ...
    #[must_use]
    pub const fn get_gl_id(&self) -> GLuint {
        use self::VertexAttributeType::{Double, Float, UnsignedByte, UnsignedInt, UnsignedShort};
        match self {
            Float => gl::FLOAT,
            Double => gl::DOUBLE,
    ...
    #[must_use]
    pub const fn get_mem_size(&self) -> usize {
        use core::mem;
...
impl IndexBufferFormat {
    ...
    pub const fn get_gl_id(&self) -> GLuint {
        use self::IndexBufferFormat::{
            LineStrip, Lines, Points, TriangleFan, TriangleStrip, Triangles,
        };
        match self {
            Points => gl::POINTS,
            Lines => gl::LINES,
...
impl UniformType {
    ...
    pub fn set(self, gl_context: &Rc<GenericGlContext>, location: GLint) {
        use self::UniformType::{
            Float, FloatVec2, FloatVec3, FloatVec4, Int, IntVec2, IntVec3, IntVec4, Matrix2,
            Matrix3, Matrix4, UnsignedInt, UnsignedIntVec2, UnsignedIntVec3, UnsignedIntVec4,
        };
        match self {
...
impl ::core::fmt::Display for GlShaderCompileError {
    fn fmt(&self, f: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result {
        use self::GlShaderCompileError::{Fragment, Vertex};
        match self {
    ...
    pub fn new(
        ...
        if let Some(error_id) = get_gl_shader_error(gl_context, vertex_shader_object) {
            ...
            return Err(GlShaderCreateError::Compile(GlShaderCompileError::Vertex(
                VertexShaderCompileError { error_id, info_log },
            )));
        }
        ...
        if let Some(error_id) = get_gl_shader_error(gl_context, fragment_shader_object) {
            ...
            return Err(GlShaderCreateError::Compile(
                GlShaderCompileError::Fragment(FragmentShaderCompileError { error_id, info_log }),
            ));
        }
            ...
            for uniform in *uniforms {
                if !uniform_locations.contains_key(&uniform.uniform_name) {
                    uniform_locations.insert(
                        ...
                        gl_context
                            .get_uniform_location(shader_program_id, uniform.uniform_name.as_str()),
                    );
                }
            }
