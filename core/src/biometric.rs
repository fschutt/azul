//! POD types for the biometric-authentication surface . Defined here in `azul-core`
//! so the request config and result types can cross the FFI without `azul-layout`
//! having to be a dependency.

use azul_css::AzString;

/// What biometric hardware the device can authenticate with right now. This is the
/// *sync availability probe* (iOS `LAContext.biometryType` / `canEvaluatePolicy`;
/// Android `BiometricManager.canAuthenticate`), not the outcome of an auth attempt -
/// that is [`BiometricResult`].
#[repr(C)]
...
/// The outcome of one `request_biometric_auth` attempt, delivered to the caller's
/// completion callback once the OS prompt resolves. Maps onto every platform's
/// result enum: iOS `LAError`, Android `BiometricPrompt.AuthenticationCallback`,
/// Windows `UserConsentVerificationResult`, Linux polkit / PAM (research/02 §6).
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum BiometricResult {
    /// The user matched their face / finger / iris. Unlock granted.
    Authenticated,
    /// The user presented a biometric but it did not match (wrong finger / face).
    /// Distinct from `Cancelled` - the prompt is still up or retries were exhausted
    /// without a deliberate cancel.
    Failed,
    /// The user dismissed the prompt (tapped Cancel / pressed back).
    Cancelled,
    /// Biometrics failed but the user authenticated via the OS passcode / PIN /
    /// device-credential fallback. Still a successful unlock - only delivered when
    /// [`BiometricPrompt::allow_device_credential`] was set.
    FellBackToPasscode,
    /// No usable biometric is enrolled / available on this device, so the prompt
    /// could not be shown (Linux degraded path, or hardware absent). Pairs with
    /// [`BiometricKind::NotAvailable`].
    Unavailable,
    /// A platform error occurred (sensor busy, lockout, key invalidated, or an
    /// unmapped native error code).
    Error,
}

impl BiometricResult {
    /// `true` when the user successfully unlocked - either by biometric match
    /// (`Authenticated`) or by the OS passcode fallback (`FellBackToPasscode`). The
    /// vault gate keys off this.
...
// Option<BiometricResult>` returns `None` until the first request completes; this is
// the no-codegen prerequisite for that accessor (mirrors `OptionLocationFix`).
impl_option!(
    BiometricResult,
...
/// Configuration for one biometric-auth request - what the OS prompt shows and
/// which fallbacks are allowed. Passed to `App::request_biometric_auth`.
#[repr(C)]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BiometricPrompt {
    /// Reason shown in the OS prompt - required on iOS (`localizedReason`; the
    /// `NSFaceIDUsageDescription` plist key is declared separately), shown as the
    /// Android subtitle and the Windows / Linux message line. Empty is accepted but
    /// discouraged.
    pub reason: AzString,
    /// Label for the cancel / negative button (Android requires one; iOS
    /// `localizedCancelTitle`). Empty → platform default ("Cancel").
    pub cancel_label: AzString,
    /// Allow the OS passcode / PIN / device-credential fallback when biometrics
    /// fail or aren't enrolled. When the user takes that path the result is
...
impl BiometricPrompt {
    /// Convenience constructor: a biometric-only prompt showing `reason`, with the
    /// platform-default cancel label and no passcode fallback.
...
mod autotest_generated {
    ...
    /// basic_true_false: one known-false (`NotAvailable`) and the three known-true
    /// sensor variants return the documented bool.
    #[test]
    ...
    /// edge_inputs: `Default::default()` is `NotAvailable`, so the default gate
    /// must report "no biometric offered" - never panics.
    #[test]
    ...
    /// invariant: `is_available()` is exactly "not the NotAvailable variant" for
    /// every variant - catches a mis-mapped `matches!`.
    #[test]
    ...
    /// The `const fn` really is usable in a const context (compile-time evaluation)
    /// - a regression guard against dropping `const`.
    #[test]
    ...
    /// `FellBackToPasscode` count as success. In particular `Failed` (biometric
    /// mismatch) and `Cancelled` must NOT unlock the vault.
    #[test]
    ...
    /// invariant: exactly two of the six variants are successes - a naive impl that
    /// treated `FellBackToPasscode` as failure, or `Failed` as success, would break
    /// this count.
    #[test]
    ...
    /// invariants_hold: `new` copies `reason` verbatim and applies the documented
    /// defaults (empty cancel label, no device-credential fallback). Uses AzString
    /// content-equality.
    #[test]
    ...
    /// no_panic: a multi-megabyte ASCII reason is stored without truncation or
    /// panic.
    #[test]
    ...
    /// no_panic + unicode: emoji, combining marks, CJK and RTL text survive
    /// byte-for-byte through the constructor.
    #[test]
    ...
    /// A multibyte reason repeated to ~1 MB: char count and byte count stay
    /// consistent (no UTF-8 boundary corruption).
    #[test]
    ...
    /// A reason containing embedded NUL / control bytes must NOT be truncated at
    /// the NUL (guards against C-string-style handling).
    #[test]
    ...
    /// Boundary reason lengths around common allocation/size thresholds all
    /// round-trip their length exactly.
    #[test]
    ...
    /// `new` equals a hand-built struct with the same reason and the documented
    /// default fields - pins the field wiring.
    #[test]
    ...
    /// `new` never enables the device-credential fallback, whatever the reason - it
    /// is documented as biometric-only.
    #[test]
    ...
    /// Clone of a constructed prompt compares equal (Clone/PartialEq consistency
    /// with the deep AzString buffer).
    #[test]
    ...
    /// `Option<BiometricResult>` <-> `OptionBiometricResult` round-trips losslessly
    /// for every variant and for `None`.
    #[test]
    ...
    // ------------------------------------------------------------------
    // FFI discriminant stability (`#[repr(C)]` ABI contract) Both enums cross the
    // FFI boundary as C enums; the platform backends (iOS/Android/Windows/Linux) map
    // their native codes onto these discriminants. Re-ordering or inserting a
    // variant silently re-numbers them, which would turn a `Cancelled` into a
    // `Failed` - or worse, an `Error` into an `Authenticated` - for any already
    // compiled shell.
    // ------------------------------------------------------------------
    ...
    /// `BiometricResult` discriminants are 0..=5 in declaration order. Note
    /// `Authenticated == 0`: a zeroed result is a *success*, so the backends must
    /// never hand out a default-initialised result.
    #[test]
    ...
    /// encode == decode: variant -> discriminant -> variant is the identity for
    /// every variant, and out-of-range codes (the boundary / overflow cases a native
    /// backend could hand us) decode to `None` rather than being reinterpreted as a
    /// valid variant.
    #[test]
    ...
    /// Both C enums share one underlying repr, and the FFI Option wrapper is at
    /// least as large as its payload (no niche-packing surprise that would break the
    /// `#[repr(C, u8)]` layout the shells rely on).
    #[test]
    ...
    /// The derived total order follows declaration order (it is the same order the
    /// discriminants encode), and sorting is stable against it.
    #[test]
    fn ord_matches_declaration_order() {
        ...
        // The one unavailable kind sorts first - `kinds[1..]` are all real sensors.
    ...
    /// Hash is variant-distinguishing (no two variants collide in a set) and equal
    /// values hash equal - the manager keys cached results by these types.
    #[test]
    ...
    /// Both enums are `Copy`: reading a result does not consume it, so a manager
    /// can hand the same value to several callbacks.
    #[test]
    fn enums_are_copy() {
    ...
    /// The wrapper defaults to `None` ("no request has completed yet") - crucially
    /// NOT to `Some(Authenticated)`, which discriminant 0 of the payload would be.
    #[test]
    ...
    /// `replace` has `mem::replace` semantics: it returns the *previous* value and
    /// installs the new one. A vault gate that mis-read the return value would act
    /// on the stale result.
    #[test]
    ...
    /// `map` / `and_then` / `as_ref` / `as_mut` / `into_option` behave exactly like
    /// the std `Option` equivalents for every variant.
    #[test]
    fn option_result_combinators_match_std_option() {
        for original in [
            ...
            // as_mut writes through to the wrapper (and is `None` exactly when the
            // source Option was `None` - it must not conjure a slot out of the
            // `None` variant).
    ...
    /// `Some(_)` follows the payload order - pins the `#[repr(C, u8)]` tag ordering
    /// that codegen mirrors.
    #[test]
    ...
    /// `PartialEq` is sensitive to *every* field: two prompts that differ only in
    /// the cancel label, only in the reason, or only in the device-credential flag
    /// must not compare equal. A derive that dropped a field would let a
    /// biometric-only prompt compare equal to a passcode-fallback one.
    #[test]
    ...
    /// `as_bytes().len()` agree for ASCII, multibyte, embedded-NUL and empty reasons
    /// - no off-by-one or capacity/len confusion.
    #[test]
    ...
    /// No Unicode normalization happens: an NFC "é" and an NFD "e" + U+0301 stay
    /// distinct byte sequences (a normalizing string type would make these two
    /// prompts compare equal and silently change what the OS modal displays).
    #[test]
    ...
    /// The constructor does not trim, collapse or otherwise rewrite the reason -
    /// leading/trailing whitespace survives verbatim.
    #[test]
    ...
    /// (`into_library_owned_string` gives back exactly what went in), including for
    /// a multibyte reason.
    #[test]
    ...
    /// A prompt whose *both* strings are ~1 MB clones deeply and compares equal -
    /// exercises the deep-copy path for the non-defaulted field too (the constructor
    /// never sets it, so it is built by hand).
    #[test]
    ...
    /// `Default::default()` is idempotent and fully empty - two defaults compare
    /// equal and neither enables the passcode fallback.
    #[test]
    fn prompt_default_is_empty_and_stable() {
        ...
        // An empty reason is accepted (documented as "discouraged", not rejected) -
        // the constructor must not panic on it.
        assert_eq!(BiometricPrompt::new(AzString::from_const_str("")), a);
    }

    /// `Debug` renders the reason (used in backend logs); it must not lose the
    /// field or print a placeholder.
    #[test]
