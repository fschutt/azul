...
mod autotest_generated {
    ...
    fn page_content_height_returns_paged_height_verbatim_and_ignores_width() {
        for h in HOSTILE_F32 {
            ...
            for w in HOSTILE_F32 {
                let got =
                    FragmentationContext::new_paged(LogicalSize::new(w, h)).page_content_height();
                assert!(
                    same_f32(got, h),
    ...
    fn f32_max_tall_page_collides_with_continuous_sentinel_but_is_paged_disambiguates() {
        ...
        let max_page = FragmentationContext::new_paged(LogicalSize::new(595.0, f32::MAX));
    ...
    fn page_margins_copy_does_not_alias_the_original() {
        ...
        assert_eq!(
            original.top, 5.0,
            "mutating a copy wrote through to the original"
        );
