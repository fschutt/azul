...
mod autotest_generated {
    ...
    fn probe_config_default_is_permissive_zero() {
        ...
        assert_eq!(
            d.max_accuracy_m.to_bits(),
            0,
            "default must be +0.0, not -0.0"
        );
    ...
    fn probe_config_ord_field_precedence() {
        ...
        assert!(
            fg_maxed < bg_minimal,
            "background dominates the numeric fields"
        );
    ...
    fn probe_config_ord_is_bitwise_not_numeric() {
        ...
        assert!(
            neg.max_accuracy_m < pos.max_accuracy_m,
            "numerically: -1 < 1"
        );
        assert_eq!(
            neg.cmp(&pos),
