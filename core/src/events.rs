...
pub enum EasingFunction {
    ...
    EaseInOut,
    EaseOut,
    /// Critically-damped spring settle (ledger #28, per the animation design doc):
    /// position eases like a spring released toward the target - fast initial pull,
    /// asymptote-free exact landing. Evaluated analytically from normalized time
...
impl CallbackToCall {
    ...
    /// Build a list of `CallbackToCall` entries for every node hit by the given hit
    /// test under the given DOM, tagged with `event_filter`. Returns an empty `Vec`
    /// when there is no hit test data for the DOM.
...
pub enum ProcessEventResult {
    ...
    // GPU transforms changed: do another hit-test and recurse until nothing has
    // changed anymore
    UpdateHitTesterAndProcessAgain = 3,
    // Restyle or runtime edit changed layout-affecting properties: re-run layout on
    // the EXISTING StyledDom (no DOM rebuild).
...
/// Tracks the origin of an event for proper handling. This allows the system to
/// distinguish between user input, programmatic changes, and synthetic events
...
/// Event propagation phase (similar to DOM Level 2 Events). Events can be
/// intercepted at different phases: - **Capture**: Event travels from root down to
/// target (rarely used) - **Target**: Event is at the target element - **Bubble**:
/// Event travels from target back up to root (most common)
#[derive(Debug, Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(C)]
#[derive(Default)]
pub enum EventPhase {
...
// ============================================================================ W3C
// CSSOM View Module - Scroll Into View Types
// ============================================================================

/// W3C-compliant scroll-into-view options These options control how an element is
/// scrolled into view, following the CSSOM View Module specification.
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Default)]
pub struct ScrollIntoViewOptions {
    /// Vertical alignment: start, center, end, nearest (default: nearest)
    pub block: ScrollLogicalPosition,
    /// Horizontal alignment: start, center, end, nearest (default: nearest) Note:
    /// Named `inline_axis` to avoid conflict with C keyword `inline`
...
/// Scroll alignment for vertical (block) or horizontal (inline) axis Determines
/// where the target element should be positioned within the scroll container's
/// visible area.
#[repr(C)]
...
/// Scroll animation behavior for scrollIntoView API This is distinct from the CSS
/// `scroll-behavior` property, as it also supports the `Instant` option which CSS
/// does not have.
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, Default)]
...
pub enum LifecycleReason {
    ...
    Unmount,
    /// A `<transient-window>` was closed by the USER (outside click, Escape), not
    /// by the app flipping `open`.
    Dismiss,
    /// A `<transient-window>` was torn off its anchor into a free toplevel by the
    /// user's drag. `current_bounds` is where it went, in the parent.
    TearOff,
    /// A torn-off `<transient-window>` was docked back (onto its anchor or a drop
    /// zone) by the user's drag.
    Dock,
}
...
/// Type-specific event data for text-input (editing) events. Carried by
/// `EventType::Input` events so that text-input callbacks can read the edit details
/// directly off the event - matching how mouse/keyboard/scroll callbacks read their
/// data - instead of having to reach into the `TextInputManager`'s pending
/// changeset.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Identifies WHICH pending structural changeset a notification is for. Carried by
/// `EventType::DocumentEdit` events; the app acks with the same id via
/// `mark_document_edit_applied`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// High-level event type classification. This enum categorizes all possible events
/// that can occur in the UI.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(C)]
pub enum EventType {
    // Mouse Events / Mouse cursor is over the element
    MouseOver,
    ...
    // IME Composition Events / IME composition started
    CompositionStart,
    ...
    // Focus Events / Element received focus
    Focus,
    /// Element lost focus
    Blur,
    ...
    // Input Events / Input value is being changed (fires on every keystroke)
    Input,
    ...
    // Scroll Events / Element is being scrolled
    Scroll,
    /// Scroll started
    ScrollStart,
    /// Scroll ended
    ...
    // Drag Events / Drag operation started
    DragStart,
    ...
    // Pen / Stylus Events (W3C PointerEvent, pointerType "pen") / Pen tip made
    // contact (or pen entered while down)
    PenDown,
    ...
    // Gesture Events / Long press detected (touch or mouse held down)
    LongPress,
    ...
    // Clipboard Events / Content copied to clipboard
    Copy,
    ...
    // Media Events / Media playback started
    Play,
    ...
    // Lifecycle Events / Component was mounted to the DOM
    Mount,
    ...
    Resize,
    /// A `<transient-window>` was dismissed by the user (outside click or Escape).
    /// Fired on the transient node so the app can drop its `open` flag; the surface
    /// is already gone by the time this runs.
    Dismiss,
    /// A `<transient-window>` was torn off by the user's drag and is now a free
    /// toplevel. Fired on the transient node; the lifecycle data's `current_bounds`
    /// is the toplevel's rect in the parent's coordinates, for apps that persist
    /// their palette layout.
    TearOff,
    /// A torn-off `<transient-window>` was docked back by the user's drag.
    Dock,

    // Window Events / Window resized
    WindowResize,
    /// Window moved
    WindowMove,
    /// Window close requested
    WindowClose,
    /// The window's frame state changed - minimized, maximized, restored to normal,
    /// or entered/left fullscreen. Read the new state from `flags.frame` on the
    /// current window state; the event carries no payload of its own because the
    /// flag IS the payload and a callback that cares will already be reading window
    /// state.
    WindowFrameChanged,
    ...
    // Application Events / A monitor/display was connected
    MonitorConnected,
    /// A monitor/display was disconnected
    MonitorDisconnected,

    // File Events / File is being hovered
    FileHover,
    /// File was dropped
    FileDrop,
    ...
    // Hardware input-device Events (P6 sensors / gamepad) / A motion-sensor reading
    // (accelerometer / gyroscope / magnetometer) / changed. Read the value with
    // `CallbackInfo::get_sensor_reading`.
    SensorChanged,
    /// A gamepad's buttons / axes changed, or one was (dis)connected. Read it with
    /// `CallbackInfo::get_primary_gamepad` / `get_gamepad_state`.
    GamepadInput,

    // Geolocation Events (MWA-A1 - synthesized by the capability pump's
    // GeolocationManager EventProvider; both filter enums already carried the
    // matching variants, only this dispatch type was missing them). / A new GPS /
    // network location fix arrived.
    GeolocationFix,
    /// The native geolocation subscription errored, timed out, or was revoked.
    GeolocationError,

    // Async capability outcomes (MWA-A1b - synthesized by the capability pump's
    // manager EventProviders so idle apps observe prompt results). / A permission's
    // OS-observed state changed (granted / denied / / revoked / restricted).
    PermissionChanged,
    /// A biometric authentication prompt completed. Read the outcome via
    /// `CallbackInfo::get_biometric_result`.
    BiometricResult,
    /// The screen eyedropper started by `CallbackInfo::pick_screen_color` finished
    /// - the user picked a pixel, or cancelled. Window-level (the pick is not bound
    /// to a node); read the colour via `CallbackInfo::get_picked_screen_color`
    /// (`None` = cancelled).
    ScreenColorPicked,
    /// A keyring store / get / delete operation completed. Read the outcome via
    /// `CallbackInfo::get_keyring_result`.
    KeyringResult,

    // Structural document editing (C11 - synthesized once per recorded changeset by
    // the LayoutWindow's document-edit EventProvider). / A STRUCTURAL document edit
    // (Enter split / Backspace merge / wrap / / selection-spanning replace…) was
    // recorded and awaits the app's / apply-and-ack.
    DocumentEdit,
}

/// Unified event wrapper (similar to React's `SyntheticEvent`). All events in the
/// system are wrapped in this structure, providing a consistent interface and
/// enabling event propagation control.
#[derive(Debug, Clone, PartialEq)]
...
pub struct SyntheticEvent {
    ...
    /// Deliver at the target ONLY - no capture, no bubble. Set on the release
    /// synthesised for the node that was PRESSED when the pointer releases somewhere
    /// else (press-target capture, see `HoverManager::apply_press_target_capture`):
    /// that node owes a `MouseUp`, but its ancestors already see the real release
    /// through the hovered node's propagation path and must not see it twice.
    pub at_target_only: bool,
}

impl SyntheticEvent {
    /// Create a new synthetic event. - `timestamp`: Current time from
    ...
    /// Stop event propagation after the current phase completes. This prevents the
    /// event from reaching handlers in subsequent phases (e.g., stopping during
    /// capture prevents bubble phase).
    pub const fn stop_propagation(&mut self) {
        self.stopped = true;
    }

    /// Stop event propagation immediately. This prevents any further handlers from
    /// being called, even on the current target element.
    pub const fn stop_immediate_propagation(&mut self) {
        self.stopped_immediate = true;
        self.stopped = true;
    }

    /// Prevent the default action associated with this event. For example, prevents
    /// form submission on Enter key, or prevents text selection on drag.
...
/// Get the path from root to target node in the DOM tree. This is used for event
/// propagation - we need to know which nodes are ancestors of the target to
/// implement capture/bubble phases.
#[must_use] pub fn get_dom_path(
    ...
    // visited-set: a corrupt hierarchy with a parent cycle (or a parent chain longer
    // than the arena) would otherwise loop forever / OOM here, and this runs on
...
/// Propagate event through DOM tree with capture and bubble phases. This implements
/// DOM Level 2 event propagation: 1.
pub fn propagate_event(
    ...
    // A captured release (see `SyntheticEvent::at_target_only`) reaches its target
    // and nothing else: the ancestors get the real release through the hovered
    // node's path.
...
impl EventType {
    ...
    /// `pointerenter`/`pointerleave`): each node that gained or lost hover gets its
    /// OWN event (`event_determination` generates them per node), so bubbling a
    /// child's leave to its parent tells the parent the pointer left IT while it is
    /// still inside - which is how a slider's drag ended the moment the pointer slid
    /// off the thumb, a map pan ended on every tile crossing, and a split-pane drag
    /// ended on its first motion.
    #[must_use]
...
/// Default actions are built-in behaviors that occur in response to events. Per W3C
/// DOM Event specification: > A default action is an action that the implementation
/// is expected to take > in response to an event, unless that action is cancelled by
/// the script.
...
pub enum DefaultAction {
    ...
    ClearFocus,
    /// Activate the focused element (Enter/Space on activatable elements) This
    /// generates a synthetic Click event on the target
    ...
    SelectAllText,
    /// Enter in a contenteditable host: record a STRUCTURAL split-block changeset
    /// for the app to apply to its model (azul never mutates the DOM). Execution =
    /// `LayoutWindow::record_structural_default_action`.
    SplitBlockAtCursor {
        target: DomNodeId,
    },
    ...
    /// `white-space` preserves newlines: pre / pre-wrap / break-spaces / pre-line),
    /// and Shift+Enter in ANY contenteditable host: insert a literal `"\n"` through
    /// the standard text-input pipeline (which brings veto, undo, and caret-follow
    /// along) instead of recording a structural block split. APPENDED at the enum
...
/// Result of determining what default action should occur for an event. This is
/// computed AFTER event dispatch, based on: 1.
...
/// Trait for elements that have activation behavior (can be "clicked" via
/// keyboard). Per HTML5 spec, elements with activation behavior include: -
/// `<button>` elements - `<input type="submit">`, `<input type="button">`, `<input
/// type="reset">` - `<a>` elements with href - `<area>` elements with href - Any
/// element with a click handler (implicit activation) When an element with
/// activation behavior is focused and the user presses Enter or Space, a synthetic
...
pub trait ActivationBehavior {
    ...
    /// Returns true if this element is currently activatable (e.g., not disabled,
...
pub trait Focusable {
    ...
    /// Returns true if this element type is naturally focusable (button, input,
    /// select, textarea, a[href])
    fn is_naturally_focusable(&self) -> bool;
}

/// Check if an event filter matches the given event in the current phase. This is
/// used during event propagation to determine which callbacks should be invoked at
/// each phase.
fn matches_filter_phase(
    ...
    // guard an ancestor node's Hover/Focus callback was collected in BOTH the
    // capture and the bubble walk, so it fired TWICE whenever the hit target was a
    // descendant (e.g.
...
/// Check if a component (lifecycle) filter matches the event. Lifecycle events
/// produced by `diff::reconcile_dom` carry the target node in
/// `SyntheticEvent.target`, so dispatchers that bypass `propagate_event` and invoke
/// the target directly also need a way to compare.
...
/// Check if a hover filter matches the event. Exhaustive (filter, event-type) truth
/// table: many distinct pairs share the `=> true` body.
...
fn matches_hover_filter(
    ...
    match (filter, &event.event_type) {
        ...
        // Gestures. These filters existed, the detectors produced the events, and
        // this table had no arm for them - a `PinchOut` handler on a map could never
        // fire, whatever the gesture manager saw.
...
/// Check if a focus filter matches the event. Exhaustive (filter, event-type) truth
/// table - see matches_hover_filter.
#[allow(clippy::match_same_arms)]
fn matches_focus_filter(
    ...
    match (filter, &event.event_type) {
        ...
        (DragLeave, EventType::DragLeave) => true,
        (Drop, EventType::Drop) => true,
        // MWA-C-clipboard: W3C clipboard events on the focused element (qualified
        // paths - `use FocusEventFilter::Copy` would shadow the `Copy` trait in this
        // scope).
...
/// Check if a window filter matches the event. Exhaustive (filter, event-type)
/// truth table - see matches_hover_filter.
#[allow(clippy::match_same_arms)]
...
/// Detect lifecycle events by comparing old and new DOM state. This is the simple,
/// index-based lifecycle detection that doesn't account for node reordering.
...
/// Returns `true` iff the two logical sizes differ after fixed-point quantization
/// (~0.001 tolerance), treating a dimension that is NaN on *both* sides as unchanged
/// so a degenerate layout cannot emit a Resize every frame.
fn size_changed(old: crate::geom::LogicalSize, new: crate::geom::LogicalSize) -> bool {
    fn dim_changed(a: f32, b: f32) -> bool {
        if a.is_nan() && b.is_nan() {
            return false;
        }
        // Fixed-point quantization mirrors `LogicalSize`'s `Ord`/`Hash`. `f32 as
        // i64` saturates on overflow (no wasm32 wraparound); a lone NaN quantizes to
        // `i64::MIN` and so registers as changed.
...
fn create_resize_event(
    ...
    // `LogicalSize` used to compare f32 bit patterns, so a single NaN dimension made
    // `old != new` true *every frame forever* -> an endless Resize-event loop.
...
/// A `Resize` lifecycle event (`ComponentEventFilter::NodeResized`) for `node_id`
/// whose layout box went from `old` to `new`. `None` when its SIZE did not change: a
/// position-only move is not a resize, and a NaN dimension present on both sides
/// reads as unchanged (see `size_changed`; a raw `!=` once produced a Resize every
/// frame forever).
#[must_use]
...
/// Result of lifecycle event detection with reconciliation. Contains both the
/// generated lifecycle events and a mapping from old to new node IDs for state
/// migration (focus, scroll, etc.).
#[derive(Debug, Clone, Default)]
pub struct LifecycleEventResult {
    /// Lifecycle events (Mount, Unmount, Resize, Update)
    pub events: Vec<SyntheticEvent>,
    /// Maps old `NodeId` -> new `NodeId` for matched nodes. Use this to migrate
    /// focus, scroll state, and other node-specific state.
    pub node_id_mapping: OrderedMap<NodeId, NodeId>,
}

/// Detect lifecycle events using reconciliation with stable keys and content
/// hashing. This is the advanced lifecycle detection that can correctly identify: -
/// **Moves**: When a node changes position but keeps its identity (via key or hash)
/// - **Mounts**: When a new node appears - **Unmounts**: When an existing node
/// disappears - **Resizes**: When a node's layout bounds change - **Updates**: When
/// a keyed node's content changes The reconciliation strategy is: 1.
...
pub enum HoverEventFilter {
    ...
    PenLeave,
    /// Apple Pencil 2 / Surface Slim Pen 2 barrel squeeze on the hovered element.
    /// Fires once per squeeze.
    PenSqueeze,
    /// Apple Pencil 2 side double-tap on the hovered element. Fires once per
    /// gesture.
    PenDoubleTap,
    /// Pen/stylus is hovering above the hovered element (in proximity, not in
    /// contact). Continuous: fires per pen-axis update while the stylus is held
    /// above the surface.
    PenHover,
    /// New GPS / network location fix arrived for a `GeolocationProbe` in this
    /// node's subtree. Payload accessor: `CallbackInfo::get_geolocation_fix()`.
    GeolocationFix,
    /// Native geolocation subscription errored / was revoked / timed out.
    GeolocationError,
    ...
    // W3C MouseOut event (bubbling version of MouseLeave) / Mouse left the element
    // OR moved to a child element (W3C `mouseout`, bubbles)
    MouseOut,

    // W3C Focus events (bubbling versions) / Focus is about to move INTO this
    // element or a descendant (W3C `focusin`, bubbles)
    FocusIn,
    /// Focus is about to move OUT of this element or a descendant (W3C `focusout`,
    /// bubbles)
    FocusOut,

    // IME Composition events / IME composition started (W3C `compositionstart`)
    CompositionStart,
    ...
    // Async capability outcomes (MWA-A1b) / A permission's OS-observed state
    // changed while this node (the / capability's most recent subscriber) is in the
    // target chain.
    PermissionChanged,
...
impl HoverEventFilter {
    /// Check if this is an internal system event that should not be exposed to user
    /// callbacks
    ...
    // Exhaustive On -> Option<FocusEventFilter> mapping table; the several `=>
    // None` rows (window-only events) are intentional 1:1 rows - merging would
    // collapse the table.
    #[allow(clippy::match_same_arms)]
...
/// Event filter similar to `HoverEventFilter` that only fires when the element is
/// focused. **Important**: In order for this to fire, the item must have a
/// `tabindex` attribute (to indicate that the item is focus-able).
#[derive(Debug, Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(C)]
pub enum FocusEventFilter {
    ...
    // W3C Focus events (bubbling versions, fires on focused element when focus
    // changes) / Focus moved into this element or a descendant (W3C `focusin`)
    FocusIn,
    ...
    // Clipboard events (W3C clipboard-events; MWA-C-clipboard: fire on the focused
    // element BEFORE the OS default action, which preventDefault suppresses).
    // APPENDED at the end for ABI stability - sync to api.json via azul-doc autofix
    // in Phase D.
    Copy,
    ...
    Paste,
    /// A structural document edit was recorded on (or under) the focused element
    /// and awaits the app's apply-and-ack (see `EventType::DocumentEdit`). APPENDED
    /// at the end for ABI stability.
    DocumentEdit,
}

/// Event filter that fires when any action fires on the entire window (regardless
/// of whether any element is hovered or focused over).
#[derive(Debug, Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(C)]
pub enum WindowEventFilter {
    ...
    GeolocationFix,
    /// Native geolocation subscription dropped or errored (signal lost, no
    /// provider, permission revoked mid-session).
    GeolocationError,
    /// A motion-sensor reading changed (P6). Fires window-level (the device isn't
    /// bound to a node); read via `CallbackInfo::get_sensor_reading`.
    SensorChanged,
    ...
    /// different scaling). The new DPI is available via
    /// `CallbackInfo::get_hidpi_factor()`.
    DpiChanged,
    /// The window moved to a different monitor. The new monitor is available via
    /// `CallbackInfo::get_current_monitor()`.
    MonitorChanged,

    // Async capability outcomes (MWA-A1b) - window-level mirrors (the outcome isn't
    // inherently bound to a node). / A permission's OS-observed state changed.
    PermissionChanged,
    /// A biometric authentication prompt completed.
    BiometricResult,
    /// The screen eyedropper finished (picked or cancelled).
...
impl WindowEventFilter {
    // Exhaustive On -> Option<HoverEventFilter> mapping table (see
    // to_focus_event_filter).
    #[allow(clippy::match_same_arms)]
    #[must_use] pub const fn to_hover_event_filter(&self) -> Option<HoverEventFilter> {
        match self {
            ...
            // MouseEnter and MouseLeave on the **window** - does not mean a
            // mouseenter and a mouseleave on the hovered element
            Self::MouseEnter => None,
            Self::MouseLeave => None,
            Self::Resized => None,
            Self::Moved => None,
            // A frame transition is a WINDOW fact; there is no per-element hover
...
pub enum ComponentEventFilter {
    ...
    /// Escape). The popup is closed by the engine regardless; this is where the app
    /// clears its own `open` state so the next layout agrees.
    Dismissed,
    /// Fired on a `<transient-window>` the user tore off its anchor by dragging its
    /// `-azul-app-region: drag` strip; it is a free toplevel now. The event's
    /// `current_bounds` is its rect in the parent.
    TornOff,
    /// Fired on a torn-off `<transient-window>` the user docked back - onto its
    /// anchor, or onto a `tearoff-zone` node, which is its anchor from here on.
    Docked,
}
...
pub enum ApplicationEventFilter {
    ...
    DeviceDisconnected,
    /// Fired when a new monitor/display is connected to the system. Callback
    /// receives updated monitor list via `CallbackInfo::get_monitors()`.
    MonitorConnected,
    /// Fired when a monitor/display is disconnected from the system.
    MonitorDisconnected,
}

/// Sets the target for what events can reach the callbacks specifically. This
/// determines the condition under which an event is fired, such as whether the node
/// is hovered, focused, or if the event is window-global.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(C, u8)]
pub enum EventFilter {
    /// Calls the attached callback when the mouse is actively over the given
    /// element.
    Hover(HoverEventFilter),
    /// Calls the attached callback when the element is currently focused.
    Focus(FocusEventFilter),
    /// Calls the callback when anything related to the window is happening. The
    /// "hit item" will be the root item of the DOM.
    Window(WindowEventFilter),
    /// API stub: Something happened with the node itself (node resized, created or
    /// removed).
    Component(ComponentEventFilter),
    /// Something happened with the application (started, shutdown, device plugged
    /// in).
...
/// Convert from `On` enum to `EventFilter`. This determines which specific filter
/// variant is used based on the event type.
impl From<On> for EventFilter {
    // Exhaustive On -> EventFilter mapping table; the a11y events
    // (Default/Collapse/ Expand/Increment/Decrement) all map to MouseUp ("click") as
    // intentional 1:1 documented rows - merging would drop the per-row rationale
    // comments.
    #[allow(clippy::match_same_arms)]
...
// Cross-Platform Event Dispatch System NOTE: The old dispatch_synthetic_events /
// CallbackTarget / CallbackToInvoke / EventDispatchResult pipeline has been removed.
// Event dispatch now goes through dispatch_events_propagated() in event_v2.rs which
// uses propagate_event() for W3C Capture→Target→Bubble propagation.

/// Trait for managers to provide their pending events. Each manager
/// (`TextInputManager`, `ScrollManager`, etc.) implements this to report what events
/// occurred since the last frame.
pub trait EventProvider {
    /// Get all pending events from this manager. Events should include: - `target`:
    /// The `DomNodeId` that was affected - `event_type`: What happened (Input,
    /// Scroll, Focus, etc.) - `source`: `EventSource::User` for input,
    /// `EventSource::Programmatic` for API calls - `data`: Type-specific event data
    /// After calling this, the manager should mark events as "read" so they aren't
    /// returned again next frame.
    fn get_pending_events(&self, timestamp: Instant) -> Vec<SyntheticEvent>;
}

/// Deduplicate synthetic events by (target node, event type). Groups by
/// (target.dom, target.node, `event_type`), keeping the latest timestamp.
...
/// Convert `EventType` to `EventFilters` (returns multiple filters for generic +
/// specific events) For mouse button events, returns both generic (`MouseUp`) AND
/// button-specific (LeftMouseUp/RightMouseUp). The button-specific filter is derived
/// from the `EventData::Mouse` payload.
#[allow(clippy::match_same_arms)]
...
#[must_use] pub fn event_type_to_filters(event_type: EventType, event_data: &EventData) -> Vec<EventFilter> {
    ...
    match event_type {
        ...
        // *release* over the target (left-button only). Mapping it to LeftMouseDown
        // fired synthesized clicks as a duplicate MouseDown (press semantics)
        // instead of a completed click.
        ...
        // Gestures: the detectors emitted these for years and this table sent them
        // to `vec![]` - no filter, no callback, whatever the widget registered.
        // Every gesture reaches its hover / focus / window filters.
        ...
        // File events MWA-B7: node-level Hover mirror + the window-level filter.
        // Without the Window mirrors, even WindowEventFilter::DroppedFile
        // registrations were unreachable (file events dispatched to Hover filters
        // only).
        ...
        // Lifecycle events - dispatched on the target node via
        // EventFilter::Component. Both Mount and Unmount map to their respective
        // Component filters so that
        // `.add_callback(EventFilter::Component(ComponentEventFilter::AfterMount))`
        // actually fires after reconcile_dom emits a
        // SyntheticEvent{EventType::Mount,..}.
        ...
        // Geolocation (MWA-A1): node-level Hover mirror + the window-level filter
        // (a fix isn't bound to a node). The fix itself is read via
        // CallbackInfo::get_geolocation_fix.
        ...
        // MWA-C-clipboard: W3C clipboard events - fire on the focused element
        // before the OS default action (preventDefault suppresses the default
        // copy/cut/paste).
...
/// Framework-determined side effects (system changes). Unlike `CallbackChange`
/// (from user callbacks), these are determined by the framework's event analysis:
/// hit tests, gesture detection, focus rules, text selection, keyboard shortcuts,
/// etc.
#[derive(Debug, Clone, PartialEq, Eq)]
#[must_use = "SystemChange must be processed through apply_system_change()"]
pub enum SystemChange {
    /// Advance layout animations by an EXACT step, bypassing the wall clock. Only
    /// the E2E surface emits this.
    ...
    /// Unified selection operation: cursor movement, selection extension, or
    /// deletion. Replaces the old `ArrowKeyNavigation` and `DeleteTextSelection`
    /// variants.
    ApplySelectionOp {
        target: DomNodeId,
        op: SelectionOp,
    ...
    /// Add a cursor at the clicked position (Ctrl+Click). The position will be
    /// hit-tested to find the text cursor location.
    AddCursorAtClick {
        position: LogicalPosition,
    },
    /// Select the next occurrence of the current selection's text (Ctrl+D). If the
    /// primary selection is a cursor, expand it to the word first.
    ...
    /// Change focus to a new target (or clear focus if None). Handles:
    /// `set_focused_node`, `apply_focus_restyle`, `scroll_node_into_view`,
...
/// Flattened focus/selection state for the input interpreter (replaces trait
/// objects).
#[derive(Debug, Clone, Copy)]
...
/// Passed to the interpreter callback.
#[derive(Debug)]
...
/// The `extern "C"` callback type for the input interpreter. The first `RefAny` is
/// the user data (vim mode, repeat counter, etc.) held in
/// `InputInterpreterCallback.ctx`.
...
/// Configurable input interpreter callback. Maps raw platform events + window state
/// → semantic `SystemChange` actions.
#[repr(C)]
...
/// What JSON type an op argument expects. Spelled out rather than left to prose,
/// because this is read by machines: an agent choosing arguments and a UI validating
/// a macro form both need the type, not a sentence describing it.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// A worked example: what to send, and what comes back. Both halves matter.
#[derive(Debug, Clone, PartialEq, Default)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize, serde::Deserialize))]
pub struct E2eOpExample {
    pub description: String,
    /// Example arguments - a real JSON value, not a string containing JSON. These
    /// were `String` first, which serialized as escaped JSON inside JSON: every
    /// consumer parsed twice and nothing checked the inner text was even
    /// well-formed, so a malformed example sat in the schema looking fine.
    pub args: crate::json::Json,
    /// What the op returns for those arguments. MUST contain a `success` boolean -
    /// validated when the schema is installed, see `E2eOpSchema::validate`.
...
/// Everything an application advertises about its ops. Built in memory as a normal
/// Rust struct and serialized to `Json` at the boundary - on BOTH sides, framework
/// and application.
#[derive(Debug, Clone, PartialEq, Default)]
#[cfg_attr(feature = "serde-json", derive(serde::Serialize, serde::Deserialize))]
...
/// Does this value have a top-level `success` boolean? Deliberately requires a
/// BOOLEAN, not merely the key: `"success": "yes"` and `"success": null` are the
/// shapes a hand-written schema actually produces, and each would otherwise pass
/// while telling a consumer nothing.
...
fn json_has_success_bool(v: &crate::json::Json) -> bool {
    ...
    {
        // Without serde there is no parser here. Returning TRUE would silently pass
        // every schema; the honest fallback is a textual check that can only reject
...
pub enum E2eSchemaError {
    ...
    /// An example's `returns` has no `success` boolean. The contract is that every
    /// op result says whether it worked.
...
impl E2eOpSchema {
    /// Check the schema is usable, BEFORE anything reads or dispatches it. Called
    /// when the schema is installed, not on first invocation.
    ...
    pub fn to_json(&self) -> crate::json::Json {
        #[cfg(feature = "serde-json")]
        {
            // Falling back to an empty-op object rather than to null: a consumer
            // must never be handed something that reads as "not parseable" when the
            // truth is "serialization failed". serde_json preserves declaration
            // order; `Json::parse` does NOT (it re-serializes sorted, which buried
            // `name` and `summary` under `args`/`description`/`examples`).
...
pub struct CustomE2eOpResult {
    /// Whether the application RECOGNISED this op name. This is not "did it
    /// succeed" - it is "was this op mine at all".
    pub handled: bool,
    /// Result payload, JSON. Reported back to the scenario as-is, so a scenario can
    /// assert on it.
    pub json: AzString,
}

impl Default for CustomE2eOpResult {
    /// "Not my op": `handled: false` with an empty payload - the value a bridge
    /// (e.g. the generated Python trampoline) returns when no user handler can run.
    fn default() -> Self {
        Self { handled: false, json: AzString::from_const_str("") }
    }
}

/// The `extern "C"` callback type for a user-defined E2E op. Receives the op name
/// and its arguments as a JSON string, exactly as they appeared in the scenario, and
/// returns a `CustomE2eOpResult`.
...
pub struct CustomE2eOpCallback {
    ...
    pub ctx: crate::refany::OptionRefAny,
    /// Describes every op `cb` answers: name, summary, description, the JSON type
    /// each argument expects, usage examples, and example returns. DATA, not a
    /// second callback - discovery is a field read, so it needs no invocation and no
    /// debug HTTP server.
    pub op_schema: crate::json::Json,
}

// `impl_callback_traits!` only ever reads `self.cb`, so it is correct here. The
// full `impl_callback!` is NOT usable: its generated `Clone` and `From` construct
// `Self { cb, ctx }` literally, and this struct has a third field.
...
impl From<CustomE2eOpCallbackType> for CustomE2eOpCallback {
    /// Installs a handler that advertises NOTHING. A bare fn pointer carries no
    /// schema, so this cannot invent one.
...
impl Default for CustomE2eOpCallback {
    fn default() -> Self {
        Self {
            ...
            ctx: crate::refany::OptionRefAny::None,
            // An empty LIST, not an empty string or a null. A consumer must be able
            // to tell "this app advertises no ops" from "this app returned nothing
            // parseable"; those mean different things to a plugin deciding whether
            // the host is usable at all.
            op_schema: E2eOpSchema::default().to_json(),
        }
    }
}

/// Default handler: recognises NOTHING. `handled: false` is the load-bearing part.
#[must_use]
...
impl ArrowDirection {
    ...
    /// Convert to a `(SelectionDirection, SelectionStep)` pair for the selection-op
    /// interpreter. `ctrl` upgrades arrow keys to word jumps.
...
/// Granularity of cursor movement or selection expansion. Combined with
/// `SelectionDirection`, determines how far a cursor moves or a selection expands.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[repr(C)]
...
/// A unified selection operation that replaces all cursor movement, selection
/// extension, and text deletion commands. Every keyboard shortcut for cursor
/// movement or deletion maps to this: - Arrow Left = (Backward, Character, Move, 1)
/// - Shift+Right = (Forward, Character, Extend, 1) - Ctrl+Backspace = (Backward,
/// Word, Delete, 1) - Home = (Backward, Line, Move, 1) - Ctrl+End = (Forward,
/// Document, Move, 1) The `repeat` field enables vim-style commands: 3w = (Forward,
/// Word, Move, 3).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
#[repr(C)]
...
impl KeyboardShortcut {
    /// Map a `(VirtualKeyCode, primary, shift)` triple to a text-editing shortcut.
    /// Returns `None` if the key combination is not a recognized shortcut or if
    /// `primary` is not held.
...
/// Default input interpreter: standard desktop keybindings. This is the default
/// `InputInterpreterFn` that handles arrow keys, Home/End, Backspace/Delete,
/// Ctrl+C/V/A/Z, mouse clicks, and drag selection.
...
/// Releasing the button ends a text-selection drag, so the autoscroll timer has to
/// go. `StopAutoScrollTimer` existed and was handled, but NOTHING emitted it:
/// teardown relied entirely on the 60Hz callback noticing on its next tick that the
/// button had been released and terminating itself.
...
/// Extract the front-most hovered node from a hit test. Picks the node with the
/// minimum `hit_depth` (0 = frontmost/topmost in z-order) across every hovered DOM.
...
/// Extract mouse position from event data, falling back to `mouse_state` if not
/// available
fn get_mouse_position_with_fallback(
    ...
    match &event.data {
        ...
        _ => {
            // Fallback: use current cursor position from mouse_state This handles
            // synthetic events from debug API and automation where EventData may not
...
/// Handle `MouseDown` event - detect text selection clicks and Ctrl+Click for
/// multi-cursor
fn handle_mouse_down(
    ...
    // Ctrl+Click (or Cmd+Click on macOS): add cursor at click position. Use the
    // platform PRIMARY modifier so this fires on Cmd on macOS (where Ctrl+Click is
    // the secondary-click gesture) - `ctrl_down()` was wrong there.
...
fn handle_mouse_over(
    ...
    // Deliberately NOT gated on a hovered hit node. A drag that leaves the text -
    // into the container's padding, over a gap, past the last line - still extends
    // the selection in every native editor, and the endpoint is resolved from the
    // pointer position against the ANCHOR block's layout
    // (`process_mouse_drag_for_selection`), not from whatever happens to be under
    // the cursor.
...
fn handle_key_down(
    ...
    // reading it here could act on the wrong key/modifiers.
    let _ = keyboard_state;

    // MWA-A2: standard shortcuts key off the PRIMARY modifier (Cmd on macOS, Ctrl
    // elsewhere); word-jump / word-delete keys off the platform's word modifier
    ...
    // Check keyboard shortcuts (primary+key) → emit specific SystemChange variants.
    // Standard editing shortcuts are routed through the `KeyboardShortcut` enum, and
    // a couple of additional Azul-specific primary-modifier combos are matched
    // after.
    if primary {
    ...
    let selection_op = if let Some(arrow) = ArrowDirection::from_key(*vk, word_mod) {
        ...
        match vk {
            // Backspace/Delete = Delete mode (word modifier upgrades to Word:
...
/// Trait for querying selection manager state. This allows
/// `pre_callback_filter_internal_events` to query manager state without depending on
/// the concrete `SelectionManager` type from layout crate.
...
/// Trait for querying focus manager state. This allows
/// `pre_callback_filter_internal_events` to query manager state without depending on
/// the concrete `FocusManager` type from layout crate.
pub trait FocusManagerQuery {
    /// Get the currently focused node ID
    fn get_focused_node_id(&self) -> Option<DomNodeId>;
}

/// Post-callback filter: Determine additional system changes needed after user
/// callbacks. Takes the pre-callback system changes and focus state to determine
/// what post-callback system changes are needed (text input, scrolling, timers).
...
mod tests {
    ...
    fn keydown_without_keyboard_data_generates_no_system_change() {
        ...
        // This test documents the bug we just fixed: EventData::None causes the
        // handle_key_down function to return None (early exit at line 2737)
        assert!(result.system_changes.is_empty(),
    ...
    fn ctrl_c_generates_copy() {
        // MWA-A2/MWA-D: shortcuts key off the PRIMARY modifier - Cmd on macOS, Ctrl
        // elsewhere - so this test presses the platform's primary key. (The old
        // version hardcoded LControl and correctly started failing on macOS hosts
        // once primary_down() landed: Ctrl+C must NOT copy on macOS, Cmd+C does.)
    ...
    fn restyle_relayout_aliases_are_btreemap_compatible() {
        // RestyleNodes / RelayoutNodes are aliases for BTreeMap<NodeId,
        // Vec<ChangedCssProperty>>. Confirm we can construct empty ones via the
        // alias and that they accept the same keys.
    ...
    fn detect_lifecycle_events_with_reconciliation_is_callable() {
        // Smoke test: empty old/new node data must produce no events and an empty
        // migration map. This proves the function is callable from the public API
        // and threads through `crate::diff::reconcile_dom`.
        let dom_id = DomId { inner: 0 };
    ...
    fn first_hovered_node_picks_frontmost_by_depth() {
        ...
        // Front-most node (depth 0) has the HIGHER NodeId; back node (depth 5) has
        // the lower id. The old `.next()` logic returned the lowest id (node 2, the
        // back one).
        let mut regular = BTreeMap::new();
...
mod autotest_generated {
    ...
    /// Modifiers with the platform's PRIMARY modifier held (Cmd on macOS, Ctrl
    /// elsewhere).
    ...
    fn size_changed_single_sided_nan_is_a_change() {
        // NaN on ONE side only must register as changed (the both-sides NaN case is
        // the loop-guard covered by `size_changed_nan_guard_stops_resize_loop`).
        assert!(size_changed(
    ...
    fn size_changed_negative_and_infinite_do_not_panic() {
        ...
        // f32 * 1000.0 overflows to +/-inf, and `inf as i64` SATURATES (it does not
        // wrap or UB). So the comparison stays total and panic-free.
        assert!(!size_changed(
        ...
        // documented quantization trade-off, asserted here so a future change of the
        // quantizer (e.g. to i128 or a float compare) is a deliberate one.
        assert!(!size_changed(
    ...
    fn create_lifecycle_event_survives_extreme_node_ids() {
        ...
        // (NodeId::new(usize::MAX) would overflow that encoding - out of scope
        // here.)
        ...
        // NodeId 0 (the root) must round-trip too - the 1-based encoding makes 0
        // the value most likely to collide with the `None` sentinel.
    ...
    fn detect_lifecycle_events_without_layout_emits_nothing() {
        // Hierarchies differ, but no layout maps -> the fn must not fabricate
        // events.
    ...
    fn window_to_hover_filter_mapping_never_yields_an_internal_filter() {
        // Every window filter that has a hover twin must map onto a filter the user
        // is actually allowed to register (never a SystemText* internal).
        for w in [
    ...
    fn virtual_keycode_round_trips_for_every_key_events_rs_interprets() {
        ...
        // Out-of-range key codes must decode to None rather than index out of
        // bounds.
    ...
    fn arrow_direction_ctrl_only_upgrades_horizontal_arrows_to_words() {
        // ctrl must upgrade Left/Right to Word steps, and must NOT change the step
        // for Up/Down/Home/End (those are already line/document scoped).
    ...
    fn keyboard_shortcut_from_key_requires_primary_for_every_key() {
        ...
        // D is handled separately (SelectNextOccurrence), not as a
        // KeyboardShortcut.
    ...
    fn component_filter_matches_only_its_own_lifecycle_event() {
        ...
        // DefaultAction / Selected have no EventType twin: they must never match a
        // lifecycle event (they are driven by the a11y layer instead).
    ...
    fn event_type_to_filters_never_panics_and_stays_synced_with_the_hover_matcher() {
        ...
        // (see shell2/common/event.rs). If the two tables disagree, the callback is
        // collected and then silently dropped - a dead filter.
    ...
    fn event_type_to_filters_omits_button_specific_filter_for_exotic_buttons() {
        ...
        // Gestures ARE mapped (they were not, which is why a Pinch callback never
        // fired) - see every_gesture_event_matches_its_same_named_filter.
    ...
    fn get_dom_path_handles_a_deep_chain_without_recursing() {
        // 5000 levels deep: an iterative walk copes, a recursive one would blow the
        // stack.
    ...
    /// Bug class: a child's enter/leave reaching its parent. W3C `mouseleave` does
    /// not bubble, and every node that lost hover already gets its own event, so a
    /// parent that handles `MouseLeave` must hear ONLY about the pointer leaving the
    /// parent - not a child inside it.
    #[test]
    ...
    fn an_at_target_only_event_reaches_its_target_and_no_ancestor() {
        // The captured release for a pressed node: its ancestors already saw the
        // real release through the hovered node's path.
        let hier = hierarchy_chain(3); // 0 <- 1 <- 2
        let mut callbacks: BTreeMap<NodeId, Vec<EventFilter>> = BTreeMap::new();
        for i in 0..3 {
    ...
    fn deduplicate_synthetic_events_collapses_a_large_duplicate_burst() {
        // 10k identical events (e.g. a scroll storm) must collapse to one, and the
        // result must be the newest - no quadratic blowup, no overflow.
    ...
    fn handle_mouse_down_saturates_above_a_triple_click() {
        ...
        // 4 and above (up to the u8 boundary) are dropped - no wraparound, no
        // panic.
    ...
    /// `StopAutoScrollTimer` at all, so a lost release left a 60Hz timer running for
    ...
    fn handle_mouse_over_requires_a_held_button_and_a_drag_origin() {
        ...
        // selection gets extended, and the endpoint resolves against the anchor
        // block, not against whatever is under the pointer.
    ...
    fn handle_key_down_rejects_undecodable_key_codes() {
        ...
        // u32::MAX / out-of-table codes must fall out via `from_u32` -> None, never
        // index a table or panic.
    ...
    fn handle_key_down_reads_modifiers_from_the_event_not_the_live_keyboard() {
        ...
        // ...and conversely, a live primary key must NOT rewrite an unmodified
        // event.
    ...
    fn post_filter_handles_a_large_pre_change_list_without_blowing_up() {
        // 5000 cursor ops -> 1 flush + 5000 scroll-into-views. Bounded, no
        // overflow.
    ...
    /// The default schema must be an empty op LIST, not null and not a string. A
    /// plugin deciding whether a host is usable has to distinguish "this app
    /// advertises no ops" from "this app returned nothing parseable".
    #[test]
    fn default_op_schema_is_an_empty_list_not_a_null() {
        ...
        // Positive control: a NON-empty schema must serialize its contents, so this
        // test cannot pass by everything being empty.
    ...
    fn schema_validation_requires_a_success_boolean_in_every_example() {
        ...
        // Present but not a BOOLEAN is still a reject - `"success":"yes"` is what a
        // hand-written schema actually produces and tells a consumer nothing.
    ...
    fn default_custom_op_handler_recognises_nothing() {
        // handled=false is the load-bearing default: an app with no handler must
        // make a scenario naming a custom op FAIL, not pass quietly.
    ...
    fn default_post_filter_extern_decodes_the_none_focus_sentinel() {
        // old_focus = the `None` sentinel, new_focus = a real node => a focus
        // change.
