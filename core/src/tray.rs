//! System tray / status icon - platform-agnostic model. The actual OS plumbing
...
/// RGBA8 image for a tray icon, at one specific size. Unlike
/// [`crate::window::WindowIcon`], which is fixed at 16x16 / 32x32, a tray icon needs
/// arbitrary sizes: Windows wants `GetSystemMetricsForDpi(SM_CXSMICON)` (16 / 20 /
/// 24 / 32 px as the taskbar's DPI changes), macOS wants an 18x18 *point* template
/// image (so 36x36 px on a 2x display), and SNI wants an array of whatever sizes we
/// care to publish so the panel can pick.
#[derive(Debug, Clone)]
#[repr(C)]
...
impl TrayIconImage {
    ...
    /// Returns `OptionTrayIconImage` rather than `Option<Self>` so the signature
    /// crosses the C ABI unchanged.
    #[must_use]
    ...
    /// The icon's pixels as ARGB32 in **network (big-endian) byte order**, the wire
    /// format `org.kde.StatusNotifierItem`'s `IconPixmap` (`a(iiay)`) requires.
    /// Nothing else uses this layout, so it is computed on demand.
    #[must_use]
...
pub enum TrayIconSource {
    /// No icon. Most desktops render this as an invisible item, so it is almost
    /// never what you want.
    #[default]
    None,
    /// Explicit RGBA bitmaps, ideally at several sizes so each platform can pick -
    /// see [`TrayIconData::best_icon`]. Only needed for an icon that genuinely is
    /// not in a pack - typically one generated at runtime, since the icon registry
    /// is frozen once the provider is shared (`App::run` consumes the handle).
    Rgba(TrayIconImageVec),
    /// An **icon spec** - exactly the string an `<icon>` node takes: a bare name
    /// (`"settings"`), a pack-qualified name (`"mypack:logo"`), or a comma-separated
    /// fallback list (`"mypack:logo, settings"`). This is the preferred form, for
    /// two reasons.
    Named(AzString),
}

/// Hint about what the tray item represents. Maps to SNI's `Category`; Windows and
/// macOS have no equivalent and ignore it.
...
pub enum TrayEventType {
    /// Primary activation. **Do not document a precise gesture**: it is left click
    /// or keyboard Enter on Windows, a click on macOS, and entirely
    /// desktop-dependent on Linux (the SNI spec does not specify it).
    Activate,
    /// Middle click on Windows/Linux; not emitted on macOS.
    SecondaryActivate,
    /// The user asked for the context menu. On Windows and macOS we then open it;
    /// **on Linux the panel already opened it itself** from the exported dbusmenu,
    /// so this is informational there.
    ContextMenu,
    /// Scroll wheel over the icon. Linux only (SNI `Scroll`); Windows and macOS
    /// never emit it.
    Scroll,
...
pub struct TrayIconData {
    /// Stable identifier for this app's tray item. Used as SNI's `Id`, and as the
    /// seed for macOS's `NSStatusItem.autosaveName` (which is what makes the item
    /// keep its position in the menu bar across launches).
    pub id: AzString,
    /// Human-readable application name (SNI `Title`).
    pub title: AzString,
    /// Where the icon's pixels come from - a named icon-pack entry (preferred) or
    /// explicit RGBA bitmaps.
    ...
    /// The retained context menu. `None` means "no menu": on Windows and macOS a
    /// context-menu request then just reports [`TrayEventType::ContextMenu`], and on
    /// Linux the `Menu` property is left unset and hosts fall back to calling
    /// `ContextMenu()`.
    pub menu: OptionMenu,
}
...
impl TrayIconData {
    ...
    /// Use an icon from the icon registry, by the same spec an `<icon>` node takes
    /// - `"settings"`, `"mypack:logo"`, or a fallback list. Preferred: it renders at
    /// whatever size each platform asks for.
    #[must_use]
    ...
    /// least `target_px` (upscaling a small icon looks far worse than downscaling a
    /// large one - this is why Windows' own `LoadIconMetric` scales down from a
    /// larger frame rather than up). Only meaningful for [`TrayIconSource::Rgba`]; a
    /// `Named` icon is rasterized at the exact size instead, so it never needs
    /// picking.
    #[must_use]
...
mod tests {
    ...
    fn new_rejects_a_buffer_that_is_not_w_times_h_times_4() {
        ...
        // A zero dimension would make `expected` 0 and let an empty buffer through,
        // which every backend would then read as a valid icon.
    ...
    fn best_icon_prefers_the_smallest_that_still_covers_the_target() {
        ...
        // A named icon has no bitmap to pick from - it is rasterized at the exact
        // size instead, so best_icon must not invent one.
        assert!(
