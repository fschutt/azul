...
//!
//! * **evdev** (`from_evdev`) — Wayland's `wl_keyboard.key` IS an evdev code, X11 keycodes are
//!   `evdev + 8`, and Android's `KeyEvent` scan code is evdev too. One table serves all three.
//! * **PS/2 set 1** (`from_windows_scancode`) — what `WM_KEYDOWN`'s `lParam` carries, where the
//!   `E0` prefix (bit 24 of `lParam`) is what separates NumpadEnter from Enter, ControlRight from
//!   ControlLeft, and the arrow cluster from the numpad.
//! * **Carbon virtual keycodes** (`from_macos_keycode`) — `NSEvent.keyCode`. These are POSITIONAL
//!   despite the name; `kVK_ANSI_A` is the position, not the letter.
//!
...
impl PhysicalKey {
    ...
    pub const fn from_evdev(code: u32) -> Self {
        use PhysicalKey::{
            ...
            Backspace, BracketLeft, BracketRight, CapsLock, Comma, ContextMenu, ControlLeft,
            ControlRight, Convert, Delete, Digit0, Digit1, Digit2, Digit3, Digit4, Digit5, Digit6,
            Digit7, Digit8, Digit9, End, Enter, Equal, Escape, Home, Insert, IntlBackslash, IntlRo,
            IntlYen, KanaMode, KeyA, KeyB, KeyC, KeyD, KeyE, KeyF, KeyG, KeyH, KeyI, KeyJ, KeyK,
            KeyL, KeyM, KeyN, KeyO, KeyP, KeyQ, KeyR, KeyS, KeyT, KeyU, KeyV, KeyW, KeyX, KeyY,
            KeyZ, Lang1, Lang2, MetaLeft, MetaRight, Minus, NonConvert, NumLock, Numpad0, Numpad1,
            Numpad2, Numpad3, Numpad4, Numpad5, Numpad6, Numpad7, Numpad8, Numpad9, NumpadAdd,
            NumpadComma, NumpadDecimal, NumpadDivide, NumpadEnter, NumpadEqual, NumpadMultiply,
            NumpadSubtract, PageDown, PageUp, Pause, Period, PrintScreen, Quote, ScrollLock,
            Semicolon, ShiftLeft, ShiftRight, Slash, Space, Tab, Unidentified, F1, F10, F11, F12,
            F13, F14, F15, F16, F17, F18, F19, F2, F20, F21, F22, F23, F24, F3, F4, F5, F6, F7, F8,
            F9,
        };
        match code {
            1 => Escape,
    ...
    pub const fn from_windows_scancode(scancode: u32, extended: bool) -> Self {
        use PhysicalKey::{
            ...
            IntlYen, KanaMode, KeyA, KeyB, KeyC, KeyD, KeyE, KeyF, KeyG, KeyH, KeyI, KeyJ, KeyK,
            KeyL, KeyM, KeyN, KeyO, KeyP, KeyQ, KeyR, KeyS, KeyT, KeyU, KeyV, KeyW, KeyX, KeyY,
            KeyZ, MetaLeft, MetaRight, Minus, NonConvert, NumLock, Numpad0, Numpad1, Numpad2,
            Numpad3, Numpad4, Numpad5, Numpad6, Numpad7, Numpad8, Numpad9, NumpadAdd,
            NumpadDecimal, NumpadDivide, NumpadEnter, NumpadEqual, NumpadMultiply, NumpadSubtract,
            PageDown, PageUp, Pause, Period, PrintScreen, Quote, ScrollLock, Semicolon, ShiftLeft,
            ShiftRight, Slash, Space, Tab, Unidentified, F1, F10, F11, F12, F13, F14, F15, F16,
    ...
    pub const fn from_macos_keycode(keycode: u16) -> Self {
        use PhysicalKey::{
            AltLeft, AltRight, ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Backquote, Backslash,
            Backspace, BracketLeft, BracketRight, CapsLock, Comma, ContextMenu, ControlLeft,
            ControlRight, Delete, Digit0, Digit1, Digit2, Digit3, Digit4, Digit5, Digit6, Digit7,
            Digit8, Digit9, End, Enter, Equal, Escape, Home, Insert, IntlBackslash, IntlRo,
            ...
            Lang2, MetaLeft, MetaRight, Minus, NumLock, Numpad0, Numpad1, Numpad2, Numpad3,
            Numpad4, Numpad5, Numpad6, Numpad7, Numpad8, Numpad9, NumpadAdd, NumpadComma,
            NumpadDecimal, NumpadDivide, NumpadEnter, NumpadEqual, NumpadMultiply, NumpadSubtract,
            PageDown, PageUp, Period, Quote, Semicolon, ShiftLeft, ShiftRight, Slash, Space, Tab,
            Unidentified, F1, F10, F11, F12, F13, F14, F15, F16, F17, F18, F19, F2, F20, F3, F4,
            F5, F6, F7, F8, F9,
        };
        match keycode {
            0x00 => KeyA,
...
mod tests {
    ...
    fn the_same_position_gets_the_same_name_on_every_platform() {
        ...
        for (want, evdev, win, mac) in cases {
            ...
            assert_eq!(
                PhysicalKey::from_macos_keycode(*mac),
                *want,
                "macos {mac:#04X}"
            );
            // X11 is the evdev table shifted by the protocol's +8.
            assert_eq!(
                PhysicalKey::from_x11_keycode(evdev + 8),
                *want,
                "x11 {}",
                evdev + 8
            );
    ...
    fn the_windows_extended_bit_separates_the_duplicated_scancodes() {
        ...
        for (sc, plain, ext) in pairs {
            assert_eq!(
                PhysicalKey::from_windows_scancode(*sc, false),
                *plain,
                "{sc:#04X}"
            );
            assert_eq!(
                PhysicalKey::from_windows_scancode(*sc, true),
                *ext,
                "E0 {sc:#04X}"
            );
            assert_ne!(plain, ext);
    ...
    fn an_unnamed_code_is_unidentified() {
        ...
        assert_eq!(
            PhysicalKey::from_windows_scancode(0xFE, false),
            PhysicalKey::Unidentified
        );
        assert_eq!(
            PhysicalKey::from_windows_scancode(0x02, true),
            PhysicalKey::Unidentified
        );
        assert_eq!(
            PhysicalKey::from_macos_keycode(0xFF),
            PhysicalKey::Unidentified
        );
        // kVK_Function has no W3C `code` and must not be invented.
        assert_eq!(
            PhysicalKey::from_macos_keycode(0x3F),
            PhysicalKey::Unidentified
        );
    }
    ...
    fn an_x11_keycode_below_the_offset_cannot_wrap() {
        for kc in 0..8 {
            assert_eq!(
                PhysicalKey::from_x11_keycode(kc),
                PhysicalKey::Unidentified,
                "{kc}"
            );
        }
    ...
    fn left_and_right_modifiers_are_distinct_positions() {
        ...
        assert_eq!(
            PhysicalKey::from_macos_keycode(0x36),
