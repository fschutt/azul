...
/// AZ web-lift DIAGNOSTIC gate (REVERT): set true by AzStartup_solveLayoutReal in the lifted
/// wasm; false in the native pre-render path so the fixed-addr markers are skipped natively.
pub static AZ_IN_WASM_SOLVE: core::sync::atomic::AtomicBool = core::sync::atomic::AtomicBool::new(false);

#[inline(always)]
fn az_fuf_marker(v: u32) {
    if AZ_IN_WASM_SOLVE.load(core::sync::atomic::Ordering::Relaxed) {
        unsafe { core::ptr::write_volatile(0x40830 as *mut u32, v); }
    }
}

#[inline(always)]
fn az_fuf_val(addr: usize, v: u32) {
    if AZ_IN_WASM_SOLVE.load(core::sync::atomic::Ordering::Relaxed) {
...
impl FcFontCache {
    ...
    fn resolve_font_chain_impl(
        ...
        az_fuf_val(0x40850, 0xCC000010); // DIAG: uncached returned, before chain_cache lock
        az_fuf_val(0x40854, chain.unicode_fallbacks.len() as u32); // DIAG: chain state
        az_fuf_val(0x4085C, chain.css_fallbacks.len() as u32);
        // DIAG (2026-06-24): the chain_cache HashMap insert (hashbrown FontChainCacheKey) TRAPS in the
        // lifted wasm — confirmed CC000013 (chain.clone() completes, then cache.insert traps), PROBE1's
        // FIRST insert into the empty table (reserve_rehash). The cache is PERF-ONLY; skipping it keeps
        // resolve fully correct (every call re-resolves). Unblocks the lifted layout path; the hashbrown
        // insert lift remains a separate deep-fix. cache_key is still borrowed by the get() above.
        az_fuf_val(0x40850, 0xCC0000FF); // chain_cache.insert SKIPPED (lift-bug workaround)
        let _ = &cache_key;
    ...
    fn resolve_font_chain_uncached(
        ...
        az_fuf_val(0x40850, 0xCC000001); // DIAG (REVERT): find_unicode_fallbacks returned
        let mut unicode_fallbacks = unicode_fallbacks;
        if css_fallbacks.is_empty() && unicode_fallbacks.is_empty() {
            az_fuf_val(0x40850, 0xCC000002); // DIAG: LAST-RESORT enter (state_read + patterns BTreeMap iter().next())
            ...
            az_fuf_val(0x40850, 0xCC000003); // DIAG: LAST-RESORT done
        }
        az_fuf_val(0x40850, 0xCC000004); // DIAG: before FontFallbackChain build (uncached returns next)
    ...
    fn find_unicode_fallbacks(
        ...
        // DIAG (2026-06-24, REVERT): capture coverage-loop values to settle FontMatch-corrupt
        // vs loop-lift-bug. 0x40838=step((ci<<8)|sub), 0x4083C=candidates.len, 0x40840=uncov.len,
        // 0x40844=candidate.unicode_ranges.len, 0x40848=...as_ptr low32 (Vec header reads — safe,
        // no buffer deref). If len/ptr garbage → FontMatch corrupt; if sane but traps → loop lift.
        az_fuf_val(0x40838, 0x00000001);
        az_fuf_val(0x4083C, candidates.len() as u32);
        az_fuf_val(0x40840, uncovered_ranges.len() as u32);
        let mut __ci = 0u32;
        for candidate in candidates {
            az_fuf_val(0x40838, (__ci << 8) | 0x10);
            az_fuf_val(0x40844, candidate.unicode_ranges.len() as u32);
            az_fuf_val(0x40848, candidate.unicode_ranges.as_ptr() as usize as u32);
            // DIAG: fallbacks field (always Vec::new() in source) — if len/ptr garbage, the
            // lift DROPPED the fallbacks-init store → chain.clone() later OOBs (the trap).
            az_fuf_val(0x4084C, candidate.fallbacks.len() as u32);
            az_fuf_val(0x40858, candidate.fallbacks.as_ptr() as usize as u32);
            // Check which ranges this font covers
            let mut covers_new_range = false;

            for (i, range) in uncovered_ranges.iter().enumerate() {
                az_fuf_val(0x40838, (__ci << 8) | 0x20);
                if remaining_uncovered[i] {
                    az_fuf_val(0x40838, (__ci << 8) | 0x30);
                    // Check if this font covers this range
                    for font_range in &candidate.unicode_ranges {
                        az_fuf_val(0x40838, (__ci << 8) | 0x40);
