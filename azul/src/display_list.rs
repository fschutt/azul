...
use app_units::{Au, AU_PER_PX, MAX_AU, MIN_AU};
use azul_css::{
    BoxShadowClipMode, BoxShadowPreDisplayItem, ColorU as StyleColorU, Css, CssProperty,
    FloatValue, FontId, LayoutMargin, LayoutOverflow, LayoutPadding, LayoutPosition, PixelValue,
    RectLayout, RectStyle, SizeMetric, StyleBackground, StyleBackgroundColor, StyleBorder,
    StyleBorderRadius, StyleBoxShadow, StyleFontSize, StyleTextAlignmentHorz,
    StyleTextAlignmentVert, StyleTextColor,
};
...
use webrender::api::{
    AddFont, AddFontInstance, AddImage, AlphaType, BorderDetails, BorderRadius, BorderSide,
    BorderStyle, ClipMode, ColorF, ColorU, ComplexClipRegion, DisplayListBuilder, Epoch,
    ExternalImageData, ExternalImageId, ExternalImageType, ExternalScrollId, FontInstanceFlags,
    FontInstanceKey, FontKey, FontRenderMode, GlyphOptions, GradientStop, ImageData,
    ImageDescriptor, ImageFormat, ImageKey, ImageRendering, LayoutPixel, LayoutPoint,
    LayoutPrimitiveInfo, LayoutRect, LayoutSize, NormalBorder, PipelineId, PrimitiveInfo,
    RenderApi, ResourceUpdate, TextureTarget,
};
use {
    ...
    dom::{
        new_scroll_tag_id, DomHash, GlTextureCallback, IFrameCallback, NodeData,
    ...
    images::ImageId,
    text_cache::TextInfo,
    text_layout::{FontMetrics, ScrollbarInfo, TextOverflowPass2, Words},
    traits::Layout,
    ui_description::{StyledNode, UiDescription},
    ui_state::UiState,
    window::{FakeWindow, HidpiAdjustedBounds, ScrollStates, Window, WindowInfo},
    FastHashMap,
};

const DEFAULT_FONT_COLOR: StyleTextColor = StyleTextColor(StyleColorU {
    r: 0,
    b: 0,
    g: 0,
    a: 255,
...
impl<'a, T: Layout + 'a> DisplayList<'a, T> {
    ...
    pub(crate) fn new_from_ui_description(
        ...
        let display_rect_arena = arena.node_data.transform(|node, node_id| {
            let style = ui_description
                .styled_nodes
                .get(&node_id)
                .unwrap_or(&ui_description.default_style_of_node);
            let tag = ui_state
                .node_ids_to_tag_ids
                .get(&node_id)
                .and_then(|tag| Some(*tag));
    ...
    fn update_image_resources(
        ...
        use images::{ImageInfo, ImageState};
        ...
        for (resource_key, (data, descriptor)) in updated_images.into_iter() {
            ...
            resource_updates.push(ResourceUpdate::AddImage(AddImage {
                key,
                descriptor,
                data,
                tiling: None,
            ...
            match &*(*value.2).borrow() {
                ...
                FontState::AboutToBeDeleted(ref font_key) => {
                    let to_delete_font_instances = font_key.and_then(|f_key| {
                        let to_delete_font_instances =
                            app_resources.fonts[&f_key].values().cloned().collect();
    ...
    pub(crate) fn into_display_list_builder(
        ...
        let rects_in_rendering_order =
            determine_rendering_order(node_hierarchy, &self.rectangles, &laid_out_rectangles);
...
fn determine_rendering_order_inner<'a>(
    ...
    fn traverse_simple(
        root_id: NodeId,
        current_node: NodeEdge<NodeId>,
        node_hierarchy: &NodeHierarchy,
    ) -> Option<NodeEdge<NodeId>> {
        // returns the next item
        match current_node {
            NodeEdge::Start(current_node) => match node_hierarchy[current_node].first_child {
            ...
            NodeEdge::End(current_node) => {
                if current_node == root_id {
                    ...
                    match node_hierarchy[current_node].next_sibling {
                        ...
                        None => node_hierarchy[current_node]
                            .parent
                            .and_then(|parent| Some(NodeEdge::End(parent))),
                    }
                }
            }
        }
    ...
    while let Some(next_node_id) =
        traverse_simple(root_id, current_node_edge.clone(), node_hierarchy)
    {
...
fn do_the_layout<'a, 'b, T: Layout>(
    ...
    NodeDataContainer<LayoutRect>,
    Vec<(usize, NodeId)>,
    WordCache,
) {
    use text_layout::{get_words_cached, split_text_into_words};
    use ui_solver::{
        get_x_positions, get_y_positions, solve_flex_layout_height, solve_flex_layout_width,
    };
        ...
        .filter_map(|id| {
            let (font, font_metrics, font_id, font_size) = match node_data[id].node_type {
                NodeType::Label(_) | NodeType::Text(_) => {
                    ...
                    let font_size_app_units =
                        Au((font_size.0.to_pixels() as i32) * AU_PER_PX as i32);
                    let font_instance_key = push_font(
                        &font_id,
                        font_size_app_units,
                        resource_updates,
                        app_resources,
            ...
            match &node_data[id].node_type {
                NodeType::Label(ref string_to_render) => Some((
                    id,
                    (
    ...
    let preferred_widths =
        node_data.transform(|node, _| node.node_type.get_preferred_width(&app_resources.images));
    ...
    let preferred_heights = node_data.transform(|node, id| {
        ...
        node.node_type
            .get_preferred_height_based_on_width(
...
fn node_needs_to_clip_children(style: &RectStyle) -> bool {
    ...
    overflow.horizontal.clips_children() || overflow.vertical.clips_children()
}
        ...
        fn push_rect<'a, 'b, 'c, 'd, 'e, 'f, T: Layout>(
            ...
            if item.clip_children {
                if let Some(last_child) = referenced_content.node_hierarchy[item.node_id].last_child
                {
                    ...
                    let solved_rect = solved_rects_data[item.node_id];
                    let clip = get_clip_region(solved_rect, &styled_node).unwrap_or(
                        ComplexClipRegion::new(solved_rect, BorderRadius::zero(), ClipMode::Clip),
                    );
                    let clip_id = referenced_mutable_content.builder.define_clip(
                        solved_rect,
                        vec![clip],
                        /* image_mask: */ None,
                    );
...
fn get_clip_region<'a>(
    bounds: LayoutRect,
    rect: &DisplayRectangle<'a>,
) -> Option<ComplexClipRegion> {
...
fn displaylist_handle_rect<'a, 'b, 'c, 'd, 'e, 'f, 'g, T: Layout>(
    ...
    let info = LayoutPrimitiveInfo {
        ...
        tag: rect.tag.and_then(|tag| Some((tag, 0))).or({
            scrollable_nodes
                .overflowing_nodes
                .get(&rect_idx)
                .and_then(|scrolled| Some((scrolled.scroll_tag_id.0, 0)))
        }),
    };
    ...
    let clip_region_id = get_clip_region(bounds, &rect).and_then(|clip| {
        Some(
            referenced_mutable_content
                .builder
                .define_clip(bounds, vec![clip], None),
        )
    });
    ...
    if let Some(bg_col) = &rect.style.background_color {
        ...
        const TRANSPARENT_BG: StyleBackgroundColor = StyleBackgroundColor(StyleColorU {
            r: 0,
            g: 0,
            b: 0,
            a: 0,
        });
        push_rect(&info, referenced_mutable_content.builder, &TRANSPARENT_BG);
    }
    ...
    let scrollbar_style = ScrollbarInfo {
        ...
        background_color: StyleBackgroundColor(StyleColorU {
            r: 241,
            g: 241,
            b: 241,
            a: 255,
        }),
        triangle_color: StyleBackgroundColor(StyleColorU {
            r: 163,
            g: 163,
            b: 163,
            a: 255,
        }),
        bar_color: StyleBackgroundColor(StyleColorU {
            r: 193,
            g: 193,
            b: 193,
            a: 255,
        ...
        |text_info: &TextInfo,
         ...
         resource_updates: &mut Vec<ResourceUpdate>| {
            ...
            let mut text_bounds = rect
                ...
                .padding
                .as_ref()
                .and_then(|padding| Some(subtract_padding(&bounds, padding)))
                .unwrap_or(bounds);
    ...
    let overflow_result = match html_node {
        ...
        GlTexture(callback) => push_opengl_texture(
            callback,
            &info,
            rectangle,
            referenced_content,
...
fn push_opengl_texture<'a, 'b, 'c, 'd, 'e, 'f, 'g, T: Layout>(
    ...
    let bounds =
        HidpiAdjustedBounds::from_bounds(&referenced_mutable_content.fake_window, info.rect);
...

    {
        ...
        let gl_context = referenced_mutable_content
            .fake_window
            .read_only_window()
            .get_gl_context();
    ...
    let descriptor = ImageDescriptor::new(
        info.rect.size.width as i32,
        info.rect.size.height as i32,
        ImageFormat::BGRA8,
        opaque,
        allow_mipmaps,
    );
...
fn push_iframe<'a, 'b, 'c, 'd, 'e, 'f, 'g, T: Layout>(
    ...
    parent_scrollable_nodes
        .overflowing_nodes
        .extend(scrollable_nodes.overflowing_nodes.into_iter());
    parent_scrollable_nodes
        .tags_to_node_ids
        .extend(scrollable_nodes.tags_to_node_ids.into_iter());
...
fn push_scrollbar(
    ...
    let scrollbar_vertical_background = TypedRect::<f32, LayoutPixel> {
        origin: TypedPoint2D::new(
            bounds.origin.x + bounds.size.width - scrollbar_style.width.0,
            bounds.origin.y,
        ),
    ...
    push_rect(
        &scrollbar_vertical_background_info,
        builder,
        &scrollbar_style.background_color,
    );
    ...
    push_rect(
        &scrollbar_vertical_bar_info,
        builder,
        &scrollbar_style.bar_color,
    );
    ...
    push_triangle(
        &scrollbar_triangle_rect,
        builder,
        &scrollbar_style.triangle_color,
        TriangleDirection::PointUp,
    );

    // Triangle bottom
    scrollbar_triangle_rect.origin.y +=
        bounds.size.height - scrollbar_style.width.0 + scrollbar_style.padding.0;
    push_triangle(
        &scrollbar_triangle_rect,
        builder,
        &scrollbar_style.triangle_color,
        TriangleDirection::PointDown,
    );
}
...
fn push_triangle(
    ...
    use css::webrender_translate::wr_translate_color_u;
    use webrender::api::{BorderRadius, LayoutSideOffsets};
...
fn push_box_shadow(
    ...
    fn push_box_shadow_inner(
        ...
        use css::webrender_translate::{
            wr_translate_border_radius, wr_translate_box_shadow_clip_mode, wr_translate_color_u,
        };
    ...
    match what_shadow_to_push {
        ShouldPushShadow::PushOneShadow => {
            let current_shadow = match (top, left, bottom, right) {
                ...
                | (None, None, None, Some(Some(shadow))) => shadow,
        ...
        ShouldPushShadow::PushAllShadows => {
            ...
            let clip_rect = top_shadow
                .as_ref()
                .and_then(|top_shadow| Some(get_clip_rect(top_shadow, bounds)))
                .unwrap_or(*bounds);
            push_box_shadow_inner(
                builder,
                &top_shadow,
...
fn push_background(
    ...
    match background {
        RadialGradient(gradient) => {
            ...
            let mut stops: Vec<GradientStop> = gradient
                .stops
                .iter()
                .map(|gradient_pre| GradientStop {
            ...
            let radius = match gradient.shape {
                Shape::Ellipse => {
                    TypedSize2D::new(bounds.size.width / 2.0, bounds.size.height / 2.0)
                }
                Shape::Circle => {
        ...
        LinearGradient(gradient) => {
            ...
            let (mut begin_pt, mut end_pt) = gradient
                .direction
                .to_points(&wr_translate_layout_rect(*bounds));
        ...
        Image(style_image_id) => {
            ...
            if let Some(image_id) = app_resources.css_ids_to_image_ids.get(&style_image_id.0) {
                ...
                let image_dimensions = app_resources
                    .images
                    .get(image_id)
                    .and_then(|i| Some(i.get_dimensions()))
...
fn push_border(
    ...
    use css::webrender_translate::{wr_translate_border_details, wr_translate_layout_side_offsets};
...
fn push_font(
    ...
    match &*borrow {
        FontState::Uploaded(font_key) => {
            let font_sizes_hashmap = app_resources
                .fonts
                .entry(*font_key)
                .or_insert(FastHashMap::default());
            let font_instance_key = font_sizes_hashmap
                .entry(font_size_app_units)
                .or_insert_with(|| {
...
fn subtract_padding(
    ...
    let top = padding
        .top
        .and_then(|top| Some(top.to_pixels()))
        .unwrap_or(0.0);
    let bottom = padding
        .bottom
        .and_then(|bottom| Some(bottom.to_pixels()))
        .unwrap_or(0.0);
    let left = padding
        .left
        .and_then(|left| Some(left.to_pixels()))
        .unwrap_or(0.0);
    let right = padding
        .right
        .and_then(|right| Some(right.to_pixels()))
        .unwrap_or(0.0);
...
fn populate_css_properties(
    ...
    fn apply_style_property(rect: &mut DisplayRectangle, property: &CssProperty) {
        match property {
            BorderRadius(b) => {
                rect.style.border_radius = Some(*b);
            }
            BackgroundColor(c) => {
                rect.style.background_color = Some(*c);
            }
            TextColor(t) => {
                rect.style.font_color = Some(*t);
            }
            Border(b) => {
                StyleBorder::merge(&mut rect.style.border, &b);
            }
            Background(b) => {
                rect.style.background = Some(b.clone());
            }
            FontSize(f) => {
                rect.style.font_size = Some(*f);
            }
            FontFamily(f) => {
                rect.style.font_family = Some(f.clone());
            }
            LetterSpacing(l) => {
                rect.style.letter_spacing = Some(*l);
            }
            Overflow(o) => {
                LayoutOverflow::merge(&mut rect.style.overflow, &o);
            }
            TextAlign(ta) => {
                rect.style.text_align = Some(*ta);
            }
            BoxShadow(b) => {
                StyleBoxShadow::merge(&mut rect.style.box_shadow, b);
            }
            LineHeight(lh) => {
                rect.style.line_height = Some(*lh);
            }

            Width(w) => {
                rect.layout.width = Some(*w);
            }
            Height(h) => {
                rect.layout.height = Some(*h);
            }
            MinWidth(mw) => {
                rect.layout.min_width = Some(*mw);
            }
            MinHeight(mh) => {
                rect.layout.min_height = Some(*mh);
            }
            MaxWidth(mw) => {
                rect.layout.max_width = Some(*mw);
            }
            MaxHeight(mh) => {
                rect.layout.max_height = Some(*mh);
            }

            Position(p) => {
                rect.layout.position = Some(*p);
            }
            Top(t) => {
                rect.layout.top = Some(*t);
            }
            Bottom(b) => {
                rect.layout.bottom = Some(*b);
            }
            Right(r) => {
                rect.layout.right = Some(*r);
            }
            Left(l) => {
                rect.layout.left = Some(*l);
            }

            Padding(p) => {
                LayoutPadding::merge(&mut rect.layout.padding, &p);
            }
            Margin(m) => {
                LayoutMargin::merge(&mut rect.layout.margin, &m);
            }

            FlexGrow(g) => rect.layout.flex_grow = Some(*g),
            FlexShrink(s) => rect.layout.flex_shrink = Some(*s),
            FlexWrap(w) => {
                rect.layout.wrap = Some(*w);
            }
            FlexDirection(d) => {
                rect.layout.direction = Some(*d);
            }
            JustifyContent(j) => {
                rect.layout.justify_content = Some(*j);
            }
            AlignItems(a) => {
                rect.layout.align_items = Some(*a);
            }
            AlignContent(a) => {
                rect.layout.align_content = Some(*a);
            }
    ...
    for constraint in &rect.styled_node.css_constraints {
        ...
        match constraint {
            ...
            Dynamic(dynamic_property) => {
                if let Some(overridden_property) = css_overrides
                    .get(&node_id)
                    .and_then(|overrides| overrides.get(&dynamic_property.dynamic_id))
                {
