use azul_css::{LayoutMargin, LayoutPadding, LayoutPosition, RectLayout};
        ...
        impl NodeDataContainer<$struct_name> {
            ...
            fn $bubble_fn_name(
                ...
                for (_node_depth, non_leaf_id) in non_leaf_nodes.iter().rev() {
                    ...
                    let children_flex_basis =
                        self.sum_children_flex_basis(*non_leaf_id, node_hierarchy, arena_data);
            ...
            fn apply_flex_grow(
                ...
                fn distribute_space_along_main_axis(
                    ...
                    let mut parent_node_inner_width = {
                        ...
                        parent_node.min_inner_size_px + parent_node.flex_grow_px
                            - parent_node.$get_padding_fn()
                    };
...

                    {
                        ...
                        let exact_width_childs = node_id
                            ...
                            .filter_map(|id| {
                                if let WhConstraint::EqualTo(exact) =
                                    width_calculated_arena[id].$preferred_field
                                {
                                    Some((id, exact))
                                } else {
                                    None
                        ...
                        for (exact_width_child_id, exact_width) in exact_width_childs {
                            ...
                            if arena_data[exact_width_child_id]
                                .position
                                .unwrap_or_default()
                            ...
                            width_calculated_arena[exact_width_child_id].flex_grow_px = exact_width
                                - width_calculated_arena[exact_width_child_id].min_inner_size_px;
                        }
                    }
                    ...
                    for variable_child_id in &variable_width_childs {
                        if arena_data[*variable_child_id].position.unwrap_or_default()
                            != LayoutPosition::Absolute
                        {
                            let min_width = width_calculated_arena[*variable_child_id]
                                .$preferred_field
                                .min_needed_space()
                                .unwrap_or(0.0);
                            ...
                            width_calculated_arena[*variable_child_id].flex_grow_px = min_width
                            ...
                            let nearest_relative_parent_node = positioned_node_stack
                                .get(positioned_node_stack.len() - 1)
                                .unwrap_or(&root_id);
                            ...
                            let max_space_current_node =
                                match width_calculated_arena[*variable_child_id].$preferred_field {
                    ...
                    loop {
                        if total_horizontal_space_available <= 0.0
                            || variable_width_childs.is_empty()
                        {
                            break;
                        }
                        ...
                        for variable_child_id in &variable_width_childs {
                            let flex_grow = arena_data[*variable_child_id]
                                .flex_grow
                                .and_then(|grow| Some(grow.0.get().max(1.0)))
                                .unwrap_or(DEFAULT_FLEX_GROW_FACTOR);

                            let added_space_for_one_child = total_horizontal_space_available
                                * (flex_grow / children_combined_flex_grow);

                            let current_width_of_child = width_calculated_arena[*variable_child_id]
                                .min_inner_size_px
                                + width_calculated_arena[*variable_child_id].flex_grow_px;

                            if let Some(max_width) = width_calculated_arena[*variable_child_id]
                                .$preferred_field
                                .max_available_space()
                            {
                                if (current_width_of_child + added_space_for_one_child) > max_width
                                {
                                    ...
                                    width_calculated_arena[*variable_child_id].flex_grow_px =
                                        max_width
                                            - width_calculated_arena[*variable_child_id]
                                                .min_inner_size_px;
                ...
                fn distribute_space_along_cross_axis(
                    ...
                    let last_relative_node_width = {
                        ...
                        let last_relative_node_id = positioned_node_stack
                            .get(positioned_node_stack.len() - 1)
                            .unwrap_or(&zero_node);
                        let last_relative_node = &width_calculated_arena[*last_relative_node_id];
                        last_relative_node.min_inner_size_px + last_relative_node.flex_grow_px
                            - last_relative_node.$get_padding_fn()
                    };

                    for child_id in node_id.children(node_hierarchy) {
                        let parent_node_inner_width =
                            if arena_data[child_id].position.unwrap_or_default()
                                != LayoutPosition::Absolute
                            {
                        ...
                        let preferred_width = {
                            let min_width = width_calculated_arena[child_id]
                                .$preferred_field
                                .min_needed_space()
                                .unwrap_or(0.0);
                            ...
                            if min_width > parent_node_inner_width {
                                ...
                                if let Some(max_width) = width_calculated_arena[child_id]
                                    .$preferred_field
                                    .max_available_space()
                                {
                ...
                let root_preferred_width = if let Some(max_width) =
                    self[NodeId::new(0)].$preferred_field.max_available_space()
                {
                    if root_width > max_width {
                        max_width
                    } else {
                        root_width
                    }
                ...
                for (_node_depth, parent_id) in parent_ids_sorted_by_depth {
                    ...
                    let parent_is_positioned = arena_data[*parent_id].position.unwrap_or_default()
                        != LayoutPosition::Static;
                    if parent_is_positioned {
                        positioned_node_stack.push(*parent_id);
                    }

                    if arena_data[*parent_id]
                        .direction
                        .unwrap_or_default()
                        .get_axis()
            ...
            fn sum_children_flex_basis(
                ...
                node_id
                    ...
                    .filter(|child_node_id| {
                        display_arena[*child_node_id].position != Some(LayoutPosition::Absolute)
                    })
...
pub(crate) fn solve_flex_layout_width<'a>(
    ...
    let mut width_calculated_arena =
        NodeDataContainer::<WidthCalculatedRect>::from_rect_layout_arena(
            &layout_only_arena,
            preferred_widths,
        );
    let non_leaf_nodes_sorted_by_depth = get_non_leaf_nodes_sorted_by_depth(&node_hierarchy);
    width_calculated_arena.bubble_preferred_widths_to_parents(
        node_hierarchy,
        &layout_only_arena,
        &non_leaf_nodes_sorted_by_depth,
    );
    width_calculated_arena.apply_flex_grow(
        node_hierarchy,
        &layout_only_arena,
        &non_leaf_nodes_sorted_by_depth,
        window_width,
...
pub(crate) fn solve_flex_layout_height(
    ...
    let SolvedWidthLayout {
        layout_only_arena, ..
    } = solved_widths;
    let mut height_calculated_arena =
        NodeDataContainer::<HeightCalculatedRect>::from_rect_layout_arena(
            &layout_only_arena,
            preferred_heights,
        );
    height_calculated_arena.bubble_preferred_heights_to_parents(
        node_hierarchy,
        &layout_only_arena,
        &solved_widths.non_leaf_nodes_sorted_by_depth,
    );
    height_calculated_arena.apply_flex_grow(
        node_hierarchy,
        &layout_only_arena,
        &solved_widths.non_leaf_nodes_sorted_by_depth,
        window_height,
...
///
/// Runtime: O(n) max
pub(crate) fn get_non_leaf_nodes_sorted_by_depth(
    ...
    loop {
        for id in &current_children {
            for child_id in
                id.1.children(node_layout)
                    .filter(|id| node_layout[*id].first_child.is_some())
            {
 ...
 $axis:ident
        ...
        fn $fn_name(
            ...
            fn determine_child_x_absolute(
                ...
                let child_margin_left = child_margin
                    .$left
                    .and_then(|x| Some(x.to_pixels()))
                    .unwrap_or(0.0);
                let child_margin_right = child_margin
                    .$right
                    .and_then(|x| Some(x.to_pixels()))
                    .unwrap_or(0.0);
                ...
                let last_relative_padding_left = last_relative_padding
                    .$left
                    .and_then(|x| Some(x.to_pixels()))
                    .unwrap_or(0.0);
                let last_relative_padding_right = last_relative_padding
                    .$right
                    .and_then(|x| Some(x.to_pixels()))
                    .unwrap_or(0.0);

                let last_relative_node_x =
                    arena_solved_data[*last_relative_node_id].0 + last_relative_padding_left;
                let last_relative_node_inner_width = {
                    let last_relative_node =
                        &solved_widths.$solved_widths_field[*last_relative_node_id];
                    last_relative_node.$min_width + last_relative_node.space_added
                        - (last_relative_padding_left + last_relative_padding_right)
                };

                let child_left = &arena_data[child_id]
                    .$left
                    .and_then(|s| Some(s.0.to_pixels()));
                let child_right = &arena_data[child_id]
                    .$right
                    .and_then(|s| Some(s.0.to_pixels()));

                if let Some(child_right) = child_right {
                    // align right / bottom of last relative parent
                    arena_solved_data[child_id].0 = (last_relative_node_x
                    ...
                    arena_solved_data[child_id].0 =
                        last_relative_node_x + child_margin_left + child_left.unwrap_or(0.0);
                }
            }
            ...
            let mut arena_solved_data =
                NodeDataContainer::new(vec![$height_solved_position(0.0); node_data.len()]);
            ...
            for (_node_depth, parent_id) in non_leaf_nodes {
                ...
                let parent_padding_left = parent_padding
                    .$left
                    .and_then(|x| Some(x.to_pixels()))
                    .unwrap_or(0.0);
                let parent_padding_right = parent_padding
                    .$right
                    .and_then(|x| Some(x.to_pixels()))
                    .unwrap_or(0.0);
                ...
                let parent_is_positioned =
                    parent_node.position.unwrap_or_default() != LayoutPosition::Static;
                if parent_is_positioned {
                    positioned_node_stack.push(*parent_id);
                }
                ...
                let parent_inner_width = {
                    ...
                    parent_node.$min_width + parent_node.space_added
                        - (parent_padding_left + parent_padding_right)
                };

                if parent_direction.get_axis() == LayoutAxis::$axis {
                    // Along main axis: Take X of parent
                    let main_axis_alignment =
                        node_data[*parent_id].justify_content.unwrap_or_default();
                    ...
                    let should_align_towards_end = (!parent_direction.is_reverse()
                        && main_axis_alignment == LayoutJustifyContent::End)
                        || (parent_direction.is_reverse()
                            && main_axis_alignment == LayoutJustifyContent::Start);
...
pub(crate) fn get_x_positions(
    ...
    let mut arena = get_pos_x(
        node_hierarchy,
        &solved_widths.layout_only_arena,
        &solved_widths.non_leaf_nodes_sorted_by_depth,
        solved_widths,
    );
...
mod layout_tests {
    ...
    fn get_display_rectangle_arena(
        constraints: &[(usize, RectLayout)],
    ) -> (NodeHierarchy, NodeDataContainer<RectLayout>) {
    ...
    fn test_determine_preferred_width() {
        use azul_css::{LayoutMaxWidth, LayoutMinWidth, LayoutWidth, PixelValue};
        ...
        assert_eq!(
            determine_preferred_width(&layout, None),
            WhConstraint::Unconstrained
        );
        ...
        assert_eq!(
            determine_preferred_width(&layout, None),
            WhConstraint::Between(600.0, 800.0)
        );
        ...
        assert_eq!(
            determine_preferred_width(&layout, None),
            WhConstraint::Between(0.0, 800.0)
        );
    ...
    fn test_fill_out_preferred_width() {
        ...
        let (node_hierarchy, node_data) = get_display_rectangle_arena(&[
            ...
            (
                ...
                RectLayout {
                    ...
                    padding: Some(LayoutPadding {
                        left: Some(PixelValue::px(20.0)),
                        right: Some(PixelValue::px(20.0)),
                        ..Default::default()
                    }),
                    direction: Some(LayoutDirection::Row),
                    ..Default::default()
        ...
        assert_eq!(
            width_filled_out_data[NodeId::new(0)].get_flex_basis_horizontal(),
            0.0
        );

        // Node 1 has a padding on left and right of 20, so a flex-basis of 40.0
        assert_eq!(
            width_filled_out_data[NodeId::new(1)].get_flex_basis_horizontal(),
            40.0
        );
        assert_eq!(
            width_filled_out_data[NodeId::new(1)].get_horizontal_padding(),
            40.0
        );

        assert_eq!(
            width_filled_out_data[NodeId::new(2)].get_flex_basis_horizontal(),
            0.0
        );
        assert_eq!(
            width_filled_out_data[NodeId::new(3)].get_flex_basis_horizontal(),
            0.0
        );
        assert_eq!(
            width_filled_out_data[NodeId::new(4)].get_flex_basis_horizontal(),
            0.0
        );
        assert_eq!(
            width_filled_out_data[NodeId::new(5)].get_flex_basis_horizontal(),
            0.0
        );
        ...
        assert_eq!(
            width_filled_out_data[NodeId::new(1)].preferred_width,
            WhConstraint::Between(0.0, 200.0)
        );
        assert_eq!(
            width_filled_out_data[NodeId::new(2)].preferred_width,
            WhConstraint::Unconstrained
        );
        assert_eq!(
            width_filled_out_data[NodeId::new(3)].preferred_width,
            WhConstraint::Unconstrained
        );
        assert_eq!(
            width_filled_out_data[NodeId::new(4)].preferred_width,
            WhConstraint::Unconstrained
        );
        assert_eq!(
            width_filled_out_data[NodeId::new(5)].preferred_width,
            WhConstraint::Unconstrained
        );

        // Test the flex-basis sum
        assert_eq!(
            width_filled_out_data.sum_children_flex_basis(
                NodeId::new(2),
                &node_hierarchy,
                &node_data
            ),
        ...
        width_filled_out_data.bubble_preferred_widths_to_parents(
            &node_hierarchy,
            &node_data,
            &non_leaf_nodes_sorted_by_depth,
        );
        ...
        assert_eq!(
            width_filled_out_data[NodeId::new(0)].min_inner_size_px,
            40.0
        );
        assert_eq!(
            width_filled_out_data[NodeId::new(1)].get_flex_basis_horizontal(),
        ...
        width_filled_out_data.apply_flex_grow(
            &node_hierarchy,
            &node_data,
            &non_leaf_nodes_sorted_by_depth,
            window_width,
        ...
        assert_eq!(
            width_filled_out_data[NodeId::new(0)].solved_result(),
            WidthSolvedResult {
                min_width: 40.0,
        ...
        assert_eq!(
            width_filled_out_data[NodeId::new(1)].solved_result(),
            WidthSolvedResult {
                min_width: 0.0,
                space_added: 200.0,
            }
        ...
        assert_eq!(
            width_filled_out_data[NodeId::new(2)].solved_result(),
            WidthSolvedResult {
                min_width: 0.0,
                space_added: 160.0,
            }
        ...
        assert_eq!(
            width_filled_out_data[NodeId::new(3)].solved_result(),
            WidthSolvedResult {
                min_width: 0.0,
                space_added: 80.0,
            }
        ...
        assert_eq!(
            width_filled_out_data[NodeId::new(4)].solved_result(),
            WidthSolvedResult {
                min_width: 0.0,
                space_added: 80.0,
            }
        ...
        assert_eq!(
            width_filled_out_data[NodeId::new(5)].solved_result(),
            WidthSolvedResult {
                min_width: 0.0,
