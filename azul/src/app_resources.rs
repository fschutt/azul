use app_units::Au;
use azul_css::{FontId, StyleFontSize, StyleLetterSpacing};
...
use rusttype::Font;
use std::{cell::RefCell, collections::hash_map::Entry::*, io::Read, rc::Rc};
use webrender::api::{FontInstanceKey, FontKey};
use FastHashMap;
use {
    font::{FontError, FontState},
...
pub struct AppResources {
    ...
    pub(crate) font_data:
        RefCell<FastHashMap<FontId, (Rc<Font<'static>>, Rc<Vec<u8>>, Rc<RefCell<FontState>>)>>,
...
impl AppResources {
    ...
    pub fn add_image<S: Into<String>, R: Read>(
        ...
        match self.images.entry(*image_id) {
            ...
            Vacant(v) => {
                ...
                data.read_to_end(&mut image_data)
    ...
    pub fn has_image<S: AsRef<str>>(&self, id: S) -> bool {
    ...
    pub fn get_image<S: AsRef<str>>(&self, id: S) -> Option<ImageId> {
        self.css_ids_to_image_ids
            .get(id.as_ref())
            .and_then(|id| Some(*id))
    }
    ...
    pub fn add_font<R: Read>(&mut self, id: FontId, data: &mut R) -> Result<Option<()>, FontError> {
        ...
        match self.font_data.borrow_mut().entry(id) {
            ...
            Vacant(v) => {
                ...
                data.read_to_end(&mut font_data)
                    .map_err(|e| FontError::IoError(e))?;
                let (parsed_font, fd) = font::rusttype_load_font(font_data.clone(), None)?;
                v.insert((
                    Rc::new(parsed_font),
                    Rc::new(fd),
                    Rc::new(RefCell::new(FontState::ReadyForUpload(font_data))),
                ));
                Ok(Some(()))
            }
        }
    ...
    fn get_font_internal(
        &self,
        id: &FontId,
    ) -> Option<(Rc<Font<'static>>, Rc<Vec<u8>>, Rc<RefCell<FontState>>)> {
        match id {
            FontId::BuiltinFont(b) => {
                if self.font_data.borrow().get(id).is_none() {
                    let (font, font_bytes, font_state) = Self::get_builtin_font(b.clone())?;
                    self.font_data.borrow_mut().insert(
                        id.clone(),
                        (
                            Rc::new(font),
                            Rc::new(font_bytes),
                            Rc::new(RefCell::new(font_state)),
                        ),
                    );
                }
                self.font_data
                    .borrow()
                    .get(id)
                    .and_then(|(font, bytes, state)| {
                        Some((font.clone(), bytes.clone(), state.clone()))
                    })
            }
    ...
    pub fn get_font(&self, id: &FontId) -> Option<(Rc<Font<'static>>, Rc<Vec<u8>>)> {
        self.get_font_internal(id)
            .and_then(|(font, bytes, _)| Some((font, bytes)))
    }
    ...
    pub fn delete_font(&mut self, id: &FontId) -> Option<()> {
        ...
        match self.font_data.borrow().get(&id) {
            ...
            Some(v) => match *(*v.2).borrow() {
                FontState::Uploaded(font_key) => {
                    to_delete_font_key = Some(font_key.clone());
                }
                _ => {}
            },
        }
        ...
        *borrow_mut.get_mut(&id).unwrap().2.borrow_mut() =
            FontState::AboutToBeDeleted(to_delete_font_key);
        Some(())
    }
