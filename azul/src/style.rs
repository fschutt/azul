...
use azul_css::{
    Css, CssContentGroup, CssDeclaration, CssPath, CssPathPseudoSelector, CssPathSelector,
};
    ...
    for selector in selectors {
        match selector {
            ...
            PseudoSelector(CssPathPseudoSelector::NthChild(x)) => {
                if html_node.index_in_parent != *x {
                    return false;
                }
            }
...
pub(crate) fn match_dom_selectors<T: Layout>(
    ...
    for (_depth, parent_id) in non_leaf_nodes {
        ...
        for applying_rule in css.rules.iter().filter(|rule| {
            matches_html_element(&rule.path, parent_id, &arena_borrow.node_layout, &html_tree)
        }) {
            parent_rules
                .css_constraints
                .extend(applying_rule.declarations.clone());
        }

        let inheritable_rules: Vec<CssDeclaration> = parent_rules
            .css_constraints
            .iter()
            ...
            match child_node.first_child {
                ...
                Some(_) => {
                    ...
                    styled_nodes.insert(
                        child_id,
                        StyledNode {
                            css_constraints: inheritable_rules.clone(),
                        },
                    );
                }
            }
