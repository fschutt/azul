...
impl<T> UiState<T> {
    ...
    pub fn scan_for_iframe_callbacks(&self) -> Vec<(NodeId, IFrameCallback<T>, StackCheckedPointer<T>)> {
    ...
    pub fn scan_for_gltexture_callbacks(&self) -> Vec<(NodeId, GlCallback<T>, StackCheckedPointer<T>)> {
