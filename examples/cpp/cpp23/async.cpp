...
ffi::Dom layout(ffi::RefAny data, ffi::LayoutCallbackInfo info) {
    ...
    body = body.with_css("display: flex; flex-direction: column; height: 100%; margin: 0; padding: 0; "
                         "background: #eef1f5; font-family: sans-serif;"sv);
    body = body.with_child(std::move(header));
    body = body.with_child(std::move(frame));
    body = body.with_child(std::move(footer));
