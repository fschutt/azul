...
AzDom create_button(AzRefAny* calc, const char* label, EventType evt, char digit, Operation op, const char* style) {
    ...
    AzDom_addChild(&button, AzDom_createSpanWithText(label_str));
...
AzDom layout(AzRefAny data, AzLayoutCallbackInfo info) {
    ...
    AzDom_addChild(&display, AzDom_createSpanWithText(AzString_copyFromBytes((uint8_t*)display_text, 0, strlen(display_text))));
