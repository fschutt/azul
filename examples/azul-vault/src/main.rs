...
extern "C" fn layout(mut data: RefAny, _info: LayoutCallbackInfo) -> Dom {
    ...
    if !unlocked {
        return Dom::create_body().with_child(
            Dom::create_div()
                .with_css(LOCKED)
                .with_child(Dom::create_div_with_text("🔒 AzulVault").with_css(TITLE))
                .with_child(Dom::create_div_with_text(status.as_str()).with_css(STATUS))
                .with_child(
                    Dom::create_div()
                        .with_css(BTN)
    ...
    if entries.is_empty() {
        ...
        for (k, v) in &entries {
            body = body.with_child(
                Dom::create_div()
                    .with_css(ENTRY_ROW)
                    .with_child(Dom::create_div_with_text(k.as_str()).with_css(ENTRY_K))
                    .with_child(Dom::create_div_with_text(v.as_str()).with_css(ENTRY_V)),
            );
        }
    }
    body = body.with_child(
    ...
    Dom::create_body().with_child(
        Dom::create_div()
            .with_css(ROOT)
            .with_child(Dom::create_div_with_text("🔓 AzulVault — unlocked").with_css(HEADER))
            .with_child(body),
    )
}
