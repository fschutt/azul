...
impl Layout for MyDataModel {
    fn layout(&self, _info: WindowInfo<Self>) -> Dom<Self> {
        ...
        let mut dom = Dom::new(NodeType::Div).with_child(Label::new(status.clone()).dom());

        match &self.connection_status {
            NotConnected => {
                let button = Button::with_label("Connect to database...")
                    .dom()
...
fn reset_connection(
    ...
    _event: WindowEvent<MyDataModel>,
) -> UpdateScreen {
    app_state
        .data
        .modify(|state| state.connection_status = ConnectionStatus::NotConnected);
    UpdateScreen::Redraw
}
...
fn start_connection(
    ...
    app_state
        .data
        .modify(|state| state.connection_status = status);
...
fn timer_daemon(
    state: &mut MyDataModel,
    _resources: &mut AppResources,
