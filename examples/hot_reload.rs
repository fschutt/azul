...
impl Layout for MyDataModel {
    fn layout(&self, info: WindowInfo<Self>) -> Dom<Self> {
        Dom::new(NodeType::Div)
            ...
            .with_child(
                Dom::new(NodeType::Image(info.resources.get_image("Cat01").unwrap()))
                    .with_id("cat"),
            )
    }
}
...
fn main() {
    macro_rules! CSS_PATH {
        () => {
            concat!(env!("CARGO_MANIFEST_DIR"), "/../examples/hot_reload.css")
        };
    }

    let mut app = App::new(MyDataModel, AppConfig::default());
    app.add_image("Cat01", &mut TEST_IMAGE, ImageType::Jpeg)
        .unwrap();
