...
extern "C" fn icon_demo_layout(_data: RefAny, _info: LayoutCallbackInfo) -> Dom {
    ...
    let mut note = Dom::create_div_with_text("Note: Icons show '?' when no icon provider is registered.");
    ...
    container = container
        .with_child(Dom::create_div_with_text("Navigation Icons"))
        .with_child(nav_row)
        .with_child(Dom::create_div_with_text("Action Icons"))
