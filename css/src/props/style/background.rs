...
#[cfg(feature="parser")]
use crate::parser::{
    impl_from, impl_debug_as_display, impl_display,
    parse_parentheses, ParenthesisParseError, ParenthesisParseErrorOwned,
    CssImageParseError, CssImageParseErrorOwned,
    parse_image, InvalidValueErr, InvalidValueErrOwned, split_string_respect_comma,
};
...
#[derive(Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct LinearColorStop { pub offset: OptionPercentageValue, pub color: ColorU }

/// Transient struct for parsing radial/conic color stops before normalization.
#[derive(Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct RadialColorStop { pub offset: OptionAngleValue, pub color: ColorU }
...
impl PrintAsCssValue for StyleBackgroundPosition {
    fn print_as_css_value(&self) -> String {
        format!("{} {}", self.horizontal.print_as_css_value(), self.vertical.print_as_css_value())
    }
}
impl PrintAsCssValue for StyleBackgroundPositionVec {
    fn print_as_css_value(&self) -> String {
        self.iter().map(|v| v.print_as_css_value()).collect::<Vec<_>>().join(", ")
    }
}
...
impl PrintAsCssValue for StyleBackgroundSize {
    fn print_as_css_value(&self) -> String {
        match self {
            ...
            Self::ExactSize([w, h]) => format!("{} {}", w.print_as_css_value(), h.print_as_css_value()),
        }
    }
}
...
impl<'a> CssBackgroundParseError<'a> {
    pub fn to_contained(&self) -> CssBackgroundParseErrorOwned {
        match self {
            ...
            Self::InvalidBackground(e) => CssBackgroundParseErrorOwned::InvalidBackground(e.to_contained()),
            Self::UnclosedGradient(s) => CssBackgroundParseErrorOwned::UnclosedGradient(s.to_string()),
            Self::NoDirection(s) => CssBackgroundParseErrorOwned::NoDirection(s.to_string()),
            Self::TooFewGradientStops(s) => CssBackgroundParseErrorOwned::TooFewGradientStops(s.to_string()),
            Self::DirectionParseError(e) => CssBackgroundParseErrorOwned::DirectionParseError(e.to_contained()),
            Self::GradientParseError(e) => CssBackgroundParseErrorOwned::GradientParseError(e.to_contained()),
            Self::ConicGradient(e) => CssBackgroundParseErrorOwned::ConicGradient(e.to_contained()),
            Self::ShapeParseError(e) => CssBackgroundParseErrorOwned::ShapeParseError(e.to_contained()),
            Self::ImageParseError(e) => CssBackgroundParseErrorOwned::ImageParseError(e.to_contained()),
            Self::ColorParseError(e) => CssBackgroundParseErrorOwned::ColorParseError(e.to_contained()),
        }
    }
}
...
impl CssBackgroundParseErrorOwned {
    pub fn to_shared<'a>(&'a self) -> CssBackgroundParseError<'a> {
        match self {
            ...
            Self::DirectionParseError(e) => CssBackgroundParseError::DirectionParseError(e.to_shared()),
            Self::GradientParseError(e) => CssBackgroundParseError::GradientParseError(e.to_shared()),
...
pub enum CssGradientStopParseError<'a> {
    Error(&'a str),
    Percentage(crate::props::basic::value::PercentageParseError),
...
impl<'a> CssGradientStopParseError<'a> {
    pub fn to_contained(&self) -> CssGradientStopParseErrorOwned {
        match self {
            ...
            Self::ColorParseError(e) => CssGradientStopParseErrorOwned::ColorParseError(e.to_contained()),
        }
    }
}
...
impl<'a> CssBackgroundPositionParseError<'a> {
    pub fn to_contained(&self) -> CssBackgroundPositionParseErrorOwned {
        match self {
            ...
            Self::TooManyComponents(s) => CssBackgroundPositionParseErrorOwned::TooManyComponents(s.to_string()),
            Self::FirstComponentWrong(e) => CssBackgroundPositionParseErrorOwned::FirstComponentWrong(e.to_contained()),
            Self::SecondComponentWrong(e) => CssBackgroundPositionParseErrorOwned::SecondComponentWrong(e.to_contained()),
        }
    }
}
impl CssBackgroundPositionParseErrorOwned {
    pub fn to_shared<'a>(&'a self) -> CssBackgroundPositionParseError<'a> {
        match self {
            ...
            Self::FirstComponentWrong(e) => CssBackgroundPositionParseError::FirstComponentWrong(e.to_shared()),
            Self::SecondComponentWrong(e) => CssBackgroundPositionParseError::SecondComponentWrong(e.to_shared()),
        }
    }
}
...
mod parser {
    ...
    impl GradientType {
        pub const fn get_extend_mode(&self) -> ExtendMode {
            match self {
                Self::LinearGradient | Self::RadialGradient | Self::ConicGradient => ExtendMode::Clamp,
                Self::RepeatingLinearGradient | Self::RepeatingRadialGradient | Self::RepeatingConicGradient => ExtendMode::Repeat,
            }
        }
    }
        ...
        match parse_parentheses(
            ...
            Ok((background_type, brace_contents)) => {
                let gradient_type = match background_type {
                    ...
                    "image" | "url" => return Ok(StyleBackgroundContent::Image(parse_image(brace_contents)?.into())),
                    _ => unreachable!(),
                };
    ...
    pub fn parse_style_background_position<'a>(input: &'a str) -> Result<StyleBackgroundPosition, CssBackgroundPositionParseError<'a>> {
        let input = input.trim();
        let mut whitespace_iter = input.split_whitespace();

        let first = whitespace_iter.next().ok_or(CssBackgroundPositionParseError::NoPosition(input))?;
        ...
        if let Ok(horizontal) = parse_background_position_horizontal(first) {
             let vertical = match second {
                Some(s) => parse_background_position_vertical(s).map_err(CssBackgroundPositionParseError::SecondComponentWrong)?,
        ...
        if let Ok(vertical) = parse_background_position_vertical(first) {
            let horizontal = match second {
                Some(s) => parse_background_position_horizontal(s).map_err(CssBackgroundPositionParseError::FirstComponentWrong)?,
                None => BackgroundPositionHorizontal::Center,
            };
            return Ok(StyleBackgroundPosition { horizontal, vertical });
        }

        Err(CssBackgroundPositionParseError::FirstComponentWrong(CssPixelValueParseError::InvalidPixelValue(first)))
    }
    ...
    pub fn parse_style_background_size<'a>(input: &'a str) -> Result<StyleBackgroundSize, InvalidValueErr<'a>> {
        let input = input.trim();
        match input {
    ...
    pub fn parse_style_background_repeat<'a>(input: &'a str) -> Result<StyleBackgroundRepeat, InvalidValueErr<'a>> {
        match input.trim() {
    ...
    fn parse_gradient<'a>(input: &'a str, gradient_type: GradientType) -> Result<StyleBackgroundContent, CssBackgroundParseError<'a>> {
        let input = input.trim();
        let comma_separated_items = split_string_respect_comma(input);
        let mut brace_iterator = comma_separated_items.iter();
        let first_brace_item = brace_iterator.next().ok_or(CssBackgroundParseError::NoDirection(input))?;
                ...
                loop {
                    ...
                    while let Some(word) = temp_iter.next() {
                         if let Ok(shape) = parse_shape(word) {
                            ...
                            consumed_in_iteration = true;
                            break; // position can have multiple words, so consume the rest of the item
                         }
                    }
    ...
    /// Parses "red" or "red 5%".
    fn parse_linear_color_stop<'a>(input: &'a str) -> Result<LinearColorStop, CssGradientStopParseError<'a>> {
        let input = input.trim();
        ...
        let offset = match offset_str {
            ...
            Some(s) => OptionPercentageValue::Some(parse_percentage_value(s).map_err(CssGradientStopParseError::Percentage)?)
        };
    ...
    fn parse_radial_color_stop<'a>(input: &'a str) -> Result<RadialColorStop, CssGradientStopParseError<'a>> {
        let input = input.trim();
        let (color_str, offset_str) = split_color_and_offset(input);
        ...
        let offset = match offset_str {
            ...
            Some(s) => OptionAngleValue::Some(parse_angle_value(s).map_err(CssGradientStopParseError::Angle)?)
        };
    ...
    fn split_color_and_offset<'a>(input: &'a str) -> (&'a str, Option<&'a str>) {
        ...
        // If no whitespace or the part after it is not a valid offset, the whole string is the color.
        (input, None)
    }

    /// Parses the `from <angle> at <position>` part of a conic gradient.
    fn parse_conic_first_item<'a>(input: &'a str) -> Result<Option<(AngleValue, StyleBackgroundPosition)>, CssConicGradientParseError<'a>> {
        let input = input.trim();
        ...
        let angle_part = parts.next().ok_or(CssConicGradientParseError::NoAngle(input))?.trim();
    ...
    fn parse_background_position_horizontal<'a>(input: &'a str) -> Result<BackgroundPositionHorizontal, CssPixelValueParseError<'a>> {
        Ok(match input {
    ...
    fn parse_background_position_vertical<'a>(input: &'a str) -> Result<BackgroundPositionVertical, CssPixelValueParseError<'a>> {
        Ok(match input {
    ...
    fn parse_radial_gradient_size<'a>(input: &'a str) -> Result<RadialGradientSize, InvalidValueErr<'a>> {
        match input.trim() {
