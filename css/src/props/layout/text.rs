...
impl<'a> fmt::Display for TextJustifyParseError<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            TextJustifyParseError::InvalidValue(s) => write!(f, "Invalid text-justify value: '{}'.", s),
        }
    }
}
...
impl<'a> TextJustifyParseError<'a> {
    pub fn to_owned(&self) -> TextJustifyParseErrorOwned {
        match self {
            TextJustifyParseError::InvalidValue(s) => TextJustifyParseErrorOwned::InvalidValue((*s).to_string()),
        }
    }
}
...
impl TextJustifyParseErrorOwned {
    pub fn to_borrowed(&self) -> TextJustifyParseError {
        match self {
            TextJustifyParseErrorOwned::InvalidValue(s) => TextJustifyParseError::InvalidValue(s.as_str()),
        }
    }
}
...
mod tests {
    ...
    fn test_parse_layout_text_justify() {
        assert_eq!(parse_layout_text_justify("auto"), Ok(LayoutTextJustify::Auto));
        assert_eq!(parse_layout_text_justify("none"), Ok(LayoutTextJustify::None));
        assert_eq!(parse_layout_text_justify("inter-word"), Ok(LayoutTextJustify::InterWord));
        assert_eq!(parse_layout_text_justify("inter-character"), Ok(LayoutTextJustify::InterCharacter));
        assert_eq!(parse_layout_text_justify("distribute"), Ok(LayoutTextJustify::Distribute));
