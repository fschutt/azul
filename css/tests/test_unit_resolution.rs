...
fn test_to_pixels_internal_px() {
    let value = PixelValue::px(100.0);
    let result = value.to_pixels_internal(50.0, 16.0, 16.0);
...
fn test_to_pixels_internal_percent() {
    let value = PixelValue::percent(50.0);
    let result = value.to_pixels_internal(200.0, 16.0, 16.0);
...
fn test_pixel_value_zero() {
    ...
    let result = value.to_pixels_internal(100.0, 100.0, 100.0);
...
fn test_em_chain_resolution() {
    ...
    let parent_px = parent_em.to_pixels_internal(100.0, 16.0, 16.0); // 2 * 16 = 32
    assert!((parent_px - 32.0).abs() < 0.001);

    let child_em = PixelValue::em(1.5);
    let child_px = child_em.to_pixels_internal(100.0, parent_px, 16.0); // 1.5 * 32 = 48
...
fn test_percent_over_100() {
    ...
    let result = value.to_pixels_internal(100.0, 16.0, 16.0);
...
fn test_viewport_units_return_zero_without_context() {
    ...
    assert_eq!(vw.to_pixels_internal(100.0, 16.0, 16.0), 0.0);
    assert_eq!(vh.to_pixels_internal(100.0, 16.0, 16.0), 0.0);
    assert_eq!(vmin.to_pixels_internal(100.0, 16.0, 16.0), 0.0);
    assert_eq!(vmax.to_pixels_internal(100.0, 16.0, 16.0), 0.0);
}
...
fn test_default_font_size_constant() {
    ...
    let result = default_em.to_pixels_internal(100.0, 16.0, 16.0);
    assert!(
...
fn test_mixed_calculations() {
    ...
    let padding_px = padding.to_pixels_internal(container_width, base_font, base_font);
    assert!((padding_px - 25.0).abs() < 0.001);

    let font = PixelValue::em(1.25);
    let font_px = font.to_pixels_internal(container_width, base_font, base_font);
...
fn test_percent_with_zero_base() {
    let value = PixelValue::percent(50.0);
    let result = value.to_pixels_internal(0.0, 16.0, 16.0);
...
fn test_em_with_zero_base() {
    let value = PixelValue::em(2.0);
    let result = value.to_pixels_internal(100.0, 0.0, 0.0);
